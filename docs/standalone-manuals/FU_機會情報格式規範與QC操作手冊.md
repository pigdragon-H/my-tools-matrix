...
- 有對應知識庫文章時：
  ```
  往知識庫閱讀 → [知識庫文章標題](/knowledge/{該篇文章的真實domain值}/文章slug)
  ```
...
### ⚠️ 這裡是真實出過事故的地方，務必先查值再寫，不要用猜的

**知識庫網址是兩段式`/knowledge/{domain}/{slug}`，不是`/knowledge/{slug}`！** 機會情報自己的網址是單段（`/opportunities/{slug}`），創業藍圖也是單段（`/blueprints/{slug}`），**唯獨知識庫多一層domain**。真實發生過的事故：直接套用機會情報自己單段的慣例去寫知識庫連結，漏掉了domain段，訪客點下去看到一片空白（詳見`FU_三軸金字塔架構與原理.md`第五節的完整事故記錄）。

**動手寫連結前，正確步驟是：**
1. 打開目標知識庫文章的檔案，看frontmatter裡`domain:`那一行的實際值（例如`ai-agent`、`ai-automation`）
2. 把這個真實值填進連結，不要用猜的、不要假設跟機會情報自己的domain同名同值——兩邊的domain值集合是分開維護的兩套系統

**錯誤範例（真實事故重現，不要這樣寫）**：
```
往知識庫閱讀 → [某篇文章](/knowledge/some-article-slug)
```

**正確範例**：
```
往知識庫閱讀 → [某篇文章](/knowledge/ai-agent/some-article-slug)
```

驗證腳本（`scripts/validate-ai-three-axes.mjs`）現在會擋下漏掉domain段的連結，並提示正確格式，但事先查清楚比事後被擋下重寫更有效率，也避免內容還沒進到驗證這一關、直接被人工複製貼上到別的地方時已經帶著錯誤格式。
