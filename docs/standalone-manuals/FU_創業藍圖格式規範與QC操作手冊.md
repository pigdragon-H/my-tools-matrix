...
## 十一、商業層規範：AdSense／Affiliate／Premium

**AdSense**：由 `ArticleShell` 自動渲染，`slotPrefix` 通常用 `blueprint`。固定位廣告4個自動插入正文段落之間；動態曝光廣告則是每出現一個表格、每出現一個框線字元架構圖，各自動增生一個廣告位。**創業藍圖是三軸裡內容量最大的單元（3000字以上、多個表格與計畫段落），理論上能吃到的曝光位數量也最多**，這是字數門檻定得比知識庫更嚴格的附帶效益之一。撰稿時不需手動處理版位，只需確保 `adsEnabled` 欄位正確——**草稿階段固定 `false`，唯有 `victorReviewed: true` 之後才能改 `true`**（見第五節、第九節）。

**Affiliate**：填 `affiliateTags: [...]`，比對相關聯盟商品/工具自動顯示。創業藍圖裡「工具與工作流」段落若提及具體付費工具，通常最適合對應填入 affiliate 標籤。

**Premium**：`ENABLE_PREMIUM` 功能開關目前關閉，設 `premiumGate: true` 目前不會產生任何實際付費牆效果，內容照樣完全公開顯示，Stripe 也還未接。**創業藍圖是三軸裡最有機會做成真正付費內容的單元**（稀缺、有料、訪客有付費動機），若規劃倚賴 premium 變現，撰寫前務必先跟 Victor 確認商業開通時程，不能假設設了欄位就會生效——目前即使設了 `premiumGate: true`，藍圖也還是完全公開閱讀。

## 十二、目前仍待 Victor 裁定的項目
