...
> 版本 v1.4 · 2026-07-03 · 整理者：Claude（Universe Auditor / QC）
> v1.4 變更：修正本文件範例裡`[標題](/knowledge/slug)`這個**錯誤格式**——知識庫網址是`/knowledge/{domain}/{slug}`兩段式，機會情報跟創業藍圖才是單段式（`/opportunities/{slug}`、`/blueprints/{slug}`）。這不是文字校對，是真實事故：AI視角連結曾經漏掉domain段，訪客點擊後看到空白頁面（詳見`FU_三軸金字塔架構與原理.md`第五節事故記錄）。驗證腳本已同步補強，會擋下漏掉domain段的連結。
...
[標題](/knowledge/domain/slug) 真連結，或誠實聲明「無知識庫文章」）
```
...
| **AI視角區塊缺少「往知識庫閱讀」雙狀態接點**（既無 `[標題](/knowledge/domain/slug)` 真連結，也沒有「無知識庫文章」聲明） | error（v1.3，這是訪客看得到的傳動軸接點，跟隱形的 `relatedKnowledge` frontmatter 是兩件事） |
...
| **缺少「🔭 AI視角」區塊，或字數不在300-500字（不含連結語法）** | error（v1.2，Victor 2026-07-03 裁定） |
| **AI視角區塊沒有可見的知識庫連結**（`[標題](/knowledge/domain/slug)` 格式） | ~~error~~ **已於 v1.3 併入上方雙狀態接點規則，不再單獨要求一定要有連結** |
