...
function buildUI(brief, inputBundle) {
  ...
  const zh = {
    ...
    premiumTitle: brief.premiumTitleZh || `專業版 ${nameZh}`,
    premiumText: brief.premiumTextZh || `Premium 解鎖${nameZh}的批次試算、結果歷史、PDF 匯出、多場景比較與廣告移除。`,
    premiumChips_zh: brief.premiumChipsZh || "批次試算|歷史紀錄|PDF 匯出|廣告移除",
    premiumChips_en: brief.premiumChipsEn || "Batch|History|PDF Export|Ad-free",
  ...
  const en = {
    ...
    premiumTitle: brief.premiumTitleEn || `Pro ${nameEn}`,
    premiumText: brief.premiumTextEn || `Premium unlocks batch calculation, history, PDF export, multi-scenario comparison, and ad-free for ${nameEn}.`,
    premiumChips_zh: brief.premiumChipsZh || "批次試算|歷史紀錄|PDF 匯出|廣告移除",
    premiumChips_en: brief.premiumChipsEn || "Batch|History|PDF Export|Ad-free",
