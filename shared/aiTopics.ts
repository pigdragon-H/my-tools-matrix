...
export const AI_TOPICS: AiTopicRegistryEntry[] = [
  ...
  },
  {
    ...
    targetReader: {
      zh: "想用LLM/影片生成模型做出角色一致、有真實感短內容的創作者",
      en: "Creators using LLMs and video-generation models who want consistent characters and realistic short-form output",
    },
    commercialIntent: ["opportunity_discovery", "education"],
    status: "active",
    priority: "P1",
    signal: ["單一社群貼文效果宣稱（未經查證）", "提示詞結構與企業自身測試角色一致性時使用的範本高度相似", "企業第一手實測：同一份提示詞在Grok與Gemini上得到截然不同的結果，Gemini直接生成完整一致的影片"],
    expectedOutputs: ["opportunities", "knowledge"],
    relations: {
      blueprints: [],
      knowledge: ["prompt-driven-video-generation-boundaries"],
      opportunities: ["structured-cinematic-prompt-opportunity", "gemini-direct-prompt-to-video-opportunity"],
    },
    productionState: {
      hasBlueprint: false,
      hasKnowledge: true,
      ...
      nextAction: "已完成第一手實測並取得影片佐證；下一步實測跨集角色一致性（不同對話分開生成是否仍為同一角色），驗證結果將決定是否晉升藍圖候選。",
    },
  },
];
