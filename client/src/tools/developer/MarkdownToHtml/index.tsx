...
const bands = [
  { key: "blank", range: "0 元素", label: { zh: "空白 (0 元素)", en: "Blank (0 elements)" }, desc: { zh: "尚未輸入或全空白,沒有標題、列表、連結等結構元素。建議從一個 H1 標題開始建立文件骨架,再補入段落。", en: "Empty or whitespace-only — no headings, lists, or links yet. Start with a single H1 to anchor the document, then add paragraphs." } },
  { key: "minimal", range: "1 – 5 元素", label: { zh: "極簡 (1-5 元素)", en: "Minimal (1-5 elements)" }, desc: { zh: "單頁短文 README 等級,通常一個 H1 + 幾段文字 + 1-2 個列表。SEO 上需要至少 1 個 H2 才能撐起階層,目前可能太薄。", en: "Single-page README level — typically one H1, a few paragraphs, and 1-2 lists. SEO needs at least one H2 to form a hierarchy; this is likely too thin." } },
  { key: "standard", range: "6 – 20 元素", label: { zh: "標準 (6-20 元素)", en: "Standard (6-20 elements)" }, desc: { zh: "標準部落格文章 / 文件頁面結構。H1 → 多個 H2 → 列表/連結 → 1-2 段程式碼。SEO 與閱讀體驗都健康,適合直接發佈。", en: "Standard blog post / docs page — H1 → multiple H2 → lists/links → 1-2 code blocks. Healthy for SEO and reading; ready to publish." } },
  { key: "rich", range: "21 – 50 元素", label: { zh: "豐富 (21-50 元素)", en: "Rich (21-50 elements)" }, desc: { zh: "教程、深度文章等級,有多層 H2/H3 階層、多段程式碼範例、明顯的列表結構。建議加入目錄(TOC)與錨點以利導覽。", en: "Tutorial / deep-dive level — multiple H2/H3 levels, several code examples, strong list structure. Add a TOC with anchors to aid navigation." } },
  { key: "dense", range: "51 – 120 元素", label: { zh: "密集 (51-120 元素)", en: "Dense (51-120 elements)" }, desc: { zh: "技術書籍章節 / API 參考密度。閱讀負擔開始上升,應拆分為多個獨立頁面或加入「快速跳轉」區塊。考慮分章節靜態化。", en: "Tech-book chapter / API reference density. Reading load rises — split into separate pages or add quick-jump navigation. Consider per-chapter static rendering." } },
  { key: "monster", range: "≥ 120 元素", label: { zh: "巨型 (≥ 120 元素)", en: "Monster (≥ 120 elements)" }, desc: { zh: "整本手冊或全站知識庫等級。單頁載入速度與 SEO 效益都會下滑;建議分割為多頁、加入搜尋與目錄、改用靜態網站生成器(Hugo/Docusaurus)。", en: "Full manual or wiki level. Single-page load and SEO suffer — split into multiple pages, add search and TOC, switch to a static-site generator (Hugo / Docusaurus)." } },
] as const;
...
const ui = {
  zh: {
    ...
    q1: "為什麼我的 *斜體* 沒被解析?", a1: "本工具用單顆 * 偵測斜體;若你的 * 兩側緊接其他字元(例如 `a*b*c`)會被視為文字而非斜體,這是 CommonMark 的「左/右側 flank rule」。標準寫法是 `a *b* c`(*周圍有空白)。專業版會完整實作 flank rule。",
