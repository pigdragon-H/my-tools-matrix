...
const bands = [
  { key: "every-second", range: "≥86400/天", label: { zh: "每秒級觸發", en: "Every-second" }, desc: { zh: "每天觸發 ≥86400 次,代表每秒都會跑一次。實務上幾乎不該排這個頻率;真要這麼即時,改用 message queue 或 event-driven 設計,而非 cron。", en: "≥86400 fires/day means the job runs every single second. Almost never appropriate; use a message queue or event-driven trigger instead of cron." } },
  { key: "every-minute", range: "1440–86399/天", label: { zh: "每分鐘級", en: "Every-minute" }, desc: { zh: "每分鐘觸發或更密。常見於健康檢查 ping 或快取重整;要注意 job 本身的執行時間必須遠小於 1 分鐘,否則會堆疊。", en: "Fires every minute or denser. Typical for health-check pings or cache refresh; ensure each run finishes in well under 1 minute or jobs will overlap." } },
  { key: "hourly", range: "24–1439/天", label: { zh: "每小時級", en: "Hourly" }, desc: { zh: "每小時級頻率,約每幾分鐘到每小時一次。報表彙總、log rotation、API rate-limit 重置都落在此。", en: "Hourly tier — anywhere from a few times per hour down to once per hour. Fits report aggregation, log rotation, and API rate-limit resets." } },
  { key: "daily", range: "1–23/天", label: { zh: "每日級", en: "Daily" }, desc: { zh: "每日 1 次到數次。最常見的批次窗口:夜間備份、ETL、訂閱結算、推播通知。錯峰避免與其他 daily job 同時跑。", en: "1–23 fires per day. The most common batch window: nightly backup, ETL, subscription billing, push notifications. Stagger to avoid daily-job collisions." } },
  { key: "weekly", range: "1–6/週", label: { zh: "每週級", en: "Weekly" }, desc: { zh: "每週幾次。週報、清理、整合測試、促銷活動推播。建議寫入文件「為什麼選這個 weekday」以利接手。", en: "A few times per week. Weekly digest, cleanup, integration tests, promo blasts. Document why this weekday so successors can adjust." } },
  { key: "rare", range: "<1/週", label: { zh: "罕見/年度", en: "Rare / yearly" }, desc: { zh: "每月、每季或每年一次。財報結算、年度密碼輪換、SSL 重簽。極低頻 job 必須有獨立監控,否則錯過難察覺。", en: "Once a month, quarter, or year. Financial close, annual key rotation, SSL renewal. Such low-frequency jobs need separate monitoring or misses go unnoticed." } },
] as const;
