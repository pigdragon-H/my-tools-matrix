...
const ui = {
  zh: {
    ...
    emotionConversionLayer: "情緒與轉換層", turnIntoPlan: "把壓縮整合進部署流程", conversionNote: "L9 會連動目前壓縮結果,顯示語言、原始與壓縮後大小,協助你判斷是否需要進一步用建構工具自動化壓縮。",
    progressInsight: "進度洞察卡", possibleTarget: "目前壓縮計畫", dailyGap: "原始位元組", weeklyTrend: "節省百分比", motivation: "動力卡", keepMomentum: "從單檔壓縮走向建構流程自動化",
    saveShareJourney: "儲存 / 分享", journeyTitle: "把壓縮結果帶進你的部署", journeyHint: "每次更換語言或貼上新程式碼時重新壓縮,並把結果複製到建構腳本、靜態資源或 CDN。",
    ...
    trustReferences: "信任聲明 · 相關工具 · 參考資料", trust: "信任聲明", trustText: "本工具僅在瀏覽器端壓縮,程式碼不會離開你的裝置;此為基礎壓縮,非完整建構工具。", relatedTools: "相關工具", relatedToolsText: "Base64 編碼器 · JSON 格式化 · HTML 編碼器 · Diff 比對器", references: "參考資料", referencesText: "Minification 最佳實務;HTML/CSS/JS 壓縮規則;Gzip 與 Brotli 傳輸壓縮;前端建構工具比較指南。",
    ...
    q3: "這個工具安全嗎？我的程式碼會上傳嗎？", a3: "完全安全。所有壓縮都在你的瀏覽器本機執行,程式碼不會上傳到任何伺服器,適合處理私有或內部程式碼。",
