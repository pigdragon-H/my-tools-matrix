...
const ui = {
  zh: {
    ...
    q1: "為什麼按下「比對」沒有任何 match？", a1: "可能原因：(1) Pattern 過於嚴格，例如沒加 `i` flag 而目標是大小寫不同；(2) 沒加 `g` flag 所以只能找到第一個（本工具預設加 g，若你手動清除可能漏掉）；(3) 字元類別寫錯，例如 `\\w` 不包含中文，遇到中文需用 `\\p{L}` + `u` flag；(4) Anchor 用法錯誤，例如 `^` 在多行模式（m flag）才會匹配每行起點。",
