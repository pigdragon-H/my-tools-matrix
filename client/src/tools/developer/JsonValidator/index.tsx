        ...
        <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
          ...
          <section className="grid gap-5 lg:grid-cols-[1fr_0.8fr]">
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">
              <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">{lang === "zh" ? "JSON 驗證旅程" : "JSON Validation Journey"}</p>
              <h2 className="mt-2 text-3xl font-black">{lang === "zh" ? "JSON 數據驗證，確保正確" : "Validate JSON, Ensure Correctness"}</h2>
              <div className="mt-6 space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-black">{lang === "zh" ? "輸入 JSON" : "Input JSON"}</h3>
                    <p className="mt-1 text-sm text-slate-600">{lang === "zh" ? "貼上 JSON 數據" : "Paste JSON data"}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 rounded-full bg-blue-100 px-3 py-1 text-xs font-black text-blue-700">{lang === "zh" ? "步驟 2" : "Step 2"}</div>
                  <div>
                    <h3 className="font-black">{lang === "zh" ? "驗證語法" : "Validate Syntax"}</h3>
                    <p className="mt-1 text-sm text-slate-600">{lang === "zh" ? "檢查格式" : "Check format"}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 rounded-full bg-blue-100 px-3 py-1 text-xs font-black text-blue-700">{lang === "zh" ? "步驟 3" : "Step 3"}</div>
                  <div>
                    <h3 className="font-black">{lang === "zh" ? "驗證結果" : "Validation Result"}</h3>
                    <p className="mt-1 text-sm text-slate-600">{lang === "zh" ? "驗證是否成功" : "Check if valid"}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-black">{lang === "zh" ? "修正使用" : "Fix & Use"}</h3>
                    <p className="mt-1 text-sm text-slate-600">{lang === "zh" ? "修正錯誤" : "Fix errors"}</p>
                  </div>
                </div>
              </div>
            </div>
