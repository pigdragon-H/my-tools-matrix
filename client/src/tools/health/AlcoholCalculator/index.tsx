// @profile B
// Profile B · 計算機-YMYL · Alcohol Calculator - standard drinks and estimated blood alcohol content（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<0.02%", label: { zh: "幾乎無影響", en: "Negligible" }, desc: { zh: "血液酒精濃度極低,對行為影響不明顯。", en: "A very low blood alcohol level with little noticeable effect." } },
  { key: "normal", range: "0.02-0.03%", label: { zh: "輕微", en: "Slight" }, desc: { zh: "輕微放鬆感,仍接近法定駕駛標準。", en: "A slight, relaxed feeling while staying near the legal driving line." } },
  { key: "notable", range: "0.03-0.05%", label: { zh: "接近上限", en: "Near limit" }, desc: { zh: "已達或接近法定駕駛上限,不建議開車。", en: "At or near the legal driving limit; driving is not advised." } },
  { key: "high", range: "0.05-0.08%", label: { zh: "明顯影響", en: "Impaired" }, desc: { zh: "判斷與反應明顯變慢,務必避免駕駛。", en: "Judgement and reaction are clearly slowed; do not drive." } },
  { key: "major", range: "0.08-0.15%", label: { zh: "嚴重影響", en: "Heavy" }, desc: { zh: "協調與意識明顯受損,風險很高。", en: "Coordination and awareness are heavily impaired; risk is high." } },
  { key: "executive", range: "0.15%+", label: { zh: "危險", en: "Dangerous" }, desc: { zh: "酒精濃度危險,可能危及健康,建議就醫。", en: "A dangerous alcohol level that may threaten health; seek care." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "酒精濃度計算機",
    subtitle: "由飲用量與酒精濃度,估算標準杯數與血液酒精濃度。",
    intro: "輸入飲用量、酒精濃度、體重與飲後經過時數,本工具估算純酒精克數、標準杯數與血液酒精濃度 (BAC),並概估代謝至清醒所需時間,協助你了解飲酒對身體的影響。此為一般教育用途的估算,實際數值因個人體質而異,切勿作為駕駛判斷依據。",
    trustNoteLabel: "注意事項:",
    ...
    calculator: "計算機",
    participants: "飲用量 (毫升)",
    averageHourlyRate: "酒精濃度 (%)",
    durationHours: "體重 (公斤)",
    meetingsPerMonth: "飲後經過時數",
  ...
  en: {
    ...
    subtitle: "Estimate standard drinks and blood alcohol content from volume and ABV.",
    intro: "Enter the volume, alcohol by volume, body weight and hours since drinking; the tool estimates grams of pure alcohol, standard drinks and blood alcohol content (BAC), plus an approximate time to sober, helping you understand alcohol's effect. This is an educational estimate that varies by individual and must never be used to decide whether to drive.",
    trustNoteLabel: "Note:",
    ...
    participants: "Volume (ml)",
    averageHourlyRate: "Alcohol by volume (%)",
    durationHours: "Body weight (kg)",
    meetingsPerMonth: "Hours since drinking",
...
export default function AlcoholCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("5");
  ...
  const result = useMemo(() => {
    ...
    const volumeMl = v1; const abvPercent = v2; const bodyWeight = v3; const hoursElapsed = v4;
    const gramsAlcohol = volumeMl * (abvPercent / 100) * 0.789;
    const standardDrinks = gramsAlcohol / 10;
    const r = 0.68;
    const bacRaw = bodyWeight > 0 ? (gramsAlcohol / (bodyWeight * 1000 * r)) * 100 : 0;
    const bac = Math.max(0, bacRaw - hoursElapsed * 0.015);
    const soberHours = bac > 0 ? bac / 0.015 : 0;
    return { bac, standardDrinks, gramsAlcohol, soberHours };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.bac, 3);
  const monthlyDisplay = fmt(result.standardDrinks, 1);

  function fillSolid() { setUnit("metric"); setParticipants("500"); setAverageHourlyRate("5"); setDurationHours("70"); setMeetingsPerMonth("1"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("1500"); setAverageHourlyRate("5"); setDurationHours("70"); setMeetingsPerMonth("1"); }

  const activeBand = bands.find(b => {
    const r = result.bac;
    if (r < 0.02) return b.key === "tiny";
    if (r < 0.03) return b.key === "normal";
    if (r < 0.05) return b.key === "notable";
    if (r < 0.08) return b.key === "high";
    if (r < 0.15) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fff1f2 0%,#ffe4e6 55%,#fce7f3 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "估計血液酒精濃度" : "estimated BAC"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.standardDrinks, 1)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.soberHours, 1)} hr</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">0.03%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "法定駕駛上限" : "Driving limit"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">0.05%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "明顯影響" : "Impaired"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "%" : "%"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "杯" : "drinks"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "標準杯數" : "Standard drinks"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.standardDrinks, 1)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "純酒精克數" : "Pure alcohol"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.gramsAlcohol, 0)}</p><p className="text-sm font-bold text-blue-700">g</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "代謝時數" : "Hours to sober"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.soberHours, 1)}</p><p className="text-sm font-bold text-slate-700"> hr</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="alcohol-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "估計血液酒精濃度" : "estimated BAC"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入飲用量" : "Enter the volume", note: t.bmrStep }, { label: lang === "zh" ? "乘以酒精濃度" : "Apply the ABV", note: t.deficitStep }, { label: lang === "zh" ? "依體重換算" : "Adjust for body weight", note: t.trendStep }, { label: lang === "zh" ? "得血液酒精濃度" : "Get the estimated BAC", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="alcohol-calculator-faq" position="inline" /></section>
