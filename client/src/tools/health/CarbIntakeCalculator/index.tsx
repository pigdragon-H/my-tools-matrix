// @profile B
// Profile B · 計算機-YMYL · Carb Intake Calculator - daily carbohydrate grams from calories and ratio（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<1 g/kg", label: { zh: "極低碳", en: "Very low" }, desc: { zh: "每公斤碳水很低,接近低碳飲食。", en: "Very low carbs per kg, near a low-carb diet." } },
  { key: "normal", range: "1-2 g/kg", label: { zh: "低碳", en: "Low" }, desc: { zh: "偏低的碳水攝取,適合減脂期。", en: "A lower carb intake suited to fat-loss phases." } },
  { key: "notable", range: "2-4 g/kg", label: { zh: "一般", en: "Moderate" }, desc: { zh: "一般活動量的碳水攝取範圍。", en: "A moderate intake for general activity." } },
  { key: "high", range: "4-6 g/kg", label: { zh: "偏高", en: "High" }, desc: { zh: "偏高碳水,適合較大訓練量。", en: "A higher carb intake for larger training loads." } },
  { key: "major", range: "6-8 g/kg", label: { zh: "很高", en: "Very high" }, desc: { zh: "高碳水,常見於耐力運動。", en: "A high carb intake common in endurance sport." } },
  { key: "executive", range: "8+ g/kg", label: { zh: "極高", en: "Extreme" }, desc: { zh: "極高碳水,通常為賽前肝醣超補。", en: "An extreme intake, typical of carb loading." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "碳水攝取計算機",
    subtitle: "由每日熱量與碳水比例,計算建議的碳水克數。",
    intro: "輸入每日總熱量、碳水比例、體重與每日餐數,本工具以每克碳水 4 大卡換算,計算每日碳水克數、每餐分配與每公斤攝取量,協助你規劃飲食結構。此為一般教育用途的估算,實際需求依運動量與目標而異。",
    trustNoteLabel: "注意事項:",
  ...
  en: {
    ...
    subtitle: "Compute recommended carbohydrate grams from calories and ratio.",
    intro: "Enter daily calories, carb ratio, body weight and meals per day; using 4 kcal per gram the tool computes daily carbohydrate grams, per-meal distribution and intake per kilogram, helping you plan your diet structure. This is an educational estimate that varies with activity and goals.",
    trustNoteLabel: "Note:",
    ...
    averageHourlyRate: "Carb ratio (%)",
    durationHours: "Weight (kg)",
...
export default function CarbIntakeCalculator() {
  ...
  const result = useMemo(() => {
    ...
    const dailyKcal = v1; const carbPercent = v2; const weightKg = v3; const meals = v4;
    const carbKcal = dailyKcal * (carbPercent / 100);
    const carbGrams = carbKcal / 4;
    const perKg = weightKg > 0 ? carbGrams / weightKg : 0;
    const perMeal = meals > 0 ? carbGrams / meals : 0;
    return { carbGrams, perKg, perMeal, carbKcal };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.carbGrams, 0);
  const monthlyDisplay = fmt(result.perMeal, 0);

  function fillSolid() { setUnit("metric"); setParticipants("2200"); setAverageHourlyRate("50"); setDurationHours("70"); setMeetingsPerMonth("3"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("3000"); setAverageHourlyRate("60"); setDurationHours("70"); setMeetingsPerMonth("3"); }

  const activeBand = bands.find(b => {
    const r = result.perKg;
    if (r < 1) return b.key === "tiny";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fffbeb 0%,#fef3c7 55%,#ffedd5 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "每日碳水克數" : "daily carbs"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.perMeal, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.perKg, 1)} g/kg</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">3</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "一般活動" : "General"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">6</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "耐力訓練" : "Endurance"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "克" : "g"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "克" : "g"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "碳水熱量" : "Carb calories"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.carbKcal, 0)}</p><p className="text-sm font-bold text-emerald-700"> kcal</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "每餐碳水" : "Per meal"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.perMeal, 0)}</p><p className="text-sm font-bold text-blue-700"> g</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "每公斤碳水" : "Per kg"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.perKg, 1)}</p><p className="text-sm font-bold text-slate-700"> g</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="carb-intake-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "每日碳水克數" : "daily carbs"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入每日總熱量" : "Enter daily calories", note: t.bmrStep }, { label: lang === "zh" ? "設定碳水比例" : "Set the carb ratio", note: t.deficitStep }, { label: lang === "zh" ? "換算碳水克數" : "Convert to grams", note: t.trendStep }, { label: lang === "zh" ? "得每日碳水攝取" : "Get daily carb intake", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="carb-intake-calculator-faq" position="inline" /></section>
