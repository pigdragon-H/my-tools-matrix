...
const ui = {
  zh: {
    ...
    metric: "kg 換算", imperial: "lb 換算", exampleCards: "範例卡", baselineExample: "500 kcal 標準赤字", activeExample: "維持熱量示範", dailyLabel: "每日", intakeLabel: "攝取", baselineExampleNote: "TDEE 2400 · 攝取 1900 · 7 天", activeExampleNote: "TDEE 2200 · 攝取 2200 · 14 天", flowDemo: "0 deficit", calculator: "計算機",
    tdee: "TDEE 維持熱量（kcal/day）", intake: "每日攝取（kcal/day）", days: "追蹤天數", mode: "目標模式", deficit: "赤字", maintenanceMode: "維持", surplus: "盈餘",
    resultCard: "熱量赤字結果", unit: "kcal/day", primaryValue: "主要數值", maintenanceTarget: "每週熱量差", actionTarget: "靜態體重趨勢", estimatedTdee: "每日赤字", maintenance: "每週赤字", fatLossTarget: "預估變化", deficitFormulaLabel: "TDEE − 攝取",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "熱量規劃的下一步工具", premiumTitle: "PRO 熱量赤字追蹤包", premiumText: "解鎖 TDEE 連動、週平均攝取、動態體重趨勢與個人化赤字報告。", feat1: "TDEE", feat2: "攝取量", feat3: "趨勢", feat4: "報表",
  ...
  en: {
    ...
    metric: "kg conversion", imperial: "lb conversion", exampleCards: "Example cards", baselineExample: "500 kcal standard deficit", activeExample: "Maintenance demo", dailyLabel: "Daily", intakeLabel: "Intake", baselineExampleNote: "TDEE 2400 · intake 1900 · 7 days", activeExampleNote: "TDEE 2200 · intake 2200 · 14 days", flowDemo: "0 deficit", calculator: "Calculator",
    ...
    resultCard: "Calorie Deficit Result", unit: "kcal/day", primaryValue: "Primary Value", maintenanceTarget: "Weekly calorie balance", actionTarget: "Static weight trend", estimatedTdee: "Daily deficit", maintenance: "Weekly deficit", fatLossTarget: "Estimated change", deficitFormulaLabel: "TDEE − intake",
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next tools for calorie planning", premiumTitle: "PRO Calorie Deficit Tracking Pack", premiumText: "Unlock TDEE linking, weekly average intake, dynamic weight trend, and personalized deficit reports.", feat1: "TDEE", feat2: "Intake", feat3: "Trend", feat4: "Reports",
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#dcfce7,_#f8fafc_45%,_#e0f2fe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-emerald-100 bg-white/90 p-6 shadow-2xl shadow-emerald-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-emerald-600 p-5 text-white"><div className="text-xs font-bold uppercase text-emerald-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{dailyDisplay}</div><div className="text-sm font-bold text-emerald-100">{t.unit}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{result?.mode === "deficit" ? t.deficit : result?.mode === "surplus" ? t.surplus : t.maintenanceMode}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">TDEE</div><div className="font-black">{tdee}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.intakeLabel}</div><div className="font-black">{intake}</div></div></div><button onClick={fillDeficit} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillMaintenance} className="mt-3 w-full rounded-2xl border border-orange-200 bg-orange-50 px-5 py-4 text-sm font-black text-orange-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillDeficit} className="w-full rounded-2xl border border-emerald-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">500</span></div><p className="mt-2 text-sm text-slate-600">{t.baselineExampleNote}</p></button><button onClick={fillMaintenance} className="w-full rounded-2xl border border-orange-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-700">{t.flowDemo}</span></div><p className="mt-2 text-sm text-slate-600">{t.activeExampleNote}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-emerald-400 to-blue-600" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{dailyDisplay}</div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.unit}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.mode}</div><div className="mt-1 text-xl font-black">{result?.mode === "deficit" ? t.deficit : result?.mode === "surplus" ? t.surplus : t.maintenanceMode}</div><div className="mt-1 text-xs text-slate-300">{t.deficitFormulaLabel}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.primaryValue}</div><div className="mt-1 text-xs font-black uppercase text-blue-700">{t.estimatedTdee}</div><p className="mt-2 text-3xl font-black text-blue-950">{dailyDisplay}</p><p className="text-sm font-bold text-blue-700">kcal</p></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.maintenanceTarget}</div><div className="mt-1 text-xs font-black uppercase text-emerald-700">{t.maintenance}</div><p className="mt-2 text-3xl font-black text-emerald-950">{weeklyDisplay}</p><p className="text-sm font-bold text-emerald-700">kcal/week</p></div><div className="rounded-2xl bg-orange-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-500">{t.actionTarget}</div><div className="mt-1 text-xs font-black uppercase text-orange-700">{t.fatLossTarget}</div><p className="mt-2 text-3xl font-black text-orange-950">{weightDisplay}</p><p className="text-sm font-bold text-orange-700">{unit}/week</p></div></div></div></article>
