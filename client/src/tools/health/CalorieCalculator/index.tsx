// @profile B
// Profile B · 計算機-YMYL · Calorie Calculator - daily calorie needs from BMR and activity（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<1600", label: { zh: "很低", en: "Very low" }, desc: { zh: "每日熱量需求很低,常見於小體型或久坐。", en: "A very low daily need, common in small or sedentary builds." } },
  { key: "normal", range: "1600-2000", label: { zh: "偏低", en: "Low" }, desc: { zh: "略低於一般成人的每日熱量需求。", en: "Slightly below a typical adult daily need." } },
  { key: "notable", range: "2000-2400", label: { zh: "一般", en: "Typical" }, desc: { zh: "接近一般成人的每日熱量需求。", en: "Close to a typical adult daily need." } },
  { key: "high", range: "2400-2800", label: { zh: "偏高", en: "High" }, desc: { zh: "需求偏高,常見於活躍或體型較大者。", en: "A higher need, common in active or larger builds." } },
  { key: "major", range: "2800-3200", label: { zh: "很高", en: "Very high" }, desc: { zh: "每日需求很高,常見於高強度訓練者。", en: "A very high daily need, common with intense training." } },
  { key: "executive", range: "3200+", label: { zh: "極高", en: "Extreme" }, desc: { zh: "需求極高,建議由專業營養評估規劃。", en: "An extreme need; professional nutrition planning is advised." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "熱量計算機",
    subtitle: "由體重、身高、年齡與活動量,計算每日熱量需求。",
    intro: "輸入體重、身高、年齡與活動係數,本工具以 Mifflin-St Jeor 公式計算基礎代謝率 (BMR),再乘以活動係數得到每日維持熱量,並提供減重與增重的建議熱量區間,協助你規劃飲食目標。此為一般教育用途的估算,實際需求因人而異。",
    trustNoteLabel: "注意事項:",
    ...
    calculator: "計算機",
    participants: "體重 (公斤)",
    averageHourlyRate: "身高 (公分)",
    durationHours: "年齡",
    meetingsPerMonth: "活動係數",
  ...
  en: {
    ...
    subtitle: "Compute daily calorie needs from weight, height, age and activity.",
    intro: "Enter weight, height, age and an activity factor; using the Mifflin-St Jeor equation the tool computes your basal metabolic rate (BMR), multiplies it by the activity factor for daily maintenance calories, and suggests intake ranges for loss and gain, helping you plan dietary goals. This is an educational estimate that varies by individual.",
    trustNoteLabel: "Note:",
    ...
    averageHourlyRate: "Height (cm)",
    durationHours: "Age",
    meetingsPerMonth: "Activity factor",
...
export default function CalorieCalculator() {
  ...
  const [participants, setParticipants] = useState("70");
  const [averageHourlyRate, setAverageHourlyRate] = useState("170");
  const [durationHours, setDurationHours] = useState("30");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("1.55");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const weightKg = v1; const heightCm = v2; const age = v3; const activityFactor = v4;
    const bmr = 10 * weightKg + 6.25 * heightCm - 5 * age + 5;
    const maintenance = bmr * activityFactor;
    const mildLoss = maintenance - 500;
    const surplus = maintenance + 300;
    return { maintenance, bmr, mildLoss, surplus };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.maintenance, 0);
  const monthlyDisplay = fmt(result.bmr, 0);

  function fillSolid() { setUnit("metric"); setParticipants("70"); setAverageHourlyRate("170"); setDurationHours("30"); setMeetingsPerMonth("1.55"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("85"); setAverageHourlyRate("180"); setDurationHours("30"); setMeetingsPerMonth("1.725"); }

  const activeBand = bands.find(b => {
    const r = result.maintenance;
    if (r < 1600) return b.key === "tiny";
    if (r < 2000) return b.key === "normal";
    if (r < 2400) return b.key === "notable";
    if (r < 2800) return b.key === "high";
    if (r < 3200) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fffbeb 0%,#fef3c7 55%,#ffedd5 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "每日維持熱量" : "daily maintenance"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.bmr, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.mildLoss, 0)} kcal</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">2000</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "一般成人" : "Typical adult"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">2500</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "活躍族群" : "Active"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "大卡" : "kcal"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "大卡" : "kcal"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "基礎代謝率" : "BMR"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.bmr, 0)}</p><p className="text-sm font-bold text-emerald-700"> kcal</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "減重熱量" : "Mild loss"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.mildLoss, 0)}</p><p className="text-sm font-bold text-blue-700"> kcal</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "增重熱量" : "Surplus"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.surplus, 0)}</p><p className="text-sm font-bold text-slate-700"> kcal</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="calorie-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入體重與身高" : "Enter weight and height", note: t.bmrStep }, { label: lang === "zh" ? "計算基礎代謝率" : "Compute the BMR", note: t.deficitStep }, { label: lang === "zh" ? "乘以活動係數" : "Apply the activity factor", note: t.trendStep }, { label: lang === "zh" ? "得每日熱量需求" : "Get the daily calorie need", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="calorie-calculator-faq" position="inline" /></section>
