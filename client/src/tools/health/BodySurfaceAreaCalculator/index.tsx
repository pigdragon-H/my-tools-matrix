// @profile B
// Profile B · 計算機-YMYL · Body Surface Area Calculator - Mosteller and DuBois BSA from height and weight（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<1.3", label: { zh: "很小", en: "Very small" }, desc: { zh: "體表面積偏小,常見於兒童或小體型。", en: "A small body surface area, common in children or small builds." } },
  { key: "normal", range: "1.3-1.6", label: { zh: "偏小", en: "Small" }, desc: { zh: "略小於成人平均的體表面積。", en: "Slightly below the adult average surface area." } },
  { key: "notable", range: "1.6-1.8", label: { zh: "平均", en: "Average" }, desc: { zh: "接近成人平均的體表面積。", en: "Close to the adult average surface area." } },
  { key: "high", range: "1.8-2.0", label: { zh: "偏大", en: "Large" }, desc: { zh: "高於平均,常見於高大或健壯體型。", en: "Above average, common in tall or muscular builds." } },
  { key: "major", range: "2.0-2.3", label: { zh: "很大", en: "Very large" }, desc: { zh: "體表面積很大,用藥劑量計算需留意。", en: "A very large surface area; dose calculations need care." } },
  { key: "executive", range: "2.3+", label: { zh: "極大", en: "Extra large" }, desc: { zh: "體表面積極大,建議由專業評估。", en: "An extra-large surface area; professional review is advised." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "體表面積計算機",
    subtitle: "由身高與體重,以 Mosteller 與 DuBois 公式計算體表面積。",
    intro: "輸入身高、體重與劑量係數,本工具以 Mosteller 與 DuBois 兩種公式計算體表面積 (BSA),並依體表面積估算用藥劑量,常用於醫療與營養評估。此為一般教育用途的估算,實際臨床用藥請依專業醫療人員指示。",
    trustNoteLabel: "注意事項:",
    ...
    calculator: "計算機",
    participants: "身高 (公分)",
    averageHourlyRate: "體重 (公斤)",
    durationHours: "劑量係數 (mg/m²)",
    meetingsPerMonth: "備用欄位",
  ...
  en: {
    ...
    subtitle: "Compute body surface area with the Mosteller and DuBois formulas.",
    intro: "Enter height, weight and a dose factor; the tool computes body surface area (BSA) with both the Mosteller and DuBois formulas and estimates a dose from the BSA, as commonly used in clinical and nutrition assessment. This is an educational estimate; follow professional guidance for any clinical dosing.",
    trustNoteLabel: "Note:",
    ...
    averageHourlyRate: "Weight (kg)",
    durationHours: "Dose factor (mg/m2)",
    meetingsPerMonth: "Reserved",
...
export default function BodySurfaceAreaCalculator() {
  ...
  const [participants, setParticipants] = useState("170");
  ...
  const result = useMemo(() => {
    ...
    const heightCm = v1; const weightKg = v2; const doseFactor = v3;
    const mosteller = Math.sqrt((heightCm * weightKg) / 3600);
    const dubois = 0.007184 * Math.pow(heightCm, 0.725) * Math.pow(weightKg, 0.425);
    const dose = mosteller * doseFactor;
    return { mosteller, dubois, dose, heightCm };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.mosteller, 2);
  const monthlyDisplay = fmt(result.dubois, 2);

  function fillSolid() { setUnit("metric"); setParticipants("170"); setAverageHourlyRate("70"); setDurationHours("100"); setMeetingsPerMonth("0"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("185"); setAverageHourlyRate("95"); setDurationHours("100"); setMeetingsPerMonth("0"); }

  const activeBand = bands.find(b => {
    const r = result.mosteller;
    if (r < 1.3) return b.key === "tiny";
    if (r < 1.6) return b.key === "normal";
    if (r < 1.8) return b.key === "notable";
    if (r < 2.0) return b.key === "high";
    if (r < 2.3) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#f0fdfa 0%,#ccfbf1 55%,#cffafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "Mosteller 體表面積" : "Mosteller BSA"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.dubois, 2)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.dose, 0)} mg</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">1.7</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "成人平均" : "Adult average"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">2.0</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "體型較大" : "Larger build"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "m²" : "m2"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "m²" : "m2"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "Mosteller BSA" : "Mosteller BSA"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.mosteller, 2)}</p><p className="text-sm font-bold text-emerald-700"> m2</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "DuBois BSA" : "DuBois BSA"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.dubois, 2)}</p><p className="text-sm font-bold text-blue-700"> m2</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "估計劑量" : "Estimated dose"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.dose, 0)}</p><p className="text-sm font-bold text-slate-700"> mg</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="body-surface-area-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "Mosteller 體表面積" : "Mosteller BSA"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入身高" : "Enter the height", note: t.bmrStep }, { label: lang === "zh" ? "輸入體重" : "Enter the weight", note: t.deficitStep }, { label: lang === "zh" ? "套用 Mosteller 公式" : "Apply the Mosteller formula", note: t.trendStep }, { label: lang === "zh" ? "得體表面積" : "Get the body surface area", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="body-surface-area-calculator-faq" position="inline" /></section>
