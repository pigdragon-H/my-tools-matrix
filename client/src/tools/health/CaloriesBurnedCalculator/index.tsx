// @profile B
// Profile B · 計算機-YMYL · Calories Burned Calculator - MET-based energy expenditure for an activity（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<150", label: { zh: "很低", en: "Very low" }, desc: { zh: "單次消耗很低,屬輕度活動。", en: "A very low burn from light activity." } },
  { key: "normal", range: "150-300", label: { zh: "輕度", en: "Light" }, desc: { zh: "輕度運動的熱量消耗。", en: "A light-exercise calorie burn." } },
  { key: "notable", range: "300-500", label: { zh: "中等", en: "Moderate" }, desc: { zh: "中等強度運動的常見消耗。", en: "A common burn from moderate exercise." } },
  { key: "high", range: "500-750", label: { zh: "較高", en: "High" }, desc: { zh: "較高的單次熱量消耗。", en: "A higher single-session burn." } },
  { key: "major", range: "750-1000", label: { zh: "很高", en: "Very high" }, desc: { zh: "高強度或長時間運動的消耗。", en: "A burn from intense or long sessions." } },
  { key: "executive", range: "1000+", label: { zh: "極高", en: "Extreme" }, desc: { zh: "極高的單次消耗,注意補水與恢復。", en: "An extreme burn; mind hydration and recovery." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "運動消耗熱量計算機",
    subtitle: "由 MET 強度、體重與時間,計算運動消耗的熱量。",
    intro: "輸入活動的 MET 強度值、體重、運動時間與每週次數,本工具以 MET 公式計算單次與每週消耗的熱量,並換算等值脂肪克數,協助你評估運動的能量效益。此為一般教育用途的估算,實際消耗因個人代謝與運動方式而異。",
    trustNoteLabel: "注意事項:",
    ...
    flowDemo: "儲蓄率",
    calculator: "計算機",
    participants: "MET 強度值",
    averageHourlyRate: "體重 (公斤)",
    durationHours: "運動時間 (分鐘)",
    meetingsPerMonth: "每週次數",
  ...
  en: {
    ...
    subtitle: "Compute calories burned from MET intensity, weight and duration.",
    intro: "Enter the activity MET value, body weight, duration and weekly sessions; using the MET formula the tool computes calories burned per session and per week and converts to an equivalent fat mass, helping you gauge the energy benefit of exercise. This is an educational estimate that varies by metabolism and exercise style.",
    trustNoteLabel: "Note:",
    ...
    participants: "MET value",
    averageHourlyRate: "Weight (kg)",
    durationHours: "Duration (minutes)",
    meetingsPerMonth: "Sessions per week",
...
export default function CaloriesBurnedCalculator() {
  ...
  const result = useMemo(() => {
    ...
    const metValue = v1; const weightKg = v2; const minutes = v3; const sessions = v4;
    const perMinute = (metValue * 3.5 * weightKg) / 200;
    const burned = perMinute * minutes;
    const weekly = burned * sessions;
    const fatGrams = burned / 7.7;
    return { burned, weekly, perMinute, fatGrams };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.burned, 0);
  const monthlyDisplay = fmt(result.weekly, 0);

  function fillSolid() { setUnit("metric"); setParticipants("8"); setAverageHourlyRate("70"); setDurationHours("45"); setMeetingsPerMonth("4"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("12"); setAverageHourlyRate("85"); setDurationHours("60"); setMeetingsPerMonth("4"); }

  const activeBand = bands.find(b => {
    const r = result.burned;
    if (r < 150) return b.key === "tiny";
    if (r < 300) return b.key === "normal";
    if (r < 500) return b.key === "notable";
    if (r < 750) return b.key === "high";
    if (r < 1000) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fffbeb 0%,#fef3c7 55%,#ffedd5 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "本次消耗熱量" : "calories burned"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.weekly, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.fatGrams, 0)} g</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">300</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "中等運動" : "Moderate"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">600</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "高強度" : "Intense"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "大卡" : "kcal"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "大卡" : "kcal"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "每分鐘消耗" : "Per minute"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.perMinute, 1)}</p><p className="text-sm font-bold text-emerald-700"> kcal</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "每週消耗" : "Weekly burn"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.weekly, 0)}</p><p className="text-sm font-bold text-blue-700"> kcal</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "等值脂肪" : "Fat equivalent"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.fatGrams, 0)}</p><p className="text-sm font-bold text-slate-700"> g</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="calories-burned-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "本次消耗熱量" : "calories burned"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "選擇活動 MET 值" : "Choose the activity MET", note: t.bmrStep }, { label: lang === "zh" ? "輸入體重" : "Enter the body weight", note: t.deficitStep }, { label: lang === "zh" ? "乘以運動時間" : "Multiply by the duration", note: t.trendStep }, { label: lang === "zh" ? "得消耗熱量" : "Get the calories burned", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="calories-burned-calculator-faq" position="inline" /></section>
