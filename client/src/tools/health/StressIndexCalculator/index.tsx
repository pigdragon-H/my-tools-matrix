...
const ui = {
  zh: {
    ...
    intro: "Stress Index Calculator 依據睡眠品質、工作負荷、放鬆頻率、身體症狀數與情緒低落天數，累加成一個概念性的壓力指數分數與等級，協助您進行自我覺察。這是教育性自評，不是臨床診斷工具。",
    trustNoteLabel: "重要聲明：", trustNote: "本工具僅為教育性自我覺察參考，無法診斷焦慮、憂鬱或任何身心狀況。若您長期感到難以承受、情緒低落或有自我傷害念頭，請立即聯繫專業心理師、醫師或當地危機支援專線。",
    ...
    examplesCalculator: "範例 → 計算機", enterValues: "輸入您的狀況", examplesHelper: "先用範例理解分數怎麼算，再改成您自己的睡眠、負荷與情緒狀態。", metric: "標準模式", imperial: "簡易模式",
    exampleCards: "範例卡", baselineExample: "低壓範例", activeExample: "高壓範例", indexLabel: "指數", baselineExampleNote: "睡眠佳 · 低負荷 · 放鬆 5 次", activeExampleNote: "睡眠差 · 高負荷 · 放鬆 0 次", calculator: "計算機",
    ...
    resultCard: "壓力指數評估結果", primaryValue: "壓力指數", quickScoreLabel: "快速分數", maintenanceTarget: "分數", maintenance: "壓力指數分數", actionTarget: "等級", fatLossTarget: "壓力等級",
    resultIntelligence: "結果解讀", tdeeMatrix: "六格壓力等級判讀矩陣", tdeeMatrixNote: "L7 固定六格，將分數對應到常見壓力區間；這是自我覺察參考，不是醫療或心理診斷。",
    scenarioLayer: "情境比較", scenarioTitle: "不同生活情境下的壓力對照", scenarioNote: "L8 將您目前的分數與低壓、典型與高壓三種情境並列，幫您理解相對位置。",
    ...
    knowledge: "知識層", knowledgeTitle: "關於壓力與身心韌性，您該知道的事",
    ...
    q2: "資料會被儲存嗎？", a2: "不會。所有計算都在您的瀏覽器本機完成，不會上傳或保存個人資料。",
    q3: "分數有絕對標準嗎？", a3: "沒有。不同量表計分方式不同，本工具的區間僅作概念性參考。",
    q4: "壓力一定是壞事嗎？", a4: "適度壓力可提升專注與表現，長期過高且無法恢復的壓力才需要特別注意。",
    q5: "我覺得很痛苦但分數不高怎麼辦？", a5: "請相信自己的感受並尋求協助，工具分數不能取代您的主觀經驗與專業判斷。",
    q6: "可以給家人朋友用嗎？", a6: "可以，但同樣僅供自我覺察；任何持續或嚴重的困擾都應交由專業人員評估。",
    affiliate: "推薦工具", affiliateTitle: "搭配使用的相關工具",
    premiumTitle: "Pro 進階壓力洞察", premiumText: "解鎖多日壓力趨勢圖、放鬆習慣追蹤與個人化覺察報告，協助長期觀察身心節奏。", feat1: "趨勢分析", feat2: "記錄追蹤", feat3: "習慣追蹤", feat4: "報表",
  ...
  en: {
    ...
    exampleCards: "Example cards", baselineExample: "Low-stress example", activeExample: "High-stress example", indexLabel: "Index", baselineExampleNote: "Good sleep · Low load · 5x relax", activeExampleNote: "Poor sleep · High load · 0x relax", calculator: "Calculator",
    ...
    resultCard: "Stress Index Result", primaryValue: "Stress index", quickScoreLabel: "Quick score", maintenanceTarget: "Score", maintenance: "Stress index score", actionTarget: "Band", fatLossTarget: "Stress band",
    ...
    premiumTitle: "Pro Stress Insights", premiumText: "Unlock multi-day stress trend charts, relaxation-habit tracking and a personalized awareness report to observe your mind-body rhythm over time.", feat1: "Trends", feat2: "Logging", feat3: "Habits", feat4: "Report",
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">{/* L4-InputGuidance */}
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillStandard} className="w-full rounded-2xl border border-emerald-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">3</span></div><p className="mt-2 text-sm text-slate-600">{t.baselineExampleNote}</p></button><button onClick={fillCut} className="w-full rounded-2xl border border-orange-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-700">24</span></div><p className="mt-2 text-sm text-slate-600">{t.activeExampleNote}</p></button></div></div>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-emerald-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9-Emotion-Upper */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black uppercase text-slate-500">{t.indexLabel}</div><div className="mt-1 text-3xl font-black">{scoreDisplay}</div></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-xs font-black uppercase text-blue-600">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-blue-950">{bandLabel}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-emerald-950">{fmt(result.relaxN, 0)}</div></div></div></article>
