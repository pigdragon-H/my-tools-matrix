// @profile B
// Profile B · 計算機-YMYL · Fat Loss Calculator - time and rate to lose fat from a daily deficit（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<0.25 kg/wk", label: { zh: "很緩", en: "Very slow" }, desc: { zh: "每週減脂很慢,容易維持。", en: "A very slow weekly loss, easy to sustain." } },
  { key: "normal", range: "0.25-0.45 kg/wk", label: { zh: "緩和", en: "Gentle" }, desc: { zh: "溫和的減脂速度。", en: "A gentle fat-loss pace." } },
  { key: "notable", range: "0.45-0.7 kg/wk", label: { zh: "穩健", en: "Steady" }, desc: { zh: "穩健且常見的減脂速度。", en: "A steady, common fat-loss pace." } },
  { key: "high", range: "0.7-0.9 kg/wk", label: { zh: "積極", en: "Aggressive" }, desc: { zh: "較快的減脂速度,需注意營養。", en: "A faster pace; mind nutrition quality." } },
  { key: "major", range: "0.9-1.2 kg/wk", label: { zh: "很積極", en: "Very fast" }, desc: { zh: "速度偏快,建議專業監督。", en: "A fast pace; professional oversight is advised." } },
  { key: "executive", range: "1.2+ kg/wk", label: { zh: "過快", en: "Too fast" }, desc: { zh: "減脂過快,不建議,請放緩。", en: "Too fast to be advisable; slow the pace." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "減脂計算機",
    subtitle: "由想減脂肪量與每日缺口,計算達標所需時間與速度。",
    intro: "輸入想減去的脂肪量、每日熱量缺口與起始體重,本工具以每公斤脂肪約 7700 大卡換算,計算達標所需天數、週數與每週減脂速度,協助你設定合理的減脂時程。此為一般教育用途的估算,實際成效因代謝與執行而異。",
    trustNoteLabel: "注意事項:",
    ...
    flowDemo: "儲蓄率",
    calculator: "計算機",
    participants: "想減脂肪 (公斤)",
    averageHourlyRate: "每日熱量缺口 (大卡)",
    durationHours: "起始體重 (公斤)",
  ...
  en: {
    ...
    subtitle: "Compute the time and pace to lose fat from a daily deficit.",
    intro: "Enter the amount of fat to lose, your daily calorie deficit and starting weight; using roughly 7700 kcal per kilogram of fat the tool computes the days and weeks to reach the goal and the weekly loss rate, helping you set a sensible timeline. This is an educational estimate that varies with metabolism and adherence.",
    trustNoteLabel: "Note:",
    ...
    participants: "Fat to lose (kg)",
    averageHourlyRate: "Daily deficit (kcal)",
    durationHours: "Start weight (kg)",
...
export default function FatLossCalculator() {
  ...
  const result = useMemo(() => {
    ...
    const fatToLoseKg = v1; const dailyDeficit = v2; const startWeightKg = v3;
    const kcalPerKgFat = 7700;
    const totalDeficit = fatToLoseKg * kcalPerKgFat;
    const days = dailyDeficit > 0 ? totalDeficit / dailyDeficit : 0;
    const weeks = days / 7;
    const weeklyKg = dailyDeficit > 0 ? (dailyDeficit * 7) / kcalPerKgFat : 0;
    return { weeks, days, weeklyKg, fatToLoseKg };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.weeks, 1);
  const monthlyDisplay = fmt(result.days, 0);

  function fillSolid() { setUnit("metric"); setParticipants("6"); setAverageHourlyRate("500"); setDurationHours("80"); setMeetingsPerMonth("0"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("10"); setAverageHourlyRate("1000"); setDurationHours("80"); setMeetingsPerMonth("0"); }

  const activeBand = bands.find(b => {
    const r = result.weeklyKg;
    if (r < 0.25) return b.key === "tiny";
    if (r < 0.45) return b.key === "normal";
    if (r < 0.7) return b.key === "notable";
    if (r < 0.9) return b.key === "high";
    if (r < 1.2) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#ecfdf5 0%,#d1fae5 55%,#dbeafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "達標所需週數" : "weeks to goal"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.days, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.weeklyKg, 2)} kg/wk</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">0.5</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "穩健速度" : "Steady"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">1.0</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "積極速度" : "Aggressive"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "週" : "weeks"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "天" : "days"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "所需天數" : "Days needed"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.days, 0)}</p><p className="text-sm font-bold text-emerald-700"> d</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "每週減脂" : "Weekly loss"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.weeklyKg, 2)}</p><p className="text-sm font-bold text-blue-700"> kg</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "目標減脂" : "Target loss"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.fatToLoseKg, 1)}</p><p className="text-sm font-bold text-slate-700"> kg</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="fat-loss-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "達標所需週數" : "weeks to goal"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "設定想減脂肪量" : "Set the fat to lose", note: t.bmrStep }, { label: lang === "zh" ? "輸入每日缺口" : "Enter the daily deficit", note: t.deficitStep }, { label: lang === "zh" ? "換算所需時間" : "Convert to time needed", note: t.trendStep }, { label: lang === "zh" ? "得達標週數" : "Get the weeks to goal", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="fat-loss-calculator-faq" position="inline" /></section>
