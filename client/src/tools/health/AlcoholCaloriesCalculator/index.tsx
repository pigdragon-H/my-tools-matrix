...
const ui = {
  zh: {
    ...
    metric: "啤酒 / 葡萄酒", imperial: "烈酒 / 調酒", exampleCards: "範例卡", baselineExample: "啤酒 355 ml · 5%", activeExample: "調酒 200 ml · 15%", flowDemo: "ABV", calculator: "計算機",
    weight: "份量 (ml)", tdee: "酒精濃度 ABV (%)", goal: "糖分 (g)", goalCut: "啤酒", goalMaintain: "葡萄酒", goalBulk: "烈酒",
    resultCard: "酒精熱量估算結果", unit: "kcal/杯", primaryValue: "純酒精", maintenanceTarget: "酒精熱量 (kcal)", actionTarget: "糖分熱量 (kcal)", estimatedTdee: "純酒精克數", maintenance: "酒精", fatLossTarget: "糖分",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "飲食規劃的下一步工具", premiumTitle: "PRO 飲酒追蹤包", premiumText: "解鎖每杯記錄、週飲量趨勢圖、無酒精日提醒與個人化飲食報告。",
  ...
  en: {
    ...
    metric: "Beer / Wine", imperial: "Spirit / Cocktail", exampleCards: "Example cards", baselineExample: "Beer 355 ml · 5%", activeExample: "Cocktail 200 ml · 15%", flowDemo: "ABV", calculator: "Calculator",
    weight: "Volume (ml)", tdee: "Alcohol by volume ABV (%)", goal: "Sugar (g)", goalCut: "Beer", goalMaintain: "Wine", goalBulk: "Spirit",
    resultCard: "Alcohol Calorie Estimate", unit: "kcal/drink", primaryValue: "Pure alcohol", maintenanceTarget: "Alcohol kcal", actionTarget: "Sugar kcal", estimatedTdee: "Grams of pure alcohol", maintenance: "Alcohol", fatLossTarget: "Sugar",
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next tools for diet planning", premiumTitle: "PRO Drink Tracking Pack", premiumText: "Unlock per-drink logging, weekly intake trend charts, alcohol-free-day reminders, and personalized diet reports.",
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillStandard} className="w-full rounded-2xl border border-emerald-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">~150</span></div><p className="mt-2 text-sm text-slate-600">Beer · 355 ml · 5% · 13 g</p></button><button onClick={fillCut} className="w-full rounded-2xl border border-orange-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-700">~286</span></div><p className="mt-2 text-sm text-slate-600">Cocktail · 200 ml · 15% · 25 g</p></button></div></div>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-emerald-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9-Emotion-Upper */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black uppercase text-slate-500">Total</div><div className="mt-1 text-3xl font-black">{totalDisplay}</div></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-xs font-black uppercase text-blue-600">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-blue-950">{gramsDisplay}g</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-emerald-950">{result && Number(ml) > 0 ? fmt(result.total / Number(ml), 2) : "—"}</div></div></div></article>
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO"><article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["Logging", "Trends", "Reminder", "Report"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
