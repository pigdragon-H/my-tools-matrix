// @profile B
// Profile B · 計算機-YMYL · Blood Pressure Calculator - MAP, pulse pressure and category from systolic/diastolic（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<90", label: { zh: "偏低", en: "Low" }, desc: { zh: "收縮壓偏低,若伴隨頭暈宜留意。", en: "A low systolic reading; watch for dizziness." } },
  { key: "normal", range: "90-120", label: { zh: "正常", en: "Normal" }, desc: { zh: "理想的收縮壓範圍。", en: "An ideal systolic range." } },
  { key: "notable", range: "120-130", label: { zh: "升高", en: "Elevated" }, desc: { zh: "略為升高,建議調整生活型態。", en: "Slightly elevated; lifestyle adjustments help." } },
  { key: "high", range: "130-140", label: { zh: "第一期", en: "Stage 1" }, desc: { zh: "第一期高血壓,建議規律量測。", en: "Stage 1 hypertension; monitor regularly." } },
  { key: "major", range: "140-180", label: { zh: "第二期", en: "Stage 2" }, desc: { zh: "第二期高血壓,建議就醫評估。", en: "Stage 2 hypertension; seek medical advice." } },
  { key: "executive", range: "180+", label: { zh: "危急", en: "Crisis" }, desc: { zh: "血壓危急,請立即就醫。", en: "A hypertensive crisis; seek care immediately." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "血壓計算機",
    subtitle: "由收縮壓與舒張壓,計算平均動脈壓與脈壓差並判讀分級。",
    intro: "輸入收縮壓、舒張壓、年齡與靜止心率,本工具計算平均動脈壓 (MAP)、脈壓差與心率壓力積,並依常見指引判讀血壓分級,協助你追蹤血壓變化。此為一般教育用途的估算,不能取代專業醫療診斷,如有疑慮請諮詢醫師。",
    trustNoteLabel: "注意事項:",
    ...
    flowDemo: "儲蓄率",
    calculator: "計算機",
    participants: "收縮壓 (mmHg)",
    averageHourlyRate: "舒張壓 (mmHg)",
    durationHours: "年齡",
    meetingsPerMonth: "靜止心率",
  ...
  en: {
    ...
    subtitle: "Compute mean arterial pressure and pulse pressure and read the category.",
    intro: "Enter systolic, diastolic, age and resting heart rate; the tool computes mean arterial pressure (MAP), pulse pressure and the rate pressure product, and reads the blood-pressure category from common guidelines, helping you track changes. This is an educational estimate that does not replace professional diagnosis; consult a clinician with concerns.",
    trustNoteLabel: "Note:",
    ...
    participants: "Systolic (mmHg)",
    averageHourlyRate: "Diastolic (mmHg)",
    durationHours: "Age",
    meetingsPerMonth: "Resting heart rate",
...
export default function BloodPressureCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("80");
  const [durationHours, setDurationHours] = useState("40");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("70");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const systolic = v1; const diastolic = v2; const age = v3; const restingHr = v4;
    const pulsePressure = systolic - diastolic;
    const map = diastolic + pulsePressure / 3;
    const rpp = systolic * restingHr;
    return { systolic, map, pulsePressure, rpp };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.systolic, 0);
  const monthlyDisplay = fmt(result.map, 0);

  function fillSolid() { setUnit("metric"); setParticipants("120"); setAverageHourlyRate("80"); setDurationHours("40"); setMeetingsPerMonth("70"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("160"); setAverageHourlyRate("100"); setDurationHours("40"); setMeetingsPerMonth("70"); }

  const activeBand = bands.find(b => {
    const r = result.systolic;
    if (r < 90) return b.key === "tiny";
    if (r < 120) return b.key === "normal";
    if (r < 130) return b.key === "notable";
    if (r < 140) return b.key === "high";
    if (r < 180) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fff1f2 0%,#ffe4e6 55%,#fce7f3 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "收縮壓讀數" : "systolic reading"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.map, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.pulsePressure, 0)} mmHg</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">120</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "正常上緣" : "Normal top"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">140</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "高血壓門檻" : "Hypertension"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "mmHg" : "mmHg"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "mmHg" : "mmHg"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "平均動脈壓" : "Mean arterial pressure"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.map, 0)}</p><p className="text-sm font-bold text-emerald-700"> mmHg</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "脈壓差" : "Pulse pressure"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.pulsePressure, 0)}</p><p className="text-sm font-bold text-blue-700"> mmHg</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "心率壓力積" : "Rate pressure product"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.rpp, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="blood-pressure-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "收縮壓讀數" : "systolic reading"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入收縮壓" : "Enter the systolic value", note: t.bmrStep }, { label: lang === "zh" ? "輸入舒張壓" : "Enter the diastolic value", note: t.deficitStep }, { label: lang === "zh" ? "計算平均動脈壓" : "Compute the mean arterial pressure", note: t.trendStep }, { label: lang === "zh" ? "判讀血壓分級" : "Read the blood-pressure category", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="blood-pressure-calculator-faq" position="inline" /></section>
