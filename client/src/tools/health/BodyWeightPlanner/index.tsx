// @profile B
// Profile B · 計算機-YMYL · Body Weight Planner - daily calorie change to reach a goal weight by a date（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<200", label: { zh: "很緩", en: "Very slow" }, desc: { zh: "每日缺口很小,進度緩慢但易維持。", en: "A small daily deficit; slow but easy to sustain." } },
  { key: "normal", range: "200-400", label: { zh: "緩和", en: "Gentle" }, desc: { zh: "溫和的缺口,適合長期執行。", en: "A gentle deficit suited to long-term plans." } },
  { key: "notable", range: "400-600", label: { zh: "穩健", en: "Steady" }, desc: { zh: "穩健的減重速度,平衡效果與可行性。", en: "A steady pace balancing results and feasibility." } },
  { key: "high", range: "600-900", label: { zh: "積極", en: "Aggressive" }, desc: { zh: "較大的缺口,需注意飲食品質。", en: "A larger deficit; mind nutrition quality." } },
  { key: "major", range: "900-1200", label: { zh: "很積極", en: "Very aggressive" }, desc: { zh: "缺口偏大,長期執行宜有專業指導。", en: "A large deficit; long-term use needs guidance." } },
  { key: "executive", range: "1200+", label: { zh: "過大", en: "Too large" }, desc: { zh: "每日缺口過大,不建議,請重新設定。", en: "The daily deficit is too large; reconsider the goal." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "體重規劃計算機",
    subtitle: "由目前體重、目標與時程,計算每日所需熱量缺口。",
    intro: "輸入目前體重、目標體重、預計週數與維持熱量,本工具以每公斤約 7700 大卡換算,計算達標所需的每日熱量缺口、每週體重變化與每日目標熱量,協助你規劃可行的減重節奏。此為一般教育用途的估算,實際需求因人而異,如有健康疑慮請諮詢專業人員。",
    trustNoteLabel: "注意事項:",
    ...
    flowDemo: "儲蓄率",
    calculator: "計算機",
    participants: "目前體重 (公斤)",
    averageHourlyRate: "目標體重 (公斤)",
    durationHours: "目標週數",
    meetingsPerMonth: "維持熱量 (大卡)",
  ...
  en: {
    ...
    subtitle: "Compute the daily calorie change needed to reach a goal weight.",
    intro: "Enter your current weight, goal weight, planned weeks and maintenance calories; using roughly 7700 kcal per kilogram, the tool computes the daily deficit, weekly change and daily target calories needed to reach the goal, helping you plan a feasible pace. This is an educational estimate that varies by individual; consult a professional with health concerns.",
    trustNoteLabel: "Note:",
    ...
    participants: "Current weight (kg)",
    averageHourlyRate: "Goal weight (kg)",
    durationHours: "Weeks to goal",
    meetingsPerMonth: "Maintenance kcal",
...
export default function BodyWeightPlanner() {
  ...
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("2400");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const currentKg = v1; const goalKg = v2; const weeks = v3; const maintenanceKcal = v4;
    const deltaKg = currentKg - goalKg;
    const kcalPerKg = 7700;
    const days = weeks * 7;
    const dailyDeficit = days > 0 ? (deltaKg * kcalPerKg) / days : 0;
    const targetKcal = maintenanceKcal - dailyDeficit;
    const weeklyKg = weeks > 0 ? deltaKg / weeks : 0;
    return { targetKcal, dailyDeficit, weeklyKg, deltaKg };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.targetKcal, 0);
  const monthlyDisplay = fmt(result.dailyDeficit, 0);

  function fillSolid() { setUnit("metric"); setParticipants("80"); setAverageHourlyRate("72"); setDurationHours("12"); setMeetingsPerMonth("2400"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("95"); setAverageHourlyRate("72"); setDurationHours("8"); setMeetingsPerMonth("2400"); }

  const activeBand = bands.find(b => {
    const r = result.dailyDeficit;
    if (r < 200) return b.key === "tiny";
    if (r < 400) return b.key === "normal";
    if (r < 600) return b.key === "notable";
    if (r < 900) return b.key === "high";
    if (r < 1200) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#ecfdf5 0%,#d1fae5 55%,#dbeafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "每日目標熱量" : "daily target kcal"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.dailyDeficit, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.weeklyKg, 2)} kg</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">500</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "溫和缺口" : "Gentle deficit"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">1000</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "積極缺口" : "Aggressive"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "大卡" : "kcal"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "大卡" : "kcal"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "每日缺口" : "Daily deficit"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.dailyDeficit, 0)}</p><p className="text-sm font-bold text-emerald-700"> kcal</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "每週變化" : "Weekly change"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.weeklyKg, 2)}</p><p className="text-sm font-bold text-blue-700"> kg</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "總目標變化" : "Total change"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.deltaKg, 1)}</p><p className="text-sm font-bold text-slate-700"> kg</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="body-weight-planner-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "每日目標熱量" : "daily target kcal"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入目前與目標體重" : "Enter current and goal weight", note: t.bmrStep }, { label: lang === "zh" ? "設定目標週數" : "Set the weeks to goal", note: t.deficitStep }, { label: lang === "zh" ? "換算每日熱量缺口" : "Convert to a daily deficit", note: t.trendStep }, { label: lang === "zh" ? "得每日目標熱量" : "Get the daily target kcal", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="body-weight-planner-faq" position="inline" /></section>
