...
const ui = {
  zh: {
    ...
    intro: "Fasting Calculator 依據常見斷食協定（14:10、16:8、18:6、20:4、OMAD 23:1）與你的進食窗起始時間，計算每日斷食時數、進食窗長度與開窗／關窗時刻，幫你把斷食排程具體化。",
    ...
    examplesCalculator: "範例 → 計算機", enterValues: "選擇斷食法與起始時間", examplesHelper: "先用範例理解算法，再改成你自己的斷食法與進食窗開始時間。",
    metric: "24 小時制", imperial: "排程對照", exampleCards: "範例卡", baselineExample: "16:8 · 12:00 開窗", activeExample: "20:4 示範", flowDemo: "16:8", calculator: "計算機",
    protocolLabel: "斷食法", startLabel: "進食窗開始（24 小時制）",
    resultCard: "你的斷食排程", unit: "小時/天", primaryValue: "主要數值", maintenanceTarget: "進食窗 (h)", actionTarget: "開窗時間", estimatedTdee: "斷食法", maintenance: "進食窗", fatLossTarget: "關窗時間",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "斷食排程的推薦工具", premiumTitle: "PRO 斷食計畫包", premiumText: "解鎖週期化斷食排程、進食窗營養範本、連續天數成就追蹤與個人化提醒。",
  ...
  en: {
    ...
    metric: "24-hour", imperial: "Schedule view", exampleCards: "Example cards", baselineExample: "16:8 · open at 12:00", activeExample: "20:4 demo", flowDemo: "16:8", calculator: "Calculator",
    protocolLabel: "Fasting protocol", startLabel: "Eating-window start (24h)",
    resultCard: "Your Fasting Schedule", unit: "hours/day", primaryValue: "Primary Value", maintenanceTarget: "Eating window (h)", actionTarget: "Window opens", estimatedTdee: "Protocol", maintenance: "Eating window", fatLossTarget: "Window closes",
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Recommended tools for fasting schedules", premiumTitle: "PRO Fasting Plan Pack", premiumText: "Unlock periodized fasting schedules, eating-window nutrition templates, streak achievement tracking, and personalized reminders.",
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillStandard} className="w-full rounded-2xl border border-emerald-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">16h</span></div><p className="mt-2 text-sm text-slate-600">16:8 · open 12:00 · close 20:00</p></button><button onClick={fillCut} className="w-full rounded-2xl border border-orange-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-700">20h</span></div><p className="mt-2 text-sm text-slate-600">20:4 · open 16:00 · close 20:00</p></button></div></div>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-emerald-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9-Emotion-Upper */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black uppercase text-slate-500">Fast</div><div className="mt-1 text-3xl font-black">{fastDisplay}h</div></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-xs font-black uppercase text-blue-600">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-blue-950">{eatDisplay}h</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-emerald-950">{fastPct}%</div></div></div></article>
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO"><article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["Schedule", "Nutrition", "Streaks", "Reminder"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
