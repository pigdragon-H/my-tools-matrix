...
const ui = {
  zh: {
    ...
    intro: "Workout Plan Calculator 依訓練目標、每週天數與經驗倍率，估算每肌群每週建議組數、總訓練量與分割建議，幫你把課表量化。",
    ...
    examplesCalculator: "範例 → 計算機", enterValues: "輸入目標、天數與經驗", examplesHelper: "先用範例理解算法，再換成你自己的目標與可訓練天數。",
    ...
    resultCard: "你的訓練計畫結果", unit: "組/肌群/週", primaryValue: "目前設定", maintenanceTarget: "全身週總組數", actionTarget: "每次訓練約", estimatedTdee: "每肌群週組數", maintenance: "週總組數", fatLossTarget: "每次組數",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "訓練規劃的下一步裝備", premiumTitle: "PRO 週期化課表包", premiumText: "解鎖 4 週漸進式課表、減量週安排、動作清單匯出與訓練量趨勢圖。",
  ...
  en: {
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next gear for training planning", premiumTitle: "PRO Periodized Program Pack", premiumText: "Unlock a 4-week progressive program, deload-week scheduling, exercise-list export, and volume trend charts.",
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="workout-faq" position="inline" /></section>
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">{/* L15-Affiliate */}<p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} target="_blank" rel="noopener noreferrer sponsored" className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO">{/* L16-PremiumGate */}<article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["Program", "Deload", "Export", "Trends"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
