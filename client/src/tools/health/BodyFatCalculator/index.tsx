...
const bands = [
  ...
  { key: "athlete", range: "—", label: { zh: "運動員特例", en: "Athlete exception" }, desc: { zh: "高肌肉量者可能與一般分類不同。", en: "High lean-mass athletes may classify differently." } },
] as const;
...
const ui = {
  zh: {
    ...
    metric: "公制", imperial: "英制", exampleCards: "範例卡", baselineExample: "男性標準範例", activeExample: "女性圍度範例", bodyFatLabel: "體脂", navyMethodLabel: "海軍公式", baselineExampleNote: "175cm · 頸 38 · 腰 84", activeExampleNote: "165cm · 頸 33 · 腰 76 · 臀 98", flowDemo: "Navy formula", calculator: "計算機",
    heightCm: "身高（cm）", neckCm: "頸圍（cm）", waistCm: "腰圍（cm）", hipCm: "臀圍（cm）", heightIn: "身高（in）", neckIn: "頸圍（in）", waistIn: "腰圍（in）", hipIn: "臀圍（in）", sex: "性別", male: "男性", female: "女性", weightKgLabel: "體重（kg）",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "體組成規劃的下一步工具", premiumTitle: "PRO 體組成追蹤包", premiumText: "解鎖體脂趨勢、TDEE 連動、熱量赤字模擬與個人化報告。", feat1: "體脂分析", feat2: "TDEE", feat3: "熱量赤字", feat4: "報表",
  ...
  en: {
    ...
    metric: "Metric", imperial: "Imperial", exampleCards: "Example cards", baselineExample: "Male example", activeExample: "Female example", bodyFatLabel: "Body Fat", navyMethodLabel: "Navy method", baselineExampleNote: "175cm · neck 38 · waist 84", activeExampleNote: "165cm · neck 33 · waist 76 · hip 98", flowDemo: "Navy formula", calculator: "Calculator",
    heightCm: "Height (cm)", neckCm: "Neck (cm)", waistCm: "Waist (cm)", hipCm: "Hip (cm)", heightIn: "Height (in)", neckIn: "Neck (in)", waistIn: "Waist (in)", hipIn: "Hip (in)", sex: "Sex", male: "Male", female: "Female", weightKgLabel: "Weight kg",
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next tools for body-composition planning", premiumTitle: "PRO Body Composition Tracking Pack", premiumText: "Unlock body-fat trends, TDEE linking, calorie-deficit simulation, and personalized reports.", feat1: "Body Fat", feat2: "TDEE", feat3: "Deficit", feat4: "Reports",
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillMale} className="w-full rounded-2xl border border-emerald-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">18.6%</span></div><p className="mt-2 text-sm text-slate-600">{t.baselineExampleNote}</p></button><button onClick={fillFemale} className="w-full rounded-2xl border border-orange-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-700">{t.flowDemo}</span></div><p className="mt-2 text-sm text-slate-600">{t.activeExampleNote}</p></button></div></div>
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.sex}<select className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={sex} onChange={(e) => setSex(e.target.value as Sex)}><option value="male">{t.male}</option><option value="female">{t.female}</option></select></label><label className="block text-sm font-black text-slate-700">{t.weightKgLabel}<input className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={weightKg} onChange={(e) => setWeightKg(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{unitSystem === "metric" ? t.heightCm : t.heightIn}<input className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={height} onChange={(e) => setHeight(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{unitSystem === "metric" ? t.neckCm : t.neckIn}<input className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={neck} onChange={(e) => setNeck(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{unitSystem === "metric" ? t.waistCm : t.waistIn}<input className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={waist} onChange={(e) => setWaist(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{unitSystem === "metric" ? t.hipCm : t.hipIn}<input disabled={sex === "male"} className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold disabled:bg-slate-100" value={hip} onChange={(e) => setHip(e.target.value)} /></label></div></div>
          </div>
        </section>
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-emerald-400 to-blue-600" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{bfDisplay}</div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.unit}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.sex}</div><div className="mt-1 text-xl font-black">{sex === "male" ? t.male : t.female}</div><div className="mt-1 text-xs text-slate-300">{t.navyMethodLabel}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.primaryValue}</div><div className="mt-1 text-xs font-black uppercase text-blue-700">{t.estimatedTdee}</div><p className="mt-2 text-3xl font-black text-blue-950">{bfDisplay}%</p></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.maintenanceTarget}</div><div className="mt-1 text-xs font-black uppercase text-emerald-700">{t.maintenance}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fatDisplay}</p><p className="text-sm font-bold text-emerald-700">kg</p></div><div className="rounded-2xl bg-orange-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-500">{t.actionTarget}</div><div className="mt-1 text-xs font-black uppercase text-orange-700">{t.fatLossTarget}</div><p className="mt-2 text-3xl font-black text-orange-950">{leanDisplay}</p><p className="text-sm font-bold text-orange-700">kg</p></div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-emerald-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9-Emotion-Upper */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black uppercase text-slate-500">{t.bodyFatLabel}</div><div className="mt-1 text-3xl font-black">{bfDisplay}%</div></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-xs font-black uppercase text-blue-600">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-blue-950">{result ? fmt(result.gap25) : "—"}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-emerald-950">{fatDisplay}</div></div></div></article>
