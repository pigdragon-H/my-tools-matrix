...
const ui = {
  zh: {
    ...
    metric: "公制 (kg/min)", imperial: "英制 (lb/min)", exampleCards: "範例卡", baselineExample: "自由式 30 分鐘", activeExample: "蝶式示範", flowDemo: "70 kg", calculator: "計算機",
    weight: "體重 (kg)", minutes: "游泳時間 (分鐘)", mode: "泳姿", modeFree: "自由式", modeBreast: "蛙式", modeBack: "仰式", modeFly: "蝶式", modeLeisure: "悠閒游",
    resultCard: "單次游泳消耗結果", unit: "kcal/次", primaryValue: "本次游泳", maintenanceTarget: "每分鐘消耗", actionTarget: "每週消耗", estimatedTdee: "MET 值", maintenance: "每分鐘", fatLossTarget: "每週 (3 次)",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "游泳訓練的下一步工具", premiumTitle: "PRO 游泳訓練包", premiumText: "解鎖每週訓練量趨勢、泳姿效率分析、間歇課表生成與恢復評分報告。",
  ...
  en: {
    ...
    metric: "Metric (kg/min)", imperial: "Imperial (lb/min)", exampleCards: "Example cards", baselineExample: "Freestyle 30 min", activeExample: "Butterfly demo", flowDemo: "70 kg", calculator: "Calculator",
    ...
    resultCard: "Per-Session Burn Result", unit: "kcal/session", primaryValue: "This swim", maintenanceTarget: "Per minute", actionTarget: "Per week", estimatedTdee: "MET value", maintenance: "Per minute", fatLossTarget: "Weekly (3×)",
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next tools for swim training", premiumTitle: "PRO Swim Training Pack", premiumText: "Unlock weekly volume trends, stroke-efficiency analysis, interval workout generation, and recovery score reports.",
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillStandard} className="w-full rounded-2xl border border-emerald-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">MET 8.3</span></div><p className="mt-2 text-sm text-slate-600">70 kg · 30 min · ≈ 305 kcal</p></button><button onClick={fillFly} className="w-full rounded-2xl border border-orange-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-orange-100 px-3 py-1 text-xs font-black text-orange-700">MET 13.8</span></div><p className="mt-2 text-sm text-slate-600">70 kg · 20 min · ≈ 338 kcal</p></button></div></div>
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO"><article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["Volume", "Efficiency", "Intervals", "Recovery"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
