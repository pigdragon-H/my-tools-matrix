...
const ui = {
  zh: {
    ...
    resultCard: "估算結果", unit: "百分位", primaryValue: "量測值", measurementLabel: "量測", median: "同齡中位", zScore: "標準差倍數",
  ...
  en: {
    ...
    resultCard: "Estimated Result", unit: "percentile", primaryValue: "Value", measurementLabel: "Measurement", median: "Age median", zScore: "Z-score",
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-emerald-400 to-blue-600" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{pctDisplay}</div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.unit}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.primaryValue}</div><div className="mt-1 text-xl font-black">{value} {unit}</div><div className="mt-1 text-xs text-slate-300">{sex === "boy" ? "BOY" : "GIRL"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.measurementLabel}</div><div className="mt-1 text-xs font-black uppercase text-blue-700">{t.value}</div><p className="mt-2 text-3xl font-black text-blue-950">{value}</p><p className="text-sm font-bold text-blue-700">{unit}</p></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.median}</div><div className="mt-1 text-xs font-black uppercase text-emerald-700">{t.median}</div><p className="mt-2 text-3xl font-black text-emerald-950">{result ? fmt(result.med) : "—"}</p><p className="text-sm font-bold text-emerald-700">{unit}</p></div><div className="rounded-2xl bg-orange-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-500">{t.zScore}</div><div className="mt-1 text-xs font-black uppercase text-orange-700">{t.zScore}</div><p className="mt-2 text-3xl font-black text-orange-950">{zDisplay}</p><p className="text-sm font-bold text-orange-700">σ</p></div></div></div></article>
