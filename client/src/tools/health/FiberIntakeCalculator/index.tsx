// @profile B
// Profile B · 計算機-YMYL · Fiber Intake Calculator - recommended daily fiber from calories and age（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<30%", label: { zh: "嚴重不足", en: "Very low" }, desc: { zh: "纖維攝取嚴重不足,建議大幅增加蔬果。", en: "Fiber is very low; greatly increase fruit and veg." } },
  { key: "normal", range: "30-50%", label: { zh: "不足", en: "Low" }, desc: { zh: "攝取不足,可逐步增加全穀與豆類。", en: "A low intake; add whole grains and legumes." } },
  { key: "notable", range: "50-75%", label: { zh: "接近", en: "Approaching" }, desc: { zh: "接近建議量,再補一點即可達標。", en: "Approaching the target; a small boost will do." } },
  { key: "high", range: "75-100%", label: { zh: "良好", en: "Good" }, desc: { zh: "攝取良好,接近每日建議量。", en: "A good intake, near the daily recommendation." } },
  { key: "major", range: "100-130%", label: { zh: "達標", en: "Met" }, desc: { zh: "已達建議量,維持均衡即可。", en: "The recommendation is met; keep it balanced." } },
  { key: "executive", range: "130%+", label: { zh: "偏高", en: "High" }, desc: { zh: "攝取偏高,留意水分與循序增加。", en: "A high intake; ensure hydration and increase gradually." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "纖維攝取計算機",
    subtitle: "由每日熱量,計算建議的膳食纖維攝取量。",
    intro: "輸入每日總熱量、年齡、目前纖維攝取與每日餐數,本工具以每 1000 大卡約 14 克纖維的指引,計算每日建議纖維量、與目前攝取的差距及達成率,協助你檢視膳食纖維是否足夠。此為一般教育用途的估算,個別需求請諮詢專業人員。",
    trustNoteLabel: "注意事項:",
    ...
    flowDemo: "儲蓄率",
    calculator: "計算機",
    participants: "每日總熱量 (大卡)",
    averageHourlyRate: "年齡",
    durationHours: "目前纖維攝取 (克)",
    meetingsPerMonth: "每日餐數",
  ...
  en: {
    ...
    subtitle: "Compute recommended dietary fiber from daily calories.",
    intro: "Enter daily calories, age, current fiber intake and meals per day; using the guideline of about 14 g of fiber per 1000 kcal the tool computes your daily fiber target, the gap from current intake and the percentage met, helping you check whether your fiber is adequate. This is an educational estimate; consult a professional for individual needs.",
    trustNoteLabel: "Note:",
    ...
    participants: "Daily calories (kcal)",
    averageHourlyRate: "Age",
    durationHours: "Current fiber (g)",
    meetingsPerMonth: "Meals per day",
...
export default function FiberIntakeCalculator() {
  ...
  const [participants, setParticipants] = useState("2200");
  ...
  const result = useMemo(() => {
    ...
    const dailyKcal = v1; const age = v2; const currentFiber = v3; const meals = v4;
    const recommended = (dailyKcal / 1000) * 14;
    const gap = recommended - currentFiber;
    const perMeal = meals > 0 ? recommended / meals : 0;
    const pctMet = recommended > 0 ? (currentFiber / recommended) * 100 : 0;
    return { recommended, gap, perMeal, pctMet };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.recommended, 0);
  const monthlyDisplay = fmt(result.gap, 0);

  function fillSolid() { setUnit("metric"); setParticipants("2200"); setAverageHourlyRate("30"); setDurationHours("15"); setMeetingsPerMonth("3"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("2800"); setAverageHourlyRate("30"); setDurationHours("30"); setMeetingsPerMonth("3"); }

  const activeBand = bands.find(b => {
    const r = result.pctMet;
    if (r < 30) return b.key === "tiny";
    if (r < 50) return b.key === "normal";
    if (r < 75) return b.key === "notable";
    if (r < 100) return b.key === "high";
    if (r < 130) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#ecfdf5 0%,#d1fae5 55%,#dbeafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "每日建議纖維" : "recommended fiber"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.gap, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.pctMet, 0)}%</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">25</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "女性建議" : "Women"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">38</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "男性建議" : "Men"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "克" : "g"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "克" : "g"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "與建議差距" : "Gap to target"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.gap, 0)}</p><p className="text-sm font-bold text-emerald-700"> g</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "每餐纖維" : "Per meal"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.perMeal, 0)}</p><p className="text-sm font-bold text-blue-700"> g</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "達成率" : "Met percent"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.pctMet, 0)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="fiber-intake-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "每日建議纖維" : "recommended fiber"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入每日總熱量" : "Enter daily calories", note: t.bmrStep }, { label: lang === "zh" ? "套用每千卡 14 克" : "Apply 14 g per 1000 kcal", note: t.deficitStep }, { label: lang === "zh" ? "比對目前攝取" : "Compare current intake", note: t.trendStep }, { label: lang === "zh" ? "得建議纖維量" : "Get the fiber target", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="fiber-intake-calculator-faq" position="inline" /></section>
