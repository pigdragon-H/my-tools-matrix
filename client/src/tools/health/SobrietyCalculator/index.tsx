...
const ui = {
  zh: {
    ...
    resultCard: "估算結果", unit: "BAC %", primaryValue: "目前 BAC", currentBacLabel: "目前酒精濃度", bacRaw: "初始 BAC", sobriety: "至清醒時間", rFactor: "分布係數",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天的安全提醒帶回家", journeyHint: "代謝速率因人而異，請務必保留充足時間，必要時改搭計程車或代駕。",
  ...
  en: {
    ...
    resultCard: "Estimated Result", unit: "BAC %", primaryValue: "Current BAC", currentBacLabel: "Current alcohol level", bacRaw: "Initial BAC", sobriety: "Time to sober", rFactor: "Distribution factor",
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-emerald-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9-Emotion-Upper */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black uppercase text-slate-500">{t.currentBacLabel}</div><div className="mt-1 text-3xl font-black">{bacDisplay}%</div></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-xs font-black uppercase text-blue-600">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-blue-950">{bacRawDisplay}%</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-emerald-950">{sobrietyDisplay}</div></div></div></article>
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="sobriety-faq" position="inline" /></section>
