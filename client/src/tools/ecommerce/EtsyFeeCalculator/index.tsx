...
const ui = {
  zh: {
    ...
    intro: "本工具把您輸入的售價,扣除 Etsy 刊登費（$0.20）、交易費（6.5%）、支付處理費（3% + $0.25）、站外廣告（12-15%,可選）與商品成本,即時算出淨收入與利潤率,協助您在上架與定價前看清每件商品扣完費用後實際留下多少。所有計算都在瀏覽器本機完成。",
    ...
    examplesCalculator: "範例 → 計算器", enterValues: "輸入售價、成本與站外廣告", examplesHelper: "先用範例了解 Etsy 費用結構,再輸入您自己的售價、商品成本與站外廣告選項,即可得到各項費用明細、淨收入與利潤率。",
    ...
    emotionConversionLayer: "情緒與轉換層", turnIntoPlan: "把費用計算整合進上架與定價策略", conversionNote: "L9 會連動目前計算結果,顯示淨收入、利潤率與總費用,協助您判斷該商品的定價是否合理與是否值得開站外廣告。",
    progressInsight: "進度洞察卡", possibleTarget: "目前商品計算", dailyGap: "淨收入", weeklyTrend: "售價", motivation: "動力卡", keepMomentum: "從單件試算走向整店費用與定價管理",
    saveShareJourney: "儲存 / 分享", journeyTitle: "把這次試算帶進您的商品清單", journeyHint: "每次更換商品、調整售價或站外廣告選項時重新計算,並把結果記錄到商品定價表或店鋪損益系統。",
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section aria-label="L14 常見問題補充區" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="etsy-fee-faq" position="inline" /></section>
