...
export default function CpmCalculator(){const [values,setValues]=useState<Record<string,number>>({cost: 15000,
    impressions: 80000}); const results=useMemo<Result[]>(()=>{const v=(key:string)=>Number(values[key]??0); const cpm=v('impressions')>0?v('cost')/v('impressions')*1000:0; return [{label:'CPM',value:money(cpm)},{label:'總花費',value:money(v('cost'))},{label:'曝光數',value:num(v('impressions'))}];},[values]); return <div className="mx-auto max-w-3xl space-y-6 rounded-2xl border bg-white p-6 shadow-sm dark:bg-slate-950"><div><h1 className="text-2xl font-bold">CPM 千次曝光成本計算器</h1><p className="mt-2 text-sm text-slate-600 dark:text-slate-300">用花費與曝光計算 CPM。</p></div><div className="grid gap-4 md:grid-cols-2">{fields.map(f=><label key={f.key} className="space-y-1 text-sm font-medium">{f.label}<input type="number" value={values[f.key]??0} onChange={e=>setValues(prev=>({...prev,[f.key]:Number(e.target.value)}))} className="w-full rounded-lg border p-2 dark:bg-slate-900"/></label>)}</div><div className="grid gap-4 md:grid-cols-3">{results.map(r=><div key={r.label} className="rounded-xl bg-fuchsia-50 p-4 text-fuchsia-950 dark:bg-fuchsia-950 dark:text-fuchsia-50"><p className="text-sm">{r.label}</p><p className="mt-1 text-xl font-bold">{r.value}</p></div>)}</div></div>}
