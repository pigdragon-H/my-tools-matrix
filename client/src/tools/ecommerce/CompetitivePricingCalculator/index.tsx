...
const ui = {
  zh: {
    ...
    intro: "Competitive Pricing Calculator 依據競品平均售價與你的市場定位（低價、貼齊、溢價），計算建議售價、與市場的價差與相對市場指數，協助你在競爭中找到既有競爭力又守得住毛利的價位。",
    trustNoteLabel: "注意事項：", trustNote: "本工具以競品均價估算定位，未考量你的成本結構與品牌價值；最終定價應再回頭用定價計算機檢查該價位下的毛利是否健康。",
    ...
    knowledge: "知識", knowledgeTitle: "競爭定價在電商營運中的意義", definition: "定義", definitionText: "競爭定價是以競品平均售價為基準，依市場定位調整出建議售價；市場指數衡量你的售價相對市場均價的高低，是定位策略的核心指標。", formula: "公式", formulaText: "建議售價 = 競品均價 × 定位係數。市場價差 = (建議售價 − 競品均價) ÷ 競品均價 × 100%。市場指數 = 建議售價 ÷ 競品均價 × 100。", limitations: "限制", limitationsText: "本工具以競品均價估算；真實定價還需考量自身成本、品牌價值與需求彈性，且競品價會隨促銷與季節變動。", interpretation: "解讀", interpretationText: "市場指數 100 代表貼齊均價；低於 100 是價格優勢但壓縮毛利，高於 100 須有價值主張支撐，否則訂單會下滑。", context: "脈絡", contextText: "競爭定價應與定價、批發定價與轉換率一起看，才能確認該定位既有競爭力又守得住毛利。", example: "範例", exampleText: "競品均價 500、貼齊市場 → 建議售價 500，市場指數 100，價差 0%。",
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "競爭定價的下一步工具", premiumTitle: "PRO 競爭定價分析包", premiumText: "解鎖多競品價格帶分析、動態定位建議、價格彈性模擬與多通路競爭報告。",
  ...
  en: {
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next tools for competitive pricing", premiumTitle: "PRO Competitive Pricing Analytics Pack", premiumText: "Unlock multi-competitor price-band analysis, dynamic positioning suggestions, price-elasticity simulation, and multi-channel competition reports.",
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="competitive-pricing-faq" position="inline" /></section>
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO"><article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["PriceBand", "Dynamic", "Elasticity", "MultiChannel"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
