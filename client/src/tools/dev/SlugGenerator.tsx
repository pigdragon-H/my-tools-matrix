...
export default function SlugGenerator() {
  const [input, setInput] = useState('Hello 世界 My Tools Matrix');
  const [copied, setCopied] = useState(false);


  const result = useMemo<Output>(() => {
    try {
      const slug = input.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g, "-").replace(/^-+|-+$/g, "");
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">Slug Generator</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">Slug生成器：輸入文字後產生 URL-friendly slug，支援多語言基本轉換。</p>
      </section>
