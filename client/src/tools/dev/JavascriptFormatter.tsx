...
export default function JavascriptFormatter() {
  const [input, setInput] = useState('function hello(name){console.log("Hi "+name);return true;}');
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">JavaScript Formatter</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">JavaScript格式化工具：輸入原始內容後輸出縮排美化版本，支援一鍵複製。</p>
      </section>
