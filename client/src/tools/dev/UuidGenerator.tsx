...
export default function UuidGenerator() {
  ...
  const result = useMemo<Output>(() => {
    try {
      const count = Math.min(Math.max(Number(input) || 1, 1), 100);
      return { value: Array.from({ length: count }, () => crypto.randomUUID()).join("\n"), error: "" };
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">UUID Generator</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">UUID生成器：生成 UUID v4，支援批量生成最多 100 個。</p>
      </section>
