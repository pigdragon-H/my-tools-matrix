...
export default function CsvToJson() {
  ...
  const result = useMemo<Output>(() => {
    try {
      ...
      if (lines.length < 2) throw new Error("請至少提供 header 與一筆資料。");
      const parse = (line: string) => line.split(",").map((x) => x.trim());
      const headers = parse(lines[0]);
      const rows = lines.slice(1).map((line) => Object.fromEntries(parse(line).map((cell, i) => [headers[i] || `col${i + 1}`, cell])));
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">CSV to JSON Converter</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">CSV轉JSON轉換器：輸入 CSV 後輸出 JSON 陣列，第一列自動視為 header。</p>
      </section>
