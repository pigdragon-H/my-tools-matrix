...
export default function JsonToXml() {
  const [input, setInput] = useState('{"user":{"name":"Victor","active":true}}');
  const [copied, setCopied] = useState(false);


  const result = useMemo<Output>(() => {
    try {
      const toXml = (value: unknown, key = "root", level = 0): string => {
        const pad = "  ".repeat(level);
        if (Array.isArray(value)) return value.map((item) => toXml(item, key, level)).join("\n");
        if (value && typeof value === "object") {
          const inner = Object.entries(value as Record<string, unknown>).map(([k, v]) => toXml(v, k, level + 1)).join("\n");
          return `${pad}<${key}>\n${inner}\n${pad}</${key}>`;
        }
        return `${pad}<${key}>${String(value ?? "")}</${key}>`;
      };
      return { value: toXml(JSON.parse(input)), error: "" };
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">JSON to XML Converter</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">JSON轉XML轉換器：輸入 JSON 後輸出格式化 XML，支援一鍵複製。</p>
      </section>
