...
export default function HexEncoderDecoder() {
  ...
  const result = useMemo<Output>(() => {
    try {
      if (mode === "encode") return { value: Array.from(new TextEncoder().encode(input)).map((b) => b.toString(16).padStart(2, "0")).join(" "), error: "" };
      const bytes = input.trim().split(/\s+/).map((part) => parseInt(part, 16));
      if (bytes.some((b) => Number.isNaN(b) || b < 0 || b > 255)) throw new Error("HEX 格式無效。請使用例如 48 65 6c 6c 6f。");
      return { value: new TextDecoder().decode(Uint8Array.from(bytes)), error: "" };
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">HEX Encoder/Decoder</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">HEX編碼解碼器：文字與 HEX 雙向轉換。</p>
      </section>
