...
export default function XmlValidator() {
  const [input, setInput] = useState('<root><item>ok</item></root>');
  ...
  const result = useMemo<Output>(() => {
    try {
      ...
      if (err) throw new Error(err.textContent || "XML 語法無效。");
      return { value: "XML 有效", error: "" };
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">XML Validator</h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">XML驗證器：驗證 XML 基本語法，顯示錯誤位置。</p>
      </section>
