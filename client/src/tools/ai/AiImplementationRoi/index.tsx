...
const ui = {
  zh: {
    ...
    intro: "AI Implementation ROI Calculator 依據年度效益（千元）、年度營運成本（千元）與一次性導入規模（試點、標準或全面），計算總投入、淨回報、投報率與回本月數，協助你判斷這項 AI 導入是否值得投資、該選哪個導入規模、何時能回本，讓你在簽下 AI 導入專案前就把投報率算清楚。",
    trustNoteLabel: "注意事項：", trustNote: "本工具以你輸入的導入規模一次性成本估算，未含維運風險、變更管理與效益遞延；實際投報率請以你公司財務與專案實績為準。",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "導入的下一步工具", premiumTitle: "PRO 導入投報分析包", premiumText: "解鎖多年度現金流折現、風險加權情境、採用率模擬與多專案投報比較矩陣。",
    trustReferences: "信任聲明 · 相關工具 · 參考資料", trust: "信任聲明", trustText: "本工具只供投資規劃與教育用途，不取代你公司財務分析、會計明細或專案實績報告。", relatedTools: "相關工具", relatedToolsText: "AI ROI · Automation Savings · AI Labor · Prompt ROI", references: "參考資料", referencesText: "企業 AI 導入效益研究；專案投報率分析指南;變更管理成本報告;採用率與生產力研究。",
    ...
    q6: "這個工具能取代財務分析嗎？", a6: "不能。它只是快速估算與教育用途；實際投報率應以你公司財務分析與專案實績為準。",
  },
  en: {
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO"><article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["DiscountedFlow", "RiskWeighted", "AdoptionScenario", "ProjectMatrix"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
