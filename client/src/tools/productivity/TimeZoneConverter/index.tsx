...
const ui = {
  zh: {
    ...
    intro: "本工具根據你與對方的 UTC 偏移、各自工作起訖時間,計算雙方工作時段的重疊小時數、最佳會議時段建議與週度可同步小時,協助跨國團隊規劃可持續的協作節奏。",
    ...
    yourTzOffset: "你的時區(UTC 偏移,小時)", theirTzOffset: "對方時區(UTC 偏移,小時)", yourWorkStart: "你的上班時刻(0–23)", yourWorkEnd: "你的下班時刻(0–23)",
    ...
    overlapHours: "今日重疊小時", weeklySync: "週度同步小時", theirWorkWindow: "對方工作時段(你的時間)", suggestedMeetingSlot: "建議會議時段(你的時間)", timeDiffHours: "時差(小時)",
    ...
    formula: "公式", formulaText: "時差(小時) = 對方 UTC 偏移 − 你的 UTC 偏移。對方上班(你的時間) = 對方上班 − 時差;對方下班(你的時間) = 對方下班 − 時差。重疊小時 = max(0, min(你的下班, 對方下班-你的時間) − max(你的上班, 對方上班-你的時間))。",
    ...
    example: "範例", exampleText: "你 UTC+8 上班 9–18;對方 UTC+8 上班 9–18,時差 0,重疊 9 小時。若對方 UTC−5,時差 −13,對方上班(你的時間) = 22:00–07:00,完全錯峰,重疊 0 小時。",
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-cyan-400 to-sky-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{overlapDisplay}<span className="text-3xl">h</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.weeklySync}</div><div className="mt-1 text-xl font-black">{weeklyDisplay}h</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "/週" : "/week"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.timeDiffHours}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "時差" : "Diff"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{result.timeDiff > 0 ? "+" : ""}{fmt(result.timeDiff, 1)}h</p><p className="text-sm font-bold text-emerald-700">{lang === "zh" ? "對你而言" : "from you"}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.suggestedMeetingSlot}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "建議時段" : "Suggested"}</div><p className="mt-2 text-2xl font-black text-blue-950">{result.suggestedSlot}</p><p className="text-sm font-bold text-blue-700">{lang === "zh" ? "你的時間" : "your time"}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.theirWorkWindow}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "對方時段" : "Their"}</div><p className="mt-2 text-2xl font-black text-slate-950">{result.theirWindowInMyTime}</p><p className="text-sm font-bold text-slate-700">{lang === "zh" ? "你的時間" : "your time"}</p></div></div></div></article>
