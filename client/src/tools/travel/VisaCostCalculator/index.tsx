...
const ui = {
  zh: {
    ...
    intro: "Visa Cost Calculator 依據簽證類型（觀光、商務或快件）、申請人數與每人代辦費，計算整團簽證基本費、代辦費與簽證總費用，協助你判斷費用是否合理、代辦占比多高、是否該改用電子簽或提早辦理避免加急，讓出國前的簽證成本一次算清楚。",
    trustNoteLabel: "注意事項：", trustNote: "本工具以你輸入的簽證類型與代辦費估算，未含各國最新規費、匯率與特殊文件費；實際費用請以各國使館或官方簽證系統公告為準。",
    ...
    knowledge: "知識", knowledgeTitle: "代辦占比在簽證費用中的意義", definition: "定義", definitionText: "簽證費用規劃是把每人基本規費與代辦費乘以人數得到總費用；代辦占比衡量手續費相對於整體成本的比重，是判斷直辦或代辦的核心指標。", formula: "公式", formulaText: "基本費總額 = 每人簽證費 × 人數。代辦費總額 = 每人代辦費 × 人數。簽證總費用 = 基本費總額 + 代辦費總額。代辦占比 = 代辦費總額 ÷ 總費用 × 100%。", limitations: "限制", limitationsText: "本工具以你輸入的類型與代辦費估算；真實費用還受各國最新規費、匯率波動、特殊文件費、保險與面試成本影響，且部分國家採電子簽或落地簽。", interpretation: "解讀", interpretationText: "代辦占比過高代表手續費吃掉太多預算；可透過自行線上申辦、改用電子簽、提早辦理避免加急或比較多家代辦報價來改善。", context: "脈絡", contextText: "簽證結果應與旅遊預算、貨幣換算與旅遊天數一起看，才能在費用、效期與行程之間取得平衡。", example: "範例", exampleText: "人數 2、觀光簽（$60）、每人代辦 30 → 基本費 120、代辦費 60，總費用 180 USD，代辦占比約 33%。",
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "簽證的下一步工具", premiumTitle: "PRO 簽證費用分析包", premiumText: "解鎖各國即時規費查詢、多人申辦分攤最佳化、代辦比價與效期到期提醒。",
  ...
  en: {
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next tools for visas", premiumTitle: "PRO Visa Cost Analytics Pack", premiumText: "Unlock per-country live fee lookup, multi-applicant cost-sharing optimization, agency price comparison, and validity-expiry alerts.",
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO"><article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["LiveFeeLookup", "CostSharing", "AgencyCompare", "ExpiryAlert"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
