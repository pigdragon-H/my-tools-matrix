...
const ui = {
  zh: {
    ...
    intro: "Luggage Weight Calculator 依據每件打包重量、託運件數與航司額度等級（廉航、標準或高艙），計算總打包重量、額度使用率與超重重量，協助你判斷行李是否超重、該移走多少重物、回程是否還有採買空間，讓你在櫃台過磅前就把行李重量控制在安全範圍。",
    trustNoteLabel: "注意事項：", trustNote: "本工具以你輸入的額度等級估算，未含各航司、艙等與特殊路線差異；實際託運額度與超重費請以航空公司官方規定與機場磅秤為準。",
    ...
    knowledge: "知識", knowledgeTitle: "額度使用率在行李打包中的意義", definition: "定義", definitionText: "行李重量規劃是把每件打包重量乘以件數得到總重，再與航司額度比較得到使用率；使用率與超重重量衡量行李是否超標，是櫃台過磅前的核心指標。", formula: "公式", formulaText: "總打包重量 = 每件重量 × 件數。額度使用率 = 總打包重量 ÷ 總額度 × 100%。超重重量 = 總打包重量 − 總額度。", limitations: "限制", limitationsText: "本工具以你輸入的額度等級估算；真實額度還受航司、艙等、會員等級、聯程轉機與特殊路線影響，且隨身行李另有重量與尺寸規定。", interpretation: "解讀", interpretationText: "使用率超過 100% 即超重，需付超重費或重新分配；可透過移重物到隨身、精簡物品、加購額外託運或升級額度等級來改善。", context: "脈絡", contextText: "行李結果應與旅遊預算、旅遊天數與簽證費用一起看，才能在重量、成本與行程之間取得平衡。", example: "範例", exampleText: "重量 24、標準額度（23kg）、件數 1 → 總重 24 公斤，使用率約 104%，已超重約 1 公斤。",
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "行李的下一步工具", premiumTitle: "PRO 行李重量分析包", premiumText: "解鎖各航司即時額度查詢、多件行李分配最佳化、超重費試算與過磅提醒。",
  ...
  en: {
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next tools for luggage", premiumTitle: "PRO Luggage Weight Analytics Pack", premiumText: "Unlock per-airline live allowance lookup, multi-bag distribution optimization, overweight-fee estimation, and weigh-in alerts.",
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]"><section className="flex h-full flex-col rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.affiliate}</p><h2 className="mt-2 text-3xl font-black">{t.affiliateTitle}</h2><div className="mt-5 grid gap-4 md:grid-cols-4">{affiliateItems.map((item) => <a key={item.href} href={item.href} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5 text-center font-black text-emerald-950">{l(item.label, lang)}</a>)}</div><p className="mt-3 text-xs text-emerald-700">{lang === "zh" ? "* 聯盟連結，購買後我們可能獲得佣金。" : "* Affiliate links. We may earn a commission."}</p></section><PremiumGate plan="PRO"><article className="flex h-full flex-col rounded-[2rem] border border-emerald-200 bg-gradient-to-br from-emerald-50 to-indigo-50 p-6 md:p-7"><h2 className="text-3xl font-black text-slate-950">{t.premiumTitle}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-700">{t.premiumText}</p><div className="mt-5 grid gap-3 md:grid-cols-4">{["LiveAllowance", "BagDistribution", "FeeEstimator", "WeighInAlert"].map((item) => <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-black text-violet-900 shadow-sm">{item}</div>)}</div></article></PremiumGate></section>
