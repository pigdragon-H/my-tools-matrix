...
const ui = {
  zh: {
    ...
    intro: "本工具把你輸入的飛行距離,乘上艙等乘數,即時估算可累積的常旅客里程,並依平均兌換比率換算成里程價值（美元）,協助你評估這趟飛行的哩程回饋與規劃獎勵兌換。所有計算都在瀏覽器本機完成。",
    ...
    examplesCalculator: "範例 → 計算器", enterValues: "輸入飛行距離與艙等", examplesHelper: "先用範例了解里程累積與價值的量級,再輸入你自己的飛行距離與艙等,即可得到累積里程、里程價值與可兌換獎勵的概念。",
    ...
    emotionConversionLayer: "情緒與轉換層", turnIntoPlan: "把里程估算整合進獎勵兌換規劃", conversionNote: "L9 會連動目前計算結果,顯示累積里程、里程價值與乘數,協助你判斷哪種艙位的哩程回饋較划算與何時兌換最有價值。",
    progressInsight: "進度洞察卡", possibleTarget: "目前航班計算", dailyGap: "累積里程", weeklyTrend: "距離", motivation: "動力卡", keepMomentum: "從單趟估算走向年度哩程累積與兌換管理",
    saveShareJourney: "儲存 / 分享", journeyTitle: "把這次估算帶進你的哩程帳本", journeyHint: "每次規劃新航班時重新計算,並把累積里程記錄到年度哩程表,做為獎勵兌換與升等規劃的依據。",
    ...
    q6: "這個工具會上傳我的資料嗎？", a6: "不會。所有距離與里程計算都在你的瀏覽器本機完成,輸入的數據不會上傳到任何伺服器。",
  },
  en: {
