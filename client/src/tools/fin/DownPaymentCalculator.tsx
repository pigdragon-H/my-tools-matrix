...
export default function DownPaymentCalculator() {
  ...
  const result=useMemo<Output>(()=>{
    try{
      ...
      const toolSlug: string = "down-payment-calculator";
      switch(toolSlug){
  ...
  const copyResult=async()=>{ if(!result.main) return; await navigator.clipboard.writeText(`Down Payment Calculator
${result.detail}
Formula: ${result.formula}`); setCopied(true); window.setTimeout(()=>setCopied(false),1500); };
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
    <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950"><p className="text-sm font-semibold text-blue-600">FIN · MTG</p><h1 className="text-3xl font-bold text-slate-950 dark:text-white">Down Payment Calculator</h1><p className="mt-2 text-slate-600 dark:text-slate-300">頭期款計算器：房價/頭期款比例 → 頭期款金額。</p></section>
