...
export default function CoastFireCalculator() {
  ...
  const result=useMemo<Output>(()=>{
    try{
      ...
      const toolSlug: string = "coast-fire-calculator";
      switch(toolSlug){
  ...
  const copyResult=async()=>{ if(!result.main) return; await navigator.clipboard.writeText(`Coast FIRE Calculator
${result.detail}
Formula: ${result.formula}`); setCopied(true); window.setTimeout(()=>setCopied(false),1500); };
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
    <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950"><p className="text-sm font-semibold text-blue-600">FIN · RET</p><h1 className="text-3xl font-bold text-slate-950 dark:text-white">Coast FIRE Calculator</h1><p className="mt-2 text-slate-600 dark:text-slate-300">Coast FIRE計算器：目標退休金/退休年齡/報酬率 → 現在需要金額。</p></section>
