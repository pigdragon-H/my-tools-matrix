...
export default function StockReturnCalculator() {
  ...
  const result=useMemo<Output>(()=>{
    try{
      ...
      const toolSlug: string = "stock-return-calculator";
      switch(toolSlug){
  ...
  const copyResult=async()=>{ if(!result.main) return; await navigator.clipboard.writeText(`Stock Return Calculator
${result.detail}
Formula: ${result.formula}`); setCopied(true); window.setTimeout(()=>setCopied(false),1500); };
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
    <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950"><p className="text-sm font-semibold text-blue-600">FIN · INV</p><h1 className="text-3xl font-bold text-slate-950 dark:text-white">Stock Return Calculator</h1><p className="mt-2 text-slate-600 dark:text-slate-300">股票報酬率計算器：買入價/賣出價/股數/股息 → 總報酬率。</p></section>
