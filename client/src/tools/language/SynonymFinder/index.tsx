...
// v2 修復：1) 音節→IPA音標(/rɛndər/)  2) 繁體中文釋義(編輯團隊人工撰寫，未收錄者fallback英文定義)  3) CEFR分層改用權威詞表(B1/B2完整覆蓋)
...
const ui = {
  zh: {
    ...
    intro: "Synonym Finder 串接 Datamuse 開放語料 API（rel_syn），並內建 CEFR-J 權威分級與編輯團隊人工撰寫的繁體中文釋義詞庫。輸入任何英文單字即可取得真實同義詞清單，每個結果都標註 CEFR 難度等級、IPA 音標、詞性與繁體中文釋義，並可展開查看英文定義與例句，幫您在口說、寫作、考試中挑出最貼切的替換詞。",
    ...
    resultIntelligence: "結果解讀", levelMatrix: "六級 CEFR 同義詞解讀矩陣", levelMatrixNote: "L7 將同義詞依 CEFR 等級分層，以 CEFR-J 權威詞表對照，A1 最常用、C2 最罕見；挑替換詞時對照您的目標讀者程度。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天學到的同義詞帶回家", journeyHint: "挑 2–3 個您會真正用到的同義詞造句，比死背 20 個更有效。",
    nextActionLabel: "下一步行動", nextActionTitle: "把同義詞接到下一個工具", nextActionItem1: "用反義詞查找器補齊對立詞，理解語義光譜", nextActionItem2: "用 CEFR 等級估算確認難度是否符合您的程度", nextActionItem3: "用字根分析器理解詞義從何而來，記得更牢",
