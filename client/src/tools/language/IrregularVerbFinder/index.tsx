...
const ui = {
  zh: {
    ...
    intro: "不規則動詞查找器採用自建的「動詞三態查找 + 反查」演算法：輸入一個英文動詞後，工具會比對內建 151 筆常用不規則動詞表——不論你輸入的是原形、過去式還是過去分詞，都能反查出完整的三態變化（原形 / 過去式 / 過去分詞），並標註每個形態的 KK 音標、詞類（動詞）、繁體中文釋義與一句例句。本工具為純前端演算法，三態查找不依賴外部 API，速度快、離線可用。",
    ...
    examplesCalculator: "範例 → 查找", enterValues: "輸入動詞", examplesHelper: "先用熱門範例了解三態變化、音標、CEFR 等級與中文釋義如何呈現，再換成你自己想查的動詞（原形、過去式或分詞皆可）。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天查的不規則動詞帶回家", journeyHint: "挑 2–3 個你常用錯的不規則動詞，用三態各造一句，把過去式與完成式練熟。",
    ...
    q1: "三態是怎麼查出來的？", a1: "用自建演算法建立索引：把內建 151 筆不規則動詞的原形、過去式、過去分詞三種形態全部建檔，所以不論你輸入哪個形態（包含 was/were、burnt/burned 這類多形），都能反查出完整三態。純前端判定，不需連外部 API。",
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="irregular-verb-finder-faq" position="inline" /></section>
