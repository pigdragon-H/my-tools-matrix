...
const ui = {
  zh: {
    ...
    intro: "字根分析器採用自建的「字根拆解 + 語源辨識 + 衍生字族」演算法：輸入一個英文單字後，工具會比對內建 81 筆常用字根表（拉丁與希臘語源），找出這個單字含有的字根，列出字根的語源（拉丁／希臘）、中文核心義，並展開同字根的衍生字族；每一個衍生字都會建一張四要素字卡，標註 KK 音標、詞類、繁體中文釋義與例句，讓您一次掌握一整串同源單字。本工具為純前端演算法，字根拆解不依賴外部 API，速度快、離線可用。",
    ...
    examplesCalculator: "範例 → 分析", enterValues: "輸入單字", examplesHelper: "先用熱門範例了解字根拆解、語源辨識與衍生字族如何呈現，再換成您自己想分析的單字。",
    ...
    resultIntelligence: "結果解讀", levelMatrix: "字根語源知識", levelMatrixNote: "L7 說明字根的兩大語源（拉丁與希臘）以及字根對英語學習的價值；分析時看您這個字的字根來自哪個語系。",
    ...
    q1: "字根是怎麼拆解出來的？", a1: "用自建演算法把您輸入的單字比對內建 81 筆常用字根表：依字根長度由長到短掃描，找出單字中含有的字根並依出現位置排序，最多顯示 3 個字根避免過度拆解。純前端判定，不需連外部 API。",
    ...
    q5: "本工具和「CEFR 等級估算」有什麼不同？", a5: "CEFR 估算是評估單字或文本的難度等級；本工具是「拆字根、溯語源、展字族」，比同義／聯想更深入到字根層，幫您從一個字理解一整串同源單字，是擴充字彙的根本方法。",
    q6: "一個字拆出多個字根代表什麼？", a6: "許多英文字由「字首 + 字根 + 字尾」組成，例如 transport 含 trans（橫越）與 port（搬運）。本工具最多顯示 3 個命中字根並依出現位置排序，幫您看懂這個字是由哪些語義積木組成的。",
  },
  en: {
