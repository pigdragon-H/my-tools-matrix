// @profile B
// Profile B · Language-Hub A-class (Datamuse rel_rhy) · RhymeFinder（GOLD-STANDARD MacroCalculator compatible）
// 押韻詞查找器：1) 押韻基於「發音」而非語意，IPA 音標即韻腳核心  2) 三層中文釋義(繁體優先→ECDICT簡體標「简」→英文定義標EN)  3) CEFR 分層改用權威詞表
...
const HOT_WORDS = ["light", "time", "love", "dream", "fire", "night"] as const;

const cefrBands = [
  { key: "A1", label: { zh: "A1 入門", en: "A1 Beginner" }, desc: { zh: "最常用的基礎押韻詞，童謠與口語首選。", en: "Most frequent basics; best for nursery rhymes and speech." } },
  { key: "A2", label: { zh: "A2 基礎", en: "A2 Elementary" }, desc: { zh: "常見韻腳，足以應付一般歌詞與打油詩。", en: "Common rhymes for everyday lyrics and light verse." } },
  { key: "B1", label: { zh: "B1 中級", en: "B1 Intermediate" }, desc: { zh: "讓詩句與歌詞更有層次的中階韻腳。", en: "Mid-level rhymes that add range to verse and lyrics." } },
  { key: "B2", label: { zh: "B2 中高", en: "B2 Upper-Inter" }, desc: { zh: "正式詩作與創作常見的進階韻腳。", en: "Advanced rhymes common in formal verse and songwriting." } },
  { key: "C1", label: { zh: "C1 高級", en: "C1 Advanced" }, desc: { zh: "精準、文學感強的高階押韻詞。", en: "Precise, literary high-level rhymes." } },
...
const ui = {
  zh: {
    badge: "語言 · 押韻創作 · Language Hub", switchToEnglish: "Switch to English", switchToChinese: "切換到中文", chineseShort: "中", englishShort: "EN",
    title: "押韻詞查找器 · Rhyme Finder", subtitle: "輸入一個英文字，立刻找出帶 CEFR 等級、IPA 音標與中文釋義的押韻詞",
    intro: "Rhyme Finder 串接 Datamuse 開放語料 API（rel_rhy），以發音為核心找出與您輸入單字押韻的字。押韻看的是「韻腳的聲音」而非語意，所以每個結果都以 IPA 音標凸顯韻腳，並標註 CEFR 難度等級、詞性與中文釋義，可展開查看英文定義與例句，幫您在寫詩、寫歌詞、押韻創作與口語節奏練習中快速挑出順耳又貼切的韻腳。",
    trustNoteLabel: "資料來源：", trustNote: "押韻詞來自 Datamuse（以發音語料庫比對韻腳）；CEFR 等級以 CEFR-J 與 Octanove 權威詞表對照；中文釋義以編輯團隊人工撰寫的繁體中文為優先，尚無繁體者改顯示 ECDICT 開源詞典的簡體釋義（標註「简」），繁簡皆無者展開即顯示英文定義；IPA 音標取自 ECDICT 與 ARPABET 轉換，正是判斷韻腳的依據；例句來自 Free Dictionary API。僅供學習參考。",
    quickActionCard: "快速查詢卡", tryExample: "一鍵查 light 的押韻詞", examplePreview: "找到的押韻詞數", examplePerson: "查詢字", fillExample: "查 light 的押韻詞", previewActivePath: "查 time 的押韻詞",
    examplesCalculator: "範例 → 查詢", enterValues: "輸入英文單字", examplesHelper: "先用熱門範例了解 CEFR 等級、IPA 音標與中文釋義如何呈現，再換成您自己想押韻的單字。",
    queryBtn: "查詢押韻詞", clearBtn: "清除", hotWords: "熱門查詢", inputPlaceholder: "輸入英文單字，例如 light",
    loading: "查詢中…", emptyHint: "輸入上方單字並按「查詢押韻詞」，結果會列在這裡。", noResult: "查無押韻詞，換個更常見的單字試試。",
    fallbackTitle: "查詢暫時無法使用", fallbackBody: "網路連線問題，請稍後再試。常用結果已儲存在本機快取中。",
    resultCard: "押韻詞結果", unit: "個押韻詞", primaryValue: "查詢字", ipaLabel: "音標", meaningLabel: "釋義", glossTagCn: "简", glossTagEn: "EN", enGlossHint: "展開看英文定義與例句", expandHint: "展開看例句", collapseHint: "收合", exampleLabel: "例句", enLoading: "載入例句中…", noExample: "查無例句，建議造句練習。",
    resultIntelligence: "結果解讀", levelMatrix: "六級 CEFR 押韻詞解讀矩陣", levelMatrixNote: "L7 將押韻詞依 CEFR 等級分層，以 CEFR-J 權威詞表對照，A1 最常用、C2 最罕見；挑韻腳時對照您的目標讀者程度。",
    scenarioLayer: "使用場景", scenarioTitle: "什麼時候用押韻詞", scenarioNote: "L8 列出四個典型場景，把押韻詞用在對的地方，而不是硬湊韻腳。",
    scenarioExam: "寫詩創作", scenarioExamNote: "找尾韻一致的字串成詩句，先看 IPA 音標確認韻腳真的相同。", scenarioWriting: "歌詞創作", scenarioWritingNote: "依節奏挑音節數相近、CEFR 適中的韻腳，唱起來才順口。", scenarioDaily: "口語節奏", scenarioDailyNote: "選 A1/A2 押韻詞做繞口令與 rap 練習，自然好記不卡舌。", scenarioBusiness: "標語廣告", scenarioBusinessNote: "挑短而響亮的押韻字做口號，押韻能大幅提升記憶度。",
    progressInsight: "學習洞察卡", possibleTarget: "本次查詢", dailyGap: "最常用等級", weeklyTrend: "已分級比例", motivation: "動力卡", keepMomentum: "從查押韻詞走向主動創作詩歌與歌詞",
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天找到的韻腳帶回家", journeyHint: "挑 2–3 個您真正會用到的押韻字寫成兩句押韻句，比死背 20 個更有效。",
    nextActionLabel: "下一步行動", nextActionTitle: "把押韻詞接到下一個工具", nextActionItem1: "用同義詞查找器替換同韻但更貼意的字，兼顧聲音與語意", nextActionItem2: "用 CEFR 等級估算確認韻腳難度是否符合您的程度", nextActionItem3: "用字根分析器理解韻腳字的語義從何而來，記得更牢",
    shareLinkBtn: "📋 複製結果連結", shareNativeBtn: "📤 分享給朋友", shareCopiedToast: "已複製到剪貼簿 ✓",
    decisionPath: "創作路徑", decisionTitle: "輸入 → 查詢 → 對韻 → 成句", step1: "輸入單字", step2: "查押韻詞", step3: "對 IPA 韻腳", step4: "造句成詩",
    knowledge: "知識", knowledgeTitle: "押韻詞在英語創作中的意義", definition: "定義", definitionText: "押韻詞（rhyme）是結尾發音相同或相近的字；押韻看的是「聲音」而非意義，因此判斷是否押韻要看 IPA 音標的韻腳，而不是拼字。", usage: "用法", usageText: "查到押韻詞後，先看 IPA 音標確認韻腳是否真的相同（例如 light /laɪt/ 與 night /naɪt/ 共享 /aɪt/），再看音節數與 CEFR 等級，挑唱起來順口的字。", limitations: "限制", limitationsText: "Datamuse 的押韻依發音語料比對，可能含罕見或古語；同形不同音的字（如 -ough 結尾）偶有誤差；CEFR 等級以 CEFR-J/Octanove 詞表為主，未收錄者改用詞頻啟發式推估。", interpretation: "解讀", interpretationText: "A1/A2 押韻詞適合童謠與口語節奏；B1/B2 適合一般歌詞與詩作；C1/C2 雖然典雅，用錯場合反而生硬難唱。", context: "脈絡", contextText: "押韻查詢應與同義詞、字根、CEFR 估算一起用：先用押韻找聲音、再用同義詞找貼意的字，兼顧韻腳與語意。", example: "範例", exampleText: "查 time → 得到 rhyme(B2)、climb(B1)、prime(B2)、chime(C1)；寫歌詞時 rhyme 與 time 同享 /aɪm/ 韻腳，唱起來最順。",
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "相關工具", affiliateTitle: "押韻創作的下一步工具", premiumTitle: "PRO 創作詞庫包", premiumText: "解鎖無限查詢、依 CEFR 等級篩選韻腳、依音節數過濾、自動記錄創作歷史，並把押韻字表匯出複習。",
    feat1: "無限查詢次數", feat2: "難度等級篩選", feat3: "創作歷史記錄", feat4: "押韻字表匯出",
    trustReferences: "信任聲明 · 相關工具 · 參考資料", trust: "信任聲明", trustText: "本工具僅供英語學習與押韻創作用途；CEFR 等級以 CEFR-J/Octanove 權威詞表對照，詞表未收錄者為啟發式推估，不等同官方語言檢定結果。", relatedTools: "相關工具", relatedToolsText: "Synonym Finder · Antonym Finder · Word Root Analyzer · CEFR Level Estimator", references: "參考資料", referencesText: "Datamuse API（rel_rhy，發音語料比對韻腳）；CEFR-J Wordlist v1.5（Tono Lab, TUFS）；Octanove C1/C2 Vocabulary Profile（CC BY-SA 4.0）；ECDICT 開源英漢詞典（IPA 音標與簡體釋義）；繁體中文釋義由編輯團隊人工撰寫；Free Dictionary API（例句）。",
    q1: "為什麼有些字看起來押韻卻沒列出來？", a1: "押韻看的是發音而非拼字。Datamuse 以發音語料比對韻腳，拼字相近但發音不同的字（如 love 與 move）不會互為押韻；建議以 IPA 音標確認韻腳。",
    q2: "CEFR 等級是怎麼判斷的？", a2: "優先以 CEFR-J 與 Octanove 權威詞表對照；詞表未收錄的罕見字才改用 Datamuse 詞頻啟發式推估。這是學習參考，非官方檢定。",
    q3: "為什麼有些字查不到押韻詞？", a3: "罕見字、專有名詞或拼錯的字可能沒有押韻資料；某些單音節或結尾特殊的字本來就難找到完美韻腳。換成更常見的字通常就有結果。",
    q4: "音標和中文釋義從哪來？", a4: "IPA 音標取自 ECDICT 開源英漢詞典（內建 2 萬餘字），詞庫未收錄者改以 Datamuse 音標即時轉換 IPA——這正是判斷韻腳的依據；中文釋義採三層優先序——編輯團隊人工撰寫的繁體中文優先（無標註），尚無繁體者改顯示 ECDICT 簡體釋義並標註「简」，繁簡皆無者展開即顯示英文定義（標註 EN）。全程不經機器翻譯。例句來自 Free Dictionary API。",
    q5: "結果為什麼有時不一樣？", a5: "Datamuse 依語料統計動態排序，且我們已過濾極罕見結果，因此排序可能隨時間略有變化。",
    q6: "適合寫歌詞和 rap 嗎？", a6: "適合。先用押韻詞找出共享韻腳的字，再依音節數與節奏挑選；rap 常用多音節押韻，可搭配同義詞工具兼顧聲音與語意。",
  },
  en: {
    badge: "Language · Rhyme · Language Hub", switchToEnglish: "Switch to English", switchToChinese: "切換到中文", chineseShort: "中", englishShort: "EN",
    title: "Rhyme Finder", subtitle: "Type one English word and get rhymes with CEFR level, IPA, and Chinese gloss",
    intro: "Rhyme Finder calls the open Datamuse corpus API (rel_rhy) and finds words that rhyme with your input based on pronunciation. Rhyme is about the sound of the ending, not meaning, so each result highlights its IPA transcription as the rhyme core, tagged with a CEFR difficulty level, part of speech, and Chinese gloss, expandable to show an English definition and example sentence, helping you quickly pick smooth, fitting rhymes for poetry, songwriting, rhyming creation, and spoken rhythm practice.",
    trustNoteLabel: "Data source:", trustNote: "Rhymes come from Datamuse (matched against a pronunciation corpus); CEFR levels are matched against the CEFR-J and Octanove authoritative wordlists; Chinese glosses prioritize the editorial team's hand-written Traditional Chinese, falling back to ECDICT's Simplified gloss (tagged Simp) when no Traditional one exists, and to an English definition when neither is available; IPA comes from ECDICT and ARPABET conversion and is exactly what determines the rhyme; examples come from the Free Dictionary API. For study reference only.",
    quickActionCard: "Quick Query Card", tryExample: "Find rhymes for light", examplePreview: "Rhymes found", examplePerson: "Query word", fillExample: "Find rhymes for light", previewActivePath: "Find rhymes for time",
    examplesCalculator: "Examples → Query", enterValues: "Enter an English word", examplesHelper: "Start with a popular example to see how CEFR level, IPA, and Chinese gloss appear, then swap in the word you want to rhyme.",
    queryBtn: "Find rhymes", clearBtn: "Clear", hotWords: "Popular queries", inputPlaceholder: "Type an English word, e.g. light",
    loading: "Searching…", emptyHint: "Enter a word above and press Find rhymes; results will appear here.", noResult: "No rhymes found, try a more common word.",
    ...
    resultCard: "Rhyme Results", unit: "rhymes", primaryValue: "Query word", ipaLabel: "IPA", meaningLabel: "Gloss", glossTagCn: "Simp", glossTagEn: "EN", enGlossHint: "See English definition & example on expand", expandHint: "Show example", collapseHint: "Collapse", exampleLabel: "Example", enLoading: "Loading example…", noExample: "No example found; try writing your own.",
    resultIntelligence: "Result Intelligence", levelMatrix: "Six-level CEFR rhyme matrix", levelMatrixNote: "L7 groups rhymes by CEFR level using the authoritative CEFR-J wordlist, with A1 most common and C2 rarest; match the level to your target reader when choosing a rhyme.",
    scenarioLayer: "Use scenarios", scenarioTitle: "When to use a rhyme", scenarioNote: "L8 lists four typical scenarios so you use rhymes in the right place, not just forcing a rhyme.",
    scenarioExam: "Poetry writing", scenarioExamNote: "Find words with matching end sounds; check the IPA first to confirm the rhyme is real.", scenarioWriting: "Songwriting", scenarioWritingNote: "Pick rhymes with similar syllable counts and a moderate CEFR level so the lyrics sing smoothly.", scenarioDaily: "Spoken rhythm", scenarioDailyNote: "Choose A1/A2 rhymes for tongue-twisters and rap practice that sound natural and easy.", scenarioBusiness: "Slogans & ads", scenarioBusinessNote: "Pick short, punchy rhyming words for taglines; rhyme greatly boosts memorability.",
    progressInsight: "Learning Insight Card", possibleTarget: "This query", dailyGap: "Most common level", weeklyTrend: "Graded ratio", motivation: "Motivation Card", keepMomentum: "Move from looking up rhymes to actively writing poems and lyrics",
    saveShareJourney: "Save / Share", journeyTitle: "Take today's rhymes home", journeyHint: "Write two rhyming lines with 2–3 rhymes you will actually use; it beats memorizing 20 at once.",
    nextActionLabel: "Next actions", nextActionTitle: "Connect rhymes to the next tool", nextActionItem1: "Use Synonym Finder to swap in a same-rhyme word that fits the meaning better, balancing sound and sense", nextActionItem2: "Use CEFR Level Estimator to confirm the rhyme difficulty fits your level", nextActionItem3: "Use Word Root Analyzer to see where the rhyming word's meaning comes from and remember it better",
    shareLinkBtn: "📋 Copy result link", shareNativeBtn: "📤 Share with friends", shareCopiedToast: "Copied to clipboard ✓",
    decisionPath: "Creation Path", decisionTitle: "Input → Query → Match rhyme → Compose", step1: "Type a word", step2: "Find rhymes", step3: "Match IPA rhyme", step4: "Compose a line",
    knowledge: "Knowledge", knowledgeTitle: "What rhymes mean in English creation", definition: "Definition", definitionText: "A rhyme is a word whose ending sound matches or closely resembles another's; rhyme is about sound, not meaning, so whether two words rhyme depends on the IPA ending, not the spelling.", usage: "Usage", usageText: "After finding a rhyme, check the IPA to confirm the rhyme is real (e.g. light /laɪt/ and night /naɪt/ share /aɪt/), then check syllable count and CEFR level, and pick a word that sings smoothly.", limitations: "Limitations", limitationsText: "Datamuse rhymes are matched against a pronunciation corpus and may include rare or archaic words; homographs with different sounds (like -ough endings) occasionally vary; CEFR levels primarily use the CEFR-J/Octanove wordlists, falling back to a frequency heuristic for unlisted words.", interpretation: "Interpretation", interpretationText: "A1/A2 rhymes suit nursery rhymes and spoken rhythm; B1/B2 suit general lyrics and verse; C1/C2 look elegant but feel stiff and hard to sing if used in the wrong place.", context: "Context", contextText: "Rhyme lookup should be used with synonyms, word roots, and CEFR estimation: find the sound with rhymes first, then a fitting word with synonyms, balancing rhyme and meaning.", example: "Example", exampleText: "Search time → get rhyme(B2), climb(B1), prime(B2), chime(C1); in lyrics, rhyme and time share the /aɪm/ ending and sing most smoothly.",
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Related Tools", affiliateTitle: "Next tools for rhyming creation", premiumTitle: "PRO Creation Pack", premiumText: "Unlock unlimited queries, filter rhymes by CEFR level, filter by syllable count, auto-log creation history, and export rhyme lists for review.",
    feat1: "Unlimited queries", feat2: "Level filter", feat3: "Creation history", feat4: "Export rhyme list",
    trustReferences: "Trust · Related Tools · References", trust: "Trust", trustText: "This tool is for English learning and rhyming creation only; CEFR levels are matched against the CEFR-J/Octanove wordlists, with a heuristic estimate for unlisted words, and do not equal an official language assessment.", relatedTools: "Related Tools", relatedToolsText: "Synonym Finder · Antonym Finder · Word Root Analyzer · CEFR Level Estimator", references: "References", referencesText: "Datamuse API (rel_rhy, matched against a pronunciation corpus); CEFR-J Wordlist v1.5 (Tono Lab, TUFS); Octanove C1/C2 Vocabulary Profile (CC BY-SA 4.0); ECDICT open EN-ZH dictionary (IPA and Simplified glosses); Traditional Chinese glosses hand-written by the editorial team; Free Dictionary API (examples).",
    q1: "Why do some words that look like they rhyme not appear?", a1: "Rhyme is about sound, not spelling. Datamuse matches rhymes by a pronunciation corpus, so words spelled alike but pronounced differently (like love and move) do not rhyme; confirm the rhyme by IPA.",
    ...
    q3: "Why do some words return no rhymes?", a3: "Rare words, proper nouns, or misspellings may have no rhyme data; some single-syllable or oddly-ending words are simply hard to rhyme perfectly. Switching to a more common word usually returns results.",
    q4: "Where do the IPA and Chinese gloss come from?", a4: "IPA comes from the open ECDICT EN-ZH dictionary (over 20k words built in); unlisted words convert the Datamuse pronunciation to IPA on the fly — this is exactly what determines the rhyme. Chinese glosses use a three-tier priority — the editorial team's hand-written Traditional Chinese first (no tag), then ECDICT's Simplified gloss tagged Simp, then an English definition tagged EN when neither exists. No machine translation is used. Examples come from the Free Dictionary API.",
    ...
    q6: "Is it good for writing lyrics and rap?", a6: "Yes. Find words sharing a rhyme first, then pick by syllable count and rhythm; rap often uses multi-syllable rhymes, so pair it with the synonym tool to balance sound and sense.",
  },
} as const;
...
export default function RhymeFinder() {
  ...
  const runQuery = useCallback(async (rawWord: string) => {
    ...
    setExpanded(null);
    await loadDict();
    const data = await queryDatamuse(`rel_rhy=${encodeURIComponent(word)}`, 24);
  ...
  function fillStandard() { setInput("light"); runQuery("light"); }
  function fillCut() { setInput("time"); runQuery("time"); }
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-emerald-400 to-blue-600" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{countDisplay}</div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.unit}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.primaryValue}</div><div className="mt-1 text-xl font-black">{queryWord || "—"}</div><div className="mt-1 text-xs text-slate-300">rel_rhy</div></div></div>
        ...
        <AdSenseWrapper showAds={true} adSlot="rhyme-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="rhyme-faq" position="inline" /></section>
