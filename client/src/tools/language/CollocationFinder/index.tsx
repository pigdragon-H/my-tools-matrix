...
const ui = {
  zh: {
    ...
    intro: "Collocation Finder 串接 Datamuse 開放語料 API（ml=，語意相近），以「語意相近」為核心找出與你輸入單字意思接近、可在寫作中互相搭配替換的詞。搭配看的是「語意相近度」而非拼字或發音，所以每個結果都標註 CEFR 難度等級、詞性與中文釋義，並附 IPA 音標，可展開查看英文定義與例句，幫你在寫作潤飾、用詞升級、避免重複與表達精準化中快速找到最貼意的搭配詞。",
    ...
    examplesCalculator: "範例 → 查詢", enterValues: "輸入英文單字", examplesHelper: "先用熱門範例了解 CEFR 等級、IPA 音標與中文釋義如何呈現，再換成你自己想搭配的單字。",
    ...
    resultIntelligence: "結果解讀", levelMatrix: "六級 CEFR 搭配詞解讀矩陣", levelMatrixNote: "L7 將搭配詞依 CEFR 等級分層，以 CEFR-J 權威詞表對照，A1 最常用、C2 最罕見；用詞升級時對照你的目標讀者程度。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天找到的搭配詞帶回家", journeyHint: "挑 2–3 個你真正會用到的搭配詞查釋義並造句，比死背 20 個更有效。",
    nextActionLabel: "下一步行動", nextActionTitle: "把搭配詞接到下一個工具", nextActionItem1: "用同義詞查找器比較搭配詞與同義字的細微差異", nextActionItem2: "用 CEFR 等級估算確認搭配詞難度是否符合你的程度", nextActionItem3: "用字根分析器理解搭配詞的語義從何而來，記得更牢",
    shareLinkBtn: "📋 複製結果連結", shareNativeBtn: "📤 分享給朋友", shareCopiedToast: "已複製到剪貼簿 ✓",
    decisionPath: "搭配路徑", decisionTitle: "輸入 → 查詢 → 比較 → 替換", step1: "輸入單字", step2: "查搭配詞", step3: "看 CEFR", step4: "替換用詞",
    knowledge: "知識", knowledgeTitle: "語意搭配在英語學習中的意義", definition: "定義", definitionText: "語意搭配（collocation）這裡指語意相近、可在寫作中互相搭配替換的詞；它看的是「語意相近度」而非拼字或發音，因此判斷是否可替換要看意思是否貼近與語境是否合適，而非同形或押韻。", usage: "用法", usageText: "輸入一個單字後，演算法會以語料庫統計找出意思最接近的詞（例如 make 的搭配詞有 produce、build、create），再依 CEFR 等級與詞性整理，幫你在寫作時把單調的字換成更貼意的搭配詞。", limitations: "限制", limitationsText: "Datamuse 的 ml= 依語意相近度排序，可能含多字片語或罕見詞；近義不等於完全可互換，替換前仍須看語境；CEFR 等級以 CEFR-J/Octanove 詞表為主，未收錄者改用詞頻啟發式推估。", interpretation: "解讀", interpretationText: "A1/A2 搭配詞適合基礎寫作與口語；B1/B2 適合一般寫作與報告；C1/C2 雖然精準，用錯場合反而生硬難懂。", context: "脈絡", contextText: "搭配查詢應與同義詞、字根、CEFR 估算一起用：先用搭配找出語意相近的候選詞，再用同義詞比較細微差異，挑最貼意的字。", example: "範例", exampleText: "查 strong → 得到 powerful(B1)、intense(B2)、robust(C1)；寫作時把 strong coffee 升級為 robust coffee，語感更道地。",
