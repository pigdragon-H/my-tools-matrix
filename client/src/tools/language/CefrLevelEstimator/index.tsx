...
const ui = {
  zh: {
    ...
    intro: "CEFR Level Estimator 內建 1108 筆真實常用單字，依歐洲語言共同參考架構（CEFR）分為 A1、A2、B1、B2、C1、C2 六級，每級約兩百字。輸入一個英文單字，工具會比對內建詞表，顯示該字的 CEFR 等級、詞性與繁體中文義；若查無完全相符，會列出前綴相近的單字供參考。每筆都標註 CEFR 難度等級與詞性，幫你在閱讀、寫作、選字與考試準備時快速判斷單字難度、安排學習順序。本工具為純前端查找，不依賴外部 API 取結果，速度快、離線可用。",
    ...
    examplesCalculator: "範例 → 查找", enterValues: "輸入單字", examplesHelper: "先用熱門範例了解 CEFR 等級、詞性與中文義如何呈現，再換成你自己想查的單字。",
    ...
    resultIntelligence: "結果解讀", levelMatrix: "六級 CEFR 單字分布矩陣", levelMatrixNote: "L7 將符合的單字依 CEFR 等級分層，以 CEFR-J 權威詞表對照，A1 最常用、C2 最罕見；學習時優先挑你該程度的單字。",
    scenarioLayer: "使用場景", scenarioTitle: "什麼時候用 CEFR 程度估算", scenarioNote: "L8 列出四個典型場景，把 CEFR 等級用在對的地方，而不是死背。",
    scenarioExam: "考試準備", scenarioExamNote: "多益、雅思、學測常依 CEFR 分級命題，先查單字等級，優先背你目標等級的字。", scenarioWriting: "寫作選字", scenarioWritingNote: "寫英文文章時，依等級選字，避免用太簡單或太冷僻的字，讓文章層次恰當。", scenarioDaily: "閱讀分級", scenarioDailyNote: "讀英文文章時，查生字等級，判斷這篇文章是否符合你目前的程度。", scenarioBusiness: "教學備課", scenarioBusinessNote: "老師備課時，依 CEFR 等級挑選教學單字，確保難度與學生程度相符。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天查到的單字等級帶回家", journeyHint: "挑 2–3 個你不熟的單字記下等級與中文義，依 CEFR 分級背單字最有效率。",
    ...
    q6: "適合考試準備嗎？", a6: "適合。多益、雅思、學測常依 CEFR 分級命題，查單字等級後，優先背你目標等級的字，並依等級安排學習順序。",
  },
  en: {
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="cefr-level-faq" position="inline" /></section>
