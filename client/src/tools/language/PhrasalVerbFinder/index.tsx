...
const ui = {
  zh: {
    ...
    intro: "Phrasal Verb Finder 內建 204 筆真實常見片語動詞，涵蓋 get、go、come、put、take、make、give、look、turn、bring、set、run、break、keep、hold、fall、cut、call 等十八個高頻 base 動詞，每個動詞至少十個片語。輸入一個動詞，工具會列出它構成的所有片語動詞，每筆都標註 CEFR 難度等級、繁體中文義、英文義與一個例句，幫你在閱讀、寫作、口說與考試時快速掌握片語動詞的正確意思與用法。本工具為純前端查找，不依賴外部 API 取結果，速度快、離線可用。",
    ...
    examplesCalculator: "範例 → 查找", enterValues: "輸入動詞", examplesHelper: "先用熱門範例了解 CEFR 等級、中文義與例句如何呈現，再換成你自己想查的 base 動詞。",
    ...
    resultIntelligence: "結果解讀", levelMatrix: "六級 CEFR 片語動詞解讀矩陣", levelMatrixNote: "L7 將片語動詞依 CEFR 等級分層，以 CEFR-J 權威詞表對照，A1 最常用、C2 最罕見；學習時優先挑你該程度的片語。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天查到的片語動詞帶回家", journeyHint: "挑 2–3 個你不熟的片語動詞造句並記下中文義，片語動詞背成家族最有效。",
    nextActionLabel: "下一步行動", nextActionTitle: "把片語動詞接到下一個工具", nextActionItem1: "用片語搭配查找器找出和片語動詞語意相近的詞，理解語意光譜", nextActionItem2: "用 CEFR 等級估算確認片語動詞難度是否符合你的程度", nextActionItem3: "用字根分析器理解片語中 base 動詞的語義從何而來，記得更牢",
