...
const ui = {
  zh: {
    ...
    intro: "雅思詞彙分析器採用自建的「整段文字逐字 CEFR / 雅思 Band 分析」演算法：貼上一段英文後，工具會把整段拆成單字，逐字比對內建 22000+ 字的 CEFR 詞庫定出 A1~C2 等級，並對應雅思 Band（A1≈4.0、A2≈5.0、B1≈6.0、B2≈7.0、C1≈8.0、C2≈9.0），接著逐字依等級標色、統計各等級佔比、用加權平均預估您這段文字的整體雅思詞彙 Band，再列出文中的「高階詞」(B2 以上) 與「高頻詞」清單，每個列出的詞都附 KK 音標、詞類、繁體中文釋義與一句例句。雅思考生可以一眼看出自己用詞偏簡單還是成熟、哪些高階詞撐起了分數、哪些字反覆用了太多次該替換。本工具的等級分析為純前端，逐字判定不依賴外部 API，速度快、離線可用。",
    ...
    examplesCalculator: "範例 → 分析", enterValues: "貼上一段英文", examplesHelper: "先用範例段落了解逐字標色、各等級佔比、整體 Band 與高階/高頻清單如何呈現，再貼上您自己的雅思作文或閱讀段落分析。",
    queryBtn: "分析詞彙", clearBtn: "清除", hotWords: "範例段落", inputPlaceholder: "在這裡貼上一段英文，例如您的雅思作文段落…",
    ...
    advancedTitle: "文中高階詞 (B2 以上)", advancedNote: "這些是文中達 B2/C1/C2 的較進階詞彙，撐起您的用詞分數；每個附音標、詞類、繁中釋義與例句，建議確認用法是否精準。", advancedEmpty: "文中沒有偵測到 B2 以上的高階詞，建議適量替換幾個基礎詞為同義的進階詞，拉高用詞層次。",
