...
import examplesData from "./unscrambleExamples.json";
...
export default function WordUnscrambler() {
  ...
  const runQuery = useCallback(async (rawWord: string) => {
    ...
    const mapped: ResultCard[] = matches.map((w) => {
      ...
      // ④ 例句：內建 unscrambleExamples.json（英文+繁中翻譯）優先；無則展開時補 dictionaryapi.dev
      const ex = EXAMPLES[w];
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <AdSenseWrapper showAds={true} adSlot="word-unscrambler-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="word-unscrambler-faq" position="inline" /></section>
