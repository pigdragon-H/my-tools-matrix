...
const ui = {
  zh: {
    ...
    examplesCalculator: "範例 → 驗證", enterValues: "輸入單字", examplesHelper: "先用熱門範例了解 Scrabble 計分、CEFR 等級、IPA 音標與中文釋義如何呈現，再換成你自己想驗證的單字。",
    ...
    resultIntelligence: "結果解讀", levelMatrix: "Scrabble 字母分值表", levelMatrixNote: "L7 列出英文版標準 Scrabble 字母分值，1 分字母最常見、10 分字母（Q/Z）最稀有也最值錢；驗證時看你這個字用到哪些高分字母。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天驗證的高分字帶回家", journeyHint: "挑 2–3 個你不認得的高分字查釋義並造句，玩遊戲也能順便背單字。",
    ...
    q1: "合法字是怎麼判定的？", a1: "用自建演算法把你輸入的單字比對內建 22,499 字詞庫：在詞庫中即判為有效拼字遊戲單字，不在則顯示查無此字。純前端判定，不需連外部 API。",
