...
const ui = {
  zh: {
    ...
    intro: "找字工具採用自建的「字母包含」查找演算法：把你輸入的字母當成必須出現的條件，再掃內建 22,499 字的詞庫，找出所有「含有全部這些字母」的英文單字——例如輸入 qu，會列出 queen、quick、square、unique 等含 q 與 u 的字；輸入 ght 會列出 night、light、thought 等。結果按字母數由短到長分組顯示，每個結果都標註 CEFR 難度等級、IPA 音標、詞性與中文釋義，並可展開查看英文定義與例句，是填字遊戲、Wordle 與英語拼字練習的利器。本工具為純前端演算法，不依賴外部 API 取結果，速度快、離線可用。",
    ...
    q5: "本工具和「字母重組器」有什麼不同？", a5: "字母重組器（Word Unscrambler）是「用你的字母去組字」（單字字母 ⊆ 輸入）；找字工具（Word Finder）方向相反，是「找出含有你指定字母的字」（輸入字母 ⊆ 單字），適合填字與 Wordle。",
