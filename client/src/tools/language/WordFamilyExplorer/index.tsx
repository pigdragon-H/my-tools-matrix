...
const ui = {
  zh: {
    ...
    intro: "單字家族探索器採用自建的「單字家族查找 + 反查」演算法：輸入一個英文單字後，工具會比對內建 100 組常用單字家族——不論你輸入的是 happy 還是 happiness、happily、unhappy，都能反查出整個家族，並依詞類分組（動詞 → 名詞 → 形容詞 → 副詞），列出每個家族成員的 KK 音標、詞類、繁體中文釋義與一句例句。掌握一個基底字就能一次擴充整族同源單字，比逐字死背高效許多。本工具為純前端演算法，家族查找不依賴外部 API，速度快、離線可用。",
    ...
    examplesCalculator: "範例 → 探索", enterValues: "輸入單字", examplesHelper: "先用熱門範例了解家族成員、詞類分組、音標與中文釋義如何呈現，再換成你自己想探索的單字（家族中任一成員皆可）。",
    ...
    resultCard: "探索結果", memberUnit: "個成員", baseLabel: "基底字", matchHint: "你查的字", ipaLabel: "音標", ipaPending: "/音標整理中/", meaningLabel: "釋義", glossTagCn: "(簡)", glossTagEn: "(EN)", enGlossHint: "展開看英文定義與例句", expandHint: "展開看例句", collapseHint: "收合", exampleLabel: "例句", enLoading: "載入例句中…", noExample: "查無例句，建議造句練習。",
    resultIntelligence: "結果解讀", levelMatrix: "單字家族構詞規律", levelMatrixNote: "L7 把單字家族最常見的構詞轉換分三類，看你查的字族用到哪些字綴規律，記憶時舉一反三。",
    ...
    q1: "單字家族是怎麼查出來的？", a1: "用自建演算法建立索引：把內建 100 組單字家族的每個成員全部建檔，所以不論你輸入的是基底字還是任一衍生成員，都能反查出整個家族。純前端判定，不需連外部 API。",
    q2: "為什麼輸入衍生字也能查到整族？", a2: "本工具支援反查：演算法把每個家族成員都建成索引，例如輸入 happiness 會反查出整族 happy / happiness / happily / unhappy，並標出你查的是哪個成員。這對你只知道某個衍生字、想看整族時特別有用。",
    ...
    q6: "家族成員為什麼依詞類分組？", a6: "依詞類分組（動詞 → 名詞 → 形容詞 → 副詞）能讓你一眼看出同一字族在不同詞類下的形態，例如 act（動）/ action（名）/ active（形）/ actively（副），記憶時對照字綴規律更有系統。",
  },
  en: {
