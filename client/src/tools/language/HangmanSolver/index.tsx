...
const ui = {
  zh: {
    ...
    intro: "吊人遊戲解題器採用自建的「pattern 比對 + 字母頻率推薦」演算法：你把已經確定的字母填到對應位置、未知位置用底線 _ 代替（例如 _pp_e），再把已經猜錯的字母填到「排除」欄，工具就會掃內建 21,383 字的詞庫，找出長度相符、已知位置吻合、且不含任何排除字母的所有候選單字，依字母序列出。接著它會統計這些候選字「未知位置」最常出現的字母，推薦你下一個最該猜的字母與覆蓋率，幫你用最少的猜測次數破關。每個候選字都標註 CEFR 難度、IPA 音標、詞性與繁體中文釋義，可展開查看英文定義與例句。本工具為純前端演算法，不依賴外部 API 取候選結果。",
    ...
    examplesCalculator: "範例 → 解題", enterValues: "輸入題目", examplesHelper: "先用熱門範例了解候選清單、推薦字母與四要素如何呈現，再換成你自己正在玩的吊人題目。底線 _ 代表未知位置。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天解出的高頻字帶回家", journeyHint: "挑 2–3 個候選清單裡你不認得的字查釋義並造句，玩遊戲也能順便背單字。",
    ...
    q1: "pattern 要怎麼輸入？", a1: "把你已經猜對、位置確定的字母直接填進去，還沒猜出的位置用底線 _（或英文句點 .）代替。例如吊人題顯示「_ p p _ e」，就輸入 _pp_e。長度必須和題目格數相同。",
    q2: "「排除字母」是做什麼的？", a2: "把你已經猜錯、確定不在答案裡的字母填到排除欄（如 tsr），工具會自動把含這些字母的候選字剔除，大幅縮小範圍。已知位置的字母不必再填進排除欄。",
    ...
    q5: "本工具和「字母重組器」「找字工具」有什麼不同？", a5: "字母重組器是把一堆字母重排成單字、找字工具是找含特定字母的字；本工具是「依位置 pattern 解吊人題」——你給已知字母的位置與排除字母，它依長度與位置約束找出所有可能答案，並推薦下一步該猜的字母。",
