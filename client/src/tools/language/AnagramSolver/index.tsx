// @profile B
// Profile B · Language-Hub 自建演算法 · AnagramSolver（GOLD-STANDARD MacroCalculator compatible）
...
const ui = {
  zh: {
    badge: "語言 · 字謎重組 · Language Hub", switchToEnglish: "Switch to English", switchToChinese: "切換到中文", chineseShort: "中", englishShort: "EN",
    title: "字謎重組器 · Anagram Solver", subtitle: "輸入一串字母，立刻用字母指紋演算法重組出所有可能、帶 CEFR 等級與中文釋義的英文單字",
    intro: "Anagram Solver 採用自建的「字母指紋」演算法，把你輸入的字母排序成一組指紋，再比對內建 22,499 字的詞庫，瞬間找出所有同字母異序的英文單字。每個結果都標註 CEFR 難度等級、IPA 音標、詞性與中文釋義，並可展開查看英文定義與例句，幫你在玩拼字遊戲、解字謎、Scrabble 與創意命名時快速找出所有重組答案。本工具為純前端演算法，不依賴外部 API 取結果，速度快、離線可用。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天重組出的新字帶回家", journeyHint: "挑 2–3 個你不認得的重組字查釋義並造句，玩遊戲也能順便背單字。",
  ...
  en: {
    badge: "Language · Anagram · Language Hub", switchToEnglish: "Switch to English", switchToChinese: "切換到中文", chineseShort: "中", englishShort: "EN",
    title: "Anagram Solver", subtitle: "Type a set of letters and instantly rearrange them with a letter-fingerprint algorithm into all possible words, tagged with CEFR level and Chinese gloss",
    intro: "Anagram Solver uses a custom letter-fingerprint algorithm: it sorts the letters you enter into a fingerprint, then matches it against a built-in 22,499-word dictionary to instantly find every word made of the same letters. Each result is tagged with a CEFR difficulty level, IPA transcription, part of speech, and Chinese gloss, expandable to show an English definition and example sentence, helping you quickly find every anagram for word games, puzzles, Scrabble, and creative naming. This tool is a pure front-end algorithm that fetches no external API for results, so it is fast and works offline.",
    ...
    queryBtn: "Solve anagram", clearBtn: "Clear", hotWords: "Popular sets", inputPlaceholder: "Type letters, e.g. listen",
    loading: "Solving…", emptyHint: "Enter letters above and press Solve anagram; all words made of the same letters will appear here.", noResult: "No other words with the same letters; try a different set (more letters usually means more combinations).",
    fallbackTitle: "Loading dictionary", fallbackBody: "The built-in dictionary is loading, please try again shortly.",
    resultCard: "Anagram Results", unit: "anagrams", primaryValue: "Input letters", ipaLabel: "IPA", meaningLabel: "Gloss", glossTagCn: "Simp", glossTagEn: "EN", enGlossHint: "See English definition & example on expand", expandHint: "Show example", collapseHint: "Collapse", exampleLabel: "Example", enLoading: "Loading example…", noExample: "No example found; try writing your own.",
    ...
    decisionPath: "Solving Path", decisionTitle: "Input → Solve → Understand → Apply", step1: "Type letters", step2: "Solve anagram", step3: "Read CEFR", step4: "Use in game",
    ...
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Related Tools", affiliateTitle: "Next tools for anagram solving", premiumTitle: "PRO Anagram Pack", premiumText: "Unlock unlimited solves, filter results by CEFR level, sort by word length, auto-log solving history, and export anagram lists for review.",
...
export default function AnagramSolver() {
  ...
  const runQuery = useCallback(async (rawWord: string) => {
    ...
    const mapped: ResultCard[] = matches.map((w) => {
      ...
      return { word: w, cefr, posKey: "u", ipa, meaningZh, meaningSrc };
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <AdSenseWrapper showAds={true} adSlot="anagram-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 FAQ after ad slot: AD 廣告位 · Advertisement" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="anagram-faq" position="inline" /></section>
