...
const ui = {
  zh: {
    ...
    intro: "Vocabulary DNA Engine 把一個英文單字當成「語義基因序列」：透過 Datamuse 的 rel_syn（同義）、rel_trg（聯想）、ml=（近義）三個端點，即時抓取與查詢字相關的詞，依股別組成這個字的字彙 DNA 圖譜；同時比對內建八十組真實拉丁／希臘字根，找出構成這個字的字根與其語義來源。三端點同時查詢，讓你一眼看出一個字的同義、聯想與近義網絡，並從字根理解它的語義基因。本工具結合 Datamuse 即時查詢與內建字根表，需要網路連線取得即時結果。",
    ...
    examplesCalculator: "範例 → 分析", enterValues: "輸入單字", examplesHelper: "先用熱門範例了解字彙 DNA 圖譜、三股基因與字根比對如何呈現，再換成你自己想分析的單字。",
    ...
    q5: "字彙 DNA 和同義詞查找有什麼不同？", a5: "同義詞查找器只查同義（rel_syn）一股；字彙 DNA 引擎同時查同義、聯想、近義三股並比對字根，給你一個字完整的語義基因圖譜。只想查同義詞時可改用同義詞查找器。",
