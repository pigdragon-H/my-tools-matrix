...
const ui = {
  zh: {
    ...
    intro: "Idiom Explainer 內建 101 筆真實常見英文慣用語。輸入完整慣用語或其中一個關鍵字（例如 cat、fire、ice），工具會比對內建慣用語庫，列出符合的慣用語，每筆都拆解成「字面義」與「真實意思」兩層，再附上英文真實義、一個例句與使用情境說明，並標註 CEFR 難度等級。慣用語最難的地方是字面意思常和真實意思完全不同，本工具把兩者並列，幫你在閱讀、寫作、口說與考試時真正理解慣用語。本工具為純前端查找，不依賴外部 API 取結果，速度快、離線可用。",
    ...
    examplesCalculator: "範例 → 解析", enterValues: "輸入慣用語或關鍵字", examplesHelper: "先用熱門範例了解字面義、真實意思與使用情境如何呈現，再換成你想查的慣用語或關鍵字。",
    ...
    resultIntelligence: "結果解讀", levelMatrix: "六級 CEFR 慣用語解讀矩陣", levelMatrixNote: "L7 將慣用語依 CEFR 等級分層，以 CEFR-J 權威詞表對照，A1 最常用、C2 最罕見；學習時優先挑你該程度的慣用語。",
    ...
    saveShareJourney: "儲存 / 分享", journeyTitle: "把今天解析的慣用語帶回家", journeyHint: "挑 2–3 個你不熟的慣用語，記下字面義與真實意思並造句，慣用語成對記最有效。",
    nextActionLabel: "下一步行動", nextActionTitle: "把慣用語接到下一個工具", nextActionItem1: "用片語動詞查找器理解慣用語中常見動詞構成的片語", nextActionItem2: "用 CEFR 等級估算確認慣用語難度是否符合你的程度", nextActionItem3: "用片語搭配查找器找出和慣用語關鍵字語意相近的詞，擴充表達",
