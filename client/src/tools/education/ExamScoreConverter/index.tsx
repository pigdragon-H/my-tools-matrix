...
const ui = {
  zh: {
    ...
    intro: "本工具輸入百分制分數,搭配班級平均與標準差,即時換算成美式 GPA（4.0 制）、字母等第（A-F）、台灣優甲乙丙丁,以及反映相對位置的 Z 分數與 T 分數,協助你跨制度比較成績。所有換算都在瀏覽器本機完成。",
    ...
    examplesCalculator: "範例 → 計算器", enterValues: "輸入分數、平均與標準差", examplesHelper: "先用範例分數理解換算邏輯,再輸入你的百分制分數、班級平均與標準差,即可一次得到 GPA、字母等第、台灣等第與相對位置分數。",
    ...
    emotionConversionLayer: "情緒與轉換層", turnIntoPlan: "把成績整合進升學與獎學金規劃", conversionNote: "L9 會連動目前換算結果,顯示 GPA、字母等第與相對位置,協助你判斷申請門檻與需補強的科目。",
    progressInsight: "進度洞察卡", possibleTarget: "目前成績定位", dailyGap: "台灣等第", weeklyTrend: "GPA", motivation: "動力卡", keepMomentum: "從單次成績走向長期 GPA 追蹤",
    saveShareJourney: "儲存 / 分享", journeyTitle: "把這次換算帶進你的成績紀錄", journeyHint: "每次更換分數或調整平均與標準差時重新換算,並把結果記錄到成績單或升學試算表。",
    ...
    knowledge: "知識", knowledgeTitle: "成績換算與相對位置的意義", definition: "定義", definitionText: "成績換算把不同制度的分數對應起來：百分制看絕對得分、GPA 看 4.0 制平均、字母等第看區間;Z 分數則衡量你相對全班的位置。",
    ...
    q2: "Z 分數與 T 分數有什麼用？", a2: "它們衡量你相對全班的位置。Z = (分數 − 平均) ÷ 標準差,大於 0 即高於平均;T = 50 + Z × 10,把 Z 平移成以 50 為中心、較直觀的分數。",
    ...
    q5: "及格線是幾分？", a5: "本工具以 60 分為及格線,對應字母 D-、台制丙;低於 60 則為 F／丁。實際及格標準仍以你的學校規定為準。",
    q6: "這個工具會上傳我的成績嗎？", a6: "不會。所有換算與統計都在你的瀏覽器本機完成,分數不會上傳到任何伺服器。",
  },
  en: {
