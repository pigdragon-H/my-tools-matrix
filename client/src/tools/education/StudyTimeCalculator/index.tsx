...
const ui = {
  zh: {
    ...
    nextActionLabel: "下一步行動", nextActionTitle: "把結果接到下一個工具", nextActionItem1: "用 GPA 計算機驗證目標 GPA 在你目前累積成績下是否可達", nextActionItem2: "用成績計算機反推單科 Final Grade Needed,聚焦高權重項目", nextActionItem3: "用學費成本計算機評估若延長一學期完成課程的金錢成本",
    ...
    q1: "為什麼算出來的時數比我實際讀的還多?", a1: "三個常見原因:(1)你已經有先備知識(例如高中 AP 過,大一微積分自然輕鬆),可降一級難度;(2)你高效讀書(主動回想、間隔複習),可降至 Carnegie 下限;(3)你低估了考試週與作業週的時間漲幅(平均上升 40-60%),這是平均週數,不含 crunch time。",
    ...
    q5: "在職進修(part-time)該怎麼用?", a5: "把學分數調為實際選的學分(例如 6-9 學分),目標 GPA 維持你的個人目標即可。本工具的 Carnegie Unit 計算對全職與兼職同樣適用;但兼職學生通常需要把每週時數平均到 5-7 天執行(而非全職的 5 天),才不會造成週末崩潰。",
