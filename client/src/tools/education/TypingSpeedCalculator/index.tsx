...
const ui = {
  zh: {
    ...
    intro: "本工具以你輸入的字元數、所花秒數與錯字數,即時算出每分鐘字數（WPM）的總速度與扣除錯字的淨速度、準確率與每分鐘字元數（CPM）。WPM 以「字元 ÷ 5」估算一個字,協助你客觀衡量打字效率並設定練習目標。所有計算都在瀏覽器本機完成。",
    ...
    examplesCalculator: "範例 → 計算器", enterValues: "輸入字元數、秒數與錯字", examplesHelper: "先用範例數據理解 WPM 與準確率的算法,再填入你測驗得到的字元數、花費秒數與錯字數,即可得到你的打字速度與分析。",
    ...
    emotionConversionLayer: "情緒與轉換層", turnIntoPlan: "把打字速度整合進練習計畫", conversionNote: "L9 會連動目前計算結果,顯示淨速度、等級與準確率,協助你判斷該先衝速度還是先修正錯字。",
    progressInsight: "進度洞察卡", possibleTarget: "目前打字定位", dailyGap: "準確率", weeklyTrend: "淨速度", motivation: "動力卡", keepMomentum: "從單次測驗走向長期速度追蹤",
    saveShareJourney: "儲存 / 分享", journeyTitle: "把這次測驗帶進你的練習紀錄", journeyHint: "每次更換數據或重新測驗時重新計算,並把結果記錄到練習日誌觀察進步曲線。",
    nextActionLabel: "下一步行動", nextActionTitle: "將結果接到下一個工具", nextActionItem1: "用閱讀速度計算器比較你的輸入與閱讀效率", nextActionItem2: "用學習時間計算器估算完成一份文件所需時間", nextActionItem3: "用字數計算器確認文件長度與輸入量",
    ...
    q6: "這個工具會上傳我的數據嗎？", a6: "不會。所有字數估算與速度計算都在你的瀏覽器本機完成,輸入的數據不會上傳到任何伺服器。",
  },
  en: {
