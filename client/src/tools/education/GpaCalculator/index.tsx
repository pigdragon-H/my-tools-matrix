...
const ui = {
  zh: {
    ...
    inputJson: "課程清單(每行:課名,等第,學分)", indentSize: "等第制度", sortKeys: "重修不計入舊成績(Academic Renewal)",
    ...
    decisionPath: "決策路徑", decisionTitle: "課程輸入 → 等第換算 → GPA 計算 → 學業決策", bmrStep: "課程輸入", deficitStep: "等第換算", trendStep: "GPA 計算", mealStep: "學業決策",
  ...
  en: {
    ...
    inputJson: "Course list (one per line: name, grade, credits)", indentSize: "Grading scale", sortKeys: "Exclude old grades from retakes (Academic Renewal)",
    ...
    decisionPath: "Decision path", decisionTitle: "Course input → Grade conversion → GPA calc → Academic decision", bmrStep: "Course input", deficitStep: "Grade conversion", trendStep: "GPA calc", mealStep: "Decision",
...
export default function GpaCalculator() {
  ...
  const [scale, setScale] = useState<"2" | "4" | "tab">("2"); // 2=4.0, 4=4.3, tab=100pt (本工具計算僅支援 4.0)
  ...
  function fillFreshman() { setUnit("metric"); setInputText(coursesToText(SAMPLE_FRESHMAN)); setScale("2"); setRenewal(false); }
  function fillRecovery() { setUnit("imperial"); setInputText(coursesToText(SAMPLE_RECOVERY)); setScale("2"); setRenewal(true); }
      ...
      */}
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L4/L5 */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4"><label className="block text-sm font-black text-slate-700">{t.inputJson}<textarea className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 font-mono text-xs" rows={8} value={inputText} onChange={(e) => setInputText(e.target.value)} spellCheck={false} placeholder={lang === "zh" ? "Calculus, A, 4\nEnglish, A-, 3" : "Calculus, A, 4\nEnglish, A-, 3"} /></label><div className="grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.indentSize}<div className="mt-2 grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-2"><button type="button" className={`rounded-xl px-3 py-2 text-xs font-black ${scale === "2" ? "bg-emerald-600 text-white" : "bg-white text-slate-700"}`} onClick={() => setScale("2")}>{t.indent2}</button><button type="button" className={`rounded-xl px-3 py-2 text-xs font-black ${scale === "4" ? "bg-slate-300 text-slate-500" : "bg-white text-slate-400"}`} onClick={() => setScale("2")} disabled>{t.indent4}</button><button type="button" className={`rounded-xl px-3 py-2 text-xs font-black ${scale === "tab" ? "bg-slate-300 text-slate-500" : "bg-white text-slate-400"}`} onClick={() => setScale("2")} disabled>{t.indentTab}</button></div></label><label className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-700"><input type="checkbox" checked={renewal} onChange={(e) => setRenewal(e.target.checked)} className="h-5 w-5 accent-emerald-600" /><span>{t.sortKeys}</span></label></div></div></div>
          </div>
        </section>
