...
const ui = {
  zh: {
    ...
    intro: "本工具以經典的 SM-2 間隔重複演算法,根據你這次回想的品質（0-5）、目前複習次數、難易度因子與上次間隔,算出更新後的難易度因子、下次複習間隔天數與預估記憶保留率,並推算未來五次的複習排程。所有計算都在瀏覽器本機完成,協助你用最少的複習次數記住最多內容。",
    ...
    examplesCalculator: "範例 → 計算器", enterValues: "輸入品質、次數、難易度與上次間隔", examplesHelper: "先用範例理解 SM-2 的運作,再填入你這次回想的品質、目前複習次數、難易度因子與上次間隔,即可得到下次間隔與完整排程。",
    ...
    emotionConversionLayer: "情緒與轉換層", turnIntoPlan: "把複習排程整合進學習計畫", conversionNote: "L9 會連動目前計算結果,顯示下次間隔、難易度因子與保留率,協助你安排接下來幾天的複習節奏。",
    progressInsight: "進度洞察卡", possibleTarget: "目前複習定位", dailyGap: "保留率", weeklyTrend: "下次間隔", motivation: "動力卡", keepMomentum: "從單張卡片走向長期記憶曲線",
    saveShareJourney: "儲存 / 分享", journeyTitle: "把這次排程帶進你的學習行事曆", journeyHint: "每次更換品質或調整參數時重新計算,並把下次複習日期記到行事曆或記憶卡 App。",
    ...
    trustReferences: "信任聲明 · 相關工具 · 參考資料", trust: "信任聲明", trustText: "本工具僅依 SM-2 公式做間隔與保留率估算,屬學習規劃參考;不取代你所用記憶卡 App 的實際排程。", relatedTools: "相關工具", relatedToolsText: "學習時間計算器 · 閱讀速度計算器 · 打字速度計算器 · GPA 計算器", references: "參考資料", referencesText: "SuperMemo SM-2 演算法原始定義;間隔重複與遺忘曲線研究;難易度因子的更新公式;記憶卡複習排程實務。",
    ...
    q3: "難易度因子（EF）是什麼？", a3: "EF 控制間隔的成長率,初始 2.5、最低 1.3;品質高 EF 上升、間隔拉長,品質低 EF 下降、複習更頻繁,反映卡片對你的難易度。",
    q4: "為什麼第二次間隔固定 6 天？", a4: "這是 SM-2 的設定：第一次成功複習後 1 天再見、第二次後 6 天,之後才開始以「前次間隔 × EF」拉長,讓初期先穩固記憶。",
    q5: "保留率準確嗎？", a5: "保留率是依回想品質估算的近似值,協助你直觀判斷記憶強度;實際保留受睡眠、專注、內容難度與複習時機影響,僅供參考。",
    q6: "這個工具會上傳我的資料嗎？", a6: "不會。所有 SM-2 計算與排程都在你的瀏覽器本機完成,輸入的參數不會上傳到任何伺服器。",
  },
  en: {
