// @profile B
// Profile B · 計算機-YMYL · SaasMetricsCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 0", label: { zh: "淨流失中 (< 0)", en: "Net churn (< 0)" }, desc: { zh: "落在「淨流失中」級距< 0。月淨成長率為負,流失大於新增,經常性收入正在萎縮,須優先改善留存。", en: "Falls in the \"Net churn\" band (< 0). Negative net growth; churn exceeds new revenue and recurring revenue is shrinking — fix retention first." } },
  { key: "normal", range: "0–1", label: { zh: "成長停滯 (0–1)", en: "Stalled (0–1)" }, desc: { zh: "落在「成長停滯」級距0–1。月淨成長接近零,收入停滯,新增與流失大致抵銷,須加速獲客或降低流失。", en: "Falls in the \"Stalled\" band (0–1). Near-zero net growth; revenue is stalled as adds and churn cancel out — accelerate acquisition or cut churn." } },
  { key: "notable", range: "1–3", label: { zh: "緩步成長 (1–3)", en: "Slow growth (1–3)" }, desc: { zh: "落在「緩步成長」級距1–3。月淨成長緩慢,收入逐步累積,但成長動能偏弱,仍有提升空間。", en: "Falls in the \"Slow growth\" band (1–3). Slow net growth; revenue accumulates gradually but momentum is weak with room to improve." } },
  { key: "high", range: "3–6", label: { zh: "穩健成長 (3–6)", en: "Steady growth (3–6)" }, desc: { zh: "落在「穩健成長」級距3–6。月淨成長穩健,新增明顯快於流失,經常性收入持續複利累積。", en: "Falls in the \"Steady growth\" band (3–6). Steady net growth; adds clearly outpace churn and recurring revenue compounds over time." } },
  { key: "major", range: "6–10", label: { zh: "快速成長 (6–10)", en: "Fast growth (6–10)" }, desc: { zh: "落在「快速成長」級距6–10。月淨成長快速,屬高成長 SaaS 區間,須留意規模擴張下的留存與單位經濟。", en: "Falls in the \"Fast growth\" band (6–10). Fast net growth in a high-growth SaaS range; watch retention and unit economics as you scale." } },
  { key: "executive", range: "≥ 10", label: { zh: "爆發成長 (≥ 10)", en: "Hyper growth (≥ 10)" }, desc: { zh: "落在「爆發成長」級距≥ 10。月淨成長爆發,收入快速翻倍,須確保基礎建設、留存與現金流能支撐成長。", en: "Falls in the \"Hyper growth\" band (≥ 10). Hyper net growth doubling revenue fast; ensure infrastructure, retention and cash flow can support it." } },
] as const;
...
const ui = {
  zh: {
    badge: "財務 · SaaS 指標計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "SaaS Metrics Calculator · SaaS 指標計算機",
    subtitle: "由客戶數、月收入與成長率計算 MRR、ARR 與淨成長率。",
    intro: "本工具為 SaaS 指標計算機，依公開公式於瀏覽器端試算，輸入付費客戶數、每客戶月經常性收入、月成長率(%)、月毛流失率(%)後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算SaaS 指標計算機",
    examplePreview: "月經常性收入(MRR)",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入付費客戶數、每客戶月經常性收入、月成長率(%)、月毛流失率(%)",
    ...
    flowDemo: "數字流向示範",
    calculator: "SaaS 指標計算機",
    numberOfPayingCustomers: "付費客戶數",
    monthlyRevenuePerCustomer: "每客戶月經常性收入",
    monthlyGrowthRate: "月成長率(%)",
    monthlyGrossChurnRate: "月毛流失率(%)",
    resultCard: "結果卡片",
    primaryValue: "月經常性收入(MRR)",
    primaryUnitTail: "",
    secondaryLabel: "年經常性收入(ARR)",
    secondaryTail: "",
    metricALabel: "月經常性收入(MRR)",
    metricACaption: "依公開公式試算的主要數值",
    metricATail: "",
    metricBLabel: "年經常性收入(ARR)",
    metricBCaption: "與主要結果連動的次要量值",
    metricBTail: "",
    metricCLabel: "月淨成長率",
    metricCCaption: "百分比形式的觀察點",
    metricCTail: "%",
    headlineCaption: "SaaS 指標計算機 · 即時試算",
    fatLossTarget: "一年後預估 ARR",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "SaaS 指標計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 付費客戶數 與 月成長率(%) 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "SaaS 指標計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 付費客戶數、每客戶月經常性收入、月成長率(%)、月毛流失率(%) 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "SaaS recurring revenue metrics analysis。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "工具知識庫",
    knowledgeTitle: "SaaS 指標計算機 · 觀念整理",
    definition: "定義",
    definitionText: "SaaS 指標計算機以付費客戶數、每客戶月收入、月成長率與流失率計算 MRR、ARR、月淨成長率,並推算一年後的預估 ARR。",
    formula: "公式",
    formulaText: "MRR = 付費客戶數 × 每客戶月收入;ARR = MRR × 12;淨成長率 = 月成長率 − 月流失率;預估 ARR = ARR ×(1 + 淨成長)^12。",
    limitations: "限制",
    limitationsText: "本工具以固定成長率與流失率的簡化模型估算,未計入擴張收入、淨流失、季節性與成本結構,僅供概念性估算。",
    interpretation: "解讀",
    interpretationText: "MRR 與 ARR 越高代表訂閱規模越大;月淨成長率為正且越高,代表經常性收入複利累積越快,SaaS 體質越健康。",
    context: "情境",
    contextText: "常見使用情境包括日常財務檢視、年度規劃、重大決策前的快速估算。建議搭配LTV/CAC 比率計算機 等延伸工具一起使用。",
    example: "範例",
    ...
    premiumTitle: "SaaS Metrics Pro 進階",
    premiumText: "進階版加入分群留存曲線、淨收入留存(NRR)、擴張收入拆解與多情境 ARR 預測。",
    premiumChips_zh: "分群留存|淨收入留存|擴張收入|ARR預測",
    premiumChips_en: "Cohort retention|NRR|Expansion|ARR forecast",
    ...
    q1: "MRR 和 ARR 是什麼?",
    a1: "MRR 是月經常性收入,代表每月可預期的訂閱收入;ARR 是年經常性收入,等於 MRR × 12,是衡量 SaaS 規模與成長的核心指標。",
    q2: "這些指標怎麼算?",
    a2: "MRR =付費客戶數 × 每客戶月收入;ARR = MRR × 12;淨成長率 = 月成長率 − 月毛流失率;預估 ARR = 目前 ARR ×(1 + 淨成長率)^12。",
    q3: "什麼是淨成長率?",
    a3: "淨成長率是新增收入成長扣除流失後的真實成長速度;只有淨成長為正,經常性收入才會實際增加,是評估 SaaS 健康度的關鍵。",
    q4: "為什麼要看流失率?",
    a4: "流失率反映客戶或收入的流失速度;高流失會抵銷新增、侵蝕成長,即使獲客強勁也可能原地踏步,因此留存與成長同等重要。",
    q5: "預估 ARR 怎麼推算?",
    a5: "預估 ARR 以目前 ARR 為基礎,假設淨成長率維持不變並逐月複利 12 個月推算;實際會隨成長與流失變化而不同。",
    q6: "這個結果準確嗎?",
    a6: "本工具以固定成長率與流失率的簡化模型估算,未計入擴張收入、淨流失與季節性,僅供概念性參考。"
  },
  en: {
    badge: "Finance · SaaS Metrics Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "SaaS Metrics Calculator",
    subtitle: "Calculate MRR, ARR and net growth from customer count, monthly revenue and growth rate.",
    intro: "SaaS Metrics Calculator runs the standard formula in your browser. Enter number of paying customers, monthly revenue per customer, monthly growth rate, monthly gross churn rate to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try SaaS Metrics Calculator",
    examplePreview: "Monthly recurring revenue",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter number of paying customers, monthly revenue per customer, monthly growth rate, monthly gross churn rate",
    ...
    baselineExampleValue: "Growth-stage SaaS case",
    baselineExampleNote: "Number of paying customers 500 · Monthly revenue per customer 60",
    activeExample: "Advanced example",
    activeExampleValue: "Hyper-growth SaaS case",
    activeExampleNote: "Number of paying customers doubled · watch Monthly recurring revenue react",
    flowDemo: "Data flow demo",
    calculator: "SaaS Metrics Calculator",
    numberOfPayingCustomers: "Number of paying customers",
    monthlyRevenuePerCustomer: "Monthly revenue per customer",
    monthlyGrowthRate: "Monthly growth rate",
    monthlyGrossChurnRate: "Monthly gross churn rate",
    resultCard: "Result card",
    primaryValue: "Monthly recurring revenue",
    primaryUnitTail: "",
    secondaryLabel: "Annual recurring revenue",
    secondaryTail: "",
    metricALabel: "Monthly recurring revenue",
    metricACaption: "Main figure from the standard formula",
    metricATail: "",
    metricBLabel: "Annual recurring revenue",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "",
    metricCLabel: "Net monthly growth rate",
    metricCCaption: "Percentage view",
    metricCTail: "%",
    headlineCaption: "SaaS Metrics Calculator · live calc",
    fatLossTarget: "Projected ARR in 12 months",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "SaaS Metrics Calculator · band matrix",
    ...
    dailyGap: "Daily gap",
    tertiaryTag: "Net monthly growth rate",
    ...
    nextActionItem1: "Move Number of paying customers and Monthly growth rate by ±10% to see sensitivity.",
    ...
    decisionTitle: "SaaS Metrics Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill number of paying customers, monthly revenue per customer, monthly growth rate, monthly gross churn rate.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "SaaS Metrics Calculator standard formula.",
    ...
    knowledgeTitle: "SaaS Metrics Calculator · concept primer",
    definition: "Definition",
    definitionText: "SaaS Metrics Calculator converts inputs (number of paying customers, monthly revenue per customer, monthly growth rate, monthly gross churn rate) into Monthly recurring revenue. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(number of paying customers, monthly revenue per customer, monthly growth rate, monthly gross churn rate)",
    ...
    context: "Context",
    contextText: "Common contexts include daily finance review, annual planning, and pre-decision quick estimates. Pair with LTV/CAC Ratio Calculator for a fuller picture.",
    example: "Example",
    ...
    premiumTitle: "SaaS Metrics Pro",
    premiumText: "Pro adds cohort retention curves, net revenue retention, expansion-revenue breakdown and multi-scenario ARR forecasting.",
    premiumChips_zh: "分群留存|淨收入留存|擴張收入|ARR預測",
    premiumChips_en: "Cohort retention|NRR|Expansion|ARR forecast",
    ...
    q1: "What does SaaS Metrics Calculator calculate?",
    a1: "SaaS Metrics Calculator applies the standard formula to your inputs and returns Monthly recurring revenue plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for SaaS Metrics Calculator?",
    a2: "Enter number of paying customers, monthly revenue per customer, monthly growth rate, monthly gross churn rate. SaaS Metrics Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Pro adds cohort retention curves, net revenue retention, expansion-revenue breakdown and multi-scenario ARR forecasting."
  },
} as const;
...
export default function SaasMetricsCalculator() {
  ...
  const [numberOfPayingCustomers, setNumberOfPayingCustomers] = useState("500");
  const [monthlyRevenuePerCustomer, setMonthlyRevenuePerCustomer] = useState("60");
  const [monthlyGrowthRate, setMonthlyGrowthRate] = useState("8");
  const [monthlyGrossChurnRate, setMonthlyGrossChurnRate] = useState("3");
  const t = ui[lang];

  const result = useMemo(() => {
const customers = Number(numberOfPayingCustomers) || 0; const arpu = Number(monthlyRevenuePerCustomer) || 0; const growth = Number(monthlyGrowthRate) || 0; const churn = Number(monthlyGrossChurnRate) || 0; const mrr = customers * arpu; const arr = mrr * 12; const netGrowth = growth - churn; const projectedArr = arr * Math.pow(1 + netGrowth / 100, 12); return { primaryKey: mrr, secondaryKey: arr, tertiaryKey: netGrowth, quaternaryKey: projectedArr };
  }, [numberOfPayingCustomers, monthlyRevenuePerCustomer, monthlyGrowthRate, monthlyGrossChurnRate]);
  ...
  function fillSolid() { setUnit("metric"); setNumberOfPayingCustomers("500"); setMonthlyRevenuePerCustomer("60"); setMonthlyGrowthRate("8"); setMonthlyGrossChurnRate("3"); }
  function fillHighSalary() { setUnit("imperial"); setNumberOfPayingCustomers("1000"); setMonthlyRevenuePerCustomer("80"); setMonthlyGrowthRate("12"); setMonthlyGrossChurnRate("2"); }
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ede9fe,_#f8fafc_45%,_#e0e7ff)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-violet-100 bg-white/90 p-6 shadow-2xl shadow-violet-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-violet-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-violet-600 p-5 text-white"><div className="text-xs font-bold uppercase text-violet-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-violet-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{numberOfPayingCustomers} × {monthlyRevenuePerCustomer}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-violet-200 bg-violet-50 px-5 py-4 text-sm font-black text-violet-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-emerald-700">{t.numberOfPayingCustomers}<input type="number" step="10" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={numberOfPayingCustomers} onChange={(e) => setNumberOfPayingCustomers(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.monthlyRevenuePerCustomer}<input type="number" step="5" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthlyRevenuePerCustomer} onChange={(e) => setMonthlyRevenuePerCustomer(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.monthlyGrowthRate}<input type="number" step="0.5" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthlyGrowthRate} onChange={(e) => setMonthlyGrowthRate(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.monthlyGrossChurnRate}<input type="number" step="0.5" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthlyGrossChurnRate} onChange={(e) => setMonthlyGrossChurnRate(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="saas-metrics-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="saas-metrics-calculator-faq" position="inline" /></section>
