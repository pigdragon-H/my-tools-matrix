// @profile B
// Profile B · 計算機-YMYL · AmortizationScheduleCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const ui = {
  zh: {
    badge: "財務 · 攤還排程計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Amortization Schedule Calculator · 攤還排程計算機",
    subtitle: "輸入貸款本金、利率與年限，立即估算每月還款、總利息與剩餘本金",
    intro: "本工具為 攤還排程計算機，依公開公式於瀏覽器端試算，輸入貸款本金、年利率%、貸款年限、已繳期數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算攤還排程計算機",
    examplePreview: "利息佔本金比",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入貸款本金、年利率%、貸款年限、已繳期數",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "本金 50 萬 · 5% · 30 年",
    baselineExampleNote: "貸款本金 500000 · 年利率% 5",
    activeExample: "進階範例",
    activeExampleValue: "本金 30 萬 · 3% · 15 年",
    activeExampleNote: "貸款本金 加倍 · 觀察 利息佔本金比 變化",
    flowDemo: "數字流向示範",
    calculator: "攤還排程計算機",
    loanPrincipal: "貸款本金",
    annualRatePct: "年利率%",
    ...
    metricCTail: "$",
    headlineCaption: "攤還排程計算機 · 即時試算",
    fatLossTarget: "剩餘本金",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "攤還排程計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 貸款本金 與 貸款年限 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "攤還排程計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 貸款本金、年利率%、貸款年限、已繳期數 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依本金、年利率、年限計算每月還款、總利息與目前剩餘本金。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "知識庫",
    knowledgeTitle: "攤還排程計算機 · 觀念整理",
    definition: "定義",
    definitionText: "攤還排程計算機以貸款本金、年利率、年限計算本息均攤下的每月還款、總利息與目前剩餘本金,適用房貸、車貸、信貸等等額還款貸款。",
    formula: "公式",
    formulaText: "每月還款 = P × r / (1 − (1+r)^−n);剩餘本金 = P × (1+r)^k − 每月還款 × ((1+r)^k − 1) / r",
    limitations: "限制",
    limitationsText: "本工具假設固定利率與本息均攤,不計寬限期、浮動利率、提前還款違約金與手續費;實際請以銀行攤還表為準。",
    interpretation: "解讀",
    interpretationText: "總利息越高代表年限越長或利率越高,可考慮縮短年限或提前還款;剩餘本金為再融資與提前清償的計算基礎。",
    context: "情境",
    ...
    premiumTitle: "專業版 攤還與再融資工具組",
    premiumText: "解鎖逐期攤還明細、提前還款情境模擬、本金均攤 vs 本息均攤比較、再融資省息分析與 PDF 匯出。",
    premiumChips_zh: "逐期明細|提前還款|攤還比較|PDF匯出",
    premiumChips_en: "Schedule|Prepayment|Compare|PDF",
    ...
    q1: "攤還表怎麼看?",
    a1: "攤還表列出每一期的還款金額、其中多少還利息、多少還本金,以及剩餘本金。本工具計算總覽指標:每月還款、總利息、目前剩餘本金,讓你快速掌握還款結構。",
    q2: "前幾年為什麼幾乎都在還利息?",
    a2: "本息均攤(等額本息)在前期剩餘本金最大,因此利息佔比最高。以 30 年期 5% 房貸為例,第一年約 80% 的還款都在繳利息,本金減少緩慢,這是複利的數學結果,並非銀行多收。",
    q3: "提前還款能省多少?",
    a3: "提前還款直接減少剩餘本金,後續利息按新本金重算。越早還、省越多。一般在貸款前 1/3 期間提前還款的省息效果最大。建議搭配再融資計算機比較。",
    q4: "本息均攤和本金均攤差在哪?",
    a4: "**本息均攤**每月還款固定,前期利息多、後期本金多;**本金均攤**每月還相同本金,利息隨剩餘本金遞減,初期還款最高、之後遞減,總利息較低但前期壓力大。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內以 JavaScript 完成,本金、利率等資料不會傳送到任何伺服器。",
    q6: "可以匯出完整每期攤還表嗎?",
    a6: "完整逐期攤還表(每期本金/利息/餘額)、提前還款情境模擬與 PDF 匯出屬於專業版功能,免費版提供總覽指標。"
  },
  en: {
    badge: "Finance · Amortization Schedule Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Amortization Schedule Calculator · 攤還排程計算機",
    subtitle: "Enter loan principal, rate, and term to estimate monthly payment, total interest, and remaining balance",
    intro: "Amortization Schedule Calculator runs the standard formula in your browser. Enter loan principal, annual rate pct, loan years, months paid to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Amortization Schedule Calculator",
    ...
    enterValues: "Enter loan principal, annual rate pct, loan years, months paid",
    ...
    baselineExampleValue: "P 500k · 5% · 30y",
    baselineExampleNote: "Loan Principal 500000 · Annual Rate Pct 5",
    activeExample: "Advanced example",
    activeExampleValue: "P 300k · 3% · 15y",
    activeExampleNote: "Loan Principal doubled · watch Interest-to-Principal react",
    flowDemo: "Data flow demo",
    calculator: "Amortization Schedule Calculator",
    ...
    metricCTail: "$",
    headlineCaption: "Amortization Schedule Calculator · live calc",
    fatLossTarget: "Remaining Balance",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Amortization Schedule Calculator · band matrix",
    ...
    nextActionItem1: "Move Loan Principal and Loan Years by ±10% to see sensitivity.",
    ...
    decisionTitle: "Amortization Schedule Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill loan principal, annual rate pct, loan years, months paid.",
    ...
    knowledge: "Knowledge",
    knowledgeTitle: "Amortization Schedule Calculator · concept primer",
    definition: "Definition",
    definitionText: "Amortization Schedule Calculator converts inputs (loan principal, annual rate pct, loan years, months paid) into Interest-to-Principal. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "每月還款 = P × r / (1 − (1+r)^−n);剩餘本金 = P × (1+r)^k − 每月還款 × ((1+r)^k − 1) / r",
    ...
    premiumTitle: "Pro Amortization Suite",
    premiumText: "Unlock per-period schedule, prepayment scenarios, equal-principal vs equal-payment comparison, refinance savings, and PDF export.",
    premiumChips_zh: "逐期明細|提前還款|攤還比較|PDF匯出",
    premiumChips_en: "Schedule|Prepayment|Compare|PDF",
    ...
    q1: "About: 攤還表怎麼看?",
    a1: "For \"攤還表怎麼看?\": Amortization Schedule Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 前幾年為什麼幾乎都在還利息?",
    a2: "For \"前幾年為什麼幾乎都在還利息?\": Amortization Schedule Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 提前還款能省多少?",
    a3: "For \"提前還款能省多少?\": Amortization Schedule Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 本息均攤和本金均攤差在哪?",
    a4: "For \"本息均攤和本金均攤差在哪?\": Amortization Schedule Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳嗎?",
    a5: "For \"資料會上傳嗎?\": Amortization Schedule Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 可以匯出完整每期攤還表嗎?",
    a6: "For \"可以匯出完整每期攤還表嗎?\": Amortization Schedule Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
...
export default function AmortizationScheduleCalculator() {
  ...
  }, [loanPrincipal, annualRatePct, loanYears, monthsPaid]);

  const primaryDisplay = fmt(result.interestRatio, 1);
  const secondaryDisplay = fmt(result.monthly, 2);
  const tertiaryDisplay = fmt(result.totalInterest, 2);
  const quaternaryDisplay = fmt(result.balance, 2);

  function fillSolid() { setUnit("metric"); setLoanPrincipal("500000"); setAnnualRatePct("5"); setLoanYears("30"); setMonthsPaid("12"); }
  function fillHighSalary() { setUnit("imperial"); setLoanPrincipal("300000"); setAnnualRatePct("3"); setLoanYears("15"); setMonthsPaid("24"); }
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#e0f2fe,_#f8fafc_45%,_#dbeafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-sky-100 bg-white/90 p-6 shadow-2xl shadow-sky-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-sky-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-sky-600 p-5 text-white"><div className="text-xs font-bold uppercase text-sky-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}%</div><div className="text-sm font-bold text-sky-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{loanPrincipal} × {annualRatePct}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-sky-200 bg-sky-50 px-5 py-4 text-sm font-black text-sky-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.loanPrincipal}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={loanPrincipal} onChange={(e) => setLoanPrincipal(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.annualRatePct}<input type="number" step="0.1" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={annualRatePct} onChange={(e) => setAnnualRatePct(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.loanYears}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={loanYears} onChange={(e) => setLoanYears(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.monthsPaid}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthsPaid} onChange={(e) => setMonthsPaid(e.target.value)} /></label></div></div>
          </div>
        </section>
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-sky-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-sky-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{primaryDisplay}<span className="text-3xl">%</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.primaryValue}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.secondaryLabel}</div><div className="mt-1 text-xl font-black">{secondaryDisplay}</div><div className="mt-1 text-xs text-slate-300">{t.secondaryTail}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.metricALabel}</div><div className="mt-1 text-xs font-black text-emerald-700">{t.metricACaption}</div><p className="mt-2 text-3xl font-black text-emerald-950">{tertiaryDisplay}</p><p className="text-sm font-bold text-emerald-700">{t.metricATail}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.metricBLabel}</div><div className="mt-1 text-xs font-black text-blue-700">{t.metricBCaption}</div><p className="mt-2 text-3xl font-black text-blue-950">{quaternaryDisplay}</p><p className="text-sm font-bold text-blue-700">{t.metricBTail}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.metricCLabel}</div><div className="mt-1 text-xs font-black text-slate-700">{t.metricCCaption}</div><p className="mt-2 text-3xl font-black text-slate-950">{secondaryDisplay}</p><p className="text-sm font-bold text-slate-700">{t.metricCTail}</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="amortization-schedule-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-sky-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-sky-50 p-4"><div className="text-xs font-black uppercase text-sky-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-sky-950">{secondaryDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.tertiaryTag}</div><div className="mt-1 text-3xl font-black text-emerald-950">{tertiaryDisplay}</div></div></div></article>
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="amortization-schedule-calculator-faq" position="inline" /></section>
