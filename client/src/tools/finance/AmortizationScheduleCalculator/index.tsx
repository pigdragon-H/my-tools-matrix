...
const ui = {
  zh: {
    ...
    intro: "本工具為 攤還排程計算機，依公開公式於瀏覽器端試算，輸入貸款本金、年利率%、貸款年限、已繳期數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q1: "攤還表怎麼看?",
    a1: "攤還表列出每一期的還款金額、其中多少還利息、多少還本金,以及剩餘本金。本工具計算總覽指標:每月還款、總利息、目前剩餘本金,讓您快速掌握還款結構。",
    q2: "前幾年為什麼幾乎都在還利息?",
    ...
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內以 JavaScript 完成,本金、利率等資料不會傳送到任何伺服器。",
    q6: "可以匯出完整每期攤還表嗎?",
  ...
  en: {
    ...
    deficitNote: "Amortization Schedule Calculator standard formula.",
    ...
    formula: "Formula",
    formulaText: "result = f(loan principal, annual rate pct, loan years, months paid)",
    ...
    q1: "What does Amortization Schedule Calculator calculate?",
    a1: "Amortization Schedule Calculator applies the standard formula to your inputs and returns Interest-to-Principal plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Amortization Schedule Calculator?",
    a2: "Enter loan principal, annual rate pct, loan years, months paid. Amortization Schedule Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock per-period schedule, prepayment scenarios, equal-principal vs equal-payment comparison, refinance savings, and PDF export."
  },
} as const;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#e0f2fe,_#f8fafc_45%,_#dbeafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-sky-100 bg-white/90 p-6 shadow-2xl shadow-sky-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-sky-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-sky-600 p-5 text-white"><div className="text-xs font-bold uppercase text-sky-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-sky-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{loanPrincipal} × {annualRatePct}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-sky-200 bg-sky-50 px-5 py-4 text-sm font-black text-sky-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
