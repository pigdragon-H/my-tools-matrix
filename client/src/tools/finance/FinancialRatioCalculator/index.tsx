...
const bands = [
  { key: "tiny", range: "< 0.8", label: { zh: "極弱 (< 0.8)", en: "Very low (< 0.8)" }, desc: { zh: "落在「極弱」級距< 0.8。流動比率 < 0.8,流動性嚴重不足,短期內可能無法償還即將到期的負債,需立即處理。", en: "Falls in the \"Very low\" band (< 0.8). This is the very low range for Financial Ratio Calculator." } },
  { key: "normal", range: "0.8–1.2", label: { zh: "偏弱 (0.8–1.2)", en: "Low (0.8–1.2)" }, desc: { zh: "落在「偏弱」級距0.8–1.2。0.8-1.2,屬偏弱區,接近短期償債警戒線,需加強現金管理或緊縮負債。", en: "Falls in the \"Low\" band (0.8–1.2). This is the low range for Financial Ratio Calculator." } },
  { key: "notable", range: "1.2–1.8", label: { zh: "尚可 (1.2–1.8)", en: "Moderate (1.2–1.8)" }, desc: { zh: "落在「尚可」級距1.2–1.8。1.2-1.8,尚可,屬一般中小型企業常見水準,需注意季節性現金壓力。", en: "Falls in the \"Moderate\" band (1.2–1.8). This is the moderate range for Financial Ratio Calculator." } },
  { key: "high", range: "1.8–2.5", label: { zh: "穩健 (1.8–2.5)", en: "High (1.8–2.5)" }, desc: { zh: "落在「穩健」級距1.8–2.5。1.8-2.5,穩健,流動資產足以覆蓋短債,屬健康財務結構。", en: "Falls in the \"High\" band (1.8–2.5). This is the high range for Financial Ratio Calculator." } },
  { key: "major", range: "2.5–4", label: { zh: "強健 (2.5–4)", en: "Very high (2.5–4)" }, desc: { zh: "落在「強健」級距2.5–4。2.5-4,強健,但若過高(> 3)可能代表資金運用效率不佳,有閒置資產。", en: "Falls in the \"Very high\" band (2.5–4). This is the very high range for Financial Ratio Calculator." } },
  { key: "executive", range: "≥ 4", label: { zh: "極強 (≥ 4)", en: "Extreme (≥ 4)" }, desc: { zh: "落在「極強」級距≥ 4。> 4,極強,流動性過剩,投資者可能要求發放股息或回購股票釋放股東價值。", en: "Falls in the \"Extreme\" band (≥ 4). This is the extreme range for Financial Ratio Calculator." } },
] as const;
...
const ui = {
  zh: {
    ...
    intro: "本工具為 財務比率計算機，依公開公式於瀏覽器端試算，輸入流動資產、流動負債、總負債、總權益後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q5: "資料會上傳到伺服器嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內以 JavaScript 完成,財務數據不會傳送到任何伺服器,也不會記錄到日誌或資料庫。",
    q6: "可以拿這個分析上市公司嗎?",
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#dbeafe,_#f8fafc_45%,_#e0f2fe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            <section className="space-y-6"><p className="text-sm font-black uppercase tracking-[0.24em] text-blue-700">{t.badge}</p><h1 className="max-w-3xl font-black tracking-tight text-slate-950 [font-size:clamp(1.75rem,4vw,2.5rem)] [line-height:1.2]">{t.title}</h1><p className="text-xl font-black text-blue-700">{t.subtitle}</p><p className="max-w-2xl text-lg leading-8 text-slate-700">{t.intro}</p><div className="rounded-3xl border border-blue-200 bg-blue-50 p-5 text-sm leading-6 text-blue-950"><strong>{t.trustNoteLabel}</strong> {t.trustNote}</div></section>
