// @profile B
// Profile B · 計算機-YMYL · Interest Rate Calculator - effective annual rate and total interest from nominal rate and compounding（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<2%", label: { zh: "超低利率", en: "Very low" }, desc: { zh: "利率極低,複利效果有限,適合保守理財。", en: "A very low rate with limited compounding, suited to conservative plans." } },
  { key: "normal", range: "2-5%", label: { zh: "溫和利率", en: "Moderate" }, desc: { zh: "常見的存款或債券利率區間,風險與報酬均衡。", en: "A common deposit or bond range with balanced risk and reward." } },
  { key: "notable", range: "5-8%", label: { zh: "明顯利率", en: "Notable" }, desc: { zh: "複利效果開始顯著,長期累積可觀。", en: "Compounding becomes meaningful and adds up over the long run." } },
  { key: "high", range: "8-12%", label: { zh: "偏高利率", en: "High" }, desc: { zh: "利率偏高,若為借款成本需審慎評估。", en: "A high rate; if it is a borrowing cost, evaluate carefully." } },
  { key: "major", range: "12-18%", label: { zh: "高利率", en: "Very high" }, desc: { zh: "接近信用貸款利率,負擔明顯。", en: "Near consumer-credit levels, with a clear burden." } },
  { key: "executive", range: "18%+", label: { zh: "極高利率", en: "Extreme" }, desc: { zh: "極高利率,作為借款成本應盡量避免。", en: "An extreme rate; as a borrowing cost it should be avoided where possible." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "利率計算機",
    subtitle: "由名目利率與複利次數,換算有效年利率與到期利息。",
    intro: "輸入本金、名目年利率、每年複利次數與年數,本工具計算考量複利後的有效年利率 (EAR)、到期金額與總利息,幫你比較不同存款或借款條件的真實成本。",
    trustNoteLabel: "注意事項:",
    ...
    calculator: "計算機",
    participants: "本金",
    averageHourlyRate: "名目年利率 (%)",
    durationHours: "每年複利次數",
    meetingsPerMonth: "年數",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Convert a nominal rate and compounding frequency into an effective annual rate and total interest.",
    intro: "Enter the principal, nominal annual rate, compounds per year and time horizon; the tool computes the effective annual rate after compounding, the maturity value and total interest so you can compare the true cost of different deposits or loans.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a interest-rate example",
    ...
    averageHourlyRate: "Nominal annual rate (%)",
    durationHours: "Compounds per year",
    meetingsPerMonth: "Number of years",
    resultCard: "Result",
    unit: "Primary result",
...
export default function InterestRateCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("6");
  ...
  const result = useMemo(() => {
    ...
    const principal = v1; const nominalRate = v2; const compounds = v3; const years = v4;
    const n = compounds > 0 ? compounds : 1;
    const i = nominalRate / 100 / n;
    const ear = (Math.pow(1 + i, n) - 1) * 100;
    const futureValue = principal * Math.pow(1 + i, n * years);
    const totalInterest = futureValue - principal;
    return { ear, futureValue, totalInterest, principal, nominalRate };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.ear, 2);
  const monthlyDisplay = fmt(result.totalInterest, 0);

  function fillSolid() { setUnit("metric"); setParticipants("500000"); setAverageHourlyRate("6"); setDurationHours("12"); setMeetingsPerMonth("5"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("500000"); setAverageHourlyRate("12"); setDurationHours("12"); setMeetingsPerMonth("5"); }
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#f0fdfa 0%,#ccfbf1 55%,#cffafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "有效年利率" : "effective annual rate"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">${fmt(result.futureValue, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.totalInterest, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">3%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "低利環境" : "Low-rate setting"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">8%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "較高利率" : "Higher rate"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "%" : "%"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "有效年利率" : "Effective annual rate"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.ear, 2)}</p><p className="text-sm font-bold text-emerald-700">%</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "到期金額" : "Future value"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.futureValue, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "總利息" : "Total interest"}</div><p className="mt-2 text-3xl font-black text-slate-950">${fmt(result.totalInterest, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="interest-rate-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "有效年利率" : "effective annual rate"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入本金與名目利率" : "Enter principal and rate", note: t.bmrStep }, { label: lang === "zh" ? "設定複利次數" : "Set compounding frequency", note: t.deficitStep }, { label: lang === "zh" ? "換算有效年利率" : "Convert to effective rate", note: t.trendStep }, { label: lang === "zh" ? "得到期金額與利息" : "Get future value and interest", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="interest-rate-calculator-faq" position="inline" /></section>
