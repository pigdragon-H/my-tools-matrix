// @profile B
// Profile B · 計算機-YMYL · Gold Price Calculator - value of gold holdings by weight, purity and spot price（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<10 g", label: { zh: "小額持有", en: "Small holding" }, desc: { zh: "少量黃金,適合長期累積或紀念性收藏。", en: "A small amount of gold, suited to long-term accumulation or keepsakes." } },
  { key: "normal", range: "10-50 g", label: { zh: "一般持有", en: "Modest holding" }, desc: { zh: "常見的個人持有量,流動性與保管都相對方便。", en: "A common personal holding with convenient liquidity and storage." } },
  { key: "notable", range: "50-100 g", label: { zh: "明顯持有", en: "Notable holding" }, desc: { zh: "金額已具規模,建議留意買賣價差與保管安全。", en: "A meaningful amount; watch the bid-ask spread and storage security." } },
  { key: "high", range: "100-300 g", label: { zh: "較大持有", en: "Large holding" }, desc: { zh: "資產比重提高,可納入整體投資配置考量。", en: "A larger holding that can be considered within your overall allocation." } },
  { key: "major", range: "300-600 g", label: { zh: "高額持有", en: "High holding" }, desc: { zh: "高額部位,保管、保險與變現規劃都更重要。", en: "A high-value position where storage, insurance and exit planning matter more." } },
  { key: "executive", range: "600 g+", label: { zh: "大額持有", en: "Major holding" }, desc: { zh: "大額黃金資產,建議專業保管與分散風險。", en: "A major gold asset; professional custody and diversification are advisable." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "黃金價值計算機",
    subtitle: "依重量、純度與現貨價,快速估算黃金持有的市場價值。",
    intro: "輸入黃金重量、純度 (K 數) 與每公克現貨價,再加上加工或溢價百分比,即可算出純金含量與含溢價的黃金總價值。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立黃金價格範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    flowDemo: "即時試算",
    calculator: "計算機",
    participants: "黃金重量 (公克)",
    averageHourlyRate: "純度 (K)",
    durationHours: "每公克現貨價",
    meetingsPerMonth: "加工/溢價 (%)",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Estimate the market value of your gold from weight, purity and spot price.",
    intro: "Enter the gold weight, purity in karats and spot price per gram, then add any premium to get the fine gold content and total value including markup.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a gold-price example",
    ...
    participants: "Gold weight (grams)",
    averageHourlyRate: "Purity (karat)",
    durationHours: "Spot price per gram",
    meetingsPerMonth: "Premium / markup (%)",
    resultCard: "Result",
    unit: "Primary result",
...
export default function GoldPriceCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("24");
  const [durationHours, setDurationHours] = useState("2400");
  ...
  const result = useMemo(() => {
    ...
    const weightGram = v1; const purity = v2; const spotPerGram = v3; const premiumPct = v4;
    const fineness = Math.min(1, purity / 24);
    const pureGram = weightGram * fineness;
    const baseValue = pureGram * spotPerGram;
    const totalValue = baseValue * (1 + premiumPct / 100);
    const premiumAmount = totalValue - baseValue;
    return { totalValue, baseValue, pureGram, premiumAmount, weightGram, fineness };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.totalValue, 0);
  const monthlyDisplay = fmt(result.baseValue, 0);

  function fillSolid() { setUnit("metric"); setParticipants("100"); setAverageHourlyRate("24"); setDurationHours("2400"); setMeetingsPerMonth("3"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("500"); setAverageHourlyRate("24"); setDurationHours("2400"); setMeetingsPerMonth("3"); }

  const activeBand = bands.find(b => {
    const r = result.pureGram;
    if (r < 10) return b.key === "tiny";
    if (r < 50) return b.key === "normal";
    if (r < 100) return b.key === "notable";
    if (r < 300) return b.key === "high";
    if (r < 600) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fffbeb 0%,#fef3c7 55%,#ffedd5 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "黃金總價值" : "total gold value"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.pureGram, 1)}g</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.premiumAmount, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">24K</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "純金持有" : "Pure gold holding"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">18K</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "K金飾品" : "Gold jewellery"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "純金含量" : "Fine gold"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.pureGram, 2)}</p><p className="text-sm font-bold text-emerald-700">g</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "現貨價值" : "Spot value"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.baseValue, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "含溢價總值" : "Total with premium"}</div><p className="mt-2 text-3xl font-black text-slate-950">${fmt(result.totalValue, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="gold-price-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "黃金總價值" : "total gold value"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入重量與純度" : "Enter weight and purity", note: t.bmrStep }, { label: lang === "zh" ? "換算純金含量" : "Convert to fine gold", note: t.deficitStep }, { label: lang === "zh" ? "乘以現貨價" : "Multiply by spot price", note: t.trendStep }, { label: lang === "zh" ? "加上溢價得總值" : "Add premium for total", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="gold-price-calculator-faq" position="inline" /></section>
