// @profile B
// Profile B · 計算機-YMYL · CapRateCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 3", label: { zh: "極低 (< 3)", en: "Very low (< 3)" }, desc: { zh: "落在「極低」級距< 3。Cap Rate < 3%,屬於極低,常見於蛋黃區高價低租金物件,偏資本利得導向。", en: "Falls in the \"Very low\" band (< 3). This is the very low range for Cap Rate Calculator." } },
  { key: "normal", range: "3–5", label: { zh: "低 (3–5)", en: "Low (3–5)" }, desc: { zh: "落在「低」級距3–5。3-5%,低,精華地段穩健物件,流動性高但現金流薄。", en: "Falls in the \"Low\" band (3–5). This is the low range for Cap Rate Calculator." } },
  { key: "notable", range: "5–7", label: { zh: "中等 (5–7)", en: "Moderate (5–7)" }, desc: { zh: "落在「中等」級距5–7。5-7%,中等,屬租金與風險平衡的常見區間。", en: "Falls in the \"Moderate\" band (5–7). This is the moderate range for Cap Rate Calculator." } },
  { key: "high", range: "7–9", label: { zh: "偏高 (7–9)", en: "Elevated (7–9)" }, desc: { zh: "落在「偏高」級距7–9。7-9%,偏高,郊區或老屋,現金流佳但需留意維護與空置。", en: "Falls in the \"Elevated\" band (7–9). This is the elevated range for Cap Rate Calculator." } },
  { key: "major", range: "9–12", label: { zh: "高 (9–12)", en: "High (9–12)" }, desc: { zh: "落在「高」級距9–12。9-12%,高,通常風險較高地段或商用,須審慎評估。", en: "Falls in the \"High\" band (9–12). This is the high range for Cap Rate Calculator." } },
  { key: "executive", range: "≥ 12", label: { zh: "極高 (≥ 12)", en: "Very high (≥ 12)" }, desc: { zh: "落在「極高」級距≥ 12。> 12%,極高,多伴隨高風險(地段、屋況、租客穩定度),非穩賺。", en: "Falls in the \"Very high\" band (≥ 12). This is the very high range for Cap Rate Calculator." } },
] as const;

const affiliateItems: AffiliateItem[] = [
  { label: { zh: "租金報酬率計算機", en: "Rental Yield Calculator" }, href: "/tools/finance/rental-yield-calculator" },
...
const ui = {
  zh: {
    badge: "財務 · 資本化率計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Cap Rate Calculator · 資本化率計算機",
    subtitle: "輸入年租金、營運費用與物業總價,立即算出資本化率、淨營運收入與毛租金報酬率",
    intro: "本工具為 資本化率計算機，依公開公式於瀏覽器端試算，輸入年租金收入、年營運費用、物業總價、預期空置率%後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算資本化率計算機",
    examplePreview: "資本化率",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入年租金收入、年營運費用、物業總價、預期空置率%",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "租金 60 萬 · 費用 12 萬 · 價 1000 萬",
    baselineExampleNote: "年租金收入 600000 · 年營運費用 120000",
    activeExample: "進階範例",
    activeExampleValue: "租金 90 萬 · 費用 20 萬 · 價 800 萬",
    activeExampleNote: "年租金收入 加倍 · 觀察 資本化率 變化",
    flowDemo: "數字流向示範",
    calculator: "資本化率計算機",
    annualRentIncome: "年租金收入",
    annualOperatingCost: "年營運費用",
    propertyPrice: "物業總價",
    vacancyRatePct: "預期空置率%",
    resultCard: "結果卡片",
    primaryValue: "資本化率",
    primaryUnitTail: "%",
    secondaryLabel: "淨營運收入",
    secondaryTail: "$",
    metricALabel: "資本化率",
    ...
    metricCTail: "%",
    headlineCaption: "資本化率計算機 · 即時試算",
    fatLossTarget: "有效租金收入",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "資本化率計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 年租金收入 與 物業總價 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "資本化率計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 年租金收入、年營運費用、物業總價、預期空置率% 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依淨營運收入與物業總價計算資本化率(Cap Rate)、淨營運收入與毛租金報酬率。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "工具知識庫",
    knowledgeTitle: "資本化率計算機 · 觀念整理",
    definition: "定義",
    definitionText: "資本化率計算機以年租金收入、年營運費用、物業總價與空置率,計算資本化率(Cap Rate)、淨營運收入(NOI)與毛租金報酬率,協助房地產投資人快速評估物件相對價值。",
    formula: "公式",
    formulaText: "有效租金 = 年租金 × (1 − 空置率);NOI = 有效租金 − 營運費用;Cap Rate = NOI ÷ 物業總價",
    limitations: "限制",
    limitationsText: "本工具不含貸款利息、增值報酬、稅務與資本支出(CapEx),僅衡量物業本身的現金報酬。",
    interpretation: "解讀",
    interpretationText: "Cap Rate 須與當地市場行情比較才有意義;高 Cap Rate 常伴隨高風險,應結合地段與屋況綜合判斷。",
    context: "情境",
    contextText: "常見使用情境包括日常財務檢視、年度規劃、重大決策前的快速估算。建議搭配租金報酬率計算機 等延伸工具一起使用。",
    example: "範例",
    ...
    premiumTitle: "專業版 房產投報分析",
    premiumText: "解鎖現金報酬率(CoC)、內部報酬率(IRR)、增值情境、稅務與持有成本、多物件比較與槓桿分析。",
    premiumChips_zh: "現金報酬|IRR|增值情境|多物件比較",
    premiumChips_en: "Cash-on-Cash|IRR|Appreciation|Compare",
    ...
    q1: "Cap Rate 是什麼?",
    a1: "資本化率(Cap Rate)= 淨營運收入(NOI)÷ 物業總價,衡量不考慮貸款下,物業以現金購買的年化現金報酬率。它是房地產投資人比較不同物件、不同市場相對價值的核心指標。",
    q2: "Cap Rate 越高越好嗎?",
    a2: "不一定。高 Cap Rate 通常代表高風險(地段差、屋況老、租客不穩);低 Cap Rate 多為精華地段、流動性高、增值潛力大。應在您的風險承受度與目標(現金流 vs 增值)間取捨。",
    q3: "和租金報酬率差在哪?",
    a3: "毛租金報酬率 = 年租金 ÷ 房價,沒扣費用;Cap Rate 用的是扣除營運費用與空置後的「淨」營運收入(NOI),更貼近真實收益。本工具同時提供兩者方便對照。",
    q4: "該不該含貸款利息?",
    a4: "傳統 Cap Rate 不含貸款利息,因為它衡量「物業本身」的報酬,讓不同融資條件的買家能公平比較。若要看自有資金報酬,應另算現金報酬率(Cash-on-Cash),屬專業版功能。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內以 JavaScript 完成,租金與物業價格等資料不會傳送到任何伺服器。",
    q6: "可以含增值與稅嗎?",
    a6: "含增值報酬、現金報酬率(CoC)、內部報酬率(IRR)、稅務與持有成本的完整投報分析屬於專業版功能,免費版聚焦 Cap Rate 與 NOI。"
  },
  en: {
    badge: "Finance · Cap Rate Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Cap Rate Calculator",
    subtitle: "Enter annual rent, operating cost, and property price to see the cap rate, net operating income, and gross yield",
    intro: "Cap Rate Calculator runs the standard formula in your browser. Enter annual rent income, annual operating cost, property price, vacancy rate pct to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Cap Rate Calculator",
    examplePreview: "Cap Rate",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter annual rent income, annual operating cost, property price, vacancy rate pct",
    ...
    baselineExampleValue: "Rent 600k · OpEx 120k · 10M",
    baselineExampleNote: "Annual Rent Income 600000 · Annual Operating Cost 120000",
    activeExample: "Advanced example",
    activeExampleValue: "Rent 900k · OpEx 200k · 8M",
    activeExampleNote: "Annual Rent Income doubled · watch Cap Rate react",
    flowDemo: "Data flow demo",
    calculator: "Cap Rate Calculator",
    annualRentIncome: "Annual Rent Income",
    annualOperatingCost: "Annual Operating Cost",
    propertyPrice: "Property Price",
    vacancyRatePct: "Vacancy Rate Pct",
    resultCard: "Result card",
    primaryValue: "Cap Rate",
    primaryUnitTail: "%",
    secondaryLabel: "Net Operating Income",
    secondaryTail: "$",
    metricALabel: "Cap Rate",
    metricACaption: "Main figure from the standard formula",
    metricATail: "%",
    metricBLabel: "Net Operating Income",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "$",
    metricCLabel: "Gross Yield",
    metricCCaption: "Percentage view",
    metricCTail: "%",
    headlineCaption: "Cap Rate Calculator · live calc",
    fatLossTarget: "Effective Rent",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Cap Rate Calculator · band matrix",
    ...
    nextActionItem1: "Move Annual Rent Income and Property Price by ±10% to see sensitivity.",
    ...
    decisionTitle: "Cap Rate Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill annual rent income, annual operating cost, property price, vacancy rate pct.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "Cap Rate Calculator standard formula.",
    ...
    knowledgeTitle: "Cap Rate Calculator · concept primer",
    definition: "Definition",
    definitionText: "Cap Rate Calculator converts inputs (annual rent income, annual operating cost, property price, vacancy rate pct) into Cap Rate. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(annual rent income, annual operating cost, property price, vacancy rate pct)",
    ...
    context: "Context",
    contextText: "Common contexts include daily finance review, annual planning, and pre-decision quick estimates. Pair with Rental Yield Calculator for a fuller picture.",
    example: "Example",
    ...
    premiumTitle: "Pro Property ROI",
    premiumText: "Unlock cash-on-cash return, IRR, appreciation scenarios, tax and holding costs, multi-property compare, and leverage analysis.",
    premiumChips_zh: "現金報酬|IRR|增值情境|多物件比較",
    premiumChips_en: "Cash-on-Cash|IRR|Appreciation|Compare",
    ...
    q1: "What does Cap Rate Calculator calculate?",
    a1: "Cap Rate Calculator applies the standard formula to your inputs and returns Cap Rate plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Cap Rate Calculator?",
    a2: "Enter annual rent income, annual operating cost, property price, vacancy rate pct. Cap Rate Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock cash-on-cash return, IRR, appreciation scenarios, tax and holding costs, multi-property compare, and leverage analysis."
  },
} as const;
...
export default function CapRateCalculator() {
  ...
  const [annualRentIncome, setAnnualRentIncome] = useState("600000");
  const [annualOperatingCost, setAnnualOperatingCost] = useState("120000");
  const [propertyPrice, setPropertyPrice] = useState("10000000");
  const [vacancyRatePct, setVacancyRatePct] = useState("5");
  const t = ui[lang];

  const result = useMemo(() => {
    const rent = Number(annualRentIncome) || 0;
  ...
  }, [annualRentIncome, annualOperatingCost, propertyPrice, vacancyRatePct]);

  const primaryDisplay = fmt(result.capRate, 2);
  const secondaryDisplay = fmt(result.noi, 0);
  const tertiaryDisplay = fmt(result.grossYield, 2);
  const quaternaryDisplay = fmt(result.effectiveRent, 0);

  function fillSolid() { setUnit("metric"); setAnnualRentIncome("600000"); setAnnualOperatingCost("120000"); setPropertyPrice("10000000"); setVacancyRatePct("5"); }
  function fillHighSalary() { setUnit("imperial"); setAnnualRentIncome("900000"); setAnnualOperatingCost("200000"); setPropertyPrice("8000000"); setVacancyRatePct("10"); }

  const activeBand = bands.find(b => {
    const r = result.capRate;
    if (r < 3) return 'tiny';
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ccfbf1,_#f8fafc_45%,_#cffafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-teal-100 bg-white/90 p-6 shadow-2xl shadow-teal-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-teal-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-teal-600 p-5 text-white"><div className="text-xs font-bold uppercase text-teal-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-teal-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{annualRentIncome} × {annualOperatingCost}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-teal-200 bg-teal-50 px-5 py-4 text-sm font-black text-teal-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.annualRentIncome}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={annualRentIncome} onChange={(e) => setAnnualRentIncome(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.annualOperatingCost}<input type="number" step="5000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={annualOperatingCost} onChange={(e) => setAnnualOperatingCost(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.propertyPrice}<input type="number" step="100000" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={propertyPrice} onChange={(e) => setPropertyPrice(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.vacancyRatePct}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={vacancyRatePct} onChange={(e) => setVacancyRatePct(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="cap-rate-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="cap-rate-calculator-faq" position="inline" /></section>
