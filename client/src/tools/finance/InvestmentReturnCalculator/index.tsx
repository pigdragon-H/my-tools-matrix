...
const bands = [
  { key: "tiny", range: "< 50000", label: { zh: "保守 (< 50000)", en: "Band 1 (< 50000)" }, desc: { zh: "落在「保守」級距< 50000。短期目標或退休前 5 年的緊急資金,優先考量本金安全。", en: "Falls in the \"保守\" band < 50000. This is the 保守 range for Investment Return Calculator." } },
  { key: "normal", range: "50000–150000", label: { zh: "穩健 (50000–150000)", en: "Band 2 (50000–150000)" }, desc: { zh: "落在「穩健」級距50000–150000。20–30 年穩定累積區間,核心配置為大盤指數 ETF。", en: "Falls in the \"穩健\" band 50000–150000. This is the 穩健 range for Investment Return Calculator." } },
  { key: "notable", range: "150000–300000", label: { zh: "平衡 (150000–300000)", en: "Band 3 (150000–300000)" }, desc: { zh: "落在「平衡」級距150000–300000。屬於中階累積,可考慮加入區域型或主題型指數提高分散。", en: "Falls in the \"平衡\" band 150000–300000. This is the 平衡 range for Investment Return Calculator." } },
  { key: "high", range: "300000–600000", label: { zh: "成長 (300000–600000)", en: "Band 4 (300000–600000)" }, desc: { zh: "落在「成長」級距300000–600000。長期累積到位,需要開始討論再平衡與資產配置調整。", en: "Falls in the \"成長\" band 300000–600000. This is the 成長 range for Investment Return Calculator." } },
  { key: "major", range: "600000–1000000", label: { zh: "積極 (600000–1000000)", en: "Band 5 (600000–1000000)" }, desc: { zh: "落在「積極」級距600000–1000000。可進一步討論稅務優化(401k/IRA/ROTH)與遺產規劃。", en: "Falls in the \"積極\" band 600000–1000000. This is the 積極 range for Investment Return Calculator." } },
  { key: "executive", range: "≥ 1000000", label: { zh: "極積極 (≥ 1000000)", en: "Band 6 (≥ 1000000)" }, desc: { zh: "落在「極積極」級距≥ 1000000。百萬美元級資產,建議搭配理財顧問做整體財富管理。", en: "Falls in the \"極積極\" band ≥ 1000000. This is the 極積極 range for Investment Return Calculator." } },
] as const;
...
const ui = {
  zh: {
    ...
    premiumTitle: "解鎖進階版",
    premiumText: "Premium 解鎖投資報酬率計算機的批次試算、結果歷史、PDF 匯出、多場景比較與廣告移除。",
  ...
  en: {
    ...
    premiumText: "Premium unlocks batch calculation, history, PDF export, multi-scenario comparison, and ad-free for Investment Return Calculator.",
    premiumChips_zh: "批次試算 · 歷史紀錄 · PDF 匯出 · 廣告移除",
    premiumChips_en: "Batch · History · PDF Export · Ad-free",
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#e0f2fe,_#f8fafc_45%,_#dbeafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-sky-100 bg-white/90 p-6 shadow-2xl shadow-sky-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-sky-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-sky-600 p-5 text-white"><div className="text-xs font-bold uppercase text-sky-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${primaryDisplay}$</div><div className="text-sm font-bold text-sky-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${primaryDisplay}$</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{initialInvestment} × {monthlyContribution}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-sky-200 bg-sky-50 px-5 py-4 text-sm font-black text-sky-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
