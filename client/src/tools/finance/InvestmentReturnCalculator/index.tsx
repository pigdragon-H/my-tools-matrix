...
const ui = {
  zh: {
    ...
    intro: "本工具為 投資報酬率計算機，依公開公式於瀏覽器端試算，輸入初始投資、每月加碼、年化報酬率、投資年限後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q1: "投資報酬率怎麼算才合理?",
    a1: "合理的長期年化報酬通常以「市場大盤指數含再投入股息後的歷史中位數」為錨點。S&P 500 過去 30 年含股息再投入年化約 9-10%(名目)、扣通膨後約 6-7%(實質)。本計算機輸入的「年化報酬率」是名目報酬,若您想看實質,請用 (1+名目)/(1+通膨)-1 自行換算。",
    q2: "為什麼長期 ETF 報酬約 7-10%?",
    a2: "因為大盤指數同時擁有「全市場分散」「成本低」「自動汰弱留強」三個特性,使您不需要選股就能近似拿到整個經濟體的長期成長。歷史上美股 40 年滾動年化報酬幾乎都落在 7-11% 之間,只要您不在低點離場就能拿到這個數字;但 1-3 年內的年度報酬可能 -40% 到 +30% 不等,要忍得住。",
    q3: "通膨會吃掉多少報酬?",
    a3: "美國長期通膨年化約 2-3%,代表名目 8% 的報酬,實質購買力成長約 5-6%。20 年後 100 萬美元的實質購買力約等於今天的 55-67 萬美元,不可忽略。本工具不直接扣通膨,您可在腦中以「實質報酬率 = 名目 - 通膨」估算長期購買力。",
    q4: "每月定額好還是一次投入好?",
    a4: "若您已有大筆資金,「一次投入(Lump Sum)」歷史上有約 2/3 機率優於分批,因為市場長期向上;但若您心理承受度不足,「每月定額(DCA)」能降低買在高點的後悔成本。本計算機同時支援兩者,您可以把初始投資設高、每月加碼設低,模擬 Lump Sum;反之模擬 DCA。",
    q5: "結果會上傳到伺服器嗎?",
    a5: "完全不會。本工具所有計算都在您的瀏覽器內以 JavaScript 完成,輸入的金額、報酬率、年限不會傳送到任何伺服器,也不會記錄到日誌或資料庫。您關閉分頁後資料就消失,可放心使用。",
    q6: "要不要考慮稅後報酬?",
