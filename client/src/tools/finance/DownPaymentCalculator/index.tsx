// @profile B
// Profile B · 計算機-YMYL · DownPaymentCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 20", label: { zh: "極低 (< 20)", en: "Very low (< 20)" }, desc: { zh: "落在「極低」級距< 20。達成率 < 20%,剛起步,建議建立自動轉帳儲蓄習慣。", en: "Falls in the \"Very low\" band (< 20). This is the very low range for Down Payment Calculator." } },
  { key: "normal", range: "20–40", label: { zh: "低 (20–40)", en: "Low (20–40)" }, desc: { zh: "落在「低」級距20–40。20-40%,穩定累積中,可考慮高利活存或短債提升收益。", en: "Falls in the \"Low\" band (20–40). This is the low range for Down Payment Calculator." } },
  { key: "notable", range: "40–60", label: { zh: "中等 (40–60)", en: "Moderate (40–60)" }, desc: { zh: "落在「中等」級距40–60。40-60%,過半,維持紀律即可如期達標。", en: "Falls in the \"Moderate\" band (40–60). This is the moderate range for Down Payment Calculator." } },
  { key: "high", range: "60–80", label: { zh: "偏高 (60–80)", en: "Elevated (60–80)" }, desc: { zh: "落在「偏高」級距60–80。60-80%,接近目標,可開始研究房貸利率與物件。", en: "Falls in the \"Elevated\" band (60–80). This is the elevated range for Down Payment Calculator." } },
  { key: "major", range: "80–100", label: { zh: "高 (80–100)", en: "High (80–100)" }, desc: { zh: "落在「高」級距80–100。80-100%,即將達標,建議鎖定資金、避免高風險投資。", en: "Falls in the \"High\" band (80–100). This is the high range for Down Payment Calculator." } },
  { key: "executive", range: "≥ 100", label: { zh: "極高 (≥ 100)", en: "Very high (≥ 100)" }, desc: { zh: "落在「極高」級距≥ 100。≥ 100%,已達標,可進一步準備裝潢、稅費與緊急備用金。", en: "Falls in the \"Very high\" band (≥ 100). This is the very high range for Down Payment Calculator." } },
] as const;
...
const affiliateItems: AffiliateItem[] = [
  ...
  { label: { zh: "購屋負擔力計算機", en: "Home Affordability Calculator" }, href: "/tools/finance/home-affordability-calculator" },
  { label: { zh: "緊急備用金計算機", en: "Emergency Fund Calculator" }, href: "/tools/finance/emergency-fund-calculator" },
];

const ui = {
  zh: {
    badge: "財務 · 頭期款計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Down Payment Calculator · 頭期款計算機",
    subtitle: "輸入房價、頭期比例與儲蓄進度,立即算出所需頭期、尚缺金額與達標月數",
    intro: "本工具為 頭期款計算機，依公開公式於瀏覽器端試算，輸入房屋總價、頭期款比例%、目前已存、每月可存後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算頭期款計算機",
    examplePreview: "頭期達成率",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入房屋總價、頭期款比例%、目前已存、每月可存",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "房價 1000 萬 · 2 成 · 月存 4 萬",
    baselineExampleNote: "房屋總價 10000000 · 頭期款比例% 20",
    activeExample: "進階範例",
    activeExampleValue: "房價 1500 萬 · 3 成 · 月存 3 萬",
    activeExampleNote: "房屋總價 加倍 · 觀察 頭期達成率 變化",
    flowDemo: "數字流向示範",
    calculator: "頭期款計算機",
    homePrice: "房屋總價",
    ...
    metricCTail: "$",
    headlineCaption: "頭期款計算機 · 即時試算",
    fatLossTarget: "達標月數",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "頭期款計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 房屋總價 與 目前已存 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "頭期款計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 房屋總價、頭期款比例%、目前已存、每月可存 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依房價與頭期比例計算所需頭期款、尚缺金額與達標所需月數。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "知識庫",
    knowledgeTitle: "頭期款計算機 · 觀念整理",
    definition: "定義",
    definitionText: "頭期款計算機以房屋總價、頭期款比例、目前已存金額與每月可存金額,計算所需頭期款、尚缺金額、達標所需月數與達成率,協助購屋族規劃儲蓄目標。",
    formula: "公式",
    formulaText: "所需頭期 = 房價 × 頭期比例;尚缺 = 所需頭期 − 已存;達標月數 = 尚缺 ÷ 每月可存",
    limitations: "限制",
    limitationsText: "本工具不計入儲蓄報酬利息、房價變動與通膨,僅以線性儲蓄估算達標時間。",
    interpretation: "解讀",
    interpretationText: "達成率越高代表越接近購屋目標;建議保留緊急備用金與裝潢預算後再衝刺頭期。",
    context: "情境",
    ...
    premiumTitle: "專業版 購屋儲蓄規劃",
    premiumText: "解鎖含利率與通膨的真實購買力、儲蓄報酬複利、多目標排序、PMI 試算與最佳頭期建議。",
    premiumChips_zh: "通膨調整|複利報酬|多目標|PMI試算",
    premiumChips_en: "Inflation|Compounding|Multi-goal|PMI",
    ...
    q1: "頭期款要準備幾成?",
    a1: "台灣首購房貸成數約 7-8 成,因此頭期常需 2-3 成;美國傳統貸款建議 20% 以免額外保險(PMI)。頭期越高、貸款越少、每月負擔越輕,但也壓縮手頭流動性。",
    q2: "頭期越多越好嗎?",
    a2: "不全然。頭期越多省息越多,但若把所有現金投入頭期,將缺乏緊急備用金與裝潢費。建議保留 6 個月生活費與裝潢預算後,再決定頭期金額。",
    q3: "頭期款可以借嗎?",
    a3: "原則上不建議用信貸或借款湊頭期,因為這等於用高利債支撐低利房貸,且銀行核貸時會檢視負債比。靠自有儲蓄或家人贈與較穩健。",
    q4: "除了頭期還要準備什麼?",
    a4: "除頭期款外,還需準備契稅、印花稅、代書費、仲介費、貸款開辦費(約房價 3-6%)、裝潢費,以及搬家後的緊急備用金。建議搭配緊急備用金計算機。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內以 JavaScript 完成,房價與儲蓄等資料不會傳送到任何伺服器。",
    q6: "可以模擬利率與通膨嗎?",
    a6: "含利率與通膨調整的真實購買力、儲蓄報酬複利與多目標排序屬於專業版功能,免費版提供達標進度試算。"
  },
  en: {
    badge: "Finance · Down Payment Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Down Payment Calculator",
    subtitle: "Enter home price, down payment percent, and savings to see the required amount, remaining gap, and months to goal",
    intro: "Down Payment Calculator runs the standard formula in your browser. Enter home price, down payment pct, saved so far, monthly savings to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Down Payment Calculator",
    examplePreview: "Down Payment Progress",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter home price, down payment pct, saved so far, monthly savings",
    ...
    baselineExampleValue: "10M · 20% · 40k/mo",
    baselineExampleNote: "Home Price 10000000 · Down Payment Pct 20",
    activeExample: "Advanced example",
    activeExampleValue: "15M · 30% · 30k/mo",
    activeExampleNote: "Home Price doubled · watch Down Payment Progress react",
    ...
    primaryValue: "Down Payment Progress",
    primaryUnitTail: "%",
    secondaryLabel: "Required Down Payment",
    secondaryTail: "$",
    metricALabel: "Down Payment Progress",
    metricACaption: "Main figure from the standard formula",
    metricATail: "%",
    metricBLabel: "Required Down Payment",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "$",
    metricCLabel: "Remaining Gap",
    metricCCaption: "Percentage view",
    metricCTail: "$",
    headlineCaption: "Down Payment Calculator · live calc",
    fatLossTarget: "Months to Goal",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Down Payment Calculator · band matrix",
    ...
    nextActionItem1: "Move Home Price and Saved So Far by ±10% to see sensitivity.",
    ...
    decisionTitle: "Down Payment Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill home price, down payment pct, saved so far, monthly savings.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "Down Payment Calculator standard formula.",
    ...
    knowledge: "Knowledge",
    knowledgeTitle: "Down Payment Calculator · concept primer",
    definition: "Definition",
    definitionText: "Down Payment Calculator converts inputs (home price, down payment pct, saved so far, monthly savings) into Down Payment Progress. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(home price, down payment pct, saved so far, monthly savings)",
    ...
    premiumTitle: "Pro Home Savings Planner",
    premiumText: "Unlock inflation-adjusted purchasing power, savings-return compounding, multi-goal sequencing, PMI estimate, and optimal down payment advice.",
    premiumChips_zh: "通膨調整|複利報酬|多目標|PMI試算",
    premiumChips_en: "Inflation|Compounding|Multi-goal|PMI",
    ...
    q1: "What does Down Payment Calculator calculate?",
    a1: "Down Payment Calculator applies the standard formula to your inputs and returns Down Payment Progress plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Down Payment Calculator?",
    a2: "Enter home price, down payment pct, saved so far, monthly savings. Down Payment Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock inflation-adjusted purchasing power, savings-return compounding, multi-goal sequencing, PMI estimate, and optimal down payment advice."
  },
} as const;
...
export default function DownPaymentCalculator() {
  ...
  const [savedSoFar, setSavedSoFar] = useState("800000");
  const [monthlySavings, setMonthlySavings] = useState("40000");
  const t = ui[lang];
  ...
  }, [homePrice, downPaymentPct, savedSoFar, monthlySavings]);

  const primaryDisplay = fmt(result.progressPct, 1);
  const secondaryDisplay = fmt(result.required, 0);
  const tertiaryDisplay = fmt(result.gap, 0);
  const quaternaryDisplay = fmt(result.monthsToGoal, 0);

  function fillSolid() { setUnit("metric"); setHomePrice("10000000"); setDownPaymentPct("20"); setSavedSoFar("800000"); setMonthlySavings("40000"); }
  function fillHighSalary() { setUnit("imperial"); setHomePrice("15000000"); setDownPaymentPct("30"); setSavedSoFar("1000000"); setMonthlySavings("30000"); }

  const activeBand = bands.find(b => {
    const r = result.progressPct;
    if (r < 20) return 'tiny';
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#d1fae5,_#f8fafc_45%,_#e0f2fe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-emerald-100 bg-white/90 p-6 shadow-2xl shadow-emerald-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-emerald-600 p-5 text-white"><div className="text-xs font-bold uppercase text-emerald-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-emerald-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{homePrice} × {downPaymentPct}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm font-black text-emerald-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.homePrice}<input type="number" step="100000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={homePrice} onChange={(e) => setHomePrice(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.downPaymentPct}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={downPaymentPct} onChange={(e) => setDownPaymentPct(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.savedSoFar}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={savedSoFar} onChange={(e) => setSavedSoFar(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.monthlySavings}<input type="number" step="1000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthlySavings} onChange={(e) => setMonthlySavings(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="down-payment-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="down-payment-calculator-faq" position="inline" /></section>
