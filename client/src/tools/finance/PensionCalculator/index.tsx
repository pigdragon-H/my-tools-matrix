...
const bands = [
  { key: "tiny", range: "< 3000000", label: { zh: "不足 (< 3000000)", en: "Band 1 (< 3000000)" }, desc: { zh: "落在「不足」級距< 3000000。累積 < 300 萬,僅夠基本生活,需積極補充自願提撥或個人投資。", en: "Falls in the \"不足\" band < 3000000. This is the 不足 range for Pension Calculator." } },
  { key: "normal", range: "3000000–8000000", label: { zh: "緊縮 (3000000–8000000)", en: "Band 2 (3000000–8000000)" }, desc: { zh: "落在「緊縮」級距3000000–8000000。300-800 萬,屬緊縮區,維持基本生活但無餘力旅遊或醫療緩衝。", en: "Falls in the \"緊縮\" band 3000000–8000000. This is the 緊縮 range for Pension Calculator." } },
  { key: "notable", range: "8000000–15000000", label: { zh: "尚可 (8000000–15000000)", en: "Band 3 (8000000–15000000)" }, desc: { zh: "落在「尚可」級距8000000–15000000。800-1500 萬,合理水準,搭配勞保年金可維持中等生活品質。", en: "Falls in the \"尚可\" band 8000000–15000000. This is the 尚可 range for Pension Calculator." } },
  { key: "high", range: "15000000–25000000", label: { zh: "穩定 (15000000–25000000)", en: "Band 4 (15000000–25000000)" }, desc: { zh: "落在「穩定」級距15000000–25000000。1500-2500 萬,穩定區,可享有舒適退休生活並保留醫療緩衝。", en: "Falls in the \"穩定\" band 15000000–25000000. This is the 穩定 range for Pension Calculator." } },
  { key: "major", range: "25000000–40000000", label: { zh: "充裕 (25000000–40000000)", en: "Band 5 (25000000–40000000)" }, desc: { zh: "落在「充裕」級距25000000–40000000。2500-4000 萬,充裕區,可規劃旅遊、興趣、子女傳承,財務自由。", en: "Falls in the \"充裕\" band 25000000–40000000. This is the 充裕 range for Pension Calculator." } },
  { key: "executive", range: "≥ 40000000", label: { zh: "豐厚 (≥ 40000000)", en: "Band 6 (≥ 40000000)" }, desc: { zh: "落在「豐厚」級距≥ 40000000。> 4000 萬,屬高資產退休,可考慮信託、慈善、家族傳承等進階規劃。", en: "Falls in the \"豐厚\" band ≥ 40000000. This is the 豐厚 range for Pension Calculator." } },
] as const;
...
const ui = {
  zh: {
    ...
    intro: "本工具為 退休金試算計算機，依公開公式於瀏覽器端試算，輸入當前年齡、退休年齡、月薪、雇主提撥率後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    premiumTitle: "解鎖進階版",
    premiumText: "Premium 解鎖退休金試算計算機的批次試算、結果歷史、PDF 匯出、多場景比較與廣告移除。",
    ...
    q1: "勞退新制和勞保年金有什麼不同?",
    a1: "**勞退新制(個人專戶)**: 雇主每月提撥 6% 薪資進你的個人帳戶,退休後依累積金額月領或一次領,屬於「確定提撥」。**勞保年金(老年年金)**: 由勞保局以「(平均月投保薪資 × 1.55% × 年資)」計算月退,屬於「確定給付」,但勞保財務有破產風險。兩者**可同時領**,但勞退是你的私產、勞保是社會保險。",
    q2: "薪資替代率多少才算夠?",
    a2: "國際標準建議 **70-80%** 薪資替代率(退休前 vs 退休後);若房貸已還清、子女已獨立,**60-70%** 也夠。本工具以「累積金額 / 240 個月(20 年)」概估月退,實際應結合勞保年金、自願提撥、個人投資三層退休金計算。",
    q3: "雇主提撥 6% 是天花板嗎?",
    a3: "**6% 是雇主強制提撥的法定上限**,但你自己可以再「自願提撥」最高 6%(共 12%)。自願提撥的好處:**(1) 享所得稅扣除額**(每年最高 12% 不計入綜合所得)、**(2) 帳戶享 2 年期定存最低保證收益率**、**(3) 強制儲蓄**。對 30 歲月薪 6 萬族群,自願提撥 6% 可省稅約 5-15%。",
    q4: "退休金需要自己提撥嗎?",
    a4: "**法定 6% 不需要自己提**(雇主負擔),但**強烈建議再自提 6%**。理由:本工具假設僅雇主 6% 與假設 4% 報酬率,30 歲到 65 歲約累積 1500 萬;若再自提 6%(共 12%),累積可達 3000 萬,差距巨大。自提還可省所得稅,雙重利益。",
    q5: "薪資資料會上傳到伺服器嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內以 JavaScript 完成,薪資、年齡、提撥率等資料不會傳送到任何伺服器,也不會記錄到日誌或資料庫。",
    q6: "退休後可以一次領嗎?還是只能月領?",
    a6: "**勞退新制可選一次領或月領**,60 歲滿足以下條件二選一:**(1) 工作年資 ≥ 15 年 → 月領**(終身)、**(2) 工作年資 < 15 年 → 一次領**。**勞保年金**只能月領(年資滿 15 年或 60 歲後)。一次領 vs 月領的選擇:看你健康狀況、其他資產配置、是否擔心通膨;一次領適合有高效投資能力者,月領適合追求穩定。"
  },
  en: {
    ...
    premiumText: "Premium unlocks batch calculation, history, PDF export, multi-scenario comparison, and ad-free for Pension Calculator.",
    premiumChips_zh: "批次試算 · 歷史紀錄 · PDF 匯出 · 廣告移除",
    premiumChips_en: "Batch · History · PDF Export · Ad-free",
    ...
    q1: "About: 勞退新制和勞保年金有什麼不同?",
    a1: "For \"勞退新制和勞保年金有什麼不同?\": Pension Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 薪資替代率多少才算夠?",
    a2: "For \"薪資替代率多少才算夠?\": Pension Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 雇主提撥 6% 是天花板嗎?",
    a3: "For \"雇主提撥 6% 是天花板嗎?\": Pension Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 退休金需要自己提撥嗎?",
    a4: "For \"退休金需要自己提撥嗎?\": Pension Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 薪資資料會上傳到伺服器嗎?",
    a5: "For \"薪資資料會上傳到伺服器嗎?\": Pension Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 退休後可以一次領嗎?還是只能月領?",
    a6: "For \"退休後可以一次領嗎?還是只能月領?\": Pension Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_#e0e7ff,_#f8fafc_45%,_#dbeafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-indigo-100 bg-white/90 p-6 shadow-2xl shadow-indigo-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-indigo-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-indigo-600 p-5 text-white"><div className="text-xs font-bold uppercase text-indigo-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}$</div><div className="text-sm font-bold text-indigo-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}$</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{currentAge} × {retireAge}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-indigo-200 bg-indigo-50 px-5 py-4 text-sm font-black text-indigo-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-indigo-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-indigo-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{primaryDisplay}<span className="text-3xl">$</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.primaryValue}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.secondaryLabel}</div><div className="mt-1 text-xl font-black">{secondaryDisplay}</div><div className="mt-1 text-xs text-slate-300">{t.secondaryTail}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.metricALabel}</div><div className="mt-1 text-xs font-black text-emerald-700">{t.metricACaption}</div><p className="mt-2 text-3xl font-black text-emerald-950">{tertiaryDisplay}</p><p className="text-sm font-bold text-emerald-700">{t.metricATail}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.metricBLabel}</div><div className="mt-1 text-xs font-black text-blue-700">{t.metricBCaption}</div><p className="mt-2 text-3xl font-black text-blue-950">{quaternaryDisplay}</p><p className="text-sm font-bold text-blue-700">{t.metricBTail}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.metricCLabel}</div><div className="mt-1 text-xs font-black text-slate-700">{t.metricCCaption}</div><p className="mt-2 text-3xl font-black text-slate-950">{secondaryDisplay}</p><p className="text-sm font-bold text-slate-700">{t.metricCTail}</p></div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-indigo-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black">{primaryDisplay}$</div></div><div className="rounded-2xl bg-indigo-50 p-4"><div className="text-xs font-black uppercase text-indigo-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-indigo-950">{secondaryDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.tertiaryTag}</div><div className="mt-1 text-3xl font-black text-emerald-950">{tertiaryDisplay}</div></div></div></article>
