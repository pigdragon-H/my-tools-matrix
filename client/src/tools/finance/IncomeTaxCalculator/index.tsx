// @profile B
// Profile B · 計算機-YMYL · Income Tax Calculator - progressive tax on taxable income after deductions（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<3%", label: { zh: "極低稅負", en: "Very low" }, desc: { zh: "扣除後課稅所得不高,整體有效稅率很低。", en: "Taxable income after deductions is low, so the effective rate is very low." } },
  { key: "normal", range: "3-7%", label: { zh: "輕度稅負", en: "Light burden" }, desc: { zh: "屬於較低稅率級距,稅負溫和。", en: "You sit in a lower-rate band with a mild tax burden." } },
  { key: "notable", range: "7-12%", label: { zh: "中度稅負", en: "Moderate" }, desc: { zh: "進入中間級距,合理運用扣除額可有效節稅。", en: "Into the middle band; using deductions well can meaningfully cut tax." } },
  { key: "high", range: "12-18%", label: { zh: "偏高稅負", en: "High burden" }, desc: { zh: "有效稅率上升,值得評估退休金與保險等工具。", en: "The effective rate rises; review retirement and insurance tools." } },
  { key: "major", range: "18-25%", label: { zh: "高稅負", en: "Heavy" }, desc: { zh: "接近高所得級距,稅務規劃的價值明顯。", en: "Near a high-income band, where tax planning clearly pays off." } },
  { key: "executive", range: "25%+", label: { zh: "頂級稅負", en: "Top tier" }, desc: { zh: "高所得族群,建議結合專業稅務與資產規劃。", en: "A high-income tier; combine professional tax and asset planning." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "所得稅計算機",
    subtitle: "依總所得、扣除額與累進稅率,估算年度應納所得稅與有效稅率。",
    intro: "輸入年度總所得與各項扣除額,套用適用的最高稅率與累進差額,即可估算課稅所得、應納稅額與整體有效稅率,協助你掌握稅務全貌。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立所得稅範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "年度總所得",
    averageHourlyRate: "扣除額合計",
    durationHours: "適用最高稅率 (%)",
    meetingsPerMonth: "累進差額",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Estimate your annual income tax and effective rate from gross income, deductions and progressive rates.",
    intro: "Enter your annual gross income and deductions, apply the top marginal rate and progressive difference, and see your taxable income, tax due and overall effective rate.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a income-tax example",
    ...
    participants: "Annual gross income",
    averageHourlyRate: "Total deductions",
    durationHours: "Top marginal rate (%)",
    meetingsPerMonth: "Progressive difference",
    resultCard: "Result",
    unit: "Primary result",
...
export default function IncomeTaxCalculator() {
  ...
  const [participants, setParticipants] = useState("1200000");
  const [averageHourlyRate, setAverageHourlyRate] = useState("408000");
  const [durationHours, setDurationHours] = useState("12");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("39200");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const grossIncome = v1; const deductions = v2; const topRate = v3; const progressiveDiff = v4;
    const taxable = Math.max(0, grossIncome - deductions);
    const tax = Math.max(0, taxable * (topRate / 100) - progressiveDiff);
    const effRate = grossIncome > 0 ? (tax / grossIncome) * 100 : 0;
    const netIncome = grossIncome - tax;
    return { tax, taxable, effRate, netIncome, grossIncome };
  ...
  function fillSolid() { setUnit("metric"); setParticipants("1200000"); setAverageHourlyRate("408000"); setDurationHours("12"); setMeetingsPerMonth("39200"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("3000000"); setAverageHourlyRate("408000"); setDurationHours("20"); setMeetingsPerMonth("144800"); }

  const activeBand = bands.find(b => {
    const r = result.effRate;
    if (r < 3) return b.key === "tiny";
    if (r < 7) return b.key === "normal";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#eff6ff 0%,#dbeafe 55%,#ede9fe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "應納所得稅" : "income tax due"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.effRate, 1)}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.taxable, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">5%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "低稅率級距" : "Low-rate band"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">20%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "中高稅率級距" : "Higher-rate band"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "課稅所得" : "Taxable income"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.taxable, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "應納稅額" : "Tax due"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.tax, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "有效稅率" : "Effective rate"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.effRate, 1)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="income-tax-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "應納所得稅" : "income tax due"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入總所得" : "Enter gross income", note: t.bmrStep }, { label: lang === "zh" ? "扣除免稅與扣除額" : "Subtract deductions", note: t.deficitStep }, { label: lang === "zh" ? "套用累進稅率" : "Apply progressive rate", note: t.trendStep }, { label: lang === "zh" ? "扣累進差額得稅額" : "Subtract diff for tax", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="income-tax-calculator-faq" position="inline" /></section>
