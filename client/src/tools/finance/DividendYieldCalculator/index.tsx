...
const ui = {
  zh: {
    ...
    premiumTitle: "解鎖進階版",
    premiumText: "Premium 解鎖股息殖利率計算機的批次試算、結果歷史、PDF 匯出、多場景比較與廣告移除。",
  ...
  en: {
    ...
    premiumText: "Premium unlocks batch calculation, history, PDF export, multi-scenario comparison, and ad-free for Dividend Yield Calculator.",
    premiumChips_zh: "批次試算 · 歷史紀錄 · PDF 匯出 · 廣告移除",
    premiumChips_en: "Batch · History · PDF Export · Ad-free",
    ...
    q1: "About: 殖利率高就是好的嗎?什麼是「殖利率陷阱」?",
    a1: "For \"殖利率高就是好的嗎?什麼是「殖利率陷阱」?\": Dividend Yield Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: YOC(殖利率對成本)是什麼?",
    a2: "For \"YOC(殖利率對成本)是什麼?\": Dividend Yield Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 領股息要繳稅嗎?",
    a3: "For \"領股息要繳稅嗎?\": Dividend Yield Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 為什麼要看配息成長率?",
    a4: "For \"為什麼要看配息成長率?\": Dividend Yield Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳到伺服器嗎?",
    a5: "For \"資料會上傳到伺服器嗎?\": Dividend Yield Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 高股息 ETF(0056、SCHD)值得買嗎?",
    a6: "For \"高股息 ETF(0056、SCHD)值得買嗎?\": Dividend Yield Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ecfccb,_#f8fafc_45%,_#d9f99d)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-lime-100 bg-white/90 p-6 shadow-2xl shadow-lime-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-lime-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-lime-600 p-5 text-white"><div className="text-xs font-bold uppercase text-lime-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}%</div><div className="text-sm font-bold text-lime-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{sharePrice} × {annualDividend}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-lime-200 bg-lime-50 px-5 py-4 text-sm font-black text-lime-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
