...
const ui = {
  zh: {
    ...
    intro: "本工具為 股息殖利率計算機，依公開公式於瀏覽器端試算，輸入股價、年度配息、持股股數、預期配息成長率%後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q2: "YOC(殖利率對成本)是什麼?",
    a2: "**YOC(Yield on Cost)** = 現在的年配息 / 當初買進價格 × 100%。它衡量「持股越久,配息成長越多,您的『成本殖利率』也越高」的長期效益。例如 10 年前以 $50 買進、當時殖利率 4%,若公司每年配息成長 5%,10 年後 YOC ≈ 6.5%。長期持有股息成長股的核心優勢就是 YOC 不斷攀升。",
    q3: "領股息要繳稅嗎?",
    ...
    q4: "為什麼要看配息成長率?",
    a4: "因為**今天的高殖利率股不一定是未來的高股息股**。配息成長率(Dividend Growth Rate)代表公司是否持續強化股東回饋。標普 500 股息貴族(連 25 年增息)平均年化股息成長 6-8%,長期下來 YOC 可達兩位數。本工具讓您輸入預期成長率,看 5 年後的 YOC,是長期持股決策的核心。",
    q5: "資料會上傳到伺服器嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內以 JavaScript 完成,股價、股息、持股數等資料不會傳送到任何伺服器,也不會記錄到日誌或資料庫。",
