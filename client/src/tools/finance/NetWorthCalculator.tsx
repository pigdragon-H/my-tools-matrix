...
const inputFields = [
    { key: 'cash', label: '現金與存款' },
    { key: 'investment', label: '投資資產' },
    { key: 'property', label: '房產估值' },
    { key: 'debt', label: '總負債' }
];
...
export default function NetWorthCalculator() {
  ...
  const results = useMemo<Result[]>(() => {
    ...
    const assets = v('cash') + v('investment') + v('property');
    const net = assets - v('debt');
    return [{label:'總資產', value:money(assets)}, {label:'總負債', value:money(v('debt'))}, {label:'淨資產', value:money(net)}];
  }, [values]);

  return (
    <div className="mx-auto max-w-3xl space-y-6 rounded-2xl border bg-white p-6 shadow-sm dark:bg-slate-950">
      <div>
        <h1 className="text-2xl font-bold">淨資產計算器</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">統計資產、負債與淨資產。</p>
      </div>
