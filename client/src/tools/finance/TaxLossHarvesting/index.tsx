// @profile B
// Profile B · 計算機-YMYL · TaxLossHarvesting（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 200", label: { zh: "效益極低 (< 200)", en: "Minimal (< 200)" }, desc: { zh: "落在「效益極低」級距< 200。可收割虧損少,節稅效益極低,留意是否值得交易成本。", en: "Falls in the \"Minimal\" band (< 200). Little loss to harvest; minimal savings, weigh trading costs." } },
  { key: "normal", range: "200–800", label: { zh: "效益有限 (200–800)", en: "Limited (200–800)" }, desc: { zh: "落在「效益有限」級距200–800。節稅效益有限,可搭配整體再平衡一併執行。", en: "Falls in the \"Limited\" band (200–800). Limited savings; pair it with overall rebalancing." } },
  { key: "notable", range: "800–1500", label: { zh: "效益中等 (800–1500)", en: "Moderate (800–1500)" }, desc: { zh: "落在「效益中等」級距800–1500。節稅效益中等,適度收割可降低本年度稅負。", en: "Falls in the \"Moderate\" band (800–1500). Moderate savings; measured harvesting lowers this year's tax." } },
  { key: "high", range: "1500–3000", label: { zh: "效益良好 (1500–3000)", en: "Good (1500–3000)" }, desc: { zh: "落在「效益良好」級距1500–3000。節稅效益良好,妥善配對利得與虧損可顯著節稅。", en: "Falls in the \"Good\" band (1500–3000). Good savings; matching gains and losses saves meaningfully." } },
  { key: "major", range: "3000–6000", label: { zh: "效益顯著 (3000–6000)", en: "Significant (3000–6000)" }, desc: { zh: "落在「效益顯著」級距3000–6000。節稅效益顯著,務必避免洗售(wash sale)以保留扣抵。", en: "Falls in the \"Significant\" band (3000–6000). Significant savings; avoid wash sales to preserve the deduction." } },
  { key: "executive", range: "≥ 6000", label: { zh: "效益極大 (≥ 6000)", en: "Maximal (≥ 6000)" }, desc: { zh: "落在「效益極大」級距≥ 6000。節稅效益極大,建議結合結轉規劃分年釋放虧損。", en: "Falls in the \"Maximal\" band (≥ 6000). Maximal savings; plan carryforwards to release losses across years." } },
] as const;
...
const ui = {
  zh: {
    badge: "財務 · 稅損收割計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Tax Loss Harvesting Calculator · 稅損收割計算機",
    subtitle: "估算以資本虧損抵減利得與一般所得後的節稅金額與結轉。",
    intro: "本工具為 稅損收割計算機，依公開公式於瀏覽器端試算，輸入可實現的資本虧損、本年度已實現利得、適用利得稅率、一般所得抵減上限後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算稅損收割計算機",
    examplePreview: "預估本年度節稅",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入可實現的資本虧損、本年度已實現利得、適用利得稅率、一般所得抵減上限",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "標準收割情境",
    baselineExampleNote: "可實現的資本虧損 15000 · 本年度已實現利得 8000",
    activeExample: "進階範例",
    activeExampleValue: "大額虧損結轉情境",
    activeExampleNote: "可實現的資本虧損 加倍 · 觀察 預估本年度節稅 變化",
    flowDemo: "數字流向示範",
    calculator: "稅損收割計算機",
    harvestableCapitalLoss: "可實現的資本虧損",
    realizedGainsThisYear: "本年度已實現利得",
    capitalGainsTaxRate: "適用利得稅率",
    ordinaryIncomeOffsetCap: "一般所得抵減上限",
    resultCard: "結果卡片",
    primaryValue: "預估本年度節稅",
    primaryUnitTail: "$",
    secondaryLabel: "可抵減的已實現利得",
    secondaryTail: "$",
    metricALabel: "預估本年度節稅",
    metricACaption: "依公開公式試算的主要數值",
    metricATail: "$",
    metricBLabel: "可抵減的已實現利得",
    metricBCaption: "與主要結果連動的次要量值",
    metricBTail: "$",
    metricCLabel: "抵減一般所得金額",
    metricCCaption: "百分比形式的觀察點",
    metricCTail: "$",
    headlineCaption: "稅損收割計算機 · 即時試算",
    fatLossTarget: "結轉至次年虧損",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "稅損收割計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 可實現的資本虧損 與 適用利得稅率 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "稅損收割計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 可實現的資本虧損、本年度已實現利得、適用利得稅率、一般所得抵減上限 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "Tax savings from loss harvesting。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "工具知識庫",
    knowledgeTitle: "稅損收割計算機 · 觀念整理",
    definition: "定義",
    definitionText: "稅損收割計算機估算以實現的資本虧損抵減資本利得與一般所得後的本年度節稅金額,以及結轉至次年的剩餘虧損。",
    formula: "公式",
    formulaText: "抵減利得 = min(虧損, 利得);抵減一般所得 = min(剩餘虧損, 上限);節稅 = (抵減利得 + 抵減一般所得) × 稅率;結轉 = 剩餘。",
    limitations: "限制",
    limitationsText: "本工具採單一稅率的簡化模型,未計入長短期分類、洗售規則、淨投資所得稅與州稅,實際以正式報稅為準。",
    interpretation: "解讀",
    interpretationText: "節稅金額越高代表收割效益越大;結轉虧損則可在未來年度繼續釋放抵減,具長期節稅價值。",
    context: "情境",
    ...
    premiumTitle: "Tax Loss Pro 進階",
    premiumText: "進階版加入長短期分類配對、洗售期偵測、多年結轉排程、替代標的建議與整體投組稅務最佳化。",
    premiumChips_zh: "長短配對|洗售偵測|結轉排程|投組最佳化",
    premiumChips_en: "L/S matching|Wash-sale|Carryforward|Portfolio opt",
    ...
    q1: "什麼是稅損收割?",
    a1: "稅損收割是賣出虧損部位以實現資本虧損,用來抵減同年度的資本利得,並可進一步抵減部分一般所得以節稅。",
    q2: "虧損可以抵多少?",
    a2: "資本虧損先抵減同類資本利得,超額部分可抵減一般所得(美國每年上限約 3,000 美元),再超額者結轉次年。",
    q3: "什麼是結轉虧損?",
    a3: "結轉虧損指當年用不完的資本虧損可遞延至次年繼續抵減利得與一般所得,直到用完為止。",
    q4: "洗售規則是什麼?",
    a4: "洗售規則(wash sale)禁止在賣出虧損後 30 天內買回實質相同證券,否則該虧損不得認列。",
    q5: "一般所得抵減上限多少?",
    a5: "美國規定資本虧損抵減一般所得每年上限約 3,000 美元(夫妻合併申報相同);本工具以您輸入的上限計算。",
    q6: "這個結果準確嗎?",
    a6: "本工具為簡化估算,未計入長短期分類、洗售判定與州稅,執行前請諮詢稅務專業並確認交易細節。"
  },
  en: {
    badge: "Finance · Tax Loss Harvesting Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Tax Loss Harvesting Calculator",
    subtitle: "Estimate tax saved by offsetting gains and income with harvested capital losses.",
    intro: "Tax Loss Harvesting Calculator runs the standard formula in your browser. Enter harvestable capital loss, realized gains this year, capital gains tax rate, ordinary income offset cap to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Tax Loss Harvesting Calculator",
    examplePreview: "Estimated tax saved this year",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter harvestable capital loss, realized gains this year, capital gains tax rate, ordinary income offset cap",
    ...
    baselineExampleValue: "Standard harvest case",
    baselineExampleNote: "Harvestable capital loss 15000 · Realized gains this year 8000",
    activeExample: "Advanced example",
    activeExampleValue: "Large-loss carryforward case",
    activeExampleNote: "Harvestable capital loss doubled · watch Estimated tax saved this year react",
    flowDemo: "Data flow demo",
    calculator: "Tax Loss Harvesting Calculator",
    harvestableCapitalLoss: "Harvestable capital loss",
    realizedGainsThisYear: "Realized gains this year",
    capitalGainsTaxRate: "Capital gains tax rate",
    ordinaryIncomeOffsetCap: "Ordinary income offset cap",
    resultCard: "Result card",
    primaryValue: "Estimated tax saved this year",
    primaryUnitTail: "$",
    secondaryLabel: "Gains offset",
    secondaryTail: "$",
    metricALabel: "Estimated tax saved this year",
    metricACaption: "Main figure from the standard formula",
    metricATail: "$",
    metricBLabel: "Gains offset",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "$",
    metricCLabel: "Ordinary income offset",
    metricCCaption: "Percentage view",
    metricCTail: "$",
    headlineCaption: "Tax Loss Harvesting Calculator · live calc",
    fatLossTarget: "Loss carried forward",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Tax Loss Harvesting Calculator · band matrix",
    ...
    dailyGap: "Daily gap",
    tertiaryTag: "Ordinary income offset",
    ...
    nextActionItem1: "Move Harvestable capital loss and Capital gains tax rate by ±10% to see sensitivity.",
    ...
    decisionTitle: "Tax Loss Harvesting Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill harvestable capital loss, realized gains this year, capital gains tax rate, ordinary income offset cap.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "Tax Loss Harvesting Calculator standard formula.",
    ...
    knowledgeTitle: "Tax Loss Harvesting Calculator · concept primer",
    definition: "Definition",
    definitionText: "Tax Loss Harvesting Calculator converts inputs (harvestable capital loss, realized gains this year, capital gains tax rate, ordinary income offset cap) into Estimated tax saved this year. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(harvestable capital loss, realized gains this year, capital gains tax rate, ordinary income offset cap)",
    ...
    premiumTitle: "Tax Loss Pro",
    premiumText: "Pro adds long/short matching, wash-sale window detection, multi-year carryforward scheduling, replacement-security suggestions, and portfolio-wide tax optimization.",
    premiumChips_zh: "長短配對|洗售偵測|結轉排程|投組最佳化",
    premiumChips_en: "L/S matching|Wash-sale|Carryforward|Portfolio opt",
    ...
    q1: "What does Tax Loss Harvesting Calculator calculate?",
    a1: "Tax Loss Harvesting Calculator applies the standard formula to your inputs and returns Estimated tax saved this year plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Tax Loss Harvesting Calculator?",
    a2: "Enter harvestable capital loss, realized gains this year, capital gains tax rate, ordinary income offset cap. Tax Loss Harvesting Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Pro adds long/short matching, wash-sale window detection, multi-year carryforward scheduling, replacement-security suggestions, and portfolio-wide tax optimization."
  },
} as const;
...
export default function TaxLossHarvesting() {
  ...
  const [harvestableCapitalLoss, setHarvestableCapitalLoss] = useState("15000");
  const [realizedGainsThisYear, setRealizedGainsThisYear] = useState("8000");
  const [capitalGainsTaxRate, setCapitalGainsTaxRate] = useState("15");
  const [ordinaryIncomeOffsetCap, setOrdinaryIncomeOffsetCap] = useState("3000");
  const t = ui[lang];

  const result = useMemo(() => {
const loss = Number(harvestableCapitalLoss) || 0; const gains = Number(realizedGainsThisYear) || 0; const rate = (Number(capitalGainsTaxRate) || 0) / 100; const ordinaryCap = Number(ordinaryIncomeOffsetCap) || 0; const offsetGains = Math.min(loss, gains); const remainingLoss = Math.max(0, loss - offsetGains); const ordinaryOffset = Math.min(remainingLoss, ordinaryCap); const carryforward = Math.max(0, remainingLoss - ordinaryOffset); const taxSaved = (offsetGains + ordinaryOffset) * rate; return { primaryKey: taxSaved, secondaryKey: offsetGains, tertiaryKey: ordinaryOffset, quaternaryKey: carryforward };
  }, [harvestableCapitalLoss, realizedGainsThisYear, capitalGainsTaxRate, ordinaryIncomeOffsetCap]);
  ...
  function fillSolid() { setUnit("metric"); setHarvestableCapitalLoss("15000"); setRealizedGainsThisYear("8000"); setCapitalGainsTaxRate("15"); setOrdinaryIncomeOffsetCap("3000"); }
  function fillHighSalary() { setUnit("imperial"); setHarvestableCapitalLoss("40000"); setRealizedGainsThisYear("5000"); setCapitalGainsTaxRate("24"); setOrdinaryIncomeOffsetCap("3000"); }

  const activeBand = bands.find(b => {
    const r = result.primaryKey;
    if (r < 200) return 'tiny';
    if (r < 800) return 'normal';
    if (r < 1500) return 'notable';
    if (r < 3000) return 'high';
    if (r < 6000) return 'major';
    return 'executive';
  });
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#dcfce7,_#f8fafc_45%,_#d1fae5)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-green-100 bg-white/90 p-6 shadow-2xl shadow-green-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-green-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-green-600 p-5 text-white"><div className="text-xs font-bold uppercase text-green-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-green-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{harvestableCapitalLoss} × {realizedGainsThisYear}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-green-200 bg-green-50 px-5 py-4 text-sm font-black text-green-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.harvestableCapitalLoss}<input type="number" step="1000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={harvestableCapitalLoss} onChange={(e) => setHarvestableCapitalLoss(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.realizedGainsThisYear}<input type="number" step="1000" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={realizedGainsThisYear} onChange={(e) => setRealizedGainsThisYear(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.capitalGainsTaxRate}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={capitalGainsTaxRate} onChange={(e) => setCapitalGainsTaxRate(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.ordinaryIncomeOffsetCap}<input type="number" step="500" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={ordinaryIncomeOffsetCap} onChange={(e) => setOrdinaryIncomeOffsetCap(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="tax-loss-harvesting-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="tax-loss-harvesting-faq" position="inline" /></section>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">{/* L17-TrustRelatedReferences */}<p className="text-xs font-black uppercase tracking-[0.2em] text-green-700">{t.trustReferences}</p><div className="mt-4 grid gap-5 md:grid-cols-3"><div><h2 className="text-xl font-black">{t.trust}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.trustText}</p></div><div><h2 className="text-xl font-black">{t.relatedTools}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.relatedToolsText}</p></div><div><h2 className="text-xl font-black">{t.references}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.referencesText}</p></div></div></section>
      </div>
    </main>
  );
}
