// @profile B
// Profile B · 計算機-YMYL · Tax Refund Calculator - estimated refund or balance due from withholdings（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<5%", label: { zh: "幾近持平", en: "Near even" }, desc: { zh: "退稅或補稅金額很小,扣繳與稅額相當吻合。", en: "A very small refund or balance; withholding closely matched the tax." } },
  { key: "normal", range: "5-15%", label: { zh: "小幅退補", en: "Small swing" }, desc: { zh: "差額不大,屬於正常的扣繳誤差範圍。", en: "A modest difference within the normal withholding margin." } },
  { key: "notable", range: "15-30%", label: { zh: "明顯退補", en: "Notable" }, desc: { zh: "差額明顯,可調整扣繳以更貼近實際稅額。", en: "A notable difference; adjust withholding to track actual tax better." } },
  { key: "high", range: "30-50%", label: { zh: "較大退補", en: "Large" }, desc: { zh: "扣繳與稅額落差較大,值得檢視申報資料。", en: "A large gap between withholding and tax; review your filing data." } },
  { key: "major", range: "50-80%", label: { zh: "大幅退補", en: "Heavy" }, desc: { zh: "差額偏高,可能漏報扣抵或扣繳設定有誤。", en: "A heavy difference; you may have missed credits or set withholding wrong." } },
  { key: "executive", range: "80%+", label: { zh: "極大退補", en: "Extreme" }, desc: { zh: "差額極大,建議重新核對所得與扣繳明細。", en: "An extreme difference; re-check your income and withholding details." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "退稅計算機",
    subtitle: "比對應納稅額與已扣繳稅額,估算可退稅或需補稅金額。",
    intro: "輸入課稅所得、適用稅率、已扣繳稅額與可抵減稅額,本工具計算應納稅額並與扣繳金額比對,估算你可退稅或需補繳的金額,協助你提前掌握報稅結果。",
    trustNoteLabel: "注意事項:",
    ...
    calculator: "計算機",
    participants: "課稅所得",
    averageHourlyRate: "適用稅率 (%)",
    durationHours: "已扣繳稅額",
    meetingsPerMonth: "可抵減稅額",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Compare tax due with amounts withheld to estimate a refund or balance owed.",
    intro: "Enter your taxable income, applicable rate, tax already withheld and any credits; the tool computes the tax due, compares it with withholding and estimates your refund or balance owed so you can anticipate your filing outcome.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a tax-refund example",
    ...
    averageHourlyRate: "Applicable rate (%)",
    durationHours: "Tax already withheld",
...
export default function TaxRefundCalculator() {
  ...
  const [participants, setParticipants] = useState("900000");
  const [averageHourlyRate, setAverageHourlyRate] = useState("12");
  const [durationHours, setDurationHours] = useState("120000");
  ...
  const result = useMemo(() => {
    ...
    const taxableIncome = v1; const effectiveRate = v2; const taxWithheld = v3; const taxCredits = v4;
    const taxDue = Math.max(0, taxableIncome * (effectiveRate / 100) - taxCredits);
    const refund = taxWithheld - taxDue;
    const refundAbs = Math.abs(refund);
    const refundPct = taxWithheld > 0 ? (refundAbs / taxWithheld) * 100 : 0;
    return { refund, refundAbs, taxDue, refundPct, taxWithheld };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.refund, 0);
  const monthlyDisplay = fmt(result.taxDue, 0);

  function fillSolid() { setUnit("metric"); setParticipants("900000"); setAverageHourlyRate("12"); setDurationHours("120000"); setMeetingsPerMonth("0"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("1500000"); setAverageHourlyRate("20"); setDurationHours("350000"); setMeetingsPerMonth("0"); }

  const activeBand = bands.find(b => {
    const r = result.refundPct;
    if (r < 5) return b.key === "tiny";
    if (r < 15) return b.key === "normal";
    if (r < 30) return b.key === "notable";
    if (r < 50) return b.key === "high";
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">12%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "中等稅率" : "Mid rate"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">20%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "較高稅率" : "Higher rate"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "應納稅額" : "Tax due"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.taxDue, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "已扣繳" : "Withheld"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.taxWithheld, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "退稅(+)/補稅(-)" : "Refund(+)/Due(-)"}</div><p className="mt-2 text-3xl font-black text-slate-950">${fmt(result.refund, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="tax-refund-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "預估退稅/補稅" : "refund or balance"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入課稅所得" : "Enter taxable income", note: t.bmrStep }, { label: lang === "zh" ? "套用稅率" : "Apply the tax rate", note: t.deficitStep }, { label: lang === "zh" ? "比對已扣繳稅額" : "Compare withholding", note: t.trendStep }, { label: lang === "zh" ? "得退稅或補稅" : "Get refund or balance", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="tax-refund-calculator-faq" position="inline" /></section>
