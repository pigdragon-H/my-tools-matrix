// @profile B
// Profile B · 計算機-YMYL · ROI Calculator - return on investment and annualized return（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<2%", label: { zh: "報酬偏低", en: "Low return" }, desc: { zh: "年化報酬偏低,可能跑輸通膨或定存。", en: "A low annualized return that may trail inflation or deposits." } },
  { key: "normal", range: "2-5%", label: { zh: "溫和報酬", en: "Moderate" }, desc: { zh: "與保守型投資相當的報酬水準。", en: "A return in line with conservative investments." } },
  { key: "notable", range: "5-10%", label: { zh: "良好報酬", en: "Good return" }, desc: { zh: "接近長期股市平均報酬,表現不錯。", en: "Near the long-run equity average; a solid result." } },
  { key: "high", range: "10-20%", label: { zh: "優異報酬", en: "Strong" }, desc: { zh: "報酬優異,但須留意是否伴隨較高風險。", en: "A strong return; check whether it carries higher risk." } },
  { key: "major", range: "20-40%", label: { zh: "卓越報酬", en: "Excellent" }, desc: { zh: "報酬卓越,通常難以長期持續。", en: "An excellent return that is usually hard to sustain." } },
  { key: "executive", range: "40%+", label: { zh: "極高報酬", en: "Exceptional" }, desc: { zh: "極高年化報酬,務必確認是否含高風險或運氣成分。", en: "An exceptional annualized return; check for high risk or luck." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "投資報酬率計算機",
    subtitle: "依投入成本與最終價值,計算總報酬率與年化報酬。",
    intro: "輸入投入成本、最終價值、持有年數與額外收入,本工具計算總投資報酬率 (ROI)、年化報酬率與資金倍數,協助你客觀衡量任何投資的真實績效。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立投資報酬率範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "投入成本",
    averageHourlyRate: "最終價值",
    durationHours: "持有年數",
    meetingsPerMonth: "額外收入",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Compute total and annualized return from your cost and final value.",
    intro: "Enter your initial cost, final value, holding period and any extra income; the tool computes total ROI, annualized return and a money multiple so you can objectively measure the real performance of any investment.",
    trustNoteLabel: "Note:",
    ...
    participants: "Initial cost",
    averageHourlyRate: "Final value",
    durationHours: "Holding years",
    meetingsPerMonth: "Extra income",
    resultCard: "Result",
    unit: "Primary result",
...
export default function RoiCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("150000");
  ...
  const result = useMemo(() => {
    ...
    const initialCost = v1; const finalValue = v2; const holdingYears = v3; const extraIncome = v4;
    const totalReturn = finalValue + extraIncome - initialCost;
    const roi = initialCost > 0 ? (totalReturn / initialCost) * 100 : 0;
    const years = holdingYears > 0 ? holdingYears : 1;
    const annualized = initialCost > 0 ? (Math.pow((finalValue + extraIncome) / initialCost, 1 / years) - 1) * 100 : 0;
    const multiple = initialCost > 0 ? (finalValue + extraIncome) / initialCost : 0;
    return { roi, annualized, multiple, totalReturn, initialCost };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.roi, 2);
  const monthlyDisplay = fmt(result.totalReturn, 0);

  function fillSolid() { setUnit("metric"); setParticipants("100000"); setAverageHourlyRate("150000"); setDurationHours("3"); setMeetingsPerMonth("0"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("100000"); setAverageHourlyRate("250000"); setDurationHours("3"); setMeetingsPerMonth("0"); }

  const activeBand = bands.find(b => {
    const r = result.annualized;
    if (r < 2) return b.key === "tiny";
    if (r < 5) return b.key === "normal";
    if (r < 10) return b.key === "notable";
    if (r < 20) return b.key === "high";
    if (r < 40) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#eff6ff 0%,#dbeafe 55%,#ede9fe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "總投資報酬率" : "total ROI"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.annualized, 2)}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.multiple, 2)}x</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">5%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "穩健報酬" : "Steady return"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">15%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "高報酬" : "High return"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "%" : "%"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "總獲利" : "Total return"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.totalReturn, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "年化報酬" : "Annualized return"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.annualized, 2)}</p><p className="text-sm font-bold text-blue-700">%</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "資金倍數" : "Money multiple"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.multiple, 2)}</p><p className="text-sm font-bold text-slate-700">x</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="roi-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "總投資報酬率" : "total ROI"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入投入成本" : "Enter the initial cost", note: t.bmrStep }, { label: lang === "zh" ? "設定最終價值" : "Set the final value", note: t.deficitStep }, { label: lang === "zh" ? "計算總報酬率" : "Compute total ROI", note: t.trendStep }, { label: lang === "zh" ? "年化得年報酬" : "Annualize the return", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="roi-calculator-faq" position="inline" /></section>
