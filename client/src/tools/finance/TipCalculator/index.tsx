// @profile B
// Profile B · 計算機-YMYL · TipCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const ui = {
  zh: {
    ...
    chineseShort: "中",
    englishShort: "EN",
    title: "Tip Calculator · 小費計算機",
    subtitle: "輸入帳單金額、小費比例與分攤人數，立即算出小費、總計與每人應付",
    intro: "本工具為 小費計算機，依公開公式於瀏覽器端試算，輸入帳單金額、小費比例%、分攤人數、稅率%後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    enterValues: "輸入帳單金額、小費比例%、分攤人數、稅率%",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "帳單 100 · 18% · 2 人",
    baselineExampleNote: "帳單金額 100 · 小費比例% 18",
    activeExample: "進階範例",
    activeExampleValue: "帳單 240 · 20% · 4 人 · 稅 8%",
    activeExampleNote: "帳單金額 加倍 · 觀察 小費比例 變化",
    flowDemo: "數字流向示範",
    calculator: "小費計算機",
    billAmount: "帳單金額",
    tipPercent: "小費比例%",
    ...
    metricCTail: "$",
    headlineCaption: "小費計算機 · 即時試算",
    fatLossTarget: "小費",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "小費計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 帳單金額 與 分攤人數 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "小費計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 帳單金額、小費比例%、分攤人數、稅率% 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依帳單、小費比例、人數與稅率計算小費、含稅含小費總額與每人應付。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "知識庫",
    knowledgeTitle: "小費計算機 · 觀念整理",
    definition: "定義",
    definitionText: "小費計算機以帳單金額、小費比例、分攤人數與稅率,計算應給小費、含稅含小費總額與每人應付金額,適用餐廳、外送、聚餐分帳。",
    formula: "公式",
    formulaText: "小費 = 帳單 × 小費比例;總計 = 帳單 + 稅 + 小費;每人應付 = 總計 ÷ 人數",
    limitations: "限制",
    limitationsText: "本工具平均分攤總額,不處理逐人不同消費、自動服務費(gratuity)重複計算與多幣別;特殊分帳請依實際協議。",
    interpretation: "解讀",
    interpretationText: "小費比例反映服務滿意度與地區文化;每人應付為快速分帳依據,人數越多單人負擔越低。",
    context: "情境",
    ...
    premiumTitle: "專業版 聚餐分帳工具組",
    premiumText: "解鎖逐人項目分帳、按消費比例分攤、常用比例記憶、整數進位與多幣別即時換算。",
    premiumChips_zh: "逐人分帳|比例分攤|比例記憶|多幣別",
    premiumChips_en: "Per-item|Proportional|Presets|Multi-FX",
    ...
    q1: "美國小費一般給多少?",
    a1: "美國餐廳正餐通常 **15-20%**,18% 是常見標準;吧檯每杯 1-2 美元;外送約 10-15%;計程車/Uber 約 10-20%。服務特別好可給 25%+。歐洲、亞洲多數國家小費文化較淡或已含服務費。",
    q2: "小費要不要含稅計算?",
    a2: "兩派做法都有:**算稅前**(只對食物服務給小費,較常見且合理)或**算稅後**(對含稅總額給,較大方)。本工具預設對帳單金額計算小費、稅另計,你可調整稅率欄位觀察差異。",
    q3: "外送、外帶也要給小費嗎?",
    a3: "美國外送通常給 10-15% 或至少 3-5 美元,惡劣天氣可加給;外帶傳統上不強制,但若店家有特別包裝服務,給 10% 是禮貌。各地文化不同,本工具讓你自由設定比例。",
    q4: "分帳怎麼算最公平?",
    a4: "最公平是**含小費含稅的總額平分**(本工具預設),適合大家點得差不多時;若有人點很貴,則建議按各自消費比例分攤,專業版支援逐人項目分帳。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內完成,帳單金額不會傳送到任何伺服器。",
    q6: "可以記錄常用比例嗎?",
    a6: "逐人項目分帳、常用比例記憶、四捨五入到整數與多幣別屬於專業版功能。"
  },
  en: {
    badge: "Finance · Tip Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Tip Calculator · 小費計算機",
    subtitle: "Enter bill amount, tip percent, and number of people to compute tip, total, and per-person",
    intro: "Tip Calculator runs the standard formula in your browser. Enter bill amount, tip percent, split people, tax rate pct to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Tip Calculator",
    examplePreview: "Tip Rate",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter bill amount, tip percent, split people, tax rate pct",
    ...
    baselineExampleValue: "Bill 100 · 18% · 2",
    baselineExampleNote: "Bill Amount 100 · Tip Percent 18",
    activeExample: "Advanced example",
    activeExampleValue: "Bill 240 · 20% · 4 · tax 8%",
    activeExampleNote: "Bill Amount doubled · watch Tip Rate react",
    flowDemo: "Data flow demo",
    calculator: "Tip Calculator",
    billAmount: "Bill Amount",
    tipPercent: "Tip Percent",
    splitPeople: "Split People",
    taxRatePct: "Tax Rate Pct",
    resultCard: "Result card",
    primaryValue: "Tip Rate",
    primaryUnitTail: "%",
    secondaryLabel: "Per Person",
    secondaryTail: "$",
    metricALabel: "Tip Rate",
    metricACaption: "Main figure from the standard formula",
    metricATail: "%",
    metricBLabel: "Per Person",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "$",
    metricCLabel: "Total",
    metricCCaption: "Percentage view",
    metricCTail: "$",
    headlineCaption: "Tip Calculator · live calc",
    fatLossTarget: "Tip",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Tip Calculator · band matrix",
    ...
    nextActionItem1: "Move Bill Amount and Split People by ±10% to see sensitivity.",
    ...
    decisionTitle: "Tip Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill bill amount, tip percent, split people, tax rate pct.",
    ...
    knowledge: "Knowledge",
    knowledgeTitle: "Tip Calculator · concept primer",
    definition: "Definition",
    definitionText: "Tip Calculator converts inputs (bill amount, tip percent, split people, tax rate pct) into Tip Rate. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "小費 = 帳單 × 小費比例;總計 = 帳單 + 稅 + 小費;每人應付 = 總計 ÷ 人數",
    ...
    premiumTitle: "Pro Bill Split Suite",
    premiumText: "Unlock per-item splitting, proportional split, saved tip presets, rounding, and multi-currency.",
    premiumChips_zh: "逐人分帳|比例分攤|比例記憶|多幣別",
    premiumChips_en: "Per-item|Proportional|Presets|Multi-FX",
    ...
    q1: "About: 美國小費一般給多少?",
    a1: "For \"美國小費一般給多少?\": Tip Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 小費要不要含稅計算?",
    a2: "For \"小費要不要含稅計算?\": Tip Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 外送、外帶也要給小費嗎?",
    a3: "For \"外送、外帶也要給小費嗎?\": Tip Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 分帳怎麼算最公平?",
    a4: "For \"分帳怎麼算最公平?\": Tip Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳嗎?",
    a5: "For \"資料會上傳嗎?\": Tip Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 可以記錄常用比例嗎?",
    a6: "For \"可以記錄常用比例嗎?\": Tip Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
...
export default function TipCalculator() {
  ...
  }, [billAmount, tipPercent, splitPeople, taxRatePct]);

  const primaryDisplay = fmt(result.tipRate, 1);
  const secondaryDisplay = fmt(result.perPerson, 2);
  const tertiaryDisplay = fmt(result.total, 2);
  const quaternaryDisplay = fmt(result.tip, 2);

  function fillSolid() { setUnit("metric"); setBillAmount("100"); setTipPercent("18"); setSplitPeople("2"); setTaxRatePct("0"); }
  function fillHighSalary() { setUnit("imperial"); setBillAmount("240"); setTipPercent("20"); setSplitPeople("4"); setTaxRatePct("8"); }

  const activeBand = bands.find(b => {
    const r = result.tipRate;
    if (r < 1) return 'tiny';
    if (r < 12) return 'normal';
    if (r < 18) return 'notable';
    if (r < 22) return 'high';
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ede9fe,_#f8fafc_45%,_#e0e7ff)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-violet-100 bg-white/90 p-6 shadow-2xl shadow-violet-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-violet-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-violet-600 p-5 text-white"><div className="text-xs font-bold uppercase text-violet-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}%</div><div className="text-sm font-bold text-violet-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{billAmount} × {tipPercent}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-violet-200 bg-violet-50 px-5 py-4 text-sm font-black text-violet-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.billAmount}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={billAmount} onChange={(e) => setBillAmount(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.tipPercent}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={tipPercent} onChange={(e) => setTipPercent(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.splitPeople}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={splitPeople} onChange={(e) => setSplitPeople(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.taxRatePct}<input type="number" step="0.1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={taxRatePct} onChange={(e) => setTaxRatePct(e.target.value)} /></label></div></div>
          </div>
        </section>
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-violet-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-violet-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{primaryDisplay}<span className="text-3xl">%</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.primaryValue}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.secondaryLabel}</div><div className="mt-1 text-xl font-black">{secondaryDisplay}</div><div className="mt-1 text-xs text-slate-300">{t.secondaryTail}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.metricALabel}</div><div className="mt-1 text-xs font-black text-emerald-700">{t.metricACaption}</div><p className="mt-2 text-3xl font-black text-emerald-950">{tertiaryDisplay}</p><p className="text-sm font-bold text-emerald-700">{t.metricATail}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.metricBLabel}</div><div className="mt-1 text-xs font-black text-blue-700">{t.metricBCaption}</div><p className="mt-2 text-3xl font-black text-blue-950">{quaternaryDisplay}</p><p className="text-sm font-bold text-blue-700">{t.metricBTail}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.metricCLabel}</div><div className="mt-1 text-xs font-black text-slate-700">{t.metricCCaption}</div><p className="mt-2 text-3xl font-black text-slate-950">{secondaryDisplay}</p><p className="text-sm font-bold text-slate-700">{t.metricCTail}</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="tip-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-violet-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-violet-50 p-4"><div className="text-xs font-black uppercase text-violet-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-violet-950">{secondaryDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.tertiaryTag}</div><div className="mt-1 text-3xl font-black text-emerald-950">{tertiaryDisplay}</div></div></div></article>
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="tip-calculator-faq" position="inline" /></section>
