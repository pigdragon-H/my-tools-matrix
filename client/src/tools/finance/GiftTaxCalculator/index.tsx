// @profile B
// Profile B · 計算機-YMYL · Gift Tax Calculator - taxable gift and tax after the annual exclusion (progressive bands)（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<2%", label: { zh: "幾乎免稅", en: "Near tax-free" }, desc: { zh: "贈與金額接近或低於免稅額,實質稅負極低。", en: "The gift is at or below the exclusion, so the effective tax burden is very low." } },
  { key: "normal", range: "2-5%", label: { zh: "輕度稅負", en: "Light burden" }, desc: { zh: "超出免稅額不多,稅負溫和,規劃空間充足。", en: "Only a little above the exclusion; the burden is mild with room to plan." } },
  { key: "notable", range: "5-8%", label: { zh: "明顯稅負", en: "Notable burden" }, desc: { zh: "應稅金額提高,值得評估分年贈與或保險工具。", en: "The taxable amount rises; consider spreading gifts across years or other tools." } },
  { key: "high", range: "8-11%", label: { zh: "偏高稅負", en: "High burden" }, desc: { zh: "接近最高一般稅率,建議諮詢稅務專業。", en: "Close to the top standard rate; professional tax advice is recommended." } },
  { key: "major", range: "11-14%", label: { zh: "進入高級距", en: "Upper band" }, desc: { zh: "超過門檻進入高級距,邊際稅率明顯上升。", en: "Above the threshold into the upper band, where the marginal rate clearly rises." } },
  { key: "executive", range: "14%+", label: { zh: "重度稅負", en: "Heavy burden" }, desc: { zh: "大額贈與,整體有效稅率偏高,需縝密規劃。", en: "A large gift with a high overall effective rate that needs careful planning." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "贈與稅計算機",
    subtitle: "輸入年度贈與總額與免稅額,快速估算應課稅淨額與贈與稅。",
    intro: "本工具依年度贈與總額扣除免稅額後的課稅淨額,套用級距稅率估算應納贈與稅與有效稅率,協助你做贈與規劃。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立贈與稅範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "年度贈與總額",
    averageHourlyRate: "免稅額",
    durationHours: "低級距稅率 (%)",
    meetingsPerMonth: "級距門檻",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Estimate the taxable gift and gift tax from your annual gift total and exclusion.",
    intro: "This tool subtracts the annual exclusion from your total gifts, applies band rates, and estimates the gift tax and effective rate to support your gifting plan.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a gift-tax example",
    ...
    participants: "Total gifts this year",
    averageHourlyRate: "Annual exclusion",
    durationHours: "Lower band rate (%)",
    meetingsPerMonth: "Band threshold",
    resultCard: "Result",
    unit: "Primary result",
...
export default function GiftTaxCalculator() {
  ...
  const [participants, setParticipants] = useState("5000000");
  const [averageHourlyRate, setAverageHourlyRate] = useState("2440000");
  const [durationHours, setDurationHours] = useState("10");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("25000000");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const giftAmount = v1; const exclusion = v2; const lowRate = v3; const threshold = v4;
    const taxable = Math.max(0, giftAmount - exclusion);
    const lr = lowRate / 100;
    let tax = 0;
    if (taxable <= threshold) { tax = taxable * lr; }
    else { tax = threshold * lr + (taxable - threshold) * (lr + 0.05); }
    const effRate = giftAmount > 0 ? (tax / giftAmount) * 100 : 0;
    const netGift = giftAmount - tax;
    return { tax, taxable, effRate, netGift, giftAmount, exclusion };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.tax, 0);
  const monthlyDisplay = fmt(result.taxable, 0);

  function fillSolid() { setUnit("metric"); setParticipants("5000000"); setAverageHourlyRate("2440000"); setDurationHours("10"); setMeetingsPerMonth("25000000"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("40000000"); setAverageHourlyRate("2440000"); setDurationHours("10"); setMeetingsPerMonth("25000000"); }

  const activeBand = bands.find(b => {
    const r = result.effRate;
    if (r < 2) return b.key === "tiny";
    if (r < 5) return b.key === "normal";
    if (r < 8) return b.key === "notable";
    if (r < 11) return b.key === "high";
    if (r < 14) return b.key === "major";
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fff1f2 0%,#ffe4e6 55%,#fce7f3 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "應納贈與稅" : "gift tax due"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.effRate, 1)}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.netGift, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">10%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "一般贈與級距" : "Standard gift band"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">15%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "高額贈與級距" : "High-value gift band"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "課稅贈與淨額" : "Taxable gift"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.taxable, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "應納稅額" : "Tax due"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.tax, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "有效稅率" : "Effective rate"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.effRate, 1)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="gift-tax-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "應納贈與稅" : "gift tax due"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入年度贈與總額" : "Enter total annual gifts", note: t.bmrStep }, { label: lang === "zh" ? "扣除免稅額" : "Subtract the exclusion", note: t.deficitStep }, { label: lang === "zh" ? "套用級距稅率" : "Apply band rates", note: t.trendStep }, { label: lang === "zh" ? "得出應納稅額" : "Get the tax due", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="gift-tax-calculator-faq" position="inline" /></section>
