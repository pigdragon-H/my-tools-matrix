// @profile B
// Profile B · 計算機-YMYL · Forex Profit Calculator - profit/loss on a currency trade by lot size and pip move（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<10 pips", label: { zh: "微幅波動", en: "Tiny move" }, desc: { zh: "波動極小,損益接近持平,常見於剝頭皮或盤整時段。", en: "A very small move; P/L is near flat, typical of scalping or ranging sessions." } },
  { key: "normal", range: "10-30 pips", label: { zh: "常見波段", en: "Normal swing" }, desc: { zh: "日內交易常見區間,風險與報酬大致平衡。", en: "A common intraday range with a fairly balanced risk and reward." } },
  { key: "notable", range: "30-60 pips", label: { zh: "明顯波段", en: "Notable swing" }, desc: { zh: "明顯的方向性走勢,適度槓桿下損益可觀。", en: "A clear directional move; with moderate leverage the P/L is meaningful." } },
  { key: "high", range: "60-100 pips", label: { zh: "大波段", en: "Large swing" }, desc: { zh: "大幅波動,務必確認停損與部位規模。", en: "A large swing; confirm your stop-loss and position sizing carefully." } },
  { key: "major", range: "100-200 pips", label: { zh: "劇烈波動", en: "Major move" }, desc: { zh: "劇烈走勢,常伴隨新聞事件,風險顯著放大。", en: "A major move, often around news events, with significantly amplified risk." } },
  { key: "executive", range: "200+ pips", label: { zh: "極端走勢", en: "Extreme move" }, desc: { zh: "極端行情,可能涉及高波動或滑點,需嚴格風控。", en: "An extreme move that may involve high volatility or slippage; strict risk control is essential." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "外匯交易損益計算機",
    subtitle: "依手數、進出場價與每點價值,快速算出一筆外匯交易的損益與點數。",
    intro: "輸入交易手數、進場與出場價格,以及每標準手的每點價值,即可得到本筆交易的損益金額與波動點數,協助你檢視部位規模與風險。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立外匯損益範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    flowDemo: "即時試算",
    calculator: "計算機",
    participants: "交易手數 (標準手)",
    averageHourlyRate: "進場價格",
    durationHours: "出場價格",
    meetingsPerMonth: "每標準手每點價值 (USD)",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Compute the profit or loss and pip change of a forex trade from lot size, entry/exit prices and pip value.",
    intro: "Enter your lot size, entry and exit prices, and the pip value per standard lot to see the trade's profit or loss and pip move, helping you review position size and risk.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a forex P&L example",
    ...
    participants: "Lot size (standard lots)",
    averageHourlyRate: "Entry price",
    durationHours: "Exit price",
    meetingsPerMonth: "Pip value per standard lot (USD)",
    resultCard: "Result",
    unit: "Primary result",
...
export default function ForexProfitCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("1.1000");
  const [durationHours, setDurationHours] = useState("1.1050");
  ...
  const result = useMemo(() => {
    ...
    const lotSize = v1; const entry = v2; const exit = v3; const pipValuePerLot = v4;
    const priceMove = exit - entry;
    const pips = priceMove * 10000;
    const pipValueTotal = pipValuePerLot * lotSize;
    const profit = pips * pipValueTotal;
    const absPips = Math.abs(pips);
    const direction = profit >= 0 ? 1 : 0;
    return { profit, pips, absPips, pipValueTotal, lotSize, priceMove, direction };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.profit, 2);
  const monthlyDisplay = fmt(result.absPips, 1);

  function fillSolid() { setUnit("metric"); setParticipants("1"); setAverageHourlyRate("1.1000"); setDurationHours("1.1050"); setMeetingsPerMonth("10"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("5"); setAverageHourlyRate("1.1000"); setDurationHours("1.1120"); setMeetingsPerMonth("10"); }

  const activeBand = bands.find(b => {
    const r = result.absPips;
    if (r < 10) return b.key === "tiny";
    if (r < 30) return b.key === "normal";
    if (r < 60) return b.key === "notable";
    if (r < 100) return b.key === "high";
    if (r < 200) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#ecfdf5 0%,#d1fae5 55%,#dbeafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "本筆交易損益" : "trade P/L"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.pips, 1)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.pipValueTotal, 2)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">0.5%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "小額單手交易" : "Small single-lot trade"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">2%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "較大手數交易" : "Larger position trade"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "USD" : "USD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">{monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "點" : "pips"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "點數變化" : "Pip change"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.pips, 1)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "每點總價值" : "Pip value (total)"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.pipValueTotal, 2)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "損益金額" : "Profit / loss"}</div><p className="mt-2 text-3xl font-black text-slate-950">${fmt(result.profit, 2)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="forex-profit-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "本筆交易損益" : "trade P/L"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">{monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入手數與進出場價" : "Enter lots and entry/exit", note: t.bmrStep }, { label: lang === "zh" ? "換算點數變化" : "Convert to pip change", note: t.deficitStep }, { label: lang === "zh" ? "乘以每點價值" : "Multiply by pip value", note: t.trendStep }, { label: lang === "zh" ? "得出交易損益" : "Get trade P/L", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="forex-profit-calculator-faq" position="inline" /></section>
