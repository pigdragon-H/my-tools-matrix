...
const inputFields = [
    ...
    { key: 'sellPrice', label: '賣出價格' },
    { key: 'shares', label: '股數' },
    { key: 'feeRate', label: '交易成本 (%)' }
];
...
export default function StockProfitLossCalculator() {
  ...
  const results = useMemo<Result[]>(() => {
    ...
    const cost = v('buyPrice')*v('shares'); const revenue = v('sellPrice')*v('shares'); const fee = (cost+revenue)*v('feeRate')/100; const profit = revenue-cost-fee;
    return [{label:'淨損益', value:money(profit)}, {label:'報酬率', value:pct(cost>0?profit/cost*100:0)}, {label:'交易成本', value:money(fee)}];
  }, [values]);

  return (
    <div className="mx-auto max-w-3xl space-y-6 rounded-2xl border bg-white p-6 shadow-sm dark:bg-slate-950">
      <div>
        <h1 className="text-2xl font-bold">股票損益計算器</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">計算股票買賣損益與報酬率。</p>
      </div>
