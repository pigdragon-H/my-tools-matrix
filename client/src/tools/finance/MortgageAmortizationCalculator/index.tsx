// @profile B
// Profile B · 計算機-YMYL · Mortgage Amortization Calculator - monthly payment and total interest（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<15%", label: { zh: "利息極低", en: "Very low" }, desc: { zh: "總利息佔比很低,屬於相當划算的貸款條件。", en: "Total interest is a small share of principal, a very favourable loan." } },
  { key: "normal", range: "15-30%", label: { zh: "利息溫和", en: "Moderate" }, desc: { zh: "常見的低利長年期房貸,負擔合理。", en: "A common low-rate long-term mortgage with a reasonable burden." } },
  { key: "notable", range: "30-50%", label: { zh: "利息明顯", en: "Notable" }, desc: { zh: "利息累積明顯,可評估提前還款縮短年期。", en: "Interest builds up noticeably; consider prepaying to shorten the term." } },
  { key: "high", range: "50-75%", label: { zh: "利息偏高", en: "High" }, desc: { zh: "總利息接近本金過半,利率或年期偏高。", en: "Total interest nears half the principal; the rate or term is high." } },
  { key: "major", range: "75-100%", label: { zh: "利息高昂", en: "Heavy" }, desc: { zh: "利息幾乎等於本金,務必比較其他方案。", en: "Interest approaches the principal; compare other options carefully." } },
  { key: "executive", range: "100%+", label: { zh: "利息超額", en: "Excessive" }, desc: { zh: "總利息超過本金,長年期高利率組合須重新檢視。", en: "Interest exceeds principal; a long high-rate combination should be reviewed." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "房貸攤還計算機",
    subtitle: "依貸款金額、利率與年期,計算每月還款額與總利息。",
    intro: "輸入貸款金額、年利率與貸款年數,本工具以本息平均攤還公式計算每月還款額、總還款金額與總利息,並顯示利息佔本金的比例,協助你評估房貸負擔。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立房貸攤還範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
  ...
  en: {
    ...
    subtitle: "Compute the monthly payment and total interest from loan amount, rate and term.",
    intro: "Enter the loan amount, annual rate and term; the tool uses the standard amortization formula to compute the monthly payment, total repayment and total interest, and shows interest as a share of principal so you can gauge the mortgage burden.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a mortgage amortization example",
    ...
    durationHours: "Loan term (years)",
    meetingsPerMonth: "Extra monthly payment",
    resultCard: "Result",
    unit: "Primary result",
...
export default function MortgageAmortizationCalculator() {
  ...
  const [participants, setParticipants] = useState("8000000");
  const [averageHourlyRate, setAverageHourlyRate] = useState("2.1");
  ...
  const result = useMemo(() => {
    ...
    const loanAmount = v1; const annualRate = v2; const years = v3; const extraPayment = v4;
    const n = years * 12;
    const i = annualRate / 100 / 12;
    const basePayment = i > 0 ? loanAmount * i / (1 - Math.pow(1 + i, -n)) : (n > 0 ? loanAmount / n : 0);
    const monthlyPayment = basePayment + extraPayment;
    const totalPaid = i > 0 ? basePayment * n : loanAmount;
    const totalInterest = totalPaid - loanAmount;
    const interestRatio = loanAmount > 0 ? (totalInterest / loanAmount) * 100 : 0;
    return { monthlyPayment, totalInterest, totalPaid, interestRatio, loanAmount };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.monthlyPayment, 0);
  const monthlyDisplay = fmt(result.totalInterest, 0);

  function fillSolid() { setUnit("metric"); setParticipants("8000000"); setAverageHourlyRate("2.1"); setDurationHours("30"); setMeetingsPerMonth("0"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("12000000"); setAverageHourlyRate("3.5"); setDurationHours("20"); setMeetingsPerMonth("0"); }

  const activeBand = bands.find(b => {
    const r = result.interestRatio;
    if (r < 15) return b.key === "tiny";
    if (r < 30) return b.key === "normal";
    if (r < 50) return b.key === "notable";
    if (r < 75) return b.key === "high";
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">2%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "低利房貸" : "Low-rate mortgage"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">4%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "較高利率房貸" : "Higher-rate mortgage"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元/月" : "/mo"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "總還款額" : "Total paid"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.totalPaid, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "總利息" : "Total interest"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.totalInterest, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "利息佔本金" : "Interest vs principal"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.interestRatio, 1)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="mortgage-amortization-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入貸款金額" : "Enter the loan amount", note: t.bmrStep }, { label: lang === "zh" ? "設定利率與年期" : "Set rate and term", note: t.deficitStep }, { label: lang === "zh" ? "攤還公式計算月付" : "Amortize to a monthly payment", note: t.trendStep }, { label: lang === "zh" ? "得總利息與佔比" : "Get total interest and ratio", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="mortgage-amortization-calculator-faq" position="inline" /></section>
