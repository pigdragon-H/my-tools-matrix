...
const ui = {
  zh: {
    ...
    intro: "本工具採用國際公認的「月複利 + 定期投入」標準公式，讓您輸入目前年齡、預計退休年齡、預計壽命、現有退休金、每月儲蓄與年化報酬率，即可估算退休時可累積的退休金總額、退休後每月可支取金額、以及自備款累計。並提供 40 / 50 / 55 / 60 / 65 / 70 歲退休年齡六段對照表，協助您直觀感受「退休年齡每延後 5 年，複利效應有多大差異」。",
    trustNoteLabel: "信任提醒：",
    ...
    examplesHelper: "先用範例理解年齡、儲蓄與報酬率之間的關係，再改成您自己的退休計畫。",
    metric: "新台幣",
    imperial: "美元",
    exampleCards: "範例卡",
    ...
    retireMatrixNote: "下列卡片以您目前已存退休金、月儲蓄與年化報酬率為基礎，換算不同退休年齡下的累積總額與每月可支取金額，協助您直觀感受「退休年齡每延後 5 年，複利效應呈倍數放大」。",
    ...
    q6: "FIRE 提早退休可行嗎？",
    a6: "FIRE（財務自由提早退休）需要極高儲蓄率（30-70%）與長累積期。經典 25 倍法則：年支出 × 25 = 目標退休金。例如年支出 60 萬，需 1500 萬退休金。本工具可幫您估算達標所需的月儲蓄與年化報酬率組合。",
  },
  en: {
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[1fr_1fr]">
          ...
          <PremiumGate plan="PRO">
            <article className="flex h-full flex-col rounded-[2rem] border border-violet-200 bg-gradient-to-br from-violet-50 to-indigo-50 p-6 md:p-7">{/* L16-PremiumGate */}
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">
          {/* L17-TrustRelatedReferences */}<p className="text-xs font-black uppercase tracking-[0.2em] text-violet-700">{t.trustReferences}</p>
