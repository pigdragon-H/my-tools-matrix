// @profile B
// Profile B · 計算機-YMYL · DollarCostAveraging（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 0", label: { zh: "獲利有限 (< 0)", en: "Limited gain (< 0)" }, desc: { zh: "落在「獲利有限」級距< 0。累積報酬有限,可能因期間偏短或報酬率偏低,複利效果尚未充分發酵。", en: "Falls in the \"Limited gain\" band (< 0). Limited cumulative gain; likely a short period or low return, compounding not yet in full effect." } },
  { key: "normal", range: "0–20", label: { zh: "小幅成長 (0–20)", en: "Small growth (0–20)" }, desc: { zh: "落在「小幅成長」級距0–20。累積報酬小幅成長,定期定額已開始累積,但仍需更長時間放大複利。", en: "Falls in the \"Small growth\" band (0–20). Small cumulative growth; DCA is starting to accumulate but needs more time to compound." } },
  { key: "notable", range: "20–50", label: { zh: "穩健成長 (20–50)", en: "Steady growth (20–50)" }, desc: { zh: "落在「穩健成長」級距20–50。累積報酬穩健成長,定期投入與複利共同推升資產,時間是最大助力。", en: "Falls in the \"Steady growth\" band (20–50). Steady cumulative growth; regular contributions and compounding lift assets together." } },
  { key: "high", range: "50–100", label: { zh: "良好成長 (50–100)", en: "Good growth (50–100)" }, desc: { zh: "落在「良好成長」級距50–100。累積報酬良好,長期定期定額讓本金透過複利明顯增值。", en: "Falls in the \"Good growth\" band (50–100). Good cumulative gain; long-term DCA grows principal meaningfully through compounding." } },
  { key: "major", range: "100–200", label: { zh: "顯著成長 (100–200)", en: "Strong growth (100–200)" }, desc: { zh: "落在「顯著成長」級距100–200。累積報酬顯著,長期紀律投資搭配複利,資產成長相當可觀。", en: "Falls in the \"Strong growth\" band (100–200). Strong cumulative gain; disciplined long-term investing with compounding builds substantial assets." } },
  { key: "executive", range: "≥ 200", label: { zh: "大幅成長 (≥ 200)", en: "Substantial growth (≥ 200)" }, desc: { zh: "落在「大幅成長」級距≥ 200。累積報酬大幅成長,長期間與較高報酬率使複利效果極為突出。", en: "Falls in the \"Substantial growth\" band (≥ 200). Substantial cumulative growth; a long horizon and higher return make compounding stand out." } },
] as const;
...
const affiliateItems: AffiliateItem[] = [
  ...
  { label: { zh: "FIRE 財務自由數字計算機", en: "FIRE Number Calculator" }, href: "/tools/finance/fire-number-calculator" },
  { label: { zh: "ROI 投資報酬率計算機", en: "ROI Calculator" }, href: "/tools/finance/roi-calculator" },
];

const ui = {
  zh: {
    badge: "財務 · 定期定額投資計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Dollar-Cost Averaging Calculator · 定期定額投資計算機",
    subtitle: "由每月投入、期間與報酬率計算定期定額的期末總資產與獲利。",
    intro: "本工具為 定期定額投資計算機，依公開公式於瀏覽器端試算，輸入每月投入金額、投資期間(月)、預期年報酬率(%)、期初一次性投入後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算定期定額投資計算機",
    examplePreview: "期末總資產",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入每月投入金額、投資期間(月)、預期年報酬率(%)、期初一次性投入",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "十年定期定額情境",
    baselineExampleNote: "每月投入金額 10000 · 投資期間(月) 120",
    activeExample: "進階範例",
    activeExampleValue: "二十年含期初投入情境",
    activeExampleNote: "每月投入金額 加倍 · 觀察 期末總資產 變化",
    flowDemo: "數字流向示範",
    calculator: "定期定額投資計算機",
    monthlyContribution: "每月投入金額",
    investmentPeriodInMonths: "投資期間(月)",
    expectedAnnualReturn: "預期年報酬率(%)",
    initialLumpSum: "期初一次性投入",
    resultCard: "結果卡片",
    primaryValue: "期末總資產",
    primaryUnitTail: "",
    secondaryLabel: "累積投入本金",
    secondaryTail: "",
    metricALabel: "期末總資產",
    ...
    metricCTail: "",
    headlineCaption: "定期定額投資計算機 · 即時試算",
    fatLossTarget: "累積報酬率",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "定期定額投資計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 每月投入金額 與 預期年報酬率(%) 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "定期定額投資計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 每月投入金額、投資期間(月)、預期年報酬率(%)、期初一次性投入 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "Periodic investment accumulation analysis。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "工具知識庫",
    knowledgeTitle: "定期定額投資計算機 · 觀念整理",
    definition: "定義",
    definitionText: "定期定額投資計算機以每月投入、投資期間與預期年報酬率,計算長期定期定額累積的期末總資產、投入本金、投資獲利與累積報酬率。",
    formula: "公式",
    formulaText: "月報酬 = 年報酬 ÷ 12;期末總資產 = 期初本金 ×(1 + 月報酬)^月數 + 每月投入 ×〔(1 + 月報酬)^月數 − 1〕÷ 月報酬。",
    limitations: "限制",
    limitationsText: "本工具假設報酬率固定且每月複利,未計入市場波動、交易與管理費用、稅負與通膨,實際結果會與試算有差異,僅供概念性估算。",
    interpretation: "解讀",
    interpretationText: "期末總資產越高、累積報酬率越大,代表複利與時間的效果越顯著;拉長期間與紀律投入是放大成果的關鍵。",
    context: "情境",
    ...
    premiumTitle: "DCA Pro 進階",
    premiumText: "進階版加入波動情境模擬、費用與稅負調整、通膨實質報酬與一次投入 vs 定期定額比較。",
    premiumChips_zh: "波動模擬|費用稅負|實質報酬|一次vs定額",
    premiumChips_en: "Volatility sim|Fees & tax|Real return|Lump vs DCA",
    ...
    q1: "定期定額是什麼?",
    a1: "定期定額(DCA)是每隔固定期間投入固定金額,藉由分批進場攤平成本、降低擇時風險,並透過長期複利累積資產的投資方式。",
    q2: "期末總資產怎麼算?",
    a2: "本工具以年化報酬換算月報酬率,將期初本金與每月投入分別以複利累積:期末總資產 = 期初本金複利 + 每月投入的年金終值。",
    q3: "定期定額有什麼優點?",
    a3: "定期定額可分散進場時點、攤平成本、降低情緒影響,並養成紀律儲蓄的習慣,特別適合長期累積與不擇時的投資人。",
    q4: "報酬率該設多少?",
    a4: "報酬率應依資產類別與長期歷史合理設定,股票型常用 6–8%、平衡型 4–6%;報酬率僅為假設,實際會隨市場波動。",
    q5: "期初一次投入有差嗎?",
    a5: "期初一次性投入會更早參與複利,長期下來通常使期末資產更高;但須評估自身現金流與市場進場風險。",
    q6: "這個結果準確嗎?",
    a6: "本工具假設報酬率固定且每月複利,未計入市場波動、費用、稅負與通膨,實際結果會有差異,僅供概念性參考。"
  },
  en: {
    badge: "Finance · Dollar-Cost Averaging Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Dollar-Cost Averaging Calculator",
    subtitle: "Calculate the future value and gain of dollar-cost averaging from contribution, period and return.",
    intro: "Dollar-Cost Averaging Calculator runs the standard formula in your browser. Enter monthly contribution, investment period in months, expected annual return, initial lump sum to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Dollar-Cost Averaging Calculator",
    examplePreview: "Future value",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter monthly contribution, investment period in months, expected annual return, initial lump sum",
    ...
    baselineExampleValue: "10-year DCA case",
    baselineExampleNote: "Monthly contribution 10000 · Investment period in months 120",
    activeExample: "Advanced example",
    activeExampleValue: "20-year with lump sum case",
    activeExampleNote: "Monthly contribution doubled · watch Future value react",
    flowDemo: "Data flow demo",
    calculator: "Dollar-Cost Averaging Calculator",
    monthlyContribution: "Monthly contribution",
    investmentPeriodInMonths: "Investment period in months",
    ...
    metricATail: "",
    metricBLabel: "Total invested",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "",
    metricCLabel: "Total gain",
    metricCCaption: "Percentage view",
    metricCTail: "",
    headlineCaption: "Dollar-Cost Averaging Calculator · live calc",
    fatLossTarget: "Total return rate",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Dollar-Cost Averaging Calculator · band matrix",
    ...
    nextActionItem1: "Move Monthly contribution and Expected annual return by ±10% to see sensitivity.",
    ...
    decisionTitle: "Dollar-Cost Averaging Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill monthly contribution, investment period in months, expected annual return, initial lump sum.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "Dollar-Cost Averaging Calculator standard formula.",
    ...
    knowledgeTitle: "Dollar-Cost Averaging Calculator · concept primer",
    definition: "Definition",
    definitionText: "Dollar-Cost Averaging Calculator converts inputs (monthly contribution, investment period in months, expected annual return, initial lump sum) into Future value. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(monthly contribution, investment period in months, expected annual return, initial lump sum)",
    ...
    premiumTitle: "DCA Pro",
    premiumText: "Pro adds volatility scenarios, fee and tax adjustment, inflation-adjusted real return and lump-sum vs DCA comparison.",
    premiumChips_zh: "波動模擬|費用稅負|實質報酬|一次vs定額",
    premiumChips_en: "Volatility sim|Fees & tax|Real return|Lump vs DCA",
    ...
    q1: "What does Dollar-Cost Averaging Calculator calculate?",
    a1: "Dollar-Cost Averaging Calculator applies the standard formula to your inputs and returns Future value plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Dollar-Cost Averaging Calculator?",
    a2: "Enter monthly contribution, investment period in months, expected annual return, initial lump sum. Dollar-Cost Averaging Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Pro adds volatility scenarios, fee and tax adjustment, inflation-adjusted real return and lump-sum vs DCA comparison."
  },
} as const;
...
export default function DollarCostAveraging() {
  ...
  const [investmentPeriodInMonths, setInvestmentPeriodInMonths] = useState("120");
  const [expectedAnnualReturn, setExpectedAnnualReturn] = useState("7");
  const [initialLumpSum, setInitialLumpSum] = useState("0");
  const t = ui[lang];

  const result = useMemo(() => {
const monthly = Number(monthlyContribution) || 0; const months = Number(investmentPeriodInMonths) || 0; const annual = Number(expectedAnnualReturn) || 0; const initial = Number(initialLumpSum) || 0; const r = annual / 100 / 12; let future = initial * Math.pow(1 + r, months); if (r > 0) { future += monthly * (Math.pow(1 + r, months) - 1) / r; } else { future += monthly * months; } const totalInvested = initial + monthly * months; const totalGain = future - totalInvested; const gainPct = totalInvested > 0 ? totalGain / totalInvested * 100 : 0; return { primaryKey: future, secondaryKey: totalInvested, tertiaryKey: totalGain, quaternaryKey: gainPct };
  }, [monthlyContribution, investmentPeriodInMonths, expectedAnnualReturn, initialLumpSum]);
  ...
  function fillSolid() { setUnit("metric"); setMonthlyContribution("10000"); setInvestmentPeriodInMonths("120"); setExpectedAnnualReturn("7"); setInitialLumpSum("0"); }
  function fillHighSalary() { setUnit("imperial"); setMonthlyContribution("15000"); setInvestmentPeriodInMonths("240"); setExpectedAnnualReturn("8"); setInitialLumpSum("100000"); }
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ffedd5,_#fff7ed_45%,_#fef3c7)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-orange-100 bg-white/90 p-6 shadow-2xl shadow-orange-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-orange-600 p-5 text-white"><div className="text-xs font-bold uppercase text-orange-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-orange-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{monthlyContribution} × {investmentPeriodInMonths}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-orange-200 bg-orange-50 px-5 py-4 text-sm font-black text-orange-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.monthlyContribution}<input type="number" step="1000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthlyContribution} onChange={(e) => setMonthlyContribution(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.investmentPeriodInMonths}<input type="number" step="6" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={investmentPeriodInMonths} onChange={(e) => setInvestmentPeriodInMonths(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.expectedAnnualReturn}<input type="number" step="0.25" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={expectedAnnualReturn} onChange={(e) => setExpectedAnnualReturn(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.initialLumpSum}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={initialLumpSum} onChange={(e) => setInitialLumpSum(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="dollar-cost-averaging-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="dollar-cost-averaging-faq" position="inline" /></section>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">{/* L17-TrustRelatedReferences */}<p className="text-xs font-black uppercase tracking-[0.2em] text-orange-700">{t.trustReferences}</p><div className="mt-4 grid gap-5 md:grid-cols-3"><div><h2 className="text-xl font-black">{t.trust}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.trustText}</p></div><div><h2 className="text-xl font-black">{t.relatedTools}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.relatedToolsText}</p></div><div><h2 className="text-xl font-black">{t.references}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.referencesText}</p></div></div></section>
      </div>
    </main>
  );
}
