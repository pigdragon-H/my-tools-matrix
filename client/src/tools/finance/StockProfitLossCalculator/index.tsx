// @profile B
// Profile B · 計算機-YMYL · Stock Profit/Loss Calculator - net P/L on a share trade after fees（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<-10%", label: { zh: "明顯虧損", en: "Clear loss" }, desc: { zh: "扣除成本後虧損明顯,檢視進出場策略。", en: "A clear loss after costs; review your entry and exit strategy." } },
  { key: "normal", range: "-10-0%", label: { zh: "小幅虧損", en: "Small loss" }, desc: { zh: "報酬為負,費用侵蝕了部分價差。", en: "A negative return where fees ate into the spread." } },
  { key: "notable", range: "0-8%", label: { zh: "小幅獲利", en: "Small gain" }, desc: { zh: "扣費後仍有正報酬,屬於穩健交易。", en: "A positive return after fees; a steady trade." } },
  { key: "high", range: "8-20%", label: { zh: "良好獲利", en: "Good gain" }, desc: { zh: "報酬良好,價差足以覆蓋交易成本。", en: "A good return; the spread comfortably covers costs." } },
  { key: "major", range: "20-50%", label: { zh: "優異獲利", en: "Strong gain" }, desc: { zh: "報酬優異,適時獲利了結可鎖定收益。", en: "A strong gain; taking profit can lock in the return." } },
  { key: "executive", range: "50%+", label: { zh: "卓越獲利", en: "Exceptional" }, desc: { zh: "報酬卓越,務必檢視風險是否相稱。", en: "An exceptional gain; make sure the risk taken was proportionate." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "股票損益計算機",
    subtitle: "依買賣價、股數與費率,計算扣費後的淨損益與報酬率。",
    intro: "輸入買進價、賣出價、股數與手續費含稅費率,本工具計算毛損益、交易成本與扣費後的淨損益及報酬率,協助你檢視一筆股票交易的真實獲利。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立股票損益範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "買進價",
    averageHourlyRate: "賣出價",
    durationHours: "股數",
    meetingsPerMonth: "手續費+稅 (%)",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Compute net profit or loss and return after fees from buy/sell prices, shares and fee rate.",
    intro: "Enter the buy and sell prices, number of shares and combined fee/tax rate; the tool computes gross P/L, trading costs and the net profit or loss with return percentage so you can see the true outcome of a stock trade.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a stock P&L example",
    ...
    averageHourlyRate: "Sell price",
    durationHours: "Number of shares",
    meetingsPerMonth: "Fees + tax (%)",
    resultCard: "Result",
    unit: "Primary result",
...
export default function StockProfitLossCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("115");
  const [durationHours, setDurationHours] = useState("1000");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("0.585");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const buyPrice = v1; const sellPrice = v2; const shares = v3; const feePct = v4;
    const cost = buyPrice * shares;
    const proceeds = sellPrice * shares;
    const fees = (cost + proceeds) * (feePct / 100) / 2 * 2;
    const grossPL = proceeds - cost;
    const netPL = grossPL - fees;
    const returnPct = cost > 0 ? (netPL / cost) * 100 : 0;
    return { netPL, grossPL, fees, returnPct, cost };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.netPL, 0);
  const monthlyDisplay = fmt(result.grossPL, 0);

  function fillSolid() { setUnit("metric"); setParticipants("100"); setAverageHourlyRate("115"); setDurationHours("1000"); setMeetingsPerMonth("0.585"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("100"); setAverageHourlyRate("140"); setDurationHours("2000"); setMeetingsPerMonth("0.585"); }

  const activeBand = bands.find(b => {
    const r = result.returnPct;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#f0fdfa 0%,#ccfbf1 55%,#cffafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "淨損益" : "net profit / loss"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.returnPct, 2)}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.fees, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">0.585%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "現股交易成本" : "Cash trade cost"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">0.3%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "當沖交易成本" : "Day-trade cost"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "毛損益" : "Gross P/L"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.grossPL, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "交易成本" : "Trading fees"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.fees, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "報酬率" : "Return"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.returnPct, 2)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="stock-profit-loss-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "淨損益" : "net profit / loss"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入買進與賣出價" : "Enter buy and sell prices", note: t.bmrStep }, { label: lang === "zh" ? "設定股數" : "Set the number of shares", note: t.deficitStep }, { label: lang === "zh" ? "扣除手續費與稅" : "Subtract fees and tax", note: t.trendStep }, { label: lang === "zh" ? "得淨損益與報酬" : "Get net P/L and return", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="stock-profit-loss-calculator-faq" position="inline" /></section>
