// @profile B
// Profile B · 計算機-YMYL · Retirement Savings Calculator - projected nest egg from contributions and growth（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<3M", label: { zh: "起步階段", en: "Getting started" }, desc: { zh: "資產仍在累積初期,提高投入或時間都有幫助。", en: "The nest egg is still early; more contributions or time help a lot." } },
  { key: "normal", range: "3M-8M", label: { zh: "穩定累積", en: "Building up" }, desc: { zh: "穩定成長中,可覆蓋基本退休生活。", en: "Growing steadily and able to cover a basic retirement." } },
  { key: "notable", range: "8M-15M", label: { zh: "中度充裕", en: "Comfortable" }, desc: { zh: "資產充裕,退休生活有更多彈性。", en: "A comfortable amount giving more flexibility in retirement." } },
  { key: "high", range: "15M-30M", label: { zh: "相當充裕", en: "Well-funded" }, desc: { zh: "資金充足,可支撐較高品質的退休生活。", en: "Well-funded for a higher-quality retirement lifestyle." } },
  { key: "major", range: "30M-60M", label: { zh: "財務自由", en: "Financially free" }, desc: { zh: "接近財務自由,被動收入可覆蓋多數支出。", en: "Near financial freedom, with passive income covering most spending." } },
  { key: "executive", range: "60M+", label: { zh: "豐沛資產", en: "Abundant" }, desc: { zh: "資產豐沛,可考慮傳承與資產配置規劃。", en: "Abundant assets; consider legacy and allocation planning." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "退休儲蓄計算機",
    subtitle: "依目前儲蓄、每月投入與報酬率,推估退休時的資產總額。",
    intro: "輸入目前儲蓄、每月投入金額、距退休年數與預期年報酬率,本工具以複利推估退休時的資產總額、投資成長與依 4% 法則的退休月領金額,協助你檢視退休準備是否充足。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立退休儲蓄範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "目前儲蓄",
    averageHourlyRate: "每月投入",
    durationHours: "距退休年數",
    meetingsPerMonth: "年報酬率 (%)",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Project your retirement nest egg from current savings, monthly contributions and returns.",
    intro: "Enter your current savings, monthly contribution, years to retirement and expected annual return; the tool compounds these into a projected nest egg, total growth and a 4%-rule monthly income so you can check whether your retirement plan is on track.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a retirement-savings example",
    ...
    participants: "Current savings",
    averageHourlyRate: "Monthly contribution",
    durationHours: "Years to retirement",
    meetingsPerMonth: "Annual return (%)",
    resultCard: "Result",
    unit: "Primary result",
...
export default function RetirementSavingsCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("15000");
  const [durationHours, setDurationHours] = useState("25");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("6");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const currentSavings = v1; const monthlyContribution = v2; const yearsToRetire = v3; const annualReturn = v4;
    const months = yearsToRetire * 12;
    const r = annualReturn / 100 / 12;
    const fvCurrent = currentSavings * Math.pow(1 + r, months);
    const fvContrib = r > 0 ? monthlyContribution * ((Math.pow(1 + r, months) - 1) / r) : monthlyContribution * months;
    const nestEgg = fvCurrent + fvContrib;
    const totalContributed = currentSavings + monthlyContribution * months;
    const growth = nestEgg - totalContributed;
    const monthlyIncome = nestEgg * 0.04 / 12;
    return { nestEgg, growth, totalContributed, monthlyIncome, yearsToRetire };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.nestEgg, 0);
  const monthlyDisplay = fmt(result.monthlyIncome, 0);

  function fillSolid() { setUnit("metric"); setParticipants("500000"); setAverageHourlyRate("15000"); setDurationHours("25"); setMeetingsPerMonth("6"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("1000000"); setAverageHourlyRate("25000"); setDurationHours("30"); setMeetingsPerMonth("8"); }

  const activeBand = bands.find(b => {
    const r = result.nestEgg;
    if (r < 3000000) return b.key === "tiny";
    if (r < 8000000) return b.key === "normal";
    if (r < 15000000) return b.key === "notable";
    if (r < 30000000) return b.key === "high";
    if (r < 60000000) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#f5f3ff 0%,#ede9fe 55%,#fae8ff 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "退休資產總額" : "retirement nest egg"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">${fmt(result.growth, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.monthlyIncome, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">6%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "穩健報酬" : "Steady return"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">8%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "積極報酬" : "Growth return"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元/月" : "/mo"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "累計投入" : "Total contributed"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.totalContributed, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "投資成長" : "Investment growth"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.growth, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "退休月領 (4%)" : "Monthly income (4%)"}</div><p className="mt-2 text-3xl font-black text-slate-950">${fmt(result.monthlyIncome, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="retirement-savings-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "退休資產總額" : "retirement nest egg"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入目前儲蓄" : "Enter current savings", note: t.bmrStep }, { label: lang === "zh" ? "設定每月投入" : "Set monthly contribution", note: t.deficitStep }, { label: lang === "zh" ? "複利成長至退休" : "Compound to retirement", note: t.trendStep }, { label: lang === "zh" ? "得退休資產與月領" : "Get nest egg and income", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="retirement-savings-calculator-faq" position="inline" /></section>
