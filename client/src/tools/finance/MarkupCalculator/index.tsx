...
const bands = [
  { key: "tiny", range: "< 1", label: { zh: "虧損 (< 1)", en: "Very low (< 1)" }, desc: { zh: "落在「虧損」級距< 1。毛利率 ≤ 0%,售價低於成本,賠本銷售,務必檢視定價。", en: "Falls in the \"Very low\" band (< 1). This is the very low range for Markup Calculator." } },
  { key: "normal", range: "1–20", label: { zh: "微利 (1–20)", en: "Low (1–20)" }, desc: { zh: "落在「微利」級距1–20。< 20%,微利,常見於量大薄利的零售或大宗商品。", en: "Falls in the \"Low\" band (1–20). This is the low range for Markup Calculator." } },
  { key: "notable", range: "20–35", label: { zh: "普通 (20–35)", en: "Moderate (20–35)" }, desc: { zh: "落在「普通」級距20–35。20-35%,普通毛利,多數實體零售的常見區間。", en: "Falls in the \"Moderate\" band (20–35). This is the moderate range for Markup Calculator." } },
  { key: "high", range: "35–50", label: { zh: "健康 (35–50)", en: "High (35–50)" }, desc: { zh: "落在「健康」級距35–50。35-50%,健康毛利,品牌商品或差異化產品常見。", en: "Falls in the \"High\" band (35–50). This is the high range for Markup Calculator." } },
  { key: "major", range: "50–65", label: { zh: "高利 (50–65)", en: "Very high (50–65)" }, desc: { zh: "落在「高利」級距50–65。50-65%,高毛利,常見於軟體、服務或精品。", en: "Falls in the \"Very high\" band (50–65). This is the very high range for Markup Calculator." } },
  { key: "executive", range: "≥ 65", label: { zh: "超高 (≥ 65)", en: "Extreme (≥ 65)" }, desc: { zh: "落在「超高」級距≥ 65。> 65%,超高毛利,常見於數位產品、奢侈品或專利商品。", en: "Falls in the \"Extreme\" band (≥ 65). This is the extreme range for Markup Calculator." } },
] as const;
...
const ui = {
  zh: {
    ...
    intro: "本工具為 加成計算機，依公開公式於瀏覽器端試算，輸入商品成本、加成百分比%、目標毛利%、數量後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q3: "怎麼定價才合理?",
    a3: "合理定價需考量:**(1) 成本**(含進貨、運費、人工、攤提)、**(2) 市場行情**(競品價格)、**(3) 目標毛利**(覆蓋營運費用後仍獲利)、**(4) 顧客願付價格**。本工具幫您從成本快速算出不同加成下的售價與毛利。",
    q4: "毛利率多少算健康?",
    a4: "因產業而異:超市生鮮 5-15%、一般零售 30-50%、服飾 50-60%、餐飲 60-70%(食材成本佔比反推)、軟體/數位 80%+。健康毛利需能覆蓋固定成本並留淨利,建議對照同業基準。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內完成,成本與售價資料不會傳送到任何伺服器。",
    q6: "可以反推目標毛利的售價嗎?",
  ...
  en: {
    ...
    deficitNote: "Markup Calculator standard formula.",
    ...
    formula: "Formula",
    formulaText: "result = f(item cost, markup percent, target margin pct, quantity)",
    ...
    q1: "What does Markup Calculator calculate?",
    a1: "Markup Calculator applies the standard formula to your inputs and returns Margin plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Markup Calculator?",
    a2: "Enter item cost, markup percent, target margin pct, quantity. Markup Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock target-margin reverse pricing, tax/shipping markup, wholesale/retail tiers, platform fees, and competitor compare."
  },
} as const;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ecfccb,_#f8fafc_45%,_#d9f99d)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-lime-100 bg-white/90 p-6 shadow-2xl shadow-lime-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-lime-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-lime-600 p-5 text-white"><div className="text-xs font-bold uppercase text-lime-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-lime-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{itemCost} × {markupPercent}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-lime-200 bg-lime-50 px-5 py-4 text-sm font-black text-lime-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
