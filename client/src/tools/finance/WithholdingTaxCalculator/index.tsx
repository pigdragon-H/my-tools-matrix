// @profile B
// Profile B · 計算機-YMYL · Withholding Tax Calculator - tax withheld and net payout on income（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<5%", label: { zh: "扣繳極低", en: "Very low" }, desc: { zh: "扣繳比例很低,實領金額接近給付總額。", en: "A very low withholding share; net pay is close to the gross." } },
  { key: "normal", range: "5-12%", label: { zh: "一般扣繳", en: "Standard" }, desc: { zh: "常見的居住者扣繳水準。", en: "A common withholding level for residents." } },
  { key: "notable", range: "12-18%", label: { zh: "明顯扣繳", en: "Notable" }, desc: { zh: "含補充費後扣繳明顯,實領縮水。", en: "With the supplement added, withholding is notable and net pay shrinks." } },
  { key: "high", range: "18-25%", label: { zh: "偏高扣繳", en: "High" }, desc: { zh: "接近非居住者扣繳率,務必確認身分別。", en: "Near the non-resident rate; confirm the recipient's status." } },
  { key: "major", range: "25-32%", label: { zh: "高扣繳", en: "Heavy" }, desc: { zh: "扣繳偏高,實領僅約給付的七成。", en: "A heavy withholding; net pay is around 70% of the gross." } },
  { key: "executive", range: "32%+", label: { zh: "極高扣繳", en: "Very heavy" }, desc: { zh: "扣繳極高,建議確認適用法規與申報方式。", en: "A very heavy withholding; verify the applicable rules and filing." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "扣繳稅額計算機",
    subtitle: "依給付總額與扣繳率,計算扣繳稅額、補充保費與實領金額。",
    intro: "輸入給付總額、扣繳率、二代健保補充費率與起扣門檻,本工具計算應扣繳稅額、補充保費與實際可領取的淨額,並換算實質扣繳率,協助你掌握薪資或執行業務所得的稅後金額。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立扣繳稅額範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "給付總額",
    averageHourlyRate: "扣繳率 (%)",
    durationHours: "二代健保補充費率 (%)",
    meetingsPerMonth: "補充費門檻",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
    ...
    conversionNote: "此區會連動目前的計算結果,協助你判斷下一步該怎麼做。",
    ...
    definition: "定義",
    definitionText: "本工具把你輸入的數值轉換成可比較的結果,協助你在不同情境間做判斷。",
    formula: "公式",
    formulaText: "結果依輸入數值以固定公式計算;調整任一輸入,結果會即時更新。",
    limitations: "限制",
    limitationsText: "本工具僅提供估算,未涵蓋所有個別因素;重大決策請諮詢專業人員。",
    interpretation: "解讀",
    interpretationText: "數字本身不是答案;請結合你的目標與整體情況一起判斷。",
    context: "脈絡",
    ...
    q1: "這個結果準確嗎?",
    a1: "結果為依公式計算的估算值,準確度取決於你輸入數值的正確性與適用性。",
    q2: "我該如何選擇輸入數值?",
    ...
    q5: "數值越高或越低一定越好嗎?",
    a5: "不一定。請依你的目標與整體財務情況判斷,而非只看單一數字。",
    q6: "這個工具能取代專業建議嗎?",
  ...
  en: {
    ...
    subtitle: "Compute tax withheld, supplement and net payout from gross payment and rate.",
    intro: "Enter the gross payment, withholding rate, health-insurance supplement rate and threshold; the tool computes the tax withheld, the supplement and the net amount you actually receive, with an effective withholding rate, helping you understand the after-tax value of income.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a withholding-tax example",
    ...
    participants: "Gross payment",
    averageHourlyRate: "Withholding rate (%)",
    durationHours: "Supplement rate (%)",
    meetingsPerMonth: "Supplement threshold",
    resultCard: "Result",
    unit: "Primary result",
...
export default function WithholdingTaxCalculator() {
  ...
  const [participants, setParticipants] = useState("100000");
  const [averageHourlyRate, setAverageHourlyRate] = useState("10");
  const [durationHours, setDurationHours] = useState("2.11");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("20000");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const grossPayment = v1; const withholdingRate = v2; const supplementRate = v3; const supplementThreshold = v4;
    const withholding = grossPayment * (withholdingRate / 100);
    const supplement = grossPayment >= supplementThreshold ? grossPayment * (supplementRate / 100) : 0;
    const totalDeduction = withholding + supplement;
    const netPayout = grossPayment - totalDeduction;
    const effRate = grossPayment > 0 ? (totalDeduction / grossPayment) * 100 : 0;
    return { netPayout, withholding, supplement, effRate, grossPayment, totalDeduction };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.netPayout, 0);
  const monthlyDisplay = fmt(result.totalDeduction, 0);

  function fillSolid() { setUnit("metric"); setParticipants("100000"); setAverageHourlyRate("10"); setDurationHours("2.11"); setMeetingsPerMonth("20000"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("300000"); setAverageHourlyRate("20"); setDurationHours("2.11"); setMeetingsPerMonth("20000"); }

  const activeBand = bands.find(b => {
    const r = result.effRate;
    if (r < 5) return b.key === "tiny";
    if (r < 12) return b.key === "normal";
    if (r < 18) return b.key === "notable";
    if (r < 25) return b.key === "high";
    if (r < 32) return b.key === "major";
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#f8fafc 0%,#f1f5f9 55%,#e2e8f0 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "實領金額" : "net payout"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">${fmt(result.withholding, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.effRate, 2)}%</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">10%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "一般扣繳" : "Standard withholding"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">20%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "非居住者扣繳" : "Non-resident rate"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "扣繳稅額" : "Tax withheld"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.withholding, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "補充保費" : "Supplement"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.supplement, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "實質扣繳率" : "Effective rate"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.effRate, 2)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="withholding-tax-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "實領金額" : "net payout"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入給付總額" : "Enter the gross payment", note: t.bmrStep }, { label: lang === "zh" ? "套用扣繳率" : "Apply the withholding rate", note: t.deficitStep }, { label: lang === "zh" ? "加計補充保費" : "Add the supplement", note: t.trendStep }, { label: lang === "zh" ? "得實領金額" : "Get the net payout", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="withholding-tax-calculator-faq" position="inline" /></section>
