...
const ui = {
  zh: {
    ...
    intro: "本工具為 購屋負擔能力計算機，依公開公式於瀏覽器端試算，輸入年收入、頭期款、房貸利率%、每月債務後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q2: "頭期款要準備多少?",
    a2: "傳統建議頭期款 **20%**(可免額外貸款保險 PMI),但首購方案可低至 3-5%(需付 PMI)。頭期越高:月付越低、總利息越少、核貸越易。除頭期外還需保留**過戶費(2-5%)+ 緊急預備金(3-6 個月支出)**。",
    q3: "我能借多少房貸?",
    ...
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內完成,收入與債務資料不會傳送到任何伺服器。",
    q6: "可以含房屋稅與保險嗎?",
  ...
  en: {
    ...
    deficitNote: "Home Affordability Calculator standard formula.",
    ...
    formula: "Formula",
    formulaText: "result = f(annual income, down payment, mortgage rate pct, monthly debt)",
    ...
    q1: "What does Home Affordability Calculator calculate?",
    a1: "Home Affordability Calculator applies the standard formula to your inputs and returns Price-to-Income plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Home Affordability Calculator?",
    a2: "Enter annual income, down payment, mortgage rate pct, monthly debt. Home Affordability Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock full PITI with tax/insurance/HOA/PMI, income/rate stress tests, and multi-scenario budget comparison."
  },
} as const;
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_#f5f5f4,_#fafaf9_45%,_#e7e5e4)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-stone-100 bg-white/90 p-6 shadow-2xl shadow-stone-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-stone-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-stone-600 p-5 text-white"><div className="text-xs font-bold uppercase text-stone-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-stone-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{annualIncome} × {downPayment}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-stone-200 bg-stone-50 px-5 py-4 text-sm font-black text-stone-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">{/* L17-TrustRelatedReferences */}<p className="text-xs font-black uppercase tracking-[0.2em] text-stone-700">{t.trustReferences}</p><div className="mt-4 grid gap-5 md:grid-cols-3"><div><h2 className="text-xl font-black">{t.trust}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.trustText}</p></div><div><h2 className="text-xl font-black">{t.relatedTools}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.relatedToolsText}</p></div><div><h2 className="text-xl font-black">{t.references}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{t.referencesText}</p></div></div></section>
      </div>
    </main>
  );
}
