// @profile B
// Profile B · 計算機-YMYL · HomeAffordabilityCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const ui = {
  zh: {
    badge: "財務 · 購屋負擔能力計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Home Affordability Calculator · 購屋負擔能力計算機",
    subtitle: "輸入年收入、頭期款、房貸利率與既有債務，立即估算可負擔房價與每月房貸上限",
    intro: "本工具為 購屋負擔能力計算機，依公開公式於瀏覽器端試算，輸入年收入、頭期款、房貸利率%、每月債務後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算購屋負擔能力計算機",
    examplePreview: "房價所得倍數",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入年收入、頭期款、房貸利率%、每月債務",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "年收 120 萬 · 頭期 200 萬 · 4%",
    baselineExampleNote: "年收入 1200000 · 頭期款 2000000",
    activeExample: "進階範例",
    activeExampleValue: "年收 240 萬 · 頭期 500 萬 · 3.5%",
    activeExampleNote: "年收入 加倍 · 觀察 房價所得倍數 變化",
    flowDemo: "數字流向示範",
    calculator: "購屋負擔能力計算機",
    annualIncome: "年收入",
    downPayment: "頭期款",
    mortgageRatePct: "房貸利率%",
    monthlyDebt: "每月債務",
    resultCard: "結果卡片",
    primaryValue: "房價所得倍數",
    primaryUnitTail: "x",
    secondaryLabel: "可負擔房價",
    secondaryTail: "$",
    metricALabel: "房價所得倍數",
    ...
    metricCTail: "$",
    headlineCaption: "購屋負擔能力計算機 · 即時試算",
    fatLossTarget: "可貸金額",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "購屋負擔能力計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 年收入 與 房貸利率% 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "購屋負擔能力計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 年收入、頭期款、房貸利率%、每月債務 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依年收入、頭期、利率與既有債務，估算可負擔房價與每月房貸上限。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "知識庫",
    knowledgeTitle: "購屋負擔能力計算機 · 觀念整理",
    definition: "定義",
    definitionText: "購屋負擔能力計算機依年收入、頭期款、房貸利率與既有每月債務,套用 28/36 黃金法則,估算可負擔的房價、每月房貸上限與可貸金額,協助購屋前的預算規劃。",
    formula: "公式",
    formulaText: "每月房貸上限 = min(月收入 × 28%, 月收入 × 36% − 既有債務);可貸金額 = 月付上限 × (1 − (1+r)^−360) / r;可負擔房價 = 可貸金額 + 頭期款",
    limitations: "限制",
    limitationsText: "本工具採 28/36 法則與 30 年期假設,未含房屋稅、保險、HOA、PMI 與信用評分影響;實際核貸金額請以銀行評估為準。",
    interpretation: "解讀",
    interpretationText: "可負擔房價為上限參考,實務上建議保守抓 8 成以保留彈性;房貸佔比越低財務越穩健,越高則對利率與收入變動越敏感。",
    context: "情境",
    ...
    premiumTitle: "專業版 購屋規劃工具組",
    premiumText: "解鎖含房屋稅、保險、HOA、PMI 的完整 PITI 計算、收入/利率壓力測試與多情境購屋預算比較。",
    premiumChips_zh: "PITI計算|壓力測試|多情境|稅險HOA",
    premiumChips_en: "PITI|Stress Test|Scenarios|Tax/Ins",
    ...
    q1: "28/36 法則是什麼?",
    a1: "**28/36 法則**是傳統放款基準:**前端比(28%)**——每月房貸(本息+稅+保險)不超過月毛收入的 28%;**後端比(36%)**——房貸加上所有債務(車貸、卡債、學貸)不超過月毛收入的 36%。本工具取兩者較嚴格者算出可負擔上限。",
    q2: "頭期款要準備多少?",
    a2: "傳統建議頭期款 **20%**(可免額外貸款保險 PMI),但首購方案可低至 3-5%(需付 PMI)。頭期越高:月付越低、總利息越少、核貸越易。除頭期外還需預留**過戶費(2-5%)+ 緊急預備金(3-6 個月支出)**。",
    q3: "我能借多少房貸?",
    a3: "可貸金額取決於:**收入、既有債務、利率、年限、信用評分**。本工具用 28/36 法則反推可負擔月付,再依利率與 30 年期反推可貸本金。實際核貸還看信用、工作穩定性與銀行政策,建議多家比較。",
    q4: "除了房貸還要算哪些成本?",
    a4: "購屋總成本遠不只房貸:**(1) 房屋稅、(2) 房屋保險、(3) 管理費/HOA、(4) 維護修繕(年約房價 1%)、(5) 過戶與代書費、(6) 搬遷與裝潢**。買房前務必把這些算進每月與年度預算,別只看月付。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內完成,收入與債務資料不會傳送到任何伺服器。",
    q6: "可以含房屋稅與保險嗎?",
    a6: "房屋稅、保險、HOA、PMI 的完整 PITI 計算、多情境壓力測試與利率變動分析屬於專業版功能。"
  },
  en: {
    badge: "Finance · Home Affordability Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Home Affordability Calculator · 購屋負擔能力計算機",
    subtitle: "Enter annual income, down payment, mortgage rate, and debts to estimate affordable home price and max payment",
    intro: "Home Affordability Calculator runs the standard formula in your browser. Enter annual income, down payment, mortgage rate pct, monthly debt to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Home Affordability Calculator",
    examplePreview: "Price-to-Income",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter annual income, down payment, mortgage rate pct, monthly debt",
    ...
    baselineExampleValue: "Inc 1.2M · Down 2M · 4%",
    baselineExampleNote: "Annual Income 1200000 · Down Payment 2000000",
    activeExample: "Advanced example",
    activeExampleValue: "Inc 2.4M · Down 5M · 3.5%",
    activeExampleNote: "Annual Income doubled · watch Price-to-Income react",
    flowDemo: "Data flow demo",
    calculator: "Home Affordability Calculator",
    annualIncome: "Annual Income",
    downPayment: "Down Payment",
    mortgageRatePct: "Mortgage Rate Pct",
    monthlyDebt: "Monthly Debt",
    resultCard: "Result card",
    primaryValue: "Price-to-Income",
    primaryUnitTail: "x",
    secondaryLabel: "Affordable Price",
    secondaryTail: "$",
    metricALabel: "Price-to-Income",
    metricACaption: "Main figure from the standard formula",
    metricATail: "x",
    metricBLabel: "Affordable Price",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "$",
    metricCLabel: "Max Payment",
    metricCCaption: "Percentage view",
    metricCTail: "$",
    headlineCaption: "Home Affordability Calculator · live calc",
    fatLossTarget: "Max Loan",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Home Affordability Calculator · band matrix",
    ...
    nextActionItem1: "Move Annual Income and Mortgage Rate Pct by ±10% to see sensitivity.",
    ...
    decisionTitle: "Home Affordability Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill annual income, down payment, mortgage rate pct, monthly debt.",
    ...
    knowledge: "Knowledge",
    knowledgeTitle: "Home Affordability Calculator · concept primer",
    definition: "Definition",
    definitionText: "Home Affordability Calculator converts inputs (annual income, down payment, mortgage rate pct, monthly debt) into Price-to-Income. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "每月房貸上限 = min(月收入 × 28%, 月收入 × 36% − 既有債務);可貸金額 = 月付上限 × (1 − (1+r)^−360) / r;可負擔房價 = 可貸金額 + 頭期款",
    ...
    premiumTitle: "Pro Home Buying Suite",
    premiumText: "Unlock full PITI with tax/insurance/HOA/PMI, income/rate stress tests, and multi-scenario budget comparison.",
    premiumChips_zh: "PITI計算|壓力測試|多情境|稅險HOA",
    premiumChips_en: "PITI|Stress Test|Scenarios|Tax/Ins",
    ...
    q1: "About: 28/36 法則是什麼?",
    a1: "For \"28/36 法則是什麼?\": Home Affordability Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 頭期款要準備多少?",
    a2: "For \"頭期款要準備多少?\": Home Affordability Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 我能借多少房貸?",
    a3: "For \"我能借多少房貸?\": Home Affordability Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 除了房貸還要算哪些成本?",
    a4: "For \"除了房貸還要算哪些成本?\": Home Affordability Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳嗎?",
    a5: "For \"資料會上傳嗎?\": Home Affordability Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 可以含房屋稅與保險嗎?",
    a6: "For \"可以含房屋稅與保險嗎?\": Home Affordability Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
...
export default function HomeAffordabilityCalculator() {
  ...
  const [monthlyDebt, setMonthlyDebt] = useState("10000");
  const t = ui[lang];
  ...
  }, [annualIncome, downPayment, mortgageRatePct, monthlyDebt]);

  const primaryDisplay = fmt(result.priceToIncome, 1);
  const secondaryDisplay = fmt(result.maxHomePrice, 0);
  const tertiaryDisplay = fmt(result.maxPayment, 2);
  const quaternaryDisplay = fmt(result.maxLoan, 0);

  function fillSolid() { setUnit("metric"); setAnnualIncome("1200000"); setDownPayment("2000000"); setMortgageRatePct("4"); setMonthlyDebt("10000"); }
  function fillHighSalary() { setUnit("imperial"); setAnnualIncome("2400000"); setDownPayment("5000000"); setMortgageRatePct("3.5"); setMonthlyDebt("5000"); }

  const activeBand = bands.find(b => {
    const r = result.priceToIncome;
    if (r < 3) return 'tiny';
    if (r < 5) return 'normal';
    if (r < 7) return 'notable';
    if (r < 9) return 'high';
    if (r < 12) return 'major';
    return 'executive';
  });
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#f5f5f4,_#fafaf9_45%,_#e7e5e4)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            <section className="space-y-6"><p className="text-sm font-black uppercase tracking-[0.24em] text-stone-700">{t.badge}</p><h1 className="max-w-3xl text-4xl font-black tracking-tight text-slate-950 md:text-6xl">{t.title}</h1><p className="text-xl font-black text-stone-700">{t.subtitle}</p><p className="max-w-2xl text-lg leading-8 text-slate-700">{t.intro}</p><div className="rounded-3xl border border-stone-200 bg-stone-50 p-5 text-sm leading-6 text-stone-950"><strong>{t.trustNoteLabel}</strong> {t.trustNote}</div></section>
            <aside className="rounded-[2rem] border border-stone-100 bg-white/90 p-6 shadow-2xl shadow-stone-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-stone-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-stone-600 p-5 text-white"><div className="text-xs font-bold uppercase text-stone-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}x</div><div className="text-sm font-bold text-stone-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}x</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{annualIncome} × {downPayment}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-stone-200 bg-stone-50 px-5 py-4 text-sm font-black text-stone-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-emerald-700">{t.annualIncome}<input type="number" step="50000" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={annualIncome} onChange={(e) => setAnnualIncome(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.downPayment}<input type="number" step="100000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={downPayment} onChange={(e) => setDownPayment(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.mortgageRatePct}<input type="number" step="0.1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={mortgageRatePct} onChange={(e) => setMortgageRatePct(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.monthlyDebt}<input type="number" step="1000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthlyDebt} onChange={(e) => setMonthlyDebt(e.target.value)} /></label></div></div>
          </div>
        </section>
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-stone-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-stone-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{primaryDisplay}<span className="text-3xl">x</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.primaryValue}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.secondaryLabel}</div><div className="mt-1 text-xl font-black">{secondaryDisplay}</div><div className="mt-1 text-xs text-slate-300">{t.secondaryTail}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.metricALabel}</div><div className="mt-1 text-xs font-black text-emerald-700">{t.metricACaption}</div><p className="mt-2 text-3xl font-black text-emerald-950">{tertiaryDisplay}</p><p className="text-sm font-bold text-emerald-700">{t.metricATail}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.metricBLabel}</div><div className="mt-1 text-xs font-black text-blue-700">{t.metricBCaption}</div><p className="mt-2 text-3xl font-black text-blue-950">{quaternaryDisplay}</p><p className="text-sm font-bold text-blue-700">{t.metricBTail}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.metricCLabel}</div><div className="mt-1 text-xs font-black text-slate-700">{t.metricCCaption}</div><p className="mt-2 text-3xl font-black text-slate-950">{secondaryDisplay}</p><p className="text-sm font-bold text-slate-700">{t.metricCTail}</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="home-affordability-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-stone-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black">{primaryDisplay}x</div></div><div className="rounded-2xl bg-stone-50 p-4"><div className="text-xs font-black uppercase text-stone-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-stone-950">{secondaryDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.tertiaryTag}</div><div className="mt-1 text-3xl font-black text-emerald-950">{tertiaryDisplay}</div></div></div></article>
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="home-affordability-calculator-faq" position="inline" /></section>
