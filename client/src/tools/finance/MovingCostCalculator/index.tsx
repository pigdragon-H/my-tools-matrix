// @profile B
// Profile B · 計算機-YMYL · Moving Cost Calculator - estimate a household move from volume, distance and crew（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<15000", label: { zh: "小型搬家", en: "Small move" }, desc: { zh: "少量物品近距離搬運,費用相對輕鬆。", en: "A few items over a short distance, with a light cost." } },
  { key: "normal", range: "15000-30000", label: { zh: "一般搬家", en: "Standard move" }, desc: { zh: "常見的一房至兩房搬遷規模。", en: "A typical one to two bedroom move." } },
  { key: "notable", range: "30000-50000", label: { zh: "中型搬家", en: "Mid-size move" }, desc: { zh: "物品較多或距離較遠,建議多比價。", en: "More items or a longer distance; compare a few quotes." } },
  { key: "high", range: "50000-80000", label: { zh: "大型搬家", en: "Large move" }, desc: { zh: "完整家庭搬遷,人力與時間需求增加。", en: "A full household move with more crew and time needed." } },
  { key: "major", range: "80000-120000", label: { zh: "高額搬家", en: "High-cost move" }, desc: { zh: "長距離或大坪數,建議確認保險與條款。", en: "A long-distance or large-home move; confirm insurance and terms." } },
  { key: "executive", range: "120000+", label: { zh: "豪華搬遷", en: "Premium move" }, desc: { zh: "大型或精緻搬遷,費用高,務必書面報價。", en: "A large or premium move with high cost; insist on a written quote." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "搬家費用計算機",
    subtitle: "依物品體積、距離與人力,估算一次搬家的總費用。",
    intro: "輸入物品體積、搬運距離、人力人數與樓層雜費,本工具結合體積費、運距費與人力費估算搬家總費用,並提供每立方公尺單價,協助你比較搬家公司報價。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立搬家成本範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    flowDemo: "即時試算",
    calculator: "計算機",
    participants: "物品體積 (立方公尺)",
    averageHourlyRate: "搬運距離 (公里)",
    durationHours: "搬運人力 (人)",
    meetingsPerMonth: "樓層/雜費",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Estimate the total cost of a move from volume, distance and crew size.",
    intro: "Enter the volume, distance, crew size and miscellaneous fees; the tool combines volume, distance and labour charges to estimate the total moving cost and a per-cubic-metre rate so you can compare movers' quotes.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a moving-cost example",
    ...
    participants: "Volume (cubic metres)",
    averageHourlyRate: "Distance (km)",
    durationHours: "Crew size (workers)",
    meetingsPerMonth: "Floor / misc fee",
    resultCard: "Result",
    unit: "Primary result",
...
export default function MovingCostCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("30");
  ...
  const result = useMemo(() => {
    ...
    const volumeCbm = v1; const distanceKm = v2; const crewCount = v3; const floorFee = v4;
    const volumeCost = volumeCbm * 600;
    const distanceCost = distanceKm * 35;
    const labourCost = crewCount * 2200;
    const subtotal = volumeCost + distanceCost + labourCost + floorFee;
    const total = subtotal * 1.05;
    const perCbm = volumeCbm > 0 ? total / volumeCbm : 0;
    return { total, subtotal, labourCost, perCbm, volumeCbm };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.total, 0);
  const monthlyDisplay = fmt(result.subtotal, 0);

  function fillSolid() { setUnit("metric"); setParticipants("20"); setAverageHourlyRate("30"); setDurationHours("3"); setMeetingsPerMonth("2000"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("45"); setAverageHourlyRate("120"); setDurationHours("5"); setMeetingsPerMonth("6000"); }

  const activeBand = bands.find(b => {
    const r = result.total;
    if (r < 15000) return b.key === "tiny";
    if (r < 30000) return b.key === "normal";
    if (r < 50000) return b.key === "notable";
    if (r < 80000) return b.key === "high";
    if (r < 120000) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fffbeb 0%,#fef3c7 55%,#ffedd5 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "搬家總費用" : "total moving cost"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">${fmt(result.perCbm, 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.labourCost, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">1 BR</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "小型搬家" : "Small move"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">3 BR</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "大型搬家" : "Large move"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "費用小計" : "Subtotal"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.subtotal, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "人力費用" : "Labour cost"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.labourCost, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "含稅總價" : "Total with tax"}</div><p className="mt-2 text-3xl font-black text-slate-950">${fmt(result.total, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="moving-cost-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "搬家總費用" : "total moving cost"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入體積與距離" : "Enter volume and distance", note: t.bmrStep }, { label: lang === "zh" ? "設定人力" : "Set the crew size", note: t.deficitStep }, { label: lang === "zh" ? "加總各項費用" : "Sum all cost items", note: t.trendStep }, { label: lang === "zh" ? "含稅得搬家總費用" : "Add tax for the total", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="moving-cost-calculator-faq" position="inline" /></section>
