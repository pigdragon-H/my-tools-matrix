...
const ui = {
  zh: {
    ...
    intro: "本工具為 匯率換算計算機，依公開公式於瀏覽器端試算，輸入原幣金額、原幣匯率(對 USD)、目標幣匯率(對 USD)、手續費 %後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    definition: "定義",
    definitionText: "本工具透過「原幣 → USD → 目標幣」的交叉換算邏輯,讓您即使沒有原幣對目標幣的直接報價,也能用兩邊對美元的匯率推算實際換匯金額,並扣除指定手續費比率。",
    formula: "公式",
    ...
    interpretation: "解讀",
    interpretationText: "「有效匯率」與「中間匯率」的差距即您支付的全部成本(匯差 + 手續費),長期換匯應以此為比較基準,而非單看手續費 %。",
    context: "情境",
    ...
    a1: "中間匯率是「Google/Yahoo Finance 顯示的實時市價」,是買價與賣價的中點;銀行牌告會把「賣出/買入」拉開 0.5-2.5% 形成匯差(spread),這就是銀行的隱藏利潤。本工具讓您輸入兩邊對 USD 的匯率,自己交叉計算,可避免被牌告價的隱藏匯差混淆。",
    ...
    a4: "依金額與走向不同:**Wise(原 TransferWise)** 適合 100-50000 USD 的個人匯款,匯差通常 0.4-0.8%、固定費低;**Revolut** 假日小額免費,但匯差只在工作日以中間價計算;**Western Union** 急件現金到現金最快,但匯差高達 2-5%。本工具不推薦特定服務,輸入您查到的匯率與費率即可比較。",
    q5: "結果會上傳到伺服器嗎?",
    a5: "完全不會。所有換算都在您的瀏覽器內以 JavaScript 完成,輸入的金額、匯率、手續費不會傳送到任何伺服器。本工具不抓即時匯率,所以您必須手動填入兩邊對 USD 的匯率,這也是隱私保護的設計。",
    q6: "為什麼線上看到的匯率,實際換到的金額不一樣?",
    a6: "三個原因疊加:**(1) 中間匯率 vs 銀行牌告匯差** 0.5-2%、**(2) 隱藏手續費或最低收費門檻**、**(3) 結算時點價差** —— 您看到的匯率是現在,實際清算可能在 1-3 工作日後。本工具把這些拆開讓您看清楚,輸入「實際給的匯率」與「實收手續費 %」即可逼近真實結果。"
  },
  en: {
