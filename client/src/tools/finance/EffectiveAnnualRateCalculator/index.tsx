...
const ui = {
  zh: {
    ...
    intro: "本工具為 有效年利率計算機，依公開公式於瀏覽器端試算，輸入名目年利率%、每年複利次數、本金、年數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q3: "信用卡實際利率是多少?",
    a3: "信用卡常標示「月息」或「日息」誤導消費者。以日息 0.04%(年化看似 14.6%)為例,因每日複利,EAR 可能達 15-16%;若標 APR 18% 按日複利,EAR 接近 19.7%。本工具幫你算出真實的 EAR。",
    q4: "怎麼用 EAR 比較不同貸款?",
    a4: "把不同貸款的名目利率與複利次數都換算成 EAR,再比較。EAR 把複利頻率統一,是唯一能公平比較的指標。月複利 12% 比季複利 12% 實際更貴,只看名目會誤判。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內完成,利率與本金資料不會傳送到任何伺服器。",
    q6: "可以連續複利嗎?",
  ...
  en: {
    ...
    q1: "About: EAR 和 APR 差在哪?",
    a1: "For \"EAR 和 APR 差在哪?\": Effective Annual Rate Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 為什麼複利越頻繁 EAR 越高?",
    a2: "For \"為什麼複利越頻繁 EAR 越高?\": Effective Annual Rate Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 信用卡實際利率是多少?",
    a3: "For \"信用卡實際利率是多少?\": Effective Annual Rate Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 怎麼用 EAR 比較不同貸款?",
    a4: "For \"怎麼用 EAR 比較不同貸款?\": Effective Annual Rate Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳嗎?",
    a5: "For \"資料會上傳嗎?\": Effective Annual Rate Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 可以連續複利嗎?",
    a6: "For \"可以連續複利嗎?\": Effective Annual Rate Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#fae8ff,_#f8fafc_45%,_#fce7f3)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-fuchsia-100 bg-white/90 p-6 shadow-2xl shadow-fuchsia-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-fuchsia-600 p-5 text-white"><div className="text-xs font-bold uppercase text-fuchsia-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}%</div><div className="text-sm font-bold text-fuchsia-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{nominalRatePct} × {compoundsPerYear}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-fuchsia-200 bg-fuchsia-50 px-5 py-4 text-sm font-black text-fuchsia-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
