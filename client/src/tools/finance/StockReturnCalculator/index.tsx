// @profile B
// Profile B · 計算機-YMYL · Stock Return Calculator - total return including dividends and CAGR（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<0%", label: { zh: "負報酬", en: "Negative" }, desc: { zh: "年化報酬為負,股價含配息仍低於買進成本。", en: "A negative annualized return; price plus dividends trails your cost." } },
  { key: "normal", range: "0-5%", label: { zh: "溫和報酬", en: "Moderate" }, desc: { zh: "報酬溫和,接近保守型資產表現。", en: "A moderate return, similar to conservative assets." } },
  { key: "notable", range: "5-10%", label: { zh: "良好報酬", en: "Good" }, desc: { zh: "接近長期股市平均,表現穩健。", en: "Near the long-run equity average; a steady result." } },
  { key: "high", range: "10-18%", label: { zh: "優異報酬", en: "Strong" }, desc: { zh: "年化報酬優異,顯示選股或時機良好。", en: "A strong annualized return, reflecting good picks or timing." } },
  { key: "major", range: "18-30%", label: { zh: "卓越報酬", en: "Excellent" }, desc: { zh: "報酬卓越,通常伴隨較高波動。", en: "An excellent return, usually with higher volatility." } },
  { key: "executive", range: "30%+", label: { zh: "極高報酬", en: "Exceptional" }, desc: { zh: "極高年化報酬,難以長期維持,留意風險。", en: "An exceptional return that is hard to sustain; mind the risk." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "股票報酬計算機",
    subtitle: "結合股價變化與配息,計算總報酬率與年化報酬 (CAGR)。",
    intro: "輸入買進股價、目前股價、累計每股配息與持有年數,本工具計算含息的總報酬率、年化報酬率 (CAGR) 與配息殖利率,協助你完整評估一檔股票的長期績效。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立股票報酬範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "買進股價",
    averageHourlyRate: "目前股價",
    durationHours: "累計每股配息",
  ...
  en: {
    ...
    subtitle: "Combine price change and dividends to compute total return and CAGR.",
    intro: "Enter the buy price, current price, total dividends per share and holding period; the tool computes the total return including dividends, the annualized CAGR and the dividend yield so you can fully assess a stock's long-term performance.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a stock-return example",
    ...
    participants: "Buy price",
    averageHourlyRate: "Current price",
    durationHours: "Total dividend per share",
    meetingsPerMonth: "Holding years",
    resultCard: "Result",
    unit: "Primary result",
...
export default function StockReturnCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("72");
  ...
  const result = useMemo(() => {
    ...
    const buyPrice = v1; const currentPrice = v2; const dividendPerShare = v3; const holdingYears = v4;
    const capitalGain = currentPrice - buyPrice;
    const totalGain = capitalGain + dividendPerShare;
    const totalReturn = buyPrice > 0 ? (totalGain / buyPrice) * 100 : 0;
    const years = holdingYears > 0 ? holdingYears : 1;
    const cagr = buyPrice > 0 ? (Math.pow((currentPrice + dividendPerShare) / buyPrice, 1 / years) - 1) * 100 : 0;
    const divYield = buyPrice > 0 ? (dividendPerShare / buyPrice) * 100 : 0;
    return { totalReturn, cagr, divYield, totalGain, buyPrice };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.totalReturn, 2);
  const monthlyDisplay = fmt(result.totalGain, 2);

  function fillSolid() { setUnit("metric"); setParticipants("50"); setAverageHourlyRate("72"); setDurationHours("6"); setMeetingsPerMonth("4"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("50"); setAverageHourlyRate("110"); setDurationHours("8"); setMeetingsPerMonth("4"); }

  const activeBand = bands.find(b => {
    const r = result.cagr;
    if (r < 0) return b.key === "tiny";
    if (r < 5) return b.key === "normal";
    if (r < 10) return b.key === "notable";
    if (r < 18) return b.key === "high";
    if (r < 30) return b.key === "major";
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#eff6ff 0%,#dbeafe 55%,#ede9fe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "總報酬率" : "total return"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.cagr, 2)}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.divYield, 2)}%</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">7%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "市場平均" : "Market average"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">12%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "成長股" : "Growth stock"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? "%" : "%"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元/股" : "/share"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "每股總獲利" : "Total gain per share"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.totalGain, 2)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "年化報酬 (CAGR)" : "CAGR"}</div><p className="mt-2 text-3xl font-black text-blue-950">{fmt(result.cagr, 2)}</p><p className="text-sm font-bold text-blue-700">%</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "配息殖利率" : "Dividend yield"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.divYield, 2)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="stock-return-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "總報酬率" : "total return"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入買進與目前股價" : "Enter buy and current price", note: t.bmrStep }, { label: lang === "zh" ? "加入累計配息" : "Add total dividends", note: t.deficitStep }, { label: lang === "zh" ? "計算總報酬" : "Compute total return", note: t.trendStep }, { label: lang === "zh" ? "年化得 CAGR" : "Annualize to CAGR", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="stock-return-calculator-faq" position="inline" /></section>
