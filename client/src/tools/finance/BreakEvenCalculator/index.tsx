...
const bands = [
  { key: "tiny", range: "< 100", label: { zh: "極輕鬆 (< 100)", en: "Very low (< 100)" }, desc: { zh: "落在「極輕鬆」級距< 100。不到 100 件即可損益兩平,毛利結構非常健康,可考慮降價搶市佔。", en: "Falls in the \"Very low\" band (< 100). This is the very low range for Break-Even Calculator." } },
  { key: "normal", range: "100–500", label: { zh: "輕鬆 (100–500)", en: "Low (100–500)" }, desc: { zh: "落在「輕鬆」級距100–500。100-500 件達標,屬常見小型零售或 SaaS 月訂閱規模,行銷預算彈性大。", en: "Falls in the \"Low\" band (100–500). This is the low range for Break-Even Calculator." } },
  { key: "notable", range: "500–2000", label: { zh: "可達 (500–2000)", en: "Moderate (500–2000)" }, desc: { zh: "落在「可達」級距500–2000。500-2000 件,需穩定銷售管道,建議搭配自動化行銷與訂閱模型。", en: "Falls in the \"Moderate\" band (500–2000). This is the moderate range for Break-Even Calculator." } },
  { key: "high", range: "2000–5000", label: { zh: "吃力 (2000–5000)", en: "High (2000–5000)" }, desc: { zh: "落在「吃力」級距2000–5000。2000-5000 件門檻偏高,要重新檢視定價、變動成本或提高客單價。", en: "Falls in the \"High\" band (2000–5000). This is the high range for Break-Even Calculator." } },
  { key: "major", range: "5000–10000", label: { zh: "困難 (5000–10000)", en: "Very high (5000–10000)" }, desc: { zh: "落在「困難」級距5000–10000。5000-10000 件,單靠線下難達標,需要規模化通路或數位轉型。", en: "Falls in the \"Very high\" band (5000–10000). This is the very high range for Break-Even Calculator." } },
  { key: "executive", range: "≥ 10000", label: { zh: "不可行 (≥ 10000)", en: "Extreme (≥ 10000)" }, desc: { zh: "落在「不可行」級距≥ 10000。超過 10000 件才回本,商業模式可能需要重新設計,或固定成本過高。", en: "Falls in the \"Extreme\" band (≥ 10000). This is the extreme range for Break-Even Calculator." } },
] as const;
...
const ui = {
  zh: {
    ...
    intro: "本工具為 損益兩平計算機，依公開公式於瀏覽器端試算，輸入固定成本、單位售價、單位變動成本、目標利潤後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    premiumTitle: "專業版 損益兩平與定價沙盤",
    premiumText: "解鎖多產品加權損益兩平、安全邊際分析、價格敏感度矩陣、目標利潤反推與營運槓桿報告。",
    premiumChips_zh: "多產品加權|安全邊際|敏感度矩陣|槓桿報告",
    premiumChips_en: "Weighted BE|Safety Margin|Sensitivity|Leverage",
    ...
    a1: "貢獻邊際 = 單位售價 − 單位變動成本,代表每多賣一件能拿來「貢獻」覆蓋固定成本與利潤的金額。比毛利率更實用的原因:它能直接告訴您「再多賣 1 件,我多賺多少」,這是定價與促銷決策的核心數字;毛利率則混合了固定成本攤提,在決策邊際時容易誤判。",
    q2: "固定成本要包含老闆薪水嗎?",
    a2: "建議分兩層算:第一層只放「真正不會因銷量增減的支出」(房租、保險、軟體訂閱、會計師),老闆薪水若您能停發就放變動;第二層做「全成本 break-even」再把老闆與股東的合理報酬加進固定成本。第一層告訴您「不關門的最低門檻」,第二層告訴您「值得做這門生意的門檻」。",
    q3: "為什麼有時算出損益兩平銷量是無限大?",
    a3: "當「單位售價 ≤ 單位變動成本」(即貢獻邊際 ≤ 0),每多賣一件都在虧錢,固定成本永遠攤不平,數學上 break-even 銷量為無限大。實務上代表您必須先檢視:是否定價太低?或變動成本(原料、運費、刷卡手續費)是否被低估?降變動成本或調漲售價是唯一出路。",
    ...
    q5: "結果會上傳到伺服器嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內以 JavaScript 完成,固定成本、售價、變動成本等敏感營業資料不會傳送到任何伺服器,也不會記錄到日誌或資料庫。關閉分頁後資料就消失。",
  ...
  en: {
    ...
    deficitNote: "Break-Even Calculator standard formula.",
    ...
    formula: "Formula",
    formulaText: "result = f(fixed costs, price per unit, variable cost per unit, target profit)",
    ...
    premiumTitle: "Pro Break-Even & Pricing Sandbox",
    premiumText: "Unlock multi-product weighted break-even, margin-of-safety analysis, price-sensitivity matrix, target-profit reverse solving, and operating-leverage reports.",
    premiumChips_zh: "多產品加權|安全邊際|敏感度矩陣|槓桿報告",
    premiumChips_en: "Weighted BE|Safety Margin|Sensitivity|Leverage",
    ...
    q1: "What does Break-Even Calculator calculate?",
    a1: "Break-Even Calculator applies the standard formula to your inputs and returns Break-even Units plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Break-Even Calculator?",
    a2: "Enter fixed costs, price per unit, variable cost per unit, target profit. Break-Even Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock multi-product weighted break-even, margin-of-safety analysis, price-sensitivity matrix, target-profit reverse solving, and operating-leverage reports."
  },
} as const;
