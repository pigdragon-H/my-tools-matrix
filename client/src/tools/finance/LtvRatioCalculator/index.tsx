// @profile B
// Profile B · 計算機-YMYL · LtvRatioCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 40", label: { zh: "極低風險 (< 40)", en: "Very low risk (< 40)" }, desc: { zh: "落在「極低風險」級距< 40。LTV 極低,權益充足,通常可獲最優利率且免 PMI。", en: "Falls in the \"Very low risk\" band (< 40). Very low LTV with strong equity — typically best rates and no PMI." } },
  { key: "normal", range: "40–60", label: { zh: "低風險 (40–60)", en: "Low risk (40–60)" }, desc: { zh: "落在「低風險」級距40–60。LTV 偏低,風險小,貸款條件良好。", en: "Falls in the \"Low risk\" band (40–60). Low LTV, small risk, favorable loan terms." } },
  { key: "notable", range: "60–80", label: { zh: "標準 (60–80)", en: "Standard (60–80)" }, desc: { zh: "落在「標準」級距60–80。LTV 處於標準區間,多數放款機構可接受。", en: "Falls in the \"Standard\" band (60–80). LTV in the standard range, accepted by most lenders." } },
  { key: "high", range: "80–90", label: { zh: "偏高 (80–90)", en: "Elevated (80–90)" }, desc: { zh: "落在「偏高」級距80–90。LTV 偏高,可能需支付 PMI 或面臨較高利率。", en: "Falls in the \"Elevated\" band (80–90). Elevated LTV; PMI or higher rates may apply." } },
  { key: "major", range: "90–95", label: { zh: "高風險 (90–95)", en: "High risk (90–95)" }, desc: { zh: "落在「高風險」級距90–95。LTV 高,放款風險上升,核貸條件趨嚴。", en: "Falls in the \"High risk\" band (90–95). High LTV; lending risk rises and terms tighten." } },
  { key: "executive", range: "≥ 95", label: { zh: "極高風險 (≥ 95)", en: "Very high risk (≥ 95)" }, desc: { zh: "落在「極高風險」級距≥ 95。LTV 極高,權益薄弱,房價下跌易陷入負資產。", en: "Falls in the \"Very high risk\" band (≥ 95). Very high LTV with thin equity; a price drop can cause negative equity." } },
] as const;
...
const ui = {
  zh: {
    badge: "財務 · 貸款成數 LTV 計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "LTV Ratio Calculator · 貸款成數 LTV 計算機",
    subtitle: "計算貸款成數、自有權益與距目標 LTV 的可貸空間。",
    intro: "本工具為 貸款成數 LTV 計算機，依公開公式於瀏覽器端試算，輸入貸款金額、房產估值、目前已還本金、可接受最高 LTV後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算貸款成數 LTV 計算機",
    examplePreview: "目前 LTV 貸款成數",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入貸款金額、房產估值、目前已還本金、可接受最高 LTV",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "80% 標準情境",
    baselineExampleNote: "貸款金額 320000 · 房產估值 400000",
    activeExample: "進階範例",
    activeExampleValue: "高成數情境",
    activeExampleNote: "貸款金額 加倍 · 觀察 目前 LTV 貸款成數 變化",
    flowDemo: "數字流向示範",
    calculator: "貸款成數 LTV 計算機",
    loanAmount: "貸款金額",
    propertyAppraisedValue: "房產估值",
    principalAlreadyPaid: "目前已還本金",
    targetMaximumLtv: "可接受最高 LTV",
    resultCard: "結果卡片",
    primaryValue: "目前 LTV 貸款成數",
    primaryUnitTail: "%",
    secondaryLabel: "自有權益比例",
    secondaryTail: "%",
    metricALabel: "目前 LTV 貸款成數",
    ...
    metricCTail: "$",
    headlineCaption: "貸款成數 LTV 計算機 · 即時試算",
    fatLossTarget: "距目標 LTV 可貸空間",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "貸款成數 LTV 計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 貸款金額 與 目前已還本金 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "貸款成數 LTV 計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 貸款金額、房產估值、目前已還本金、可接受最高 LTV 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "Loan-to-value ratio。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "工具知識庫",
    knowledgeTitle: "貸款成數 LTV 計算機 · 觀念整理",
    definition: "定義",
    definitionText: "貸款成數 LTV 計算機以貸款餘額除以房產估值,計算貸款成數、自有權益與距目標 LTV 的可貸空間。",
    formula: "公式",
    formulaText: "LTV = (貸款金額 − 已還本金) ÷ 房產估值 × 100%;自有權益 = 估值 − 貸款餘額;可貸空間 = 估值 × 目標LTV − 餘額。",
    limitations: "限制",
    limitationsText: "本工具以您輸入的估值與餘額計算,未計入二胎、HELOC、放款機構鑑價差異與保險費用,實際核貸以機構評估為準。",
    interpretation: "解讀",
    interpretationText: "LTV 越低代表權益越充足、放款風險越小;可貸空間為正代表仍有增貸額度,為負代表已超過目標成數。",
    context: "情境",
    contextText: "常見使用情境包括日常財務檢視、年度規劃、重大決策前的快速估算。建議搭配房屋淨值計算機 等延伸工具一起使用。",
    example: "範例",
    ...
    premiumTitle: "LTV Pro 進階",
    premiumText: "進階版加入 PMI 取消門檻追蹤、CLTV(含二胎/HELOC)、再融資情境與分期還本對 LTV 的影響模擬。",
    premiumChips_zh: "PMI門檻|CLTV合併|再融資|還本模擬",
    premiumChips_en: "PMI threshold|CLTV|Refinance|Amortization",
    ...
    q1: "LTV 貸款成數是什麼?",
    a1: "LTV(Loan-to-Value)= 貸款餘額 ÷ 房產估值,反映貸款相對於資產價值的比例,是放款風險的核心指標。",
    q2: "為什麼 80% 是常見門檻?",
    a2: "LTV 超過 80% 時,多數機構要求房貸保險(PMI),因此 80% 常被視為免 PMI 與較優利率的分界。",
    q3: "已還本金如何影響 LTV?",
    a3: "已還本金會降低貸款餘額,進而降低 LTV、提高自有權益,有助於取消 PMI 或再融資。",
    q4: "自有權益和 LTV 有何關係?",
    a4: "自有權益 = 估值 − 貸款餘額,與 LTV 互補;LTV 越低,權益比例越高。",
    q5: "可貸空間怎麼解讀?",
    a5: "可貸空間 = 目標 LTV 下的最高貸款額 − 目前餘額,正值代表仍可增貸,負值代表已超過目標。",
    q6: "估值用市價還是鑑價?",
    a6: "放款機構通常以正式鑑價為準;本工具以您輸入的估值計算,建議填寫保守的鑑價金額。"
  },
  en: {
    badge: "Finance · LTV Ratio Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "LTV Ratio Calculator",
    subtitle: "Compute your loan-to-value ratio, home equity, and room to a target LTV.",
    intro: "LTV Ratio Calculator runs the standard formula in your browser. Enter loan amount, property appraised value, principal already paid, target maximum ltv to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try LTV Ratio Calculator",
    examplePreview: "Current LTV ratio",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter loan amount, property appraised value, principal already paid, target maximum ltv",
    ...
    baselineExampleValue: "Standard 80% case",
    baselineExampleNote: "Loan amount 320000 · Property appraised value 400000",
    activeExample: "Advanced example",
    activeExampleValue: "High-LTV case",
    activeExampleNote: "Loan amount doubled · watch Current LTV ratio react",
    flowDemo: "Data flow demo",
    calculator: "LTV Ratio Calculator",
    loanAmount: "Loan amount",
    propertyAppraisedValue: "Property appraised value",
    principalAlreadyPaid: "Principal already paid",
    targetMaximumLtv: "Target maximum LTV",
    resultCard: "Result card",
    primaryValue: "Current LTV ratio",
    primaryUnitTail: "%",
    secondaryLabel: "Equity percentage",
    secondaryTail: "%",
    metricALabel: "Current LTV ratio",
    metricACaption: "Main figure from the standard formula",
    metricATail: "%",
    metricBLabel: "Equity percentage",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "%",
    metricCLabel: "Home equity amount",
    metricCCaption: "Percentage view",
    metricCTail: "$",
    headlineCaption: "LTV Ratio Calculator · live calc",
    fatLossTarget: "Room to target LTV",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "LTV Ratio Calculator · band matrix",
    ...
    dailyGap: "Daily gap",
    tertiaryTag: "Home equity amount",
    ...
    nextActionItem1: "Move Loan amount and Principal already paid by ±10% to see sensitivity.",
    ...
    decisionTitle: "LTV Ratio Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill loan amount, property appraised value, principal already paid, target maximum ltv.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "LTV Ratio Calculator standard formula.",
    ...
    knowledgeTitle: "LTV Ratio Calculator · concept primer",
    definition: "Definition",
    definitionText: "LTV Ratio Calculator converts inputs (loan amount, property appraised value, principal already paid, target maximum ltv) into Current LTV ratio. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(loan amount, property appraised value, principal already paid, target maximum ltv)",
    ...
    context: "Context",
    contextText: "Common contexts include daily finance review, annual planning, and pre-decision quick estimates. Pair with Home Equity Calculator for a fuller picture.",
    example: "Example",
    ...
    premiumTitle: "LTV Pro",
    premiumText: "Pro adds PMI removal tracking, CLTV with second liens/HELOC, refinance scenarios, and amortization impact on LTV.",
    premiumChips_zh: "PMI門檻|CLTV合併|再融資|還本模擬",
    premiumChips_en: "PMI threshold|CLTV|Refinance|Amortization",
    ...
    q1: "What does LTV Ratio Calculator calculate?",
    a1: "LTV Ratio Calculator applies the standard formula to your inputs and returns Current LTV ratio plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for LTV Ratio Calculator?",
    a2: "Enter loan amount, property appraised value, principal already paid, target maximum ltv. LTV Ratio Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Pro adds PMI removal tracking, CLTV with second liens/HELOC, refinance scenarios, and amortization impact on LTV."
  },
} as const;

const faqKeys = [["q1","a1"],["q2","a2"],["q3","a3"],["q4","a4"],["q5","a5"],["q6","a6"]] as const;

export default function LtvRatioCalculator() {
  const { lang, setLang } = useLanguage();
  const [unit, setUnit] = useState<"metric" | "imperial">("metric");
  const [loanAmount, setLoanAmount] = useState("320000");
  const [propertyAppraisedValue, setPropertyAppraisedValue] = useState("400000");
  const [principalAlreadyPaid, setPrincipalAlreadyPaid] = useState("0");
  const [targetMaximumLtv, setTargetMaximumLtv] = useState("80");
  const t = ui[lang];

  const result = useMemo(() => {
const loan = Number(loanAmount) || 0; const value = Number(propertyAppraisedValue) || 0; const paid = Number(principalAlreadyPaid) || 0; const targetLtv = (Number(targetMaximumLtv) || 0) / 100; const currentBalance = Math.max(0, loan - paid); const ltv = value > 0 ? (currentBalance / value) * 100 : 0; const equity = value - currentBalance; const equityPct = value > 0 ? (equity / value) * 100 : 0; const maxLoanAtTarget = value * targetLtv; const roomToTarget = maxLoanAtTarget - currentBalance; return { primaryKey: ltv, secondaryKey: equityPct, tertiaryKey: equity, quaternaryKey: roomToTarget };
  }, [loanAmount, propertyAppraisedValue, principalAlreadyPaid, targetMaximumLtv]);
  ...
  function fillSolid() { setUnit("metric"); setLoanAmount("320000"); setPropertyAppraisedValue("400000"); setPrincipalAlreadyPaid("0"); setTargetMaximumLtv("80"); }
  function fillHighSalary() { setUnit("imperial"); setLoanAmount("380000"); setPropertyAppraisedValue("400000"); setPrincipalAlreadyPaid("0"); setTargetMaximumLtv("80"); }
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#cffafe,_#f8fafc_45%,_#dbeafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-cyan-100 bg-white/90 p-6 shadow-2xl shadow-cyan-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-cyan-600 p-5 text-white"><div className="text-xs font-bold uppercase text-cyan-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-cyan-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{loanAmount} × {propertyAppraisedValue}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-cyan-200 bg-cyan-50 px-5 py-4 text-sm font-black text-cyan-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.loanAmount}<input type="number" step="5000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={loanAmount} onChange={(e) => setLoanAmount(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.propertyAppraisedValue}<input type="number" step="5000" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={propertyAppraisedValue} onChange={(e) => setPropertyAppraisedValue(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.principalAlreadyPaid}<input type="number" step="1000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={principalAlreadyPaid} onChange={(e) => setPrincipalAlreadyPaid(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.targetMaximumLtv}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={targetMaximumLtv} onChange={(e) => setTargetMaximumLtv(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="ltv-ratio-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="ltv-ratio-calculator-faq" position="inline" /></section>
