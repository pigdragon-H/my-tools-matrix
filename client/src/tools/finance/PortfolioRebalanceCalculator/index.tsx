// @profile B
// Profile B · 計算機-YMYL · PortfolioRebalanceCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 0", label: { zh: "嚴重偏離 (< 0)", en: "Severe drift (< 0)" }, desc: { zh: "落在「嚴重偏離」級距< 0。股票比重與目標幾乎一致,無須再平衡,維持現狀即可。", en: "Falls in the \"Severe drift\" band (< 0). Stock weight is essentially on target; no rebalancing needed, keep the current mix." } },
  { key: "normal", range: "0–2", label: { zh: "明顯偏離 (0–2)", en: "Significant drift (0–2)" }, desc: { zh: "落在「明顯偏離」級距0–2。偏離幅度極小,仍在容許範圍內,可暫不交易以節省成本與稅負。", en: "Falls in the \"Significant drift\" band (0–2). Drift is minimal and within tolerance; you may skip trading to save costs and taxes." } },
  { key: "notable", range: "2–5", label: { zh: "中度偏離 (2–5)", en: "Moderate drift (2–5)" }, desc: { zh: "落在「中度偏離」級距2–5。偏離達中度,接近或略超過門檻,可考慮分批調整回目標配置。", en: "Falls in the \"Moderate drift\" band (2–5). Moderate drift near or slightly over the threshold; consider staged adjustments toward target." } },
  { key: "high", range: "5–10", label: { zh: "輕微偏離 (5–10)", en: "Minor drift (5–10)" }, desc: { zh: "落在「輕微偏離」級距5–10。偏離明顯超過門檻,建議再平衡將股票比重調回目標水準。", en: "Falls in the \"Minor drift\" band (5–10). Drift clearly exceeds the threshold; rebalancing back to the target weight is advised." } },
  { key: "major", range: "10–15", label: { zh: "接近目標 (10–15)", en: "Near target (10–15)" }, desc: { zh: "落在「接近目標」級距10–15。偏離幅度大,風險暴露已顯著偏離原規劃,應儘快再平衡。", en: "Falls in the \"Near target\" band (10–15). Large drift; risk exposure has deviated significantly from plan, rebalance soon." } },
  { key: "executive", range: "≥ 15", label: { zh: "完全對齊 (≥ 15)", en: "On target (≥ 15)" }, desc: { zh: "落在「完全對齊」級距≥ 15。嚴重偏離目標配置,風險結構與初衷落差過大,須立即檢視並再平衡。", en: "Falls in the \"On target\" band (≥ 15). Severe drift from target; the risk structure differs greatly from intent, rebalance immediately." } },
] as const;

const affiliateItems: AffiliateItem[] = [
  { label: { zh: "風險承受度計算機", en: "Risk Tolerance Calculator" }, href: "/tools/finance/risk-tolerance-calculator" },
...
const ui = {
  zh: {
    badge: "財務 · 投資組合再平衡計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Portfolio Rebalance Calculator · 投資組合再平衡計算機",
    subtitle: "由組合總值、股票市值與目標比重計算再平衡所需的交易金額。",
    intro: "本工具為 投資組合再平衡計算機，依公開公式於瀏覽器端試算，輸入投資組合總值、目前股票市值、目標股票比重(%)、再平衡偏離門檻(%)後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算投資組合再平衡計算機",
    examplePreview: "目前股票比重",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入投資組合總值、目前股票市值、目標股票比重(%)、再平衡偏離門檻(%)",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "股票偏高需減碼情境",
    baselineExampleNote: "投資組合總值 1000000 · 目前股票市值 700000",
    activeExample: "進階範例",
    activeExampleValue: "股票偏低需加碼情境",
    activeExampleNote: "投資組合總值 加倍 · 觀察 目前股票比重 變化",
    flowDemo: "數字流向示範",
    calculator: "投資組合再平衡計算機",
    totalPortfolioValue: "投資組合總值",
    currentStockValue: "目前股票市值",
    targetStockWeight: "目標股票比重(%)",
    rebalanceDriftThreshold: "再平衡偏離門檻(%)",
    resultCard: "結果卡片",
    primaryValue: "目前股票比重",
    primaryUnitTail: "%",
    secondaryLabel: "需調整股票金額",
    secondaryTail: "",
    metricALabel: "目前股票比重",
    metricACaption: "依公開公式試算的主要數值",
    metricATail: "%",
    metricBLabel: "需調整股票金額",
    metricBCaption: "與主要結果連動的次要量值",
    metricBTail: "",
    metricCLabel: "與目標偏離幅度",
    metricCCaption: "百分比形式的觀察點",
    metricCTail: "%",
    headlineCaption: "投資組合再平衡計算機 · 即時試算",
    fatLossTarget: "目標股票市值",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "投資組合再平衡計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 投資組合總值 與 目標股票比重(%) 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "投資組合再平衡計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 投資組合總值、目前股票市值、目標股票比重(%)、再平衡偏離門檻(%) 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "Allocation drift and rebalancing trade analysis。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "工具知識庫",
    knowledgeTitle: "投資組合再平衡計算機 · 觀念整理",
    definition: "定義",
    definitionText: "投資組合再平衡計算機以組合總值、目前股票市值與目標股票比重,計算目前比重、與目標的偏離幅度,以及調回目標所需的買賣金額。",
    formula: "公式",
    formulaText: "目前股票比重 =目前股票市值 ÷ 組合總值;需調整股票金額 = 組合總值 × 目標比重 − 目前股票市值。",
    limitations: "限制",
    limitationsText: "本工具採股票/非股票兩分法的簡化模型,未計入多資產類別、交易成本、買賣價差與資本利得稅,僅供概念性估算。",
    interpretation: "解讀",
    interpretationText: "目前比重與目標偏離越大,代表風險暴露偏離原規劃越多;當偏離超過門檻時,建議依需調整金額再平衡回目標。",
    context: "情境",
    contextText: "常見使用情境包括日常財務檢視、年度規劃、重大決策前的快速估算。建議搭配風險承受度計算機 等延伸工具一起使用。",
    example: "範例",
    ...
    premiumTitle: "Rebalance Pro 進階",
    premiumText: "進階版加入多資產類別配置、稅務最佳化再平衡、現金流注入調整與再平衡頻率回測。",
    premiumChips_zh: "多資產配置|稅務最佳化|現金流調整|頻率回測",
    premiumChips_en: "Multi-asset|Tax-optimized|Cash-flow|Frequency backtest",
    ...
    q1: "什麼是投資組合再平衡?",
    a1: "再平衡是定期或在偏離超標時,將投資組合的資產比重調回原訂目標,以維持預期的風險與報酬結構,避免單一資產過度集中。",
    q2: "這個交易金額怎麼算?",
    a2: "需調整股票金額 = 目標股票市值 − 目前股票市值;正值代表需加碼股票,負值代表需減碼股票(轉入債券或現金)。",
    q3: "為什麼要設偏離門檻?",
    a3: "設定偏離門檻(如 ±5%)可避免頻繁交易帶來的成本與稅負,只有當偏離超過門檻時才再平衡,兼顧紀律與效率。",
    q4: "多久再平衡一次比較好?",
    a4: "常見做法是定期(如每年或每半年)再平衡,或採門檻觸發法;兩者皆可,重點是有紀律地執行而非追逐市場。",
    q5: "再平衡有什麼成本?",
    a5: "再平衡可能產生交易手續費、買賣價差與資本利得稅;因此設定合理門檻、善用新資金流入調整可降低成本。",
    q6: "這個結果準確嗎?",
    a6: "本工具以單一股票/非股票兩分法的簡化模型估算,未計入多資產類別、交易成本與稅負,僅供概念性參考。"
  },
  en: {
    badge: "Finance · Portfolio Rebalance Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Portfolio Rebalance Calculator",
    subtitle: "Calculate the trade amount needed to rebalance from total value, stock value and target weight.",
    intro: "Portfolio Rebalance Calculator runs the standard formula in your browser. Enter total portfolio value, current stock value, target stock weight, rebalance drift threshold to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Portfolio Rebalance Calculator",
    examplePreview: "Current stock weight",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter total portfolio value, current stock value, target stock weight, rebalance drift threshold",
    ...
    baselineExampleValue: "Overweight stock case",
    baselineExampleNote: "Total portfolio value 1000000 · Current stock value 700000",
    activeExample: "Advanced example",
    activeExampleValue: "Underweight stock case",
    activeExampleNote: "Total portfolio value doubled · watch Current stock weight react",
    flowDemo: "Data flow demo",
    calculator: "Portfolio Rebalance Calculator",
    totalPortfolioValue: "Total portfolio value",
    currentStockValue: "Current stock value",
    targetStockWeight: "Target stock weight",
    rebalanceDriftThreshold: "Rebalance drift threshold",
    resultCard: "Result card",
    primaryValue: "Current stock weight",
    primaryUnitTail: "%",
    secondaryLabel: "Stock trade amount",
    secondaryTail: "",
    metricALabel: "Current stock weight",
    metricACaption: "Main figure from the standard formula",
    metricATail: "%",
    metricBLabel: "Stock trade amount",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "",
    metricCLabel: "Drift from target",
    metricCCaption: "Percentage view",
    metricCTail: "%",
    headlineCaption: "Portfolio Rebalance Calculator · live calc",
    fatLossTarget: "Target stock value",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Portfolio Rebalance Calculator · band matrix",
    ...
    nextActionItem1: "Move Total portfolio value and Target stock weight by ±10% to see sensitivity.",
    ...
    decisionTitle: "Portfolio Rebalance Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill total portfolio value, current stock value, target stock weight, rebalance drift threshold.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "Portfolio Rebalance Calculator standard formula.",
    ...
    knowledgeTitle: "Portfolio Rebalance Calculator · concept primer",
    definition: "Definition",
    definitionText: "Portfolio Rebalance Calculator converts inputs (total portfolio value, current stock value, target stock weight, rebalance drift threshold) into Current stock weight. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(total portfolio value, current stock value, target stock weight, rebalance drift threshold)",
    ...
    context: "Context",
    contextText: "Common contexts include daily finance review, annual planning, and pre-decision quick estimates. Pair with Risk Tolerance Calculator for a fuller picture.",
    example: "Example",
    ...
    premiumTitle: "Rebalance Pro",
    premiumText: "Pro adds multi-asset allocation, tax-optimized rebalancing, cash-flow-based adjustment and rebalance-frequency backtesting.",
    premiumChips_zh: "多資產配置|稅務最佳化|現金流調整|頻率回測",
    premiumChips_en: "Multi-asset|Tax-optimized|Cash-flow|Frequency backtest",
    ...
    q1: "What does Portfolio Rebalance Calculator calculate?",
    a1: "Portfolio Rebalance Calculator applies the standard formula to your inputs and returns Current stock weight plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Portfolio Rebalance Calculator?",
    a2: "Enter total portfolio value, current stock value, target stock weight, rebalance drift threshold. Portfolio Rebalance Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Pro adds multi-asset allocation, tax-optimized rebalancing, cash-flow-based adjustment and rebalance-frequency backtesting."
  },
} as const;
...
export default function PortfolioRebalanceCalculator() {
  ...
  const [totalPortfolioValue, setTotalPortfolioValue] = useState("1000000");
  const [currentStockValue, setCurrentStockValue] = useState("700000");
  const [targetStockWeight, setTargetStockWeight] = useState("60");
  const [rebalanceDriftThreshold, setRebalanceDriftThreshold] = useState("5");
  const t = ui[lang];

  const result = useMemo(() => {
const total = Number(totalPortfolioValue) || 0; const stock = Number(currentStockValue) || 0; const targetWeight = Number(targetStockWeight) || 0; const threshold = Number(rebalanceDriftThreshold) || 0; const currentWeight = total > 0 ? stock / total * 100 : 0; const targetStockValue = total * targetWeight / 100; const tradeAmount = targetStockValue - stock; const drift = currentWeight - targetWeight; return { primaryKey: currentWeight, secondaryKey: tradeAmount, tertiaryKey: drift, quaternaryKey: targetStockValue };
  }, [totalPortfolioValue, currentStockValue, targetStockWeight, rebalanceDriftThreshold]);
  ...
  function fillSolid() { setUnit("metric"); setTotalPortfolioValue("1000000"); setCurrentStockValue("700000"); setTargetStockWeight("60"); setRebalanceDriftThreshold("5"); }
  function fillHighSalary() { setUnit("imperial"); setTotalPortfolioValue("2000000"); setCurrentStockValue("800000"); setTargetStockWeight("60"); setRebalanceDriftThreshold("5"); }
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#fae8ff,_#f8fafc_45%,_#fce7f3)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-fuchsia-100 bg-white/90 p-6 shadow-2xl shadow-fuchsia-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-fuchsia-600 p-5 text-white"><div className="text-xs font-bold uppercase text-fuchsia-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-fuchsia-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{totalPortfolioValue} × {currentStockValue}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-fuchsia-200 bg-fuchsia-50 px-5 py-4 text-sm font-black text-fuchsia-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.totalPortfolioValue}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={totalPortfolioValue} onChange={(e) => setTotalPortfolioValue(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.currentStockValue}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={currentStockValue} onChange={(e) => setCurrentStockValue(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.targetStockWeight}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={targetStockWeight} onChange={(e) => setTargetStockWeight(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.rebalanceDriftThreshold}<input type="number" step="0.5" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={rebalanceDriftThreshold} onChange={(e) => setRebalanceDriftThreshold(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="portfolio-rebalance-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="portfolio-rebalance-calculator-faq" position="inline" /></section>
