...
const ui = {
  zh: {
    ...
    intro: "本工具為 簡單利息計算機，依公開公式於瀏覽器端試算，輸入本金、年利率%、年數、額外提存後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內完成,本金與利率資料不會傳送到任何伺服器。",
    q6: "可以加上定期提存嗎?",
  ...
  en: {
    ...
    q1: "About: 單利和複利差多少?",
    a1: "For \"單利和複利差多少?\": Simple Interest Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 什麼情況用單利?",
    a2: "For \"什麼情況用單利?\": Simple Interest Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 短期借貸算單利划算嗎?",
    a3: "For \"短期借貸算單利划算嗎?\": Simple Interest Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 怎麼從單利反推年利率?",
    a4: "For \"怎麼從單利反推年利率?\": Simple Interest Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳嗎?",
    a5: "For \"資料會上傳嗎?\": Simple Interest Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 可以加上定期提存嗎?",
    a6: "For \"可以加上定期提存嗎?\": Simple Interest Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#e0e7ff,_#f8fafc_45%,_#dbeafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-indigo-100 bg-white/90 p-6 shadow-2xl shadow-indigo-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-indigo-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-indigo-600 p-5 text-white"><div className="text-xs font-bold uppercase text-indigo-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}%</div><div className="text-sm font-bold text-indigo-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{principal} × {annualRatePct}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-indigo-200 bg-indigo-50 px-5 py-4 text-sm font-black text-indigo-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
