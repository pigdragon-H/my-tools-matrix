// @profile B
// Profile B · 計算機-YMYL · EbitdaCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 5", label: { zh: "極低 (< 5)", en: "Very low (< 5)" }, desc: { zh: "落在「極低」級距< 5。EBITDA 利潤率 < 5%,極薄,常見於低毛利通路或競爭激烈產業。", en: "Falls in the \"Very low\" band (< 5). This is the very low range for EBITDA Calculator." } },
  { key: "normal", range: "5–12", label: { zh: "低 (5–12)", en: "Low (5–12)" }, desc: { zh: "落在「低」級距5–12。5-12%,低,屬製造、零售等資本密集產業常見水準。", en: "Falls in the \"Low\" band (5–12). This is the low range for EBITDA Calculator." } },
  { key: "notable", range: "12–20", label: { zh: "中等 (12–20)", en: "Moderate (12–20)" }, desc: { zh: "落在「中等」級距12–20。12-20%,中等,健康營運的一般區間。", en: "Falls in the \"Moderate\" band (12–20). This is the moderate range for EBITDA Calculator." } },
  { key: "high", range: "20–30", label: { zh: "偏高 (20–30)", en: "Elevated (20–30)" }, desc: { zh: "落在「偏高」級距20–30。20-30%,偏高,品牌或具規模經濟企業的良好表現。", en: "Falls in the \"Elevated\" band (20–30). This is the elevated range for EBITDA Calculator." } },
  { key: "major", range: "30–45", label: { zh: "高 (30–45)", en: "High (30–45)" }, desc: { zh: "落在「高」級距30–45。30-45%,高,軟體、電信等高營運槓桿產業的優異水準。", en: "Falls in the \"High\" band (30–45). This is the high range for EBITDA Calculator." } },
  { key: "executive", range: "≥ 45", label: { zh: "極高 (≥ 45)", en: "Very high (≥ 45)" }, desc: { zh: "落在「極高」級距≥ 45。> 45%,極高,常見於 SaaS、授權與壟斷性業務,獲利能力極強。", en: "Falls in the \"Very high\" band (≥ 45). This is the very high range for EBITDA Calculator." } },
] as const;
...
const affiliateItems: AffiliateItem[] = [
  ...
  { label: { zh: "毛利計算機", en: "Gross Margin Calculator" }, href: "/tools/finance/gross-margin-calculator" },
  { label: { zh: "現金流計算機", en: "Cash Flow Calculator" }, href: "/tools/finance/cash-flow-calculator" },
];

const ui = {
  zh: {
    badge: "財務 · EBITDA 計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "EBITDA Calculator · EBITDA 計算機",
    subtitle: "輸入營收與營業費用,立即算出 EBITDA、EBITDA 利潤率與淨利",
    intro: "本工具為 EBITDA 計算機，依公開公式於瀏覽器端試算，輸入營收、營業費用、折舊攤銷、利息與稅後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算EBITDA 計算機",
    examplePreview: "EBITDA 利潤率",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入營收、營業費用、折舊攤銷、利息與稅",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "營收 500 萬 · 費用 350 萬",
    baselineExampleNote: "營收 5000000 · 營業費用 3500000",
    activeExample: "進階範例",
    activeExampleValue: "營收 800 萬 · 費用 400 萬",
    activeExampleNote: "營收 加倍 · 觀察 EBITDA 利潤率 變化",
    flowDemo: "數字流向示範",
    calculator: "EBITDA 計算機",
    revenue: "營收",
    operatingExpenses: "營業費用",
    depreciationAmortization: "折舊攤銷",
    interestAndTax: "利息與稅",
    resultCard: "結果卡片",
    primaryValue: "EBITDA 利潤率",
    primaryUnitTail: "%",
    secondaryLabel: "EBITDA",
    secondaryTail: "$",
    metricALabel: "EBITDA 利潤率",
    ...
    metricCTail: "$",
    headlineCaption: "EBITDA 計算機 · 即時試算",
    fatLossTarget: "淨利",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "EBITDA 計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 營收 與 折舊攤銷 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "EBITDA 計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 營收、營業費用、折舊攤銷、利息與稅 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依營收與費用計算 EBITDA、EBITDA 利潤率與淨利。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "知識庫",
    knowledgeTitle: "EBITDA 計算機 · 觀念整理",
    definition: "定義",
    definitionText: "EBITDA 計算機以營收、營業費用、折舊攤銷與利息稅,計算稅前息前折舊攤銷前獲利(EBITDA)、EBITDA 利潤率、EBIT 與淨利,協助評估企業核心營運的現金產生能力。",
    formula: "公式",
    formulaText: "EBITDA = 營收 − 營業費用;EBIT = EBITDA − 折舊攤銷;淨利 = EBIT − 利息與稅;EBITDA 利潤率 = EBITDA ÷ 營收",
    limitations: "限制",
    limitationsText: "本工具不計入資本支出(CapEx)與營運資金變動,EBITDA 可能高估自由現金流,高負債企業須審慎解讀。",
    interpretation: "解讀",
    interpretationText: "EBITDA 利潤率越高代表營運效率越好;應與同業比較並搭配自由現金流,避免被單一指標誤導。",
    context: "情境",
    ...
    premiumTitle: "專業版 企業估值工具組",
    premiumText: "解鎖 EV/EBITDA 估值倍數、調整後 EBITDA、自由現金流橋接、同業比較與多期趨勢分析。",
    premiumChips_zh: "EV/EBITDA|調整EBITDA|現金流橋接|同業比較",
    premiumChips_en: "EV/EBITDA|Adjusted|FCF Bridge|Peers",
    ...
    q1: "EBITDA 是什麼?",
    a1: "EBITDA = 稅前息前折舊攤銷前獲利(Earnings Before Interest, Taxes, Depreciation and Amortization),衡量企業核心營運產生現金的能力,排除融資結構、稅制與非現金費用的影響,常用於跨公司、跨國比較與併購估值。",
    q2: "EBITDA 和淨利差在哪?",
    a2: "EBITDA 加回了利息、稅、折舊與攤銷;淨利則扣除全部。EBITDA 反映「營運本身」賺現金的能力,淨利反映「股東最終」拿到多少。資本密集或高負債公司的 EBITDA 會明顯高於淨利。",
    q3: "EBITDA 利潤率多少算好?",
    a3: "因產業而異。軟體、電信常達 30-50%,製造、零售常 10-20%,低毛利通路可能低於 5%。重點是與同業比較並看趨勢:利潤率穩定或上升代表營運效率良好。",
    q4: "EBITDA 有什麼盲點?",
    a4: "EBITDA 忽略了維持營運所需的資本支出(CapEx)與折舊,可能高估自由現金流;高負債公司用 EBITDA 美化體質時尤其要小心。應搭配自由現金流與淨利一起看,避免被單一指標誤導。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內以 JavaScript 完成,財務數字不會傳送到任何伺服器。",
    q6: "可以算 EV/EBITDA 嗎?",
    a6: "EV/EBITDA 估值倍數、EBITDA 趨勢、調整後 EBITDA 與同業比較屬於專業版功能,免費版聚焦 EBITDA 與利潤率。"
  },
  en: {
    badge: "Finance · EBITDA Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "EBITDA Calculator",
    subtitle: "Enter revenue and operating expenses to see the EBITDA, EBITDA margin, and net income",
    intro: "EBITDA Calculator runs the standard formula in your browser. Enter revenue, operating expenses, depreciation amortization, interest and tax to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try EBITDA Calculator",
    examplePreview: "EBITDA Margin",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter revenue, operating expenses, depreciation amortization, interest and tax",
    ...
    baselineExampleValue: "Rev 5M · OpEx 3.5M",
    baselineExampleNote: "Revenue 5000000 · Operating Expenses 3500000",
    activeExample: "Advanced example",
    activeExampleValue: "Rev 8M · OpEx 4M",
    activeExampleNote: "Revenue doubled · watch EBITDA Margin react",
    flowDemo: "Data flow demo",
    calculator: "EBITDA Calculator",
    revenue: "Revenue",
    operatingExpenses: "Operating Expenses",
    depreciationAmortization: "Depreciation Amortization",
    interestAndTax: "Interest and Tax",
    resultCard: "Result card",
    primaryValue: "EBITDA Margin",
    primaryUnitTail: "%",
    secondaryLabel: "EBITDA",
    secondaryTail: "$",
    metricALabel: "EBITDA Margin",
    ...
    metricCTail: "$",
    headlineCaption: "EBITDA Calculator · live calc",
    fatLossTarget: "Net Income",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "EBITDA Calculator · band matrix",
    ...
    nextActionItem1: "Move Revenue and Depreciation Amortization by ±10% to see sensitivity.",
    ...
    decisionTitle: "EBITDA Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill revenue, operating expenses, depreciation amortization, interest and tax.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "EBITDA Calculator standard formula.",
    ...
    knowledge: "Knowledge",
    knowledgeTitle: "EBITDA Calculator · concept primer",
    definition: "Definition",
    definitionText: "EBITDA Calculator converts inputs (revenue, operating expenses, depreciation amortization, interest and tax) into EBITDA Margin. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(revenue, operating expenses, depreciation amortization, interest and tax)",
    ...
    premiumText: "Unlock EV/EBITDA multiples, adjusted EBITDA, free-cash-flow bridge, peer comparison, and multi-period trend analysis.",
    premiumChips_zh: "EV/EBITDA|調整EBITDA|現金流橋接|同業比較",
    premiumChips_en: "EV/EBITDA|Adjusted|FCF Bridge|Peers",
    ...
    q1: "What does EBITDA Calculator calculate?",
    a1: "EBITDA Calculator applies the standard formula to your inputs and returns EBITDA Margin plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for EBITDA Calculator?",
    a2: "Enter revenue, operating expenses, depreciation amortization, interest and tax. EBITDA Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock EV/EBITDA multiples, adjusted EBITDA, free-cash-flow bridge, peer comparison, and multi-period trend analysis."
  },
} as const;
...
export default function EbitdaCalculator() {
  ...
  const [operatingExpenses, setOperatingExpenses] = useState("3500000");
  const [depreciationAmortization, setDepreciationAmortization] = useState("400000");
  const [interestAndTax, setInterestAndTax] = useState("300000");
  const t = ui[lang];
  ...
  }, [revenue, operatingExpenses, depreciationAmortization, interestAndTax]);

  const primaryDisplay = fmt(result.ebitdaMargin, 1);
  const secondaryDisplay = fmt(result.ebitda, 0);
  const tertiaryDisplay = fmt(result.ebit, 0);
  const quaternaryDisplay = fmt(result.netIncome, 0);

  function fillSolid() { setUnit("metric"); setRevenue("5000000"); setOperatingExpenses("3500000"); setDepreciationAmortization("400000"); setInterestAndTax("300000"); }
  function fillHighSalary() { setUnit("imperial"); setRevenue("8000000"); setOperatingExpenses("4000000"); setDepreciationAmortization("500000"); setInterestAndTax("600000"); }

  const activeBand = bands.find(b => {
    const r = result.ebitdaMargin;
    if (r < 5) return 'tiny';
    if (r < 12) return 'normal';
    if (r < 20) return 'notable';
    if (r < 30) return 'high';
    if (r < 45) return 'major';
    return 'executive';
  });
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-emerald-700">{t.revenue}<input type="number" step="100000" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={revenue} onChange={(e) => setRevenue(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.operatingExpenses}<input type="number" step="100000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={operatingExpenses} onChange={(e) => setOperatingExpenses(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.depreciationAmortization}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={depreciationAmortization} onChange={(e) => setDepreciationAmortization(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.interestAndTax}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={interestAndTax} onChange={(e) => setInterestAndTax(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="ebitda-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="ebitda-calculator-faq" position="inline" /></section>
