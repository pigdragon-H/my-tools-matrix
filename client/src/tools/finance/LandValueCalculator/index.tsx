// @profile B
// Profile B · 計算機-YMYL · Land Value Calculator - market and assessed land value from area and unit price（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<20 ping", label: { zh: "小坪數", en: "Small plot" }, desc: { zh: "面積較小,適合自用或小型建案規劃。", en: "A small plot suited to personal use or a small build." } },
  { key: "normal", range: "20-50 ping", label: { zh: "一般坪數", en: "Standard plot" }, desc: { zh: "常見的住宅基地規模,流通性佳。", en: "A common residential plot size with good liquidity." } },
  { key: "notable", range: "50-100 ping", label: { zh: "明顯坪數", en: "Notable plot" }, desc: { zh: "面積已具規模,價值與用途彈性提高。", en: "A sizeable plot with higher value and more flexible uses." } },
  { key: "high", range: "100-200 ping", label: { zh: "大坪數", en: "Large plot" }, desc: { zh: "適合中型開發,須留意分區與容積規定。", en: "Suited to mid-size development; mind zoning and FAR rules." } },
  { key: "major", range: "200-400 ping", label: { zh: "大型土地", en: "Major land" }, desc: { zh: "大型土地資產,開發與稅務規劃更為重要。", en: "A major land asset where development and tax planning matter more." } },
  { key: "executive", range: "400 ping+", label: { zh: "超大型土地", en: "Estate scale" }, desc: { zh: "莊園或開發級規模,建議專業估價與顧問。", en: "Estate or development scale; professional appraisal and advice are advised." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "土地價值計算機",
    subtitle: "依面積、每坪市價與持分,估算土地市場價值與公告現值。",
    intro: "輸入土地面積 (坪)、每坪市價、公告現值比例與持分比例,本工具計算你實際持有的市場價值與公告現值,並換算坪數與平方公尺,協助你掌握土地資產。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立土地增值範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "土地面積 (坪)",
    averageHourlyRate: "每坪市價",
    durationHours: "公告現值比例 (%)",
    meetingsPerMonth: "持分比例 (%)",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Estimate market and assessed land value from area, unit price and ownership share.",
    intro: "Enter the land area in ping, market price per ping, assessed-value ratio and ownership share; the tool computes your effective market and assessed value and converts the area, helping you understand your land asset.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a land-value example",
    ...
    participants: "Land area (ping)",
    averageHourlyRate: "Market price per ping",
    durationHours: "Assessed value ratio (%)",
    meetingsPerMonth: "Ownership share (%)",
    resultCard: "Result",
    unit: "Primary result",
...
export default function LandValueCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("800000");
  const [durationHours, setDurationHours] = useState("70");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("100");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const areaPing = v1; const pricePerPing = v2; const assessedRatio = v3; const sharePct = v4;
    const share = sharePct / 100;
    const marketValue = areaPing * pricePerPing * share;
    const assessedValue = marketValue * (assessedRatio / 100);
    const ownedArea = areaPing * share;
    const sqm = ownedArea * 3.30579;
    return { marketValue, assessedValue, ownedArea, sqm, pricePerPing };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.marketValue, 0);
  const monthlyDisplay = fmt(result.assessedValue, 0);

  function fillSolid() { setUnit("metric"); setParticipants("50"); setAverageHourlyRate("800000"); setDurationHours("70"); setMeetingsPerMonth("100"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("120"); setAverageHourlyRate("1200000"); setDurationHours("70"); setMeetingsPerMonth("100"); }

  const activeBand = bands.find(b => {
    const r = result.ownedArea;
    if (r < 20) return b.key === "tiny";
    if (r < 50) return b.key === "normal";
    if (r < 100) return b.key === "notable";
    if (r < 200) return b.key === "high";
    if (r < 400) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#ecfdf5 0%,#d1fae5 55%,#dbeafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "土地市場價值" : "market land value"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.ownedArea, 1)} ping</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.assessedValue, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">70%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "一般公告比例" : "Typical assessed ratio"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">90%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "高公告比例" : "High assessed ratio"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "持有面積" : "Owned area"}</div><p className="mt-2 text-3xl font-black text-emerald-950">{fmt(result.ownedArea, 1)}</p><p className="text-sm font-bold text-emerald-700"> ping</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "市場價值" : "Market value"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.marketValue, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "公告現值" : "Assessed value"}</div><p className="mt-2 text-3xl font-black text-slate-950">${fmt(result.assessedValue, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="land-value-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "土地市場價值" : "market land value"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入面積與每坪市價" : "Enter area and unit price", note: t.bmrStep }, { label: lang === "zh" ? "乘以持分比例" : "Apply ownership share", note: t.deficitStep }, { label: lang === "zh" ? "算出市場價值" : "Compute market value", note: t.trendStep }, { label: lang === "zh" ? "套用公告比例" : "Apply assessed ratio", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="land-value-calculator-faq" position="inline" /></section>
