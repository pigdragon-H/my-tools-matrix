...
export default function CrossRateCalculator() {
  ...
  const result=useMemo<Output>(()=>{
    try{
      ...
      const toolSlug: string = "cross-rate-calculator";
      switch(toolSlug){
  ...
  const copyResult=async()=>{ if(!result.main) return; await navigator.clipboard.writeText(`Cross Rate Calculator
${result.detail}
Formula: ${result.formula}`); setCopied(true); window.setTimeout(()=>setCopied(false),1500); };
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
    <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950"><p className="text-sm font-semibold text-blue-600">FIN · FXR</p><h1 className="text-3xl font-bold text-slate-950 dark:text-white">Cross Rate Calculator</h1><p className="mt-2 text-slate-600 dark:text-slate-300">交叉匯率計算器：兩種非USD貨幣 → 交叉匯率。</p></section>
