...
const bands = [
  { key: "critical", range: "0–1 個月", label: { zh: "危急", en: "危急" }, desc: { zh: "幾乎無緩衝，任何意外都會造成財務危機。", en: "幾乎無緩衝，任何意外都會造成財務危機。" } },
  { key: "vulnerable", range: "1–3 個月", label: { zh: "脆弱", en: "脆弱" }, desc: { zh: "僅覆蓋短期風險，需加速儲蓄。", en: "僅覆蓋短期風險，需加速儲蓄。" } },
  { key: "basic", range: "3–6 個月", label: { zh: "基本安全", en: "基本安全" }, desc: { zh: "達到基本安全線，可應對多數短期突發。", en: "達到基本安全線，可應對多數短期突發。" } },
  { key: "solid", range: "6–9 個月", label: { zh: "穩健", en: "穩健" }, desc: { zh: "覆蓋中型風險，失業後有充裕找工時間。", en: "覆蓋中型風險，失業後有充裕找工時間。" } },
  { key: "strong", range: "9–12 個月", label: { zh: "強健", en: "強健" }, desc: { zh: "可承受長期失業或重大支出，壓力極低。", en: "可承受長期失業或重大支出，壓力極低。" } },
  { key: "fortress", range: "12+ 個月", label: { zh: "堡壘", en: "堡壘" }, desc: { zh: "財務防禦極強，可從容應對幾乎所有突發。", en: "財務防禦極強，可從容應對幾乎所有突發。" } },
] as const;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#fef3c7,_#f8fafc_45%,_#e0f2fe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{coverageDisplay} 個月</div><div className="text-sm font-bold text-amber-100">{t.estimatedTdee}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{coverageDisplay} 個月</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">${fmt(Number(monthlyExpenses), 0)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${gapDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillLowSavings} className="mt-3 w-full rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-sm font-black text-red-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">3.3 個月</span></div><p className="mt-2 text-sm text-slate-600">每月 $3k · 儲蓄 $10k</p></button><button onClick={fillLowSavings} className="w-full rounded-2xl border border-red-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-red-100 px-3 py-1 text-xs font-black text-red-700">0.5 個月</span></div><p className="mt-2 text-sm text-slate-600">每月 $4k · 儲蓄 $2k</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{coverageDisplay}<span className="text-3xl"> 個月</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthsToGoal}</div><div className="mt-1 text-xl font-black">{result.monthsToGoal} 個月</div><div className="mt-1 text-xs text-slate-300">達成目標</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.maintenanceTarget}</div><div className="mt-1 text-xs font-black uppercase text-emerald-700">{t.maintenance}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.targetAmount, 0)}</p><p className="text-sm font-bold text-emerald-700">目標</p></div><div className="rounded-2xl bg-red-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-red-500">{t.actionTarget}</div><div className="mt-1 text-xs font-black uppercase text-red-700">{t.fatLossTarget}</div><p className="mt-2 text-3xl font-black text-red-950">${gapDisplay}</p><p className="text-sm font-bold text-red-700">缺口</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.estimatedTdee}</div><div className="mt-1 text-xs font-black uppercase text-slate-700">已覆蓋</div><p className="mt-2 text-3xl font-black text-slate-950">{coverageDisplay} 個月</p><p className="text-sm font-bold text-slate-700">目前</p></div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9-Emotion-Upper */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black uppercase text-slate-500">目標</div><div className="mt-1 text-3xl font-black">${fmt(result.targetAmount, 0)}</div></div><div className="rounded-2xl bg-red-50 p-4"><div className="text-xs font-black uppercase text-red-600">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-red-950">${gapDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-emerald-950">{coverageDisplay} 個月</div></div></div></article>
