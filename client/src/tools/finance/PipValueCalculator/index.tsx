// @profile B
// Profile B · 計算機-YMYL · Pip Value Calculator - monetary value of one pip for a forex position（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<3", label: { zh: "微型部位", en: "Micro size" }, desc: { zh: "每點價值極小,適合新手練習與小資金。", en: "A very small pip value, suited to beginners and small accounts." } },
  { key: "normal", range: "3-9", label: { zh: "迷你部位", en: "Mini size" }, desc: { zh: "迷你手交易常見的每點價值區間。", en: "A common pip value range for mini-lot trading." } },
  { key: "notable", range: "9-15", label: { zh: "標準部位", en: "Standard" }, desc: { zh: "接近一標準手,風險與報酬更明確。", en: "Near one standard lot, with clearer risk and reward." } },
  { key: "high", range: "15-40", label: { zh: "較大部位", en: "Large" }, desc: { zh: "每點價值偏高,務必設好停損。", en: "A high pip value; set your stop-loss carefully." } },
  { key: "major", range: "40-80", label: { zh: "大部位", en: "Heavy" }, desc: { zh: "波動數十點即影響顯著,風控極重要。", en: "A few dozen pips move it sharply; risk control is vital." } },
  { key: "executive", range: "80+", label: { zh: "超大部位", en: "Very heavy" }, desc: { zh: "極高的每點價值,僅適合資金充裕且嚴格風控者。", en: "A very high pip value, only for well-funded, disciplined traders." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "點值計算機",
    subtitle: "依手數、一點大小與報價匯率,計算每一點的貨幣價值。",
    intro: "輸入交易手數、一點大小、報價匯率與合約單位,本工具計算每一點的貨幣價值、每手每點價值與十點價值,協助你管理外匯部位的風險與停損距離。",
    trustNoteLabel: "注意事項:",
    ...
    averageHourlyRate: "一點大小",
    durationHours: "報價匯率",
    meetingsPerMonth: "合約單位",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Compute the monetary value of a single pip from lot size, pip size and quote rate.",
    intro: "Enter the lot size, pip size, quote rate and contract size; the tool computes the value of a single pip, the value per pip per lot and the value of ten pips, helping you manage position risk and stop distances.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a pip-value example",
    ...
    averageHourlyRate: "Pip size",
    durationHours: "Quote rate",
    meetingsPerMonth: "Contract size",
    resultCard: "Result",
    unit: "Primary result",
...
export default function PipValueCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("0.0001");
  const [durationHours, setDurationHours] = useState("1.1000");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("100000");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const lotSize = v1; const pipSize = v2; const quoteRate = v3; const contractSize = v4;
    const units = lotSize * contractSize;
    const pipInQuote = pipSize * units;
    const pipValue = quoteRate > 0 ? pipInQuote / quoteRate : pipInQuote;
    const pipValuePerLot = lotSize > 0 ? pipValue / lotSize : 0;
    const value10pips = pipValue * 10;
    return { pipValue, pipValuePerLot, value10pips, units, lotSize };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.pipValue, 2);
  const monthlyDisplay = fmt(result.pipValuePerLot, 2);

  function fillSolid() { setUnit("metric"); setParticipants("1"); setAverageHourlyRate("0.0001"); setDurationHours("1.1000"); setMeetingsPerMonth("100000"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("5"); setAverageHourlyRate("0.0001"); setDurationHours("1.1000"); setMeetingsPerMonth("100000"); }

  const activeBand = bands.find(b => {
    const r = result.pipValue;
    if (r < 3) return b.key === "tiny";
    if (r < 9) return b.key === "normal";
    if (r < 15) return b.key === "notable";
    if (r < 40) return b.key === "high";
    if (r < 80) return b.key === "major";
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#f0fdfa 0%,#ccfbf1 55%,#cffafe 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "每點價值" : "value per pip"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">${fmt(result.value10pips, 2)}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.units, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">0.1 lot</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "小型部位" : "Small position"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">1 lot</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "標準部位" : "Standard position"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元/點" : "/pip"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元/點/手" : "/pip/lot"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "每點價值" : "Value per pip"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.pipValue, 2)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "10 點價值" : "Value of 10 pips"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.value10pips, 2)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "交易單位" : "Units traded"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.units, 0)}</p><p className="text-sm font-bold text-slate-700"></p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="pip-value-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "每點價值" : "value per pip"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入手數與合約單位" : "Enter lots and contract size", note: t.bmrStep }, { label: lang === "zh" ? "設定一點大小" : "Set the pip size", note: t.deficitStep }, { label: lang === "zh" ? "以報價匯率換算" : "Convert via quote rate", note: t.trendStep }, { label: lang === "zh" ? "得出每點價值" : "Get the value per pip", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="pip-value-calculator-faq" position="inline" /></section>
