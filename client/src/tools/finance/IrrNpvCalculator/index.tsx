// @profile B
// Profile B · 計算機-YMYL · IRR / NPV Calculator - NPV and profitability index for a level annual cashflow project（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<0.8x", label: { zh: "明顯虧損", en: "Clear loss" }, desc: { zh: "折現後現金流不足以回收投資,專案不具吸引力。", en: "Discounted cashflows fall short of the investment; the project is unattractive." } },
  { key: "normal", range: "0.8-1.0x", label: { zh: "勉強回收", en: "Marginal" }, desc: { zh: "NPV 接近零,報酬僅勉強打平折現率。", en: "NPV is near zero; returns barely meet the discount rate." } },
  { key: "notable", range: "1.0-1.2x", label: { zh: "略具效益", en: "Mildly positive" }, desc: { zh: "專案創造正向價值,但安全邊際有限。", en: "The project adds value but with a limited margin of safety." } },
  { key: "high", range: "1.2-1.5x", label: { zh: "效益良好", en: "Good return" }, desc: { zh: "獲利能力穩健,值得納入投資組合考量。", en: "A solid return worth considering within your portfolio." } },
  { key: "major", range: "1.5-2.0x", label: { zh: "高效益", en: "Strong" }, desc: { zh: "折現後仍大幅獲利,專案吸引力高。", en: "Strong profit even after discounting; an attractive project." } },
  { key: "executive", range: "2.0x+", label: { zh: "極高效益", en: "Exceptional" }, desc: { zh: "報酬遠超折現率,但須確認假設是否過於樂觀。", en: "Returns far exceed the discount rate; verify the assumptions are not too rosy." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "NPV / 投報計算機",
    subtitle: "以折現率評估專案現金流的淨現值與獲利指數。",
    intro: "輸入期初投資、每年現金流、年數與折現率,本工具將未來現金流折現後計算淨現值 (NPV) 與獲利指數 (PI),協助你判斷專案是否值得投資。",
    trustNoteLabel: "注意事項:",
    trustNote: "此工具僅供教育與規劃用途,結果為估算值,不構成專業財務、稅務或投資建議。",
    quickActionCard: "快速範例",
    tryExample: "一鍵建立IRR/NPV範例",
    examplePreview: "結果預覽",
    examplePerson: "標準範例",
    ...
    calculator: "計算機",
    participants: "期初投資",
    averageHourlyRate: "每年現金流",
    durationHours: "年數",
    meetingsPerMonth: "折現率 (%)",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Evaluate a project's cashflows with net present value and a profitability index at your discount rate.",
    intro: "Enter the initial investment, level annual cashflow, number of years and discount rate; the tool discounts future cashflows to compute net present value and a profitability index, helping you decide whether a project is worth funding.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a IRR/NPV example",
    ...
    averageHourlyRate: "Annual cashflow",
    durationHours: "Number of years",
    meetingsPerMonth: "Discount rate (%)",
    resultCard: "Result",
    unit: "Primary result",
...
export default function IrrNpvCalculator() {
  ...
  const [averageHourlyRate, setAverageHourlyRate] = useState("250000");
  const [durationHours, setDurationHours] = useState("6");
  const [meetingsPerMonth, setMeetingsPerMonth] = useState("8");
  const t = ui[lang];
  ...
  const result = useMemo(() => {
    ...
    const initialInvestment = v1; const annualCashflow = v2; const years = v3; const discountRate = v4;
    const r = discountRate / 100;
    let pv = 0;
    for (let t = 1; t <= years; t++) { pv += annualCashflow / Math.pow(1 + r, t); }
    const npv = pv - initialInvestment;
    const pi = initialInvestment > 0 ? pv / initialInvestment : 0;
    const totalCash = annualCashflow * years;
    const simpleRoi = initialInvestment > 0 ? ((totalCash - initialInvestment) / initialInvestment) * 100 : 0;
    return { npv, pv, pi, simpleRoi, initialInvestment };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.npv, 0);
  const monthlyDisplay = fmt(result.pv, 0);

  function fillSolid() { setUnit("metric"); setParticipants("1000000"); setAverageHourlyRate("250000"); setDurationHours("6"); setMeetingsPerMonth("8"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("1000000"); setAverageHourlyRate("250000"); setDurationHours("6"); setMeetingsPerMonth("12"); }

  const activeBand = bands.find(b => {
    const r = result.pi;
    if (r < 0.8) return b.key === "tiny";
    if (r < 1.0) return b.key === "normal";
    if (r < 1.2) return b.key === "notable";
    if (r < 1.5) return b.key === "high";
    if (r < 2.0) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#f5f3ff 0%,#ede9fe 55%,#fae8ff 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "淨現值 NPV" : "net present value"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.pi, 2)}x</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.simpleRoi, 1)}%</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">8%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "一般折現率" : "Standard discount rate"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">12%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "較高折現率" : "Higher discount rate"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "現金流現值" : "PV of cashflows"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.pv, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "淨現值" : "Net present value"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.npv, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "獲利指數" : "Profitability index"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.pi, 2)}</p><p className="text-sm font-bold text-slate-700">x</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="irr-npv-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "淨現值 NPV" : "net present value"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="irr-npv-calculator-faq" position="inline" /></section>
