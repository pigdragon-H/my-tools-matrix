...
const bands = [
  { key: "heavy", range: ">40%", label: { zh: "重稅", en: "Heavy tax burden" }, desc: { zh: "稅率超過 40%，可尋求稅務規劃降低有效稅率。", en: "Tax rate over 40%; consider tax planning to lower effective rate." } },
  { key: "high", range: "30–40%", label: { zh: "高稅", en: "High tax" }, desc: { zh: "稅率偏高，建議檢視扣除額與退休帳戶。", en: "Tax rate is high; review deductions and retirement accounts." } },
  { key: "moderate", range: "20–30%", label: { zh: "中等", en: "Moderate" }, desc: { zh: "稅率在常見範圍，持續善用扣除額。", en: "Tax rate in common range; keep leveraging deductions." } },
  { key: "low", range: "10–20%", label: { zh: "低稅", en: "Low tax" }, desc: { zh: "稅率較低，可將更多資金投入投資。", en: "Lower tax rate; more funds available for investing." } },
  { key: "minimal", range: "<10%", label: { zh: "極低", en: "Minimal tax" }, desc: { zh: "稅率極低，適合加速累積資產。", en: "Very low tax rate; ideal for accelerating wealth building." } },
  { key: "credit", range: "Negative", label: { zh: "退稅", en: "Net refund" }, desc: { zh: "扣除額超過收入，可能獲得退稅。", en: "Deductions exceed income; may receive a net refund." } },
] as const;
...
const ui = {
  zh: {
    badge: "財務 · 薪資稅務 · Gold Tool", switchToEnglish: "Switch to English", switchToChinese: "切換到中文", chineseShort: "中", englishShort: "EN",
    title: "稅後薪資計算機 · Salary After Tax Planner", subtitle: "計算你的實際到手薪資與有效稅率",
    intro: "Salary After Tax Calculator 根據你的年薪、州稅率與扣除額，計算聯邦稅、州稅、社會安全稅與醫療保險稅，得出實際到手薪資與有效稅率。",
    ...
    decisionPath: "決策路徑", decisionTitle: "Salary After Tax → Hourly Rate → Budget Ratio → Net Worth", bmrStep: "稅後薪資", deficitStep: "時薪", trendStep: "預算比例", mealStep: "淨資產",
    knowledge: "知識", knowledgeTitle: "Salary After Tax 在財務宇宙中的意義", definition: "定義", definitionText: "稅後薪資是扣除所有強制稅費後的實際到手收入，包括聯邦所得稅、州所得稅、社會安全稅與醫療保險稅。",
    ...
    faq: "FAQ", commonQuestions: "常見問題", affiliate: "推薦工具", affiliateTitle: "稅後薪資規劃的下一步工具", premiumTitle: "PRO 稅務規劃包", premiumText: "解鎖年度稅務趨勢圖、扣除額最佳化分析、州際比較與個人化稅務報告。",
    trustReferences: "信任聲明 · 相關工具 · 參考資料", trust: "信任聲明", trustText: "本工具僅供教育與規劃用途，不取代稅務顧問或專業財務規劃。", relatedTools: "相關工具", relatedToolsText: "Hourly Rate Calculator · Budget Ratio Calculator · Net Worth Calculator · Retirement Calculator", references: "參考資料", referencesText: "IRS 2024 Tax Brackets; Tax Foundation State Tax Rates; SSA Wage Base; CFPB Tax Withholding Guide。",
    q1: "有效稅率與邊際稅率有何不同？", a1: "有效稅率是總稅額除以總收入，反映整體稅負；邊際稅率是下一元收入適用的稅率，通常較高。",
    q2: "如何降低有效稅率？", a2: "善用 401(k)、IRA 等稅前扣除，增加標準或列舉扣除額，利用稅額抵減（Tax Credits）如子女抵稅。",
  ...
  en: {
    badge: "Finance · Salary Tax · Gold Tool", switchToEnglish: "Switch to English", switchToChinese: "切換到中文", chineseShort: "中", englishShort: "EN",
    title: "Salary After Tax Calculator · Salary After Tax Planner", subtitle: "Calculate your actual take-home pay and effective tax rate",
    intro: "This calculator uses your annual salary, state tax rate, and deductions to compute federal tax, state tax, Social Security, and Medicare, giving you your actual take-home pay and effective tax rate.",
    trustNoteLabel: "Note:", trustNote: "This tool uses simplified 2024 US single filer federal brackets; actual taxes vary by state laws, filing status, and deductions.",
    quickActionCard: "Quick Action Card", tryExample: "Create a salary after tax example instantly", examplePreview: "Tax rate preview", examplePerson: "Standard example", fillExample: "One-click standard example", previewActivePath: "Fill high-salary example",
    examplesCalculator: "Examples → Calculator", enterValues: "Enter salary and tax info", examplesHelper: "Start with an example to understand salary after tax calculation, then replace with your own numbers.",
    ...
    resultCard: "Salary After Tax Result", unit: "Take-home pay ($)", primaryValue: "Primary Value", maintenanceTarget: "Annual take-home ($)", actionTarget: "Monthly take-home", estimatedTdee: "Effective rate", maintenance: "Annual take-home", fatLossTarget: "Monthly take-home",
    ...
    resultIntelligence: "Result Intelligence", tdeeMatrix: "Six-card tax pressure matrix", tdeeMatrixNote: "L7 uses six fixed cards to compare the effective rate with common planning zones. This is planning guidance, not tax advice.",
    emotionConversionLayer: "Emotion + Conversion Layer", turnIntoPlan: "Turn salary after tax snapshot into an actionable plan", conversionNote: "L9 values update from the computed result: effective rate, monthly take-home, and savings hints.",
    progressInsight: "Progress Insight Card", possibleTarget: "Current salary after tax plan", dailyGap: "Total tax", weeklyTrend: "Effective rate", motivation: "Motivation Card", keepMomentum: "Move from salary after tax snapshot to consistent tracking",
    saveShareJourney: "Save / Share", journeyTitle: "Take today's salary after tax snapshot home", journeyHint: "Re-calculate every tax season to track effective rate changes.",
    nextActionLabel: "Next actions", nextActionTitle: "Connect this result to the next tool", nextActionItem1: "Use Hourly Rate Calculator to understand your real hourly wage", nextActionItem2: "Use Budget Ratio Calculator to plan take-home allocation", nextActionItem3: "Use Net Worth Calculator to review overall financial health",
    shareLinkBtn: "📋 Copy result link", shareNativeBtn: "📤 Share with friends", shareCopiedToast: "Copied to clipboard ✓",
    decisionPath: "Decision Path", decisionTitle: "Salary After Tax → Hourly Rate → Budget Ratio → Net Worth", bmrStep: "Salary after tax", deficitStep: "Hourly rate", trendStep: "Budget ratio", mealStep: "Net worth",
    knowledge: "Knowledge", knowledgeTitle: "What salary after tax means in the Finance universe", definition: "Definition", definitionText: "Salary after tax (take-home pay) is the income remaining after all mandatory taxes are withheld, including federal income tax, state income tax, Social Security, and Medicare.",
    formula: "Formula", formulaText: "Taxable income = Gross salary − Deductions. Federal tax uses progressive brackets. State tax = Taxable income × State rate. Social Security = min(Salary, $168,600) × 6.2%. Medicare = Salary × 1.45%. Take-home = Salary − Total tax. Effective rate = Total tax ÷ Salary × 100%.",
    limitations: "Limitations", limitationsText: "Uses simplified single filer brackets; does not compute AMT, capital gains, tax credits, or filing status differences. Consult a tax professional for actual filing.",
    interpretation: "Interpretation", interpretationText: "An effective rate of 20–30% is common for most wage earners; above 30% suggests reviewing deductions and retirement account contributions.",
    context: "Context", contextText: "Salary after tax should be viewed alongside hourly rate, budget ratio, and net worth.",
    example: "Example", exampleText: "Salary $75,000, deductions $14,600, state rate 5%. Taxable $60,400. Federal tax ≈ $8,288, state $3,020, SS $4,650, Medicare $1,088. Total tax ≈ $17,046, annual take-home $57,954, monthly $4,830, effective rate 22.7%.",
    faq: "FAQ", commonQuestions: "Common questions", affiliate: "Recommended Tools", affiliateTitle: "Next-step tools for salary after tax planning", premiumTitle: "PRO Tax Planning Pack", premiumText: "Unlock annual tax trend chart, deduction optimization, interstate comparison, and personalized tax report.",
    trustReferences: "Trust · Related Tools · References", trust: "Trust disclaimer", trustText: "This tool is for education and planning only; it does not replace tax advisors or professional financial planning.", relatedTools: "Related tools", relatedToolsText: "Hourly Rate Calculator · Budget Ratio Calculator · Net Worth Calculator · Retirement Calculator", references: "References", referencesText: "IRS 2024 Tax Brackets; Tax Foundation State Tax Rates; SSA Wage Base; CFPB Tax Withholding Guide.",
    q1: "What's the difference between effective and marginal tax rate?", a1: "Effective rate is total tax divided by total income, reflecting overall burden. Marginal rate is the rate on the next dollar earned, which is typically higher.",
    q2: "How can I lower my effective tax rate?", a2: "Maximize pre-tax deductions like 401(k) and IRA, increase standard or itemized deductions, and leverage tax credits such as the Child Tax Credit.",
    q3: "How much do state taxes vary?", a3: "Some states have no income tax (TX, FL), while others reach up to 13% (CA). Cross-state work requires special attention.",
    q4: "Can Social Security and Medicare taxes be avoided?", a4: "W-2 employees cannot avoid them. Self-employed pay double (15.3%) but can deduct half as a business expense.",
    q5: "How much tax is withheld on a year-end bonus?", a5: "Bonuses are supplemental wages; federal withholding is usually 22% (37% above $1M). Actual tax depends on total income.",
    q6: "Can this tool provide tax filing or tax avoidance advice?", a6: "No. It is an educational estimator only. For tax filing, avoidance strategies, or major financial decisions, consult a qualified professional.",
  },
} as const;
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">22.7%</span></div><p className="mt-2 text-sm text-slate-600">$75k · Take-home $57,954</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">~31%</span></div><p className="mt-2 text-sm text-slate-600">$150k · State 8%</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{effRateDisplay}<span className="text-3xl">%</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyTakeHome}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">/mo</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.maintenanceTarget}</div><div className="mt-1 text-xs font-black uppercase text-emerald-700">{t.maintenance}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.takeHome, 0)}</p><p className="text-sm font-bold text-emerald-700">annual</p></div><div className="rounded-2xl bg-red-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-red-500">{t.totalTax}</div><div className="mt-1 text-xs font-black uppercase text-red-700">TAX</div><p className="mt-2 text-3xl font-black text-red-950">${fmt(result.total, 0)}</p><p className="text-sm font-bold text-red-700">total</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveRate}</div><div className="mt-1 text-xs font-black uppercase text-slate-700">RATE</div><p className="mt-2 text-3xl font-black text-slate-950">{effRateDisplay}%</p><p className="text-sm font-bold text-slate-700">effective</p></div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9-Emotion-Upper */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black uppercase text-slate-500">Take-home</div><div className="mt-1 text-3xl font-black">${fmt(result.takeHome, 0)}</div></div><div className="rounded-2xl bg-red-50 p-4"><div className="text-xs font-black uppercase text-red-600">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-red-950">${fmt(result.total, 0)}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{effRateDisplay}%</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: "Salary", note: t.bmrStep }, { label: "Hourly", note: t.deficitStep }, { label: "Budget", note: t.trendStep }, { label: "Net Worth", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
