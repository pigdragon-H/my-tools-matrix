// @profile B
// Profile B · 計算機-YMYL · FIRE Calculator — years to financial independence given savings and return（GOLD-STANDARD-001 compatible）
...
const bands = [
  ...
  { key: "major", range: "30-45 yr", label: { zh: "需要調整", en: "Needs tuning" }, desc: { zh: "時程偏長,儲蓄率或報酬假設可能需要重新檢視。", en: "A long timeline; savings rate or return assumptions may need revisiting." } },
  { key: "executive", range: "45+ yr", label: { zh: "難以達成", en: "Hard to reach" }, desc: { zh: "以目前條件難以達成,須大幅提高儲蓄或降低目標支出。", en: "Hard to reach as is; substantially raise savings or lower target spending." } },
] as const;
...
const ui = {
  zh: {
    ...
    calculator: "計算機",
    participants: "年度支出",
    averageHourlyRate: "目前已存金額",
    durationHours: "每年新增儲蓄",
    meetingsPerMonth: "預期年報酬率 %",
  ...
  en: {
    ...
    subtitle: "Derive your FIRE number from annual expenses and estimate the years to financial independence from savings and returns",
    intro: "Using the common 25x rule, this tool converts annual expenses into a FIRE number, then projects year by year from current savings, annual contributions and expected return to estimate the years to financial independence, helping you gauge early retirement feasibility.",
    trustNoteLabel: "Note:",
    ...
    participants: "Annual expenses",
    averageHourlyRate: "Current savings",
    durationHours: "Annual savings added",
    meetingsPerMonth: "Expected annual return %",
...
export default function FireCalculator() {
  ...
  const [participants, setParticipants] = useState("40000");
  const [averageHourlyRate, setAverageHourlyRate] = useState("120000");
  const [durationHours, setDurationHours] = useState("30000");
  ...
  const result = useMemo(() => {
    ...
    const annualExpense = v1; const current = v2; const annualSaving = v3; const returnPct = v4;
    const fireNumber = annualExpense * 25;
    const r = returnPct / 100;
    let bal = current; let yrs = 0;
    while (bal < fireNumber && yrs < 100) { bal = bal * (1 + r) + annualSaving; yrs += 1; }
    const yearsToFire = bal >= fireNumber ? yrs : 100;
    const gap = Math.max(0, fireNumber - current);
    const savingsRate = (annualSaving + annualExpense) > 0 ? (annualSaving / (annualSaving + annualExpense)) * 100 : 0;
    return { fireNumber, yearsToFire, gap, savingsRate, currentSavings: current, annualSaving };
  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.yearsToFire, 0);
  const monthlyDisplay = fmt(result.fireNumber, 0);

  function fillSolid() { setUnit("metric"); setParticipants("40000"); setAverageHourlyRate("120000"); setDurationHours("30000"); setMeetingsPerMonth("5"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("40000"); setAverageHourlyRate("120000"); setDurationHours("50000"); setMeetingsPerMonth("7"); }

  const activeBand = bands.find(b => {
    const r = result.yearsToFire;
    if (r < 8) return b.key === "tiny";
    if (r < 15) return b.key === "normal";
    if (r < 22) return b.key === "notable";
    if (r < 30) return b.key === "high";
    if (r < 45) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fff1f2 0%,#ffe4e6 55%,#fce7f3 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "達成財務獨立所需年數" : "years to financial independence"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.savingsRate, 0)}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{fmt(result.gap, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">5%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "報酬 5% · 年存 30k" : "5% return · 30k/yr"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">7%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "報酬 7% · 年存 50k" : "7% return · 50k/yr"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{meetingDisplay}<span className="text-3xl">{lang === "zh" ? " 年" : " yr"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? " FIRE 目標" : " FIRE number"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "FIRE 目標金額" : "FIRE number"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.fireNumber, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "尚需累積" : "Remaining gap"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.gap, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "儲蓄率" : "Savings rate"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.savingsRate, 0)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="fire-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "達成財務獨立所需年數" : "years to financial independence"}</div><div className="mt-1 text-3xl font-black">{meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">{meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "估算年度支出" : "Estimate annual expenses", note: t.bmrStep }, { label: lang === "zh" ? "以 25 倍法則定目標" : "Set target via the 25x rule", note: t.deficitStep }, { label: lang === "zh" ? "計算儲蓄與報酬累積" : "Project savings and returns", note: t.trendStep }, { label: lang === "zh" ? "檢視時程並優化" : "Review the timeline and tune", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="fire-calculator-faq" position="inline" /></section>
