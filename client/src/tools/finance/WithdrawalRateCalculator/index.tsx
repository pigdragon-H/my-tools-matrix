// @profile B
// Profile B · 計算機-YMYL · WithdrawalRateCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 2.5", label: { zh: "極保守 (< 2.5)", en: "Very conservative (< 2.5)" }, desc: { zh: "落在「極保守」級距< 2.5。提領率 < 2.5%,極保守,資產幾乎只進不出,長壽與遺產規劃綽綽有餘。", en: "Falls in the \"Very conservative\" band (< 2.5). This is the very conservative range for Safe Withdrawal Rate Calculator." } },
  { key: "normal", range: "2.5–3.5", label: { zh: "保守 (2.5–3.5)", en: "Conservative (2.5–3.5)" }, desc: { zh: "落在「保守」級距2.5–3.5。2.5-3.5%,保守,接近多數研究建議的安全下限,跨越市場循環極穩健。", en: "Falls in the \"Conservative\" band (2.5–3.5). This is the conservative range for Safe Withdrawal Rate Calculator." } },
  { key: "notable", range: "3.5–4", label: { zh: "穩健 (3.5–4)", en: "Balanced (3.5–4)" }, desc: { zh: "落在「穩健」級距3.5–4。3.5-4%,穩健,對應經典「4% 法則」核心區間,30 年存活機率高。", en: "Falls in the \"Balanced\" band (3.5–4). This is the balanced range for Safe Withdrawal Rate Calculator." } },
  { key: "high", range: "4–5", label: { zh: "偏高 (4–5)", en: "Elevated (4–5)" }, desc: { zh: "落在「偏高」級距4–5。4-5%,偏高,需搭配彈性支出或較高報酬,空頭年份要主動減提。", en: "Falls in the \"Elevated\" band (4–5). This is the elevated range for Safe Withdrawal Rate Calculator." } },
  { key: "major", range: "5–6", label: { zh: "高風險 (5–6)", en: "High risk (5–6)" }, desc: { zh: "落在「高風險」級距5–6。5-6%,高風險,在弱市開局時耗盡機率明顯上升,建議重新檢視。", en: "Falls in the \"High risk\" band (5–6). This is the high risk range for Safe Withdrawal Rate Calculator." } },
  { key: "executive", range: "≥ 6", label: { zh: "極危險 (≥ 6)", en: "Very risky (≥ 6)" }, desc: { zh: "落在「極危險」級距≥ 6。> 6%,極危險,長期難以支撐,資產很可能在退休中段前耗盡。", en: "Falls in the \"Very risky\" band (≥ 6). This is the very risky range for Safe Withdrawal Rate Calculator." } },
] as const;

const affiliateItems: AffiliateItem[] = [
  { label: { zh: "退休金計算機", en: "Retirement Calculator" }, href: "/tools/finance/retirement-calculator" },
  { label: { zh: "401k 退休計算機", en: "401k Retirement Calculator" }, href: "/tools/finance/retirement-401k-calculator" },
...
const ui = {
  zh: {
    badge: "財務 · 安全提領率計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Safe Withdrawal Rate Calculator · 安全提領率計算機",
    subtitle: "輸入退休投資組合、提領率與報酬率,立即算出年度與每月可提領金額、期末餘額與耗盡風險",
    intro: "本工具為 安全提領率計算機，依公開公式於瀏覽器端試算，輸入退休投資組合、年度提領率(%)、預期年報酬率(%)、退休年數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算安全提領率計算機",
    examplePreview: "提領率",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入退休投資組合、年度提領率(%)、預期年報酬率(%)、退休年數",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "組合 1000 萬 · 提領 4% · 報酬 5%",
    baselineExampleNote: "退休投資組合 10000000 · 年度提領率(%) 4",
    activeExample: "進階範例",
    activeExampleValue: "組合 800 萬 · 提領 6% · 報酬 4%",
    activeExampleNote: "退休投資組合 加倍 · 觀察 提領率 變化",
    flowDemo: "數字流向示範",
    calculator: "安全提領率計算機",
    retirementPortfolio: "退休投資組合",
    annualWithdrawalRate: "年度提領率(%)",
    expectedAnnualReturn: "預期年報酬率(%)",
    retirementYears: "退休年數",
    resultCard: "結果卡片",
    primaryValue: "提領率",
    primaryUnitTail: "%",
    secondaryLabel: "年度提領金額",
    secondaryTail: "$",
    metricALabel: "提領率",
    ...
    metricCTail: "$",
    headlineCaption: "安全提領率計算機 · 即時試算",
    fatLossTarget: "期末餘額",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "安全提領率計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 退休投資組合 與 預期年報酬率(%) 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "安全提領率計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 退休投資組合、年度提領率(%)、預期年報酬率(%)、退休年數 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依投資組合、提領率、報酬率與年數計算年度與每月提領金額、實質淨成長與耗盡風險。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "知識庫",
    knowledgeTitle: "安全提領率計算機 · 觀念整理",
    definition: "定義",
    definitionText: "安全提領率計算機以退休投資組合、年度提領率、預期年報酬率與退休年數,計算可持續的年度與每月提領金額、期末餘額與是否耗盡,協助規劃退休現金流並評估 4% 法則在你情境下的可行性。",
    formula: "公式",
    formulaText: "年度提領 = 投資組合 × 提領率;每月提領 = 年度提領 ÷ 12;逐年餘額 = 前一年餘額 ×(1+報酬率) − 年度提領,任一年 ≤ 0 即視為耗盡。",
    limitations: "限制",
    limitationsText: "本工具假設報酬率固定、提領金額固定且不含通膨調整與稅負;實際市場報酬逐年波動,弱市開局(序列風險)會顯著影響存活機率。",
    interpretation: "解讀",
    interpretationText: "提領率越低越安全;3-4% 為經典穩健區間,5% 以上需搭配彈性支出與較高報酬,並在空頭年份主動減提以延長資產壽命。",
    context: "情境",
    contextText: "常見使用情境包括日常財務檢視、年度規劃、重大決策前的快速估算。建議搭配退休金計算機 等延伸工具一起使用。",
    example: "範例",
    ...
    premiumTitle: "專業版 退休提領規劃",
    premiumText: "解鎖通膨調整提領、Monte Carlo 報酬序列模擬、動態提領策略、稅務最佳化與多情境存活率分析。",
    premiumChips_zh: "通膨調整|Monte Carlo|動態提領|存活率",
    premiumChips_en: "Inflation-adj|Monte Carlo|Dynamic|Survival",
    ...
    q1: "什麼是安全提領率?",
    a1: "安全提領率(Safe Withdrawal Rate, SWR)指退休後每年從投資組合提領、又能在預期年數內不耗盡資產的比率。它把退休資產換算成可持續的年度與每月現金流,是規劃退休生活費最核心的指標之一。",
    q2: "4% 法則可靠嗎?",
    a2: "經典「4% 法則」源自 Trinity Study,指以投資組合 4% 為首年提領、其後隨通膨調整,在 30 年期間有很高機率不耗盡。它是實用起點,但受市場開局順序、資產配置與退休年數影響;本工具讓你自訂提領率、報酬率與年數試算,而非套單一數字。",
    q3: "提領率該設多少?",
    a3: "多數研究落在 3-4% 之間:追求保守或更長退休年數者可降到 3% 或以下;能接受彈性支出、退休年數較短者可略高。建議先用本工具看不同提領率下的期末餘額與是否耗盡,再決定。",
    q4: "報酬率怎麼估?",
    a4: "報酬率應使用扣除費用後的長期年化預期,而非單一年度高點。股債混合組合常見假設約 4-6%。本工具用固定報酬率簡化試算;實際報酬會逐年波動,弱市開局風險最大。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內以 JavaScript 完成,投資組合與提領率等資料不會傳送到任何伺服器。",
    q6: "可以模擬通膨嗎?",
    a6: "通膨調整提領、市場報酬序列模擬(Monte Carlo)、動態提領策略與多情境存活率屬於專業版功能,免費版聚焦固定報酬下的提領率與耗盡判斷。"
  },
  en: {
    badge: "Finance · Safe Withdrawal Rate Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Safe Withdrawal Rate Calculator",
    subtitle: "Enter your retirement portfolio, withdrawal rate, and return to see annual and monthly withdrawals, ending balance, and depletion risk",
    intro: "Safe Withdrawal Rate Calculator runs the standard formula in your browser. Enter retirement portfolio, annual withdrawal rate (%), expected annual return (%), retirement years to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Safe Withdrawal Rate Calculator",
    examplePreview: "Withdrawal Rate",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter retirement portfolio, annual withdrawal rate (%), expected annual return (%), retirement years",
    ...
    baselineExampleValue: "Portfolio 10M · 4% · Return 5%",
    baselineExampleNote: "Retirement Portfolio 10000000 · Annual Withdrawal Rate (%) 4",
    activeExample: "Advanced example",
    activeExampleValue: "Portfolio 8M · 6% · Return 4%",
    activeExampleNote: "Retirement Portfolio doubled · watch Withdrawal Rate react",
    flowDemo: "Data flow demo",
    calculator: "Safe Withdrawal Rate Calculator",
    retirementPortfolio: "Retirement Portfolio",
    annualWithdrawalRate: "Annual Withdrawal Rate (%)",
    expectedAnnualReturn: "Expected Annual Return (%)",
    retirementYears: "Retirement Years",
    resultCard: "Result card",
    primaryValue: "Withdrawal Rate",
    primaryUnitTail: "%",
    secondaryLabel: "Annual Withdrawal",
    secondaryTail: "$",
    metricALabel: "Withdrawal Rate",
    metricACaption: "Main figure from the standard formula",
    metricATail: "%",
    metricBLabel: "Annual Withdrawal",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "$",
    metricCLabel: "Monthly Withdrawal",
    metricCCaption: "Percentage view",
    metricCTail: "$",
    headlineCaption: "Safe Withdrawal Rate Calculator · live calc",
    fatLossTarget: "Ending Balance",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Safe Withdrawal Rate Calculator · band matrix",
    ...
    dailyGap: "Daily gap",
    tertiaryTag: "Monthly Withdrawal",
    ...
    nextActionItem1: "Move Retirement Portfolio and Expected Annual Return (%) by ±10% to see sensitivity.",
    ...
    decisionTitle: "Safe Withdrawal Rate Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill retirement portfolio, annual withdrawal rate (%), expected annual return (%), retirement years.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "Safe Withdrawal Rate Calculator standard formula.",
    ...
    knowledge: "Knowledge",
    knowledgeTitle: "Safe Withdrawal Rate Calculator · concept primer",
    definition: "Definition",
    definitionText: "Safe Withdrawal Rate Calculator converts inputs (retirement portfolio, annual withdrawal rate (%), expected annual return (%), retirement years) into Withdrawal Rate. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(retirement portfolio, annual withdrawal rate (%), expected annual return (%), retirement years)",
    ...
    premiumTitle: "Pro Withdrawal Planner",
    premiumText: "Unlock inflation-adjusted withdrawals, Monte Carlo return-sequence simulation, dynamic withdrawal strategies, tax optimization, and multi-scenario survival analysis.",
    premiumChips_zh: "通膨調整|Monte Carlo|動態提領|存活率",
    premiumChips_en: "Inflation-adj|Monte Carlo|Dynamic|Survival",
    ...
    q1: "What does Safe Withdrawal Rate Calculator calculate?",
    a1: "Safe Withdrawal Rate Calculator applies the standard formula to your inputs and returns Withdrawal Rate plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Safe Withdrawal Rate Calculator?",
    a2: "Enter retirement portfolio, annual withdrawal rate (%), expected annual return (%), retirement years. Safe Withdrawal Rate Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock inflation-adjusted withdrawals, Monte Carlo return-sequence simulation, dynamic withdrawal strategies, tax optimization, and multi-scenario survival analysis."
  },
} as const;
...
export default function WithdrawalRateCalculator() {
  ...
  const [retirementPortfolio, setRetirementPortfolio] = useState("10000000");
  const [annualWithdrawalRate, setAnnualWithdrawalRate] = useState("4");
  const [expectedAnnualReturn, setExpectedAnnualReturn] = useState("5");
  const [retirementYears, setRetirementYears] = useState("30");
  const t = ui[lang];

  const result = useMemo(() => {
    const portfolio = Number(retirementPortfolio) || 0;
    const rate = Number(annualWithdrawalRate) || 0;
    const ret = Number(expectedAnnualReturn) || 0;
    const years = Number(retirementYears) || 1;
    const annualWithdrawal = portfolio * (rate / 100);
    const monthlyWithdrawal = annualWithdrawal / 12;
    const realGrowth = ret - rate;
    let bal = portfolio;
    let depleted = years;
    let survived = true;
    for (let y = 1; y <= years; y++) {
      bal = bal * (1 + ret / 100) - annualWithdrawal;
      if (bal <= 0) { depleted = y; survived = false; break; }
    }
    const endBalance = survived ? bal : 0;
    return { withdrawalRate: rate, annualWithdrawal, monthlyWithdrawal, endBalance };
  }, [retirementPortfolio, annualWithdrawalRate, expectedAnnualReturn, retirementYears]);

  const primaryDisplay = fmt(result.withdrawalRate, 1);
  const secondaryDisplay = fmt(result.annualWithdrawal, 0);
  const tertiaryDisplay = fmt(result.monthlyWithdrawal, 0);
  const quaternaryDisplay = fmt(result.endBalance, 0);

  function fillSolid() { setUnit("metric"); setRetirementPortfolio("10000000"); setAnnualWithdrawalRate("4"); setExpectedAnnualReturn("5"); setRetirementYears("30"); }
  function fillHighSalary() { setUnit("imperial"); setRetirementPortfolio("8000000"); setAnnualWithdrawalRate("6"); setExpectedAnnualReturn("4"); setRetirementYears("35"); }

  const activeBand = bands.find(b => {
    const r = result.withdrawalRate;
    if (r < 2.5) return 'tiny';
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ccfbf1,_#f8fafc_45%,_#cffafe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-teal-100 bg-white/90 p-6 shadow-2xl shadow-teal-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-teal-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-teal-600 p-5 text-white"><div className="text-xs font-bold uppercase text-teal-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-teal-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{retirementPortfolio} × {annualWithdrawalRate}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-teal-200 bg-teal-50 px-5 py-4 text-sm font-black text-teal-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.retirementPortfolio}<input type="number" step="100000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={retirementPortfolio} onChange={(e) => setRetirementPortfolio(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.annualWithdrawalRate}<input type="number" step="0.1" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={annualWithdrawalRate} onChange={(e) => setAnnualWithdrawalRate(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.expectedAnnualReturn}<input type="number" step="0.1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={expectedAnnualReturn} onChange={(e) => setExpectedAnnualReturn(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.retirementYears}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={retirementYears} onChange={(e) => setRetirementYears(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="withdrawal-rate-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="withdrawal-rate-calculator-faq" position="inline" /></section>
