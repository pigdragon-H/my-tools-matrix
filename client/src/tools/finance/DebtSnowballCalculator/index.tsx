// @profile B
// Profile B · 計算機-YMYL · DebtSnowballCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const bands = [
  { key: "tiny", range: "< 5", label: { zh: "極低 (< 5)", en: "Very low (< 5)" }, desc: { zh: "落在「極低」級距< 5。利息佔債務 < 5%,還款迅速,利率低或還款力道強,負擔極輕。", en: "Falls in the \"Very low\" band (< 5). This is the very low range for Debt Snowball Calculator." } },
  { key: "normal", range: "5–12", label: { zh: "低 (5–12)", en: "Low (5–12)" }, desc: { zh: "落在「低」級距5–12。5-12%,低,健康還款節奏,1-2 年內可清。", en: "Falls in the \"Low\" band (5–12). This is the low range for Debt Snowball Calculator." } },
  { key: "notable", range: "12–25", label: { zh: "中等 (12–25)", en: "Moderate (12–25)" }, desc: { zh: "落在「中等」級距12–25。12-25%,中等,屬一般信貸或卡債分期區間。", en: "Falls in the \"Moderate\" band (12–25). This is the moderate range for Debt Snowball Calculator." } },
  { key: "high", range: "25–45", label: { zh: "偏高 (25–45)", en: "Elevated (25–45)" }, desc: { zh: "落在「偏高」級距25–45。25-45%,偏高,還款拖長或利率偏高,建議加大還款。", en: "Falls in the \"Elevated\" band (25–45). This is the elevated range for Debt Snowball Calculator." } },
  { key: "major", range: "45–80", label: { zh: "高 (45–80)", en: "High (45–80)" }, desc: { zh: "落在「高」級距45–80。45-80%,高,利息侵蝕嚴重,優先處理高利債並考慮整合。", en: "Falls in the \"High\" band (45–80). This is the high range for Debt Snowball Calculator." } },
  { key: "executive", range: "≥ 80", label: { zh: "極高 (≥ 80)", en: "Very high (≥ 80)" }, desc: { zh: "落在「極高」級距≥ 80。> 80%,極高,接近最低還款陷阱,務必重整或協商。", en: "Falls in the \"Very high\" band (≥ 80). This is the very high range for Debt Snowball Calculator." } },
] as const;
...
const ui = {
  zh: {
    badge: "財務 · 債務雪球計算機 · 黃金工具",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Debt Snowball Calculator · 債務雪球計算機",
    subtitle: "輸入總債務、平均利率與每月可還,立即算出還清月數、總利息與利息佔債務比",
    intro: "本工具為 債務雪球計算機，依公開公式於瀏覽器端試算，輸入總債務、平均年利率%、每月可還、債務筆數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算債務雪球計算機",
    examplePreview: "利息佔債務比",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入總債務、平均年利率%、每月可還、債務筆數",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "債 50 萬 · 12% · 月還 2 萬",
    baselineExampleNote: "總債務 500000 · 平均年利率% 12",
    activeExample: "進階範例",
    activeExampleValue: "債 80 萬 · 18% · 月還 1.8 萬",
    activeExampleNote: "總債務 加倍 · 觀察 利息佔債務比 變化",
    flowDemo: "數字流向示範",
    calculator: "債務雪球計算機",
    totalDebt: "總債務",
    averageRatePct: "平均年利率%",
    monthlyPayment: "每月可還",
    numberOfDebts: "債務筆數",
    resultCard: "結果卡片",
    primaryValue: "利息佔債務比",
    ...
    metricCTail: "$",
    headlineCaption: "債務雪球計算機 · 即時試算",
    fatLossTarget: "還款總額",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "債務雪球計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 總債務 與 每月可還 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "債務雪球計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 總債務、平均年利率%、每月可還、債務筆數 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依總債務、利率與每月可還計算還清月數、總利息與利息佔債務比。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "工具知識庫",
    knowledgeTitle: "債務雪球計算機 · 觀念整理",
    definition: "定義",
    definitionText: "債務雪球計算機以總債務、平均年利率與每月還款金額,估算還清所需月數、累計利息與利息佔債務比,協助規劃債務清償策略並維持還款動力。",
    formula: "公式",
    formulaText: "每月:利息 = 餘額 × 月利率,餘額 = 餘額 + 利息 − 每月還款;重複至餘額歸零得還清月數與總利息",
    limitations: "限制",
    limitationsText: "本工具以平均利率與固定月還款估算整體時程,不逐筆模擬雪球順序;若每月還款低於利息,將永遠無法還清。",
    interpretation: "解讀",
    interpretationText: "利息佔債務比越低代表還款效率越高;加大每月還款或優先處理高利債可顯著縮短還清時間。",
    context: "情境",
    contextText: "常見使用情境包括日常財務檢視、年度規劃、重大決策前的快速估算。建議搭配債務還清計算機 等延伸工具一起使用。",
    example: "範例",
    ...
    premiumTitle: "專業版 債務清償策略",
    premiumText: "解鎖逐筆債務排序、雪球 vs 雪崩比較、額外還款情境模擬、整合貸款試算與還清排程匯出。",
    premiumChips_zh: "逐筆排序|雪球雪崩|額外還款|排程匯出",
    premiumChips_en: "Per-debt|Snowball/Avalanche|Extra Pay|Schedule",
    ...
    q1: "雪球法和雪崩法差在哪?",
    a1: "**雪球法**先還餘額最小的債務,快速清掉一筆建立成就感與動力;**雪崩法**先還利率最高的,數學上最省利息。雪球法心理效果強、堅持率高;雪崩法省錢但需要紀律。本工具估算整體還清時程。",
    q2: "為什麼雪球法有效?",
    a2: "雪球法的威力在於行為心理:每還清一筆就釋放該筆的最低還款,滾入下一筆形成「雪球」,加上成就感維持動力,提高長期堅持率。對容易半途而廢的人,堅持到底往往比省一點利息更重要。",
    q3: "該先存錢還是先還債?",
    a3: "原則上先建立 1 個月的小額緊急金(避免再借新債),然後全力還高利債(尤其卡債),還清後再補滿 3-6 個月緊急備用金並開始投資。高利債的「報酬」(省下的利息)通常高於投資。",
    q4: "可以提前還清嗎?",
    a4: "可以,且強烈建議。提前還款直接減少本金,後續利息按新餘額重算,越早還、省越多。任何額外收入(獎金、退稅)優先投入高利債,能大幅縮短還清時間。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內以 JavaScript 完成,債務與利率等資料不會傳送到任何伺服器。",
    q6: "可以逐筆排序嗎?",
    a6: "逐筆債務排序、雪球 vs 雪崩比較、額外還款情境與還清排程匯出屬於專業版功能,免費版提供整體還清試算。"
  },
  en: {
    badge: "Finance · Debt Snowball Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Debt Snowball Calculator",
    subtitle: "Enter total debt, average rate, and monthly payment to see months to clear, total interest, and interest-to-debt ratio",
    intro: "Debt Snowball Calculator runs the standard formula in your browser. Enter total debt, average rate pct, monthly payment, number of debts to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Debt Snowball Calculator",
    examplePreview: "Interest-to-Debt",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter total debt, average rate pct, monthly payment, number of debts",
    ...
    baselineExampleValue: "500k · 12% · 20k/mo",
    baselineExampleNote: "Total Debt 500000 · Average Rate Pct 12",
    activeExample: "Advanced example",
    activeExampleValue: "800k · 18% · 18k/mo",
    activeExampleNote: "Total Debt doubled · watch Interest-to-Debt react",
    flowDemo: "Data flow demo",
    calculator: "Debt Snowball Calculator",
    totalDebt: "Total Debt",
    averageRatePct: "Average Rate Pct",
    monthlyPayment: "Monthly Payment",
    numberOfDebts: "Number of Debts",
    resultCard: "Result card",
    primaryValue: "Interest-to-Debt",
    primaryUnitTail: "%",
    secondaryLabel: "Months to Clear",
    secondaryTail: "mo",
    metricALabel: "Interest-to-Debt",
    metricACaption: "Main figure from the standard formula",
    metricATail: "%",
    metricBLabel: "Months to Clear",
    metricBCaption: "Secondary metric tied to the primary",
    metricBTail: "mo",
    metricCLabel: "Total Interest",
    metricCCaption: "Percentage view",
    metricCTail: "$",
    headlineCaption: "Debt Snowball Calculator · live calc",
    fatLossTarget: "Total Paid",
    resultIntelligence: "Result intelligence",
    tdeeMatrix: "Debt Snowball Calculator · band matrix",
    ...
    nextActionItem1: "Move Total Debt and Monthly Payment by ±10% to see sensitivity.",
    ...
    decisionTitle: "Debt Snowball Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill total debt, average rate pct, monthly payment, number of debts.",
    deficitStep: "Step 2 · Apply formula",
    deficitNote: "Debt Snowball Calculator standard formula.",
    ...
    knowledgeTitle: "Debt Snowball Calculator · concept primer",
    definition: "Definition",
    definitionText: "Debt Snowball Calculator converts inputs (total debt, average rate pct, monthly payment, number of debts) into Interest-to-Debt. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "result = f(total debt, average rate pct, monthly payment, number of debts)",
    ...
    context: "Context",
    contextText: "Common contexts include daily finance review, annual planning, and pre-decision quick estimates. Pair with Debt Payoff Calculator for a fuller picture.",
    example: "Example",
    ...
    premiumTitle: "Pro Debt Payoff Strategy",
    premiumText: "Unlock per-debt ordering, snowball vs avalanche compare, extra-payment scenarios, consolidation simulation, and payoff schedule export.",
    premiumChips_zh: "逐筆排序|雪球雪崩|額外還款|排程匯出",
    premiumChips_en: "Per-debt|Snowball/Avalanche|Extra Pay|Schedule",
    ...
    q1: "What does Debt Snowball Calculator calculate?",
    a1: "Debt Snowball Calculator applies the standard formula to your inputs and returns Interest-to-Debt plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Debt Snowball Calculator?",
    a2: "Enter total debt, average rate pct, monthly payment, number of debts. Debt Snowball Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock per-debt ordering, snowball vs avalanche compare, extra-payment scenarios, consolidation simulation, and payoff schedule export."
  },
} as const;
...
export default function DebtSnowballCalculator() {
  ...
  const [averageRatePct, setAverageRatePct] = useState("12");
  const [monthlyPayment, setMonthlyPayment] = useState("20000");
  const [numberOfDebts, setNumberOfDebts] = useState("4");
  const t = ui[lang];
  ...
  }, [totalDebt, averageRatePct, monthlyPayment, numberOfDebts]);

  const primaryDisplay = fmt(result.interestRatio, 1);
  const secondaryDisplay = fmt(result.monthsToClear, 0);
  const tertiaryDisplay = fmt(result.totalInterest, 0);
  const quaternaryDisplay = fmt(result.totalPaid, 0);

  function fillSolid() { setUnit("metric"); setTotalDebt("500000"); setAverageRatePct("12"); setMonthlyPayment("20000"); setNumberOfDebts("4"); }
  function fillHighSalary() { setUnit("imperial"); setTotalDebt("800000"); setAverageRatePct("18"); setMonthlyPayment("18000"); setNumberOfDebts("6"); }
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#ffedd5,_#fff7ed_45%,_#fef3c7)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-orange-100 bg-white/90 p-6 shadow-2xl shadow-orange-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-orange-600 p-5 text-white"><div className="text-xs font-bold uppercase text-orange-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-orange-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{totalDebt} × {averageRatePct}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-orange-200 bg-orange-50 px-5 py-4 text-sm font-black text-orange-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-emerald-700">{t.totalDebt}<input type="number" step="10000" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={totalDebt} onChange={(e) => setTotalDebt(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.averageRatePct}<input type="number" step="0.5" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={averageRatePct} onChange={(e) => setAverageRatePct(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.monthlyPayment}<input type="number" step="1000" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={monthlyPayment} onChange={(e) => setMonthlyPayment(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.numberOfDebts}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={numberOfDebts} onChange={(e) => setNumberOfDebts(e.target.value)} /></label></div></div>
          </div>
        </section>
        ...
        <AdSenseWrapper showAds={true} adSlot="debt-snowball-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="debt-snowball-calculator-faq" position="inline" /></section>
