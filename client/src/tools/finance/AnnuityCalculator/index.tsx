...
const ui = {
  zh: {
    ...
    intro: "本工具為 年金計算機，依公開公式於瀏覽器端試算，輸入每期投入、年利率%、年數、每年期數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在您的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在您的瀏覽器內完成,投入金額與利率資料不會傳送到任何伺服器。",
    q6: "可以算遞延年金嗎?",
  ...
  en: {
    ...
    deficitNote: "Annuity Calculator standard formula.",
    ...
    formula: "Formula",
    formulaText: "result = f(payment per period, annual rate pct, years, periods per year)",
    ...
    q1: "What does Annuity Calculator calculate?",
    a1: "Annuity Calculator applies the standard formula to your inputs and returns Compound Growth plus three supporting metrics, all computed in your browser.",
    q2: "Which inputs do I need for Annuity Calculator?",
    a2: "Enter payment per period, annual rate pct, years, periods per year. Annuity Calculator runs the standard formula client-side and updates instantly as you type.",
    ...
    a6: "Unlock annuity-due toggle, deferred annuity, payout simulation, fee-adjusted net return, and inflation adjustment."
  },
} as const;
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#dcfce7,_#f8fafc_45%,_#d1fae5)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-green-100 bg-white/90 p-6 shadow-2xl shadow-green-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-green-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-green-600 p-5 text-white"><div className="text-xs font-bold uppercase text-green-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div><div className="text-sm font-bold text-green-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}<span>{t.primaryUnitTail}</span></div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{paymentPerPeriod} × {annualRatePct}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-green-200 bg-green-50 px-5 py-4 text-sm font-black text-green-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
