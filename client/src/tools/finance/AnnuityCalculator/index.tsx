// @profile B
// Profile B · 計算機-YMYL · AnnuityCalculator（GOLD-STANDARD-001 compatible · clone of MeetingCostCalculator）
...
const affiliateItems: AffiliateItem[] = [
  ...
  { label: { zh: "退休金計算機", en: "Pension Calculator" }, href: "/tools/finance/pension-calculator" },
];
...
const ui = {
  zh: {
    ...
    chineseShort: "中",
    englishShort: "EN",
    title: "Annuity Calculator · 年金計算機",
    subtitle: "輸入每期投入、利率與年數，立即算出年金終值、總投入與利息收益",
    intro: "本工具為 年金計算機，依公開公式於瀏覽器端試算，輸入每期投入、年利率%、年數、每年期數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    trustNote: "本工具僅供一般情境試算，未涵蓋極端條件、稅務優惠、地區差異或特殊規範。涉及重大決策請以合格專業人士為準。",
    quickActionCard: "快速範例卡",
    tryExample: "試算年金計算機",
    examplePreview: "複利成長率",
    examplePerson: "標準範例",
    fillExample: "一鍵填入標準範例",
    previewActivePath: "填入進階範例",
    examplesCalculator: "範例 → 計算機",
    enterValues: "輸入每期投入、年利率%、年數、每年期數",
    examplesHelper: "先用範例了解結果輸出，再改成自己的數字。",
    metric: "標準",
    imperial: "進階",
    exampleCards: "範例卡",
    baselineExample: "標準範例",
    baselineExampleValue: "每月 1000 · 6% · 20 年",
    baselineExampleNote: "每期投入 1000 · 年利率% 6",
    activeExample: "進階範例",
    activeExampleValue: "每月 500 · 8% · 30 年",
    activeExampleNote: "每期投入 加倍 · 觀察 複利成長率 變化",
    flowDemo: "數字流向示範",
    calculator: "年金計算機",
    paymentPerPeriod: "每期投入",
    annualRatePct: "年利率%",
    years: "年數",
    periodsPerYear: "每年期數",
    resultCard: "結果卡片",
    ...
    metricCTail: "$",
    headlineCaption: "年金計算機 · 即時試算",
    fatLossTarget: "總投入",
    resultIntelligence: "結果解讀",
    tdeeMatrix: "年金計算機 · 級距矩陣",
    ...
    nextActionItem1: "把 每期投入 與 年數 各調 ±10% 觀察主要結果敏感度",
    ...
    decisionPath: "決策路徑",
    decisionTitle: "年金計算機 · 決策四步",
    bmrStep: "Step 1 · 蒐集參數",
    bmrNote: "先把 每期投入、年利率%、年數、每年期數 四個欄位填齊。",
    deficitStep: "Step 2 · 套公式",
    deficitNote: "依每期投入、利率、年數與期數計算年金終值、總投入與利息收益。",
    trendStep: "Step 3 · 看級距",
    trendNote: "對照六格級距,定位主要結果。",
    mealStep: "Step 4 · 行動",
    mealNote: "依級距提示挑一個下一步,執行 30 天後回來重算。",
    knowledge: "知識庫",
    knowledgeTitle: "年金計算機 · 觀念整理",
    definition: "定義",
    definitionText: "年金計算機以每期投入、年利率、年數與每年期數,計算累積型年金的終值、現值、總投入與利息收益,適用退休儲蓄、定期定額與年金商品試算。",
    formula: "公式",
    formulaText: "年金終值 = 每期投入 × ((1 + i)^N − 1) / i;年金現值 = 每期投入 × (1 − (1 + i)^−N) / i;i = 年利率 / 期數,N = 年數 × 期數",
    limitations: "限制",
    limitationsText: "本工具採期末年金與固定利率,不含商品費用、解約成本、稅負與期初年金;保險年金請另扣費用,實際請以保單條款為準。",
    interpretation: "解讀",
    interpretationText: "年金終值反映長期定期投入的複利成果;利息收益佔比越高,代表時間與複利貢獻越大,越早開始越有利。",
    context: "情境",
    ...
    premiumTitle: "專業版 年金規劃工具組",
    premiumText: "解鎖期初/期末年金切換、遞延年金、給付型提領模擬、含費用淨報酬與通膨調整。",
    premiumChips_zh: "期初期末|遞延年金|提領模擬|淨報酬",
    premiumChips_en: "Due/Ordinary|Deferred|Payout|Net Return",
    ...
    q1: "年金是什麼?有哪些種類?",
    a1: "**年金(Annuity)**是一連串等額、定期的現金流。常見種類:**(1) 累積型**(定期投入、未來領回,如退休儲蓄)、**(2) 給付型**(一次投入、定期領取,如退休年金)、**(3) 固定 vs 變額**(報酬固定或隨投資波動)。本工具計算累積型的終值與現值。",
    q2: "期初年金和期末年金差在哪?",
    a2: "**期末年金(ordinary annuity)**在每期「期末」付款(如房貸、多數儲蓄),本工具預設此類;**期初年金(annuity due)**在每期「期初」付款(如租金、保費),因每筆早一期生息,終值較高(乘以 1+i)。差異隨利率與期數放大。",
    q3: "年金商品費用高嗎?",
    a3: "保險公司的年金商品常含**前置費用、管理費、解約費、附約成本**,可能侵蝕 1-3% 年報酬。購買前務必看清費用結構與保證內容。本工具算的是純數學年金,實際商品報酬需扣除費用。",
    q4: "定期定額和年金一樣嗎?",
    a4: "定期定額投資(如每月買 ETF/基金)本質上就是累積型年金的應用——固定金額、固定頻率投入。差別在年金常指保險商品(有保證但費用高),而定期定額多指自行投資(彈性高但無保證)。計算邏輯相同。",
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內完成,投入金額與利率資料不會傳送到任何伺服器。",
    q6: "可以算遞延年金嗎?",
    a6: "期初/期末年金切換、遞延年金、給付型年金提領與含費用的淨報酬屬於專業版功能。"
  },
  en: {
    badge: "Finance · Annuity Calculator · Gold Tool",
    switchToEnglish: "English mode",
    switchToChinese: "切換到中文",
    chineseShort: "中",
    englishShort: "EN",
    title: "Annuity Calculator · 年金計算機",
    subtitle: "Enter payment per period, rate, and years to compute annuity future value, contributions, and interest",
    intro: "Annuity Calculator runs the standard formula in your browser. Enter payment per period, annual rate pct, years, periods per year to see the primary result and three supporting metrics. Nothing is uploaded.",
    ...
    tryExample: "Try Annuity Calculator",
    examplePreview: "Compound Growth",
    examplePerson: "Standard example",
    fillExample: "Fill standard example",
    previewActivePath: "Fill advanced example",
    examplesCalculator: "Examples → Calculator",
    enterValues: "Enter payment per period, annual rate pct, years, periods per year",
    ...
    baselineExampleValue: "1000/mo · 6% · 20y",
    baselineExampleNote: "Payment Per Period 1000 · Annual Rate Pct 6",
    activeExample: "Advanced example",
    activeExampleValue: "500/mo · 8% · 30y",
    activeExampleNote: "Payment Per Period doubled · watch Compound Growth react",
    ...
    nextActionItem1: "Move Payment Per Period and Years by ±10% to see sensitivity.",
    ...
    decisionTitle: "Annuity Calculator · 4-step decision",
    bmrStep: "Step 1 · Gather inputs",
    bmrNote: "Fill payment per period, annual rate pct, years, periods per year.",
    ...
    knowledge: "Knowledge",
    knowledgeTitle: "Annuity Calculator · concept primer",
    definition: "Definition",
    definitionText: "Annuity Calculator converts inputs (payment per period, annual rate pct, years, periods per year) into Compound Growth. It is widely used in personal finance and investment planning.",
    formula: "Formula",
    formulaText: "年金終值 = 每期投入 × ((1 + i)^N − 1) / i;年金現值 = 每期投入 × (1 − (1 + i)^−N) / i;i = 年利率 / 期數,N = 年數 × 期數",
    ...
    premiumTitle: "Pro Annuity Planning Suite",
    premiumText: "Unlock annuity-due toggle, deferred annuity, payout simulation, fee-adjusted net return, and inflation adjustment.",
    premiumChips_zh: "期初期末|遞延年金|提領模擬|淨報酬",
    premiumChips_en: "Due/Ordinary|Deferred|Payout|Net Return",
    ...
    q1: "About: 年金是什麼?有哪些種類?",
    a1: "For \"年金是什麼?有哪些種類?\": Annuity Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 期初年金和期末年金差在哪?",
    a2: "For \"期初年金和期末年金差在哪?\": Annuity Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 年金商品費用高嗎?",
    a3: "For \"年金商品費用高嗎?\": Annuity Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 定期定額和年金一樣嗎?",
    a4: "For \"定期定額和年金一樣嗎?\": Annuity Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳嗎?",
    a5: "For \"資料會上傳嗎?\": Annuity Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 可以算遞延年金嗎?",
    a6: "For \"可以算遞延年金嗎?\": Annuity Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
...
export default function AnnuityCalculator() {
  ...
  const [paymentPerPeriod, setPaymentPerPeriod] = useState("1000");
  ...
  const result = useMemo(() => {
    const pmt = Number(paymentPerPeriod) || 0;
  ...
  }, [paymentPerPeriod, annualRatePct, years, periodsPerYear]);
  ...
  function fillSolid() { setUnit("metric"); setPaymentPerPeriod("1000"); setAnnualRatePct("6"); setYears("20"); setPeriodsPerYear("12"); }
  function fillHighSalary() { setUnit("imperial"); setPaymentPerPeriod("500"); setAnnualRatePct("8"); setYears("30"); setPeriodsPerYear("12"); }

  const activeBand = bands.find(b => {
    const r = result.growthPct;
    if (r < 15) return 'tiny';
    if (r < 40) return 'normal';
    if (r < 80) return 'notable';
    if (r < 150) return 'high';
    if (r < 300) return 'major';
    return 'executive';
  });
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_#dcfce7,_#f8fafc_45%,_#d1fae5)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-green-100 bg-white/90 p-6 shadow-2xl shadow-green-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-green-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-green-600 p-5 text-white"><div className="text-xs font-bold uppercase text-green-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}%</div><div className="text-sm font-bold text-green-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{paymentPerPeriod} × {annualRatePct}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-green-200 bg-green-50 px-5 py-4 text-sm font-black text-green-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5"><h3 className="text-lg font-black">{t.calculator}</h3><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="block text-sm font-black text-slate-700">{t.paymentPerPeriod}<input type="number" step="100" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={paymentPerPeriod} onChange={(e) => setPaymentPerPeriod(e.target.value)} /></label><label className="block text-sm font-black text-emerald-700">{t.annualRatePct}<input type="number" step="0.1" className="mt-2 w-full rounded-2xl border border-emerald-200 px-4 py-3 text-lg font-bold" value={annualRatePct} onChange={(e) => setAnnualRatePct(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.years}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={years} onChange={(e) => setYears(e.target.value)} /></label><label className="block text-sm font-black text-slate-700">{t.periodsPerYear}<input type="number" step="1" className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-lg font-bold" value={periodsPerYear} onChange={(e) => setPeriodsPerYear(e.target.value)} /></label></div></div>
          </div>
        </section>
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-green-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-green-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{primaryDisplay}<span className="text-3xl">%</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.primaryValue}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.secondaryLabel}</div><div className="mt-1 text-xl font-black">{secondaryDisplay}</div><div className="mt-1 text-xs text-slate-300">{t.secondaryTail}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.metricALabel}</div><div className="mt-1 text-xs font-black text-emerald-700">{t.metricACaption}</div><p className="mt-2 text-3xl font-black text-emerald-950">{tertiaryDisplay}</p><p className="text-sm font-bold text-emerald-700">{t.metricATail}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.metricBLabel}</div><div className="mt-1 text-xs font-black text-blue-700">{t.metricBCaption}</div><p className="mt-2 text-3xl font-black text-blue-950">{quaternaryDisplay}</p><p className="text-sm font-bold text-blue-700">{t.metricBTail}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.metricCLabel}</div><div className="mt-1 text-xs font-black text-slate-700">{t.metricCCaption}</div><p className="mt-2 text-3xl font-black text-slate-950">{secondaryDisplay}</p><p className="text-sm font-bold text-slate-700">{t.metricCTail}</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="annuity-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-green-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black">{primaryDisplay}%</div></div><div className="rounded-2xl bg-green-50 p-4"><div className="text-xs font-black uppercase text-green-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-green-950">{secondaryDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.tertiaryTag}</div><div className="mt-1 text-3xl font-black text-emerald-950">{tertiaryDisplay}</div></div></div></article>
        ...
        <section aria-label="L14 常見問題後廣告位:廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="annuity-calculator-faq" position="inline" /></section>
