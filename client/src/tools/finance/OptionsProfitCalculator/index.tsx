...
const ui = {
  zh: {
    ...
    intro: "本工具為 選擇權損益計算機，依公開公式於瀏覽器端試算，輸入履約價、權利金、標的價、口數後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    premiumTitle: "解鎖進階版",
    premiumText: "Premium 解鎖選擇權損益計算機的批次試算、結果歷史、PDF 匯出、多場景比較與廣告移除。",
    ...
    a1: "**乘數**是選擇權合約的「每點價值」。美股股票選擇權每口乘數 100(每點 = $100),指數選擇權如 SPX 為 100,台指選擇權每口為 50 元/點。本工具預設乘數 100,適用美股個股 + ETF;若你交易台指或其他標的,請把結果除以對應的乘數比例自行調整。",
    q2: "為什麼最大風險就是權利金?",
    ...
    q5: "損益資料會上傳到伺服器嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內以 JavaScript 完成,履約價、權利金、標的價等資料不會傳送到任何伺服器,也不會記錄到日誌或資料庫。",
    q6: "權利金高就是好的嗎?",
  ...
  en: {
    ...
    premiumText: "Premium unlocks batch calculation, history, PDF export, multi-scenario comparison, and ad-free for Options Profit Calculator.",
    premiumChips_zh: "批次試算 · 歷史紀錄 · PDF 匯出 · 廣告移除",
    premiumChips_en: "Batch · History · PDF Export · Ad-free",
    ...
    q1: "About: 選擇權的「乘數」(multiplier)是什麼?",
    a1: "For \"選擇權的「乘數」(multiplier)是什麼?\": Options Profit Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 為什麼最大風險就是權利金?",
    a2: "For \"為什麼最大風險就是權利金?\": Options Profit Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: Call 和 Put 怎麼選?",
    a3: "For \"Call 和 Put 怎麼選?\": Options Profit Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: OTM、ATM、ITM 是什麼意思?",
    a4: "For \"OTM、ATM、ITM 是什麼意思?\": Options Profit Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 損益資料會上傳到伺服器嗎?",
    a5: "For \"損益資料會上傳到伺服器嗎?\": Options Profit Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 權利金高就是好的嗎?",
    a6: "For \"權利金高就是好的嗎?\": Options Profit Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_#fae8ff,_#f8fafc_45%,_#fce7f3)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-fuchsia-100 bg-white/90 p-6 shadow-2xl shadow-fuchsia-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-fuchsia-600 p-5 text-white"><div className="text-xs font-bold uppercase text-fuchsia-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}$</div><div className="text-sm font-bold text-fuchsia-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}$</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{strikePrice} × {premium}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-fuchsia-200 bg-fuchsia-50 px-5 py-4 text-sm font-black text-fuchsia-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-fuchsia-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-fuchsia-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{primaryDisplay}<span className="text-3xl">$</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.primaryValue}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.secondaryLabel}</div><div className="mt-1 text-xl font-black">{secondaryDisplay}</div><div className="mt-1 text-xs text-slate-300">{t.secondaryTail}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.metricALabel}</div><div className="mt-1 text-xs font-black text-emerald-700">{t.metricACaption}</div><p className="mt-2 text-3xl font-black text-emerald-950">{tertiaryDisplay}</p><p className="text-sm font-bold text-emerald-700">{t.metricATail}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.metricBLabel}</div><div className="mt-1 text-xs font-black text-blue-700">{t.metricBCaption}</div><p className="mt-2 text-3xl font-black text-blue-950">{quaternaryDisplay}</p><p className="text-sm font-bold text-blue-700">{t.metricBTail}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.metricCLabel}</div><div className="mt-1 text-xs font-black text-slate-700">{t.metricCCaption}</div><p className="mt-2 text-3xl font-black text-slate-950">{secondaryDisplay}</p><p className="text-sm font-bold text-slate-700">{t.metricCTail}</p></div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-fuchsia-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black">{primaryDisplay}$</div></div><div className="rounded-2xl bg-fuchsia-50 p-4"><div className="text-xs font-black uppercase text-fuchsia-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-fuchsia-950">{secondaryDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.tertiaryTag}</div><div className="mt-1 text-3xl font-black text-emerald-950">{tertiaryDisplay}</div></div></div></article>
