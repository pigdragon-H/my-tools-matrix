// @profile B
// Profile B · 計算機-YMYL · Inflation Calculator - future cost and lost purchasing power over time（GOLD-STANDARD-001 compatible）
...
const bands = [
  { key: "tiny", range: "<10%", label: { zh: "影響輕微", en: "Minor impact" }, desc: { zh: "購買力流失有限,短期或低通膨情境常見。", en: "Limited loss of purchasing power, typical of short or low-inflation cases." } },
  { key: "normal", range: "10-25%", label: { zh: "明顯流失", en: "Noticeable" }, desc: { zh: "長期下購買力明顯縮水,儲蓄需考慮抗通膨。", en: "Purchasing power clearly shrinks over time; savings need inflation hedges." } },
  { key: "notable", range: "25-40%", label: { zh: "顯著流失", en: "Significant" }, desc: { zh: "超過三成購買力消失,投資報酬須跑贏通膨。", en: "Over a third of value erodes; returns must outpace inflation." } },
  { key: "high", range: "40-55%", label: { zh: "大幅流失", en: "Large erosion" }, desc: { zh: "購買力減半在即,現金部位風險升高。", en: "Value nearly halves; holding cash becomes riskier." } },
  { key: "major", range: "55-70%", label: { zh: "嚴重流失", en: "Severe" }, desc: { zh: "長期高通膨,務必以實質資產保值。", en: "Long-term high inflation; real assets are essential to preserve value." } },
  { key: "executive", range: "70%+", label: { zh: "極端流失", en: "Extreme" }, desc: { zh: "購買力幾乎被吞噬,需積極通膨對沖策略。", en: "Purchasing power is nearly wiped out; aggressive hedging is needed." } },
] as const;
...
const ui = {
  zh: {
    ...
    englishShort: "EN",
    title: "通膨計算機",
    subtitle: "依通膨率與年數,估算金錢未來等值成本與購買力流失。",
    intro: "輸入目前金額、年通膨率與年數,本工具以複利方式推算同樣的東西未來需要多少錢,以及今天的金額在未來剩下多少實質價值,幫你看清通膨對長期儲蓄的侵蝕。",
    trustNoteLabel: "注意事項:",
    ...
    calculator: "計算機",
    participants: "目前金額",
    averageHourlyRate: "年通膨率 (%)",
    durationHours: "年數",
    meetingsPerMonth: "參考調整 (%)",
    resultCard: "計算結果",
    unit: "主要結果",
    primaryValue: "主要數值",
  ...
  en: {
    ...
    subtitle: "Estimate the future cost of money and lost purchasing power from an inflation rate and time horizon.",
    intro: "Enter an amount today, the annual inflation rate and a number of years; the tool compounds inflation to show what the same goods will cost later and how much real value remains, revealing how inflation erodes long-term savings.",
    trustNoteLabel: "Note:",
    ...
    tryExample: "Try a inflation example",
    ...
    participants: "Amount today",
    averageHourlyRate: "Annual inflation (%)",
    durationHours: "Number of years",
    meetingsPerMonth: "Reference adjust (%)",
    resultCard: "Result",
    unit: "Primary result",
...
export default function InflationCalculator() {
  ...
  const [participants, setParticipants] = useState("1000000");
  const [averageHourlyRate, setAverageHourlyRate] = useState("3");
  ...
  const result = useMemo(() => {
    ...
    const amount = v1; const inflationRate = v2; const years = v3; const adj = v4;
    const r = (inflationRate + adj) / 100;
    const factor = Math.pow(1 + r, years);
    const futureCost = amount * factor;
    const realValue = years > 0 ? amount / factor : amount;
    const purchasingLoss = amount - realValue;
    const lossPct = amount > 0 ? (purchasingLoss / amount) * 100 : 0;
    return { futureCost, realValue, purchasingLoss, lossPct, amount, factor };

  }, [participants, averageHourlyRate, durationHours, meetingsPerMonth]);

  const meetingDisplay = fmt(result.futureCost, 0);
  const monthlyDisplay = fmt(result.realValue, 0);

  function fillSolid() { setUnit("metric"); setParticipants("1000000"); setAverageHourlyRate("3"); setDurationHours("20"); setMeetingsPerMonth("0"); }
  function fillHighSalary() { setUnit("imperial"); setParticipants("1000000"); setAverageHourlyRate("6"); setDurationHours("30"); setMeetingsPerMonth("0"); }

  const activeBand = bands.find(b => {
    const r = result.lossPct;
    if (r < 10) return b.key === "tiny";
    if (r < 25) return b.key === "normal";
    if (r < 40) return b.key === "notable";
    if (r < 55) return b.key === "high";
    if (r < 70) return b.key === "major";
      ...
      <section className="bg-[radial-gradient(circle_at_top_left,_linear-gradient(135deg,#fff1f2 0%,#ffe4e6 55%,#fce7f3 100%))]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-amber-100 bg-white/90 p-6 shadow-2xl shadow-amber-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-amber-600 p-5 text-white"><div className="text-xs font-bold uppercase text-amber-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">${meetingDisplay}</div><div className="text-sm font-bold text-amber-100">{lang === "zh" ? "未來等值金額" : "future equivalent"}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{fmt(result.lossPct, 1)}%</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">${fmt(result.purchasingLoss, 0)}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-black text-amber-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">{/* L5-Calc */}
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><h3 className="text-lg font-black">{t.exampleCards}</h3><div className="mt-4 space-y-3"><button onClick={fillSolid} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.baselineExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">2%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "溫和通膨" : "Mild inflation"}</p></button><button onClick={fillHighSalary} className="w-full rounded-2xl border border-amber-200 bg-white p-4 text-left"><div className="flex items-center justify-between gap-3"><span className="font-black">{t.activeExample}</span><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700">5%</span></div><p className="mt-2 text-sm text-slate-600">{lang === "zh" ? "高通膨情境" : "High-inflation case"}</p></button></div></div>
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-amber-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">${meetingDisplay}<span className="text-3xl">{lang === "zh" ? "元" : "TWD"}</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.estimatedTdee}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.monthlyEquiv}</div><div className="mt-1 text-xl font-black">${monthlyDisplay}</div><div className="mt-1 text-xs text-slate-300">{lang === "zh" ? "元" : "TWD"}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.weeklyEquiv}</div><div className="mt-1 text-xs font-black text-emerald-700">{lang === "zh" ? "未來等值金額" : "Future equivalent"}</div><p className="mt-2 text-3xl font-black text-emerald-950">${fmt(result.futureCost, 0)}</p><p className="text-sm font-bold text-emerald-700"></p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.dailyEquiv}</div><div className="mt-1 text-xs font-black text-blue-700">{lang === "zh" ? "今日實質價值" : "Real value today"}</div><p className="mt-2 text-3xl font-black text-blue-950">${fmt(result.realValue, 0)}</p><p className="text-sm font-bold text-blue-700"></p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.effectiveHours}</div><div className="mt-1 text-xs font-black text-slate-700">{lang === "zh" ? "購買力流失" : "Purchasing-power loss"}</div><p className="mt-2 text-3xl font-black text-slate-950">{fmt(result.lossPct, 1)}</p><p className="text-sm font-bold text-slate-700">%</p></div></div></div></article>
        ...
        <AdSenseWrapper showAds={true} adSlot="inflation-calculator-result-intelligence" adFormat="horizontal" className="my-2" />
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-amber-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{lang === "zh" ? "未來等值金額" : "future equivalent"}</div><div className="mt-1 text-3xl font-black">${meetingDisplay}</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-xs font-black uppercase text-amber-700">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black text-amber-950">${meetingDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-emerald-950">${monthlyDisplay}</div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-700">{t.decisionPath}</p><h2 className="mt-2 text-3xl font-black">{t.decisionTitle}</h2><div className="mt-6 grid gap-4 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] md:items-center">{[{ label: lang === "zh" ? "輸入金額與通膨率" : "Enter amount and inflation", note: t.bmrStep }, { label: lang === "zh" ? "設定年數" : "Set the number of years", note: t.deficitStep }, { label: lang === "zh" ? "複利計算通膨" : "Compound the inflation", note: t.trendStep }, { label: lang === "zh" ? "得未來與實質價值" : "Get future and real value", note: t.mealStep }].map((node, index) => <div key={node.label} className="contents"><div className={`rounded-3xl border p-5 text-center ${index === 0 ? "border-amber-300 bg-amber-50" : "border-blue-200 bg-blue-50"}`}><div className="text-xs font-black uppercase text-slate-500">{index + 1}</div><div className="mt-1 text-xl font-black">{node.label}</div><p className="mt-2 text-sm leading-6 text-slate-600">{node.note}</p></div>{index < 3 && <div className="hidden text-3xl font-black text-slate-300 md:block">→</div>}</div>)}</div></section>
        ...
        <section aria-label="L14 常見問題後廣告位：廣告位" className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm md:p-5"><AdSlot slot="inflation-calculator-faq" position="inline" /></section>
