...
export default function FireCalculator() {
  ...
  const result=useMemo<Output>(()=>{
    try{
      ...
      const toolSlug: string = "fire-calculator";
      switch(toolSlug){
  ...
  const copyResult=async()=>{ if(!result.main) return; await navigator.clipboard.writeText(`FIRE Calculator
${result.detail}
Formula: ${result.formula}`); setCopied(true); window.setTimeout(()=>setCopied(false),1500); };
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
    <section className="rounded-2xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950"><p className="text-sm font-semibold text-blue-600">FIN · RET</p><h1 className="text-3xl font-bold text-slate-950 dark:text-white">FIRE Calculator</h1><p className="mt-2 text-slate-600 dark:text-slate-300">財務自由計算器：年支出/儲蓄率/預期報酬率 → FIRE 年數。</p></section>
