...
const bands = [
  { key: "tiny", range: "< -100000", label: { zh: "租賃大省 (< -100000)", en: "Band 1 (< -100000)" }, desc: { zh: "落在「租賃大省」級距< -100000。租賃總成本遠低於購買淨成本,且省去殘值風險與維護,租賃明顯划算。", en: "Falls in the \"租賃大省\" band < -100000. This is the 租賃大省 range for Lease vs Buy Calculator." } },
  { key: "normal", range: "-100000–-10000", label: { zh: "租賃略省 (-100000–-10000)", en: "Band 2 (-100000–-10000)" }, desc: { zh: "落在「租賃略省」級距-100000–-10000。租賃略省,加上彈性與免維護,短期或不確定需求時租賃較佳。", en: "Falls in the \"租賃略省\" band -100000–-10000. This is the 租賃略省 range for Lease vs Buy Calculator." } },
  { key: "notable", range: "-10000–10000", label: { zh: "接近 (-10000–10000)", en: "Band 3 (-10000–10000)" }, desc: { zh: "落在「接近」級距-10000–10000。兩者接近,決策應看彈性、現金流、稅務與使用習慣等非價格因素。", en: "Falls in the \"接近\" band -10000–10000. This is the 接近 range for Lease vs Buy Calculator." } },
  { key: "high", range: "10000–100000", label: { zh: "購買略省 (10000–100000)", en: "Band 4 (10000–100000)" }, desc: { zh: "落在「購買略省」級距10000–100000。購買略省,若長期使用且殘值穩定,購買開始占優。", en: "Falls in the \"購買略省\" band 10000–100000. This is the 購買略省 range for Lease vs Buy Calculator." } },
  { key: "major", range: "100000–300000", label: { zh: "購買大省 (100000–300000)", en: "Band 5 (100000–300000)" }, desc: { zh: "落在「購買大省」級距100000–300000。購買大幅省,長期持有下擁有資產並回收殘值,購買明顯划算。", en: "Falls in the \"購買大省\" band 100000–300000. This is the 購買大省 range for Lease vs Buy Calculator." } },
  { key: "executive", range: "≥ 300000", label: { zh: "購買壓倒 (≥ 300000)", en: "Band 6 (≥ 300000)" }, desc: { zh: "落在「購買壓倒」級距≥ 300000。購買壓倒性勝出,租賃成本遠超購買淨成本,除非極度需要彈性否則應購買。", en: "Falls in the \"購買壓倒\" band ≥ 300000. This is the 購買壓倒 range for Lease vs Buy Calculator." } },
] as const;
...
const ui = {
  zh: {
    ...
    intro: "本工具為 租賃 vs 購買計算機，依公開公式於瀏覽器端試算，輸入購買總價、每月租金、持有年數、殘值後立即得出主要結果與三個輔助指標。所有計算均不上傳，僅在你的裝置完成。",
    trustNoteLabel: "注意事項：",
    ...
    q5: "資料會上傳嗎?",
    a5: "完全不會。所有計算都在你的瀏覽器內完成,價格與租金資料不會傳送到任何伺服器。",
    q6: "可以加上利息與稅務嗎?",
  ...
  en: {
    ...
    q1: "About: 租好還是買好?關鍵看什麼?",
    a1: "For \"租好還是買好?關鍵看什麼?\": Lease vs Buy Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q2: "About: 為什麼要算殘值?",
    a2: "For \"為什麼要算殘值?\": Lease vs Buy Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q3: "About: 租賃的隱藏成本有哪些?",
    a3: "For \"租賃的隱藏成本有哪些?\": Lease vs Buy Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q4: "About: 這個工具適用車、房、設備嗎?",
    a4: "For \"這個工具適用車、房、設備嗎?\": Lease vs Buy Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional.",
    q5: "About: 資料會上傳嗎?",
    a5: "For \"資料會上傳嗎?\": Lease vs Buy Calculator runs the standard formula client-side; no data leaves the browser. Use the band guidance shown next to the result for your next step.",
    q6: "About: 可以加上利息與稅務嗎?",
    a6: "For \"可以加上利息與稅務嗎?\": Lease vs Buy Calculator runs the standard formula client-side; no data leaves the browser. For unusual scenarios, consult a qualified professional."
  },
} as const;
      ...
      */}
      <section className="bg-[radial-gradient(circle_at_top_left,_#dbeafe,_#f8fafc_45%,_#e0f2fe)]">
        <div className="mx-auto max-w-7xl px-4 py-10 md:px-8 md:py-14">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">{/* L1-Hero */}
            ...
            <aside className="rounded-[2rem] border border-blue-100 bg-white/90 p-6 shadow-2xl shadow-blue-950/10 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">{t.quickActionCard}</p><h2 className="mt-2 text-2xl font-black">{t.tryExample}</h2><div className="mt-5 rounded-3xl bg-blue-600 p-5 text-white"><div className="text-xs font-bold uppercase text-blue-100">{t.examplePreview}</div><div className="mt-1 text-5xl font-black">{primaryDisplay}$</div><div className="text-sm font-bold text-blue-100">{t.headlineCaption}</div></div><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.examplePerson}</div><div className="font-black">{primaryDisplay}$</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.flowDemo}</div><div className="font-black">{purchasePrice} × {monthlyLease}</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.fatLossTarget}</div><div className="font-black">{secondaryDisplay}</div></div></div><button onClick={fillSolid} className="mt-5 w-full rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white">{t.fillExample}</button><button onClick={fillHighSalary} className="mt-3 w-full rounded-2xl border border-blue-200 bg-blue-50 px-5 py-4 text-sm font-black text-blue-900">{t.previewActivePath}</button></aside>
          </div>
        </div>
      </section>
      <div className="mx-auto max-w-7xl space-y-7 px-4 py-8 md:px-8">
        ...
        <section className="grid gap-7 lg:grid-cols-[0.95fr_1.05fr]">{/* L6-Result */}
          <article className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="h-5 bg-gradient-to-r from-blue-400 to-blue-500" /><div className="p-6 md:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-700">{t.resultCard}</p><div className="mt-4 flex items-start justify-between gap-5"><div><div className="text-7xl font-black tracking-tight text-slate-950">{primaryDisplay}<span className="text-3xl">$</span></div><div className="mt-2 rounded-full bg-slate-100 px-4 py-2 text-sm font-black text-slate-700">{t.primaryValue}</div></div><div className="rounded-3xl bg-slate-950 p-4 text-right text-white"><div className="text-xs font-bold uppercase text-slate-300">{t.secondaryLabel}</div><div className="mt-1 text-xl font-black">{secondaryDisplay}</div><div className="mt-1 text-xs text-slate-300">{t.secondaryTail}</div></div></div><div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">{t.metricALabel}</div><div className="mt-1 text-xs font-black text-emerald-700">{t.metricACaption}</div><p className="mt-2 text-3xl font-black text-emerald-950">{tertiaryDisplay}</p><p className="text-sm font-bold text-emerald-700">{t.metricATail}</p></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500">{t.metricBLabel}</div><div className="mt-1 text-xs font-black text-blue-700">{t.metricBCaption}</div><p className="mt-2 text-3xl font-black text-blue-950">{quaternaryDisplay}</p><p className="text-sm font-bold text-blue-700">{t.metricBTail}</p></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{t.metricCLabel}</div><div className="mt-1 text-xs font-black text-slate-700">{t.metricCCaption}</div><p className="mt-2 text-3xl font-black text-slate-950">{secondaryDisplay}</p><p className="text-sm font-bold text-slate-700">{t.metricCTail}</p></div></div></div></article>
        ...
        <section className="rounded-[2rem] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-blue-50 p-6 shadow-sm md:p-7">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.9fr]">{/* L9 */}
            <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">{t.progressInsight}</p><h3 className="mt-2 text-2xl font-black">{t.possibleTarget}</h3><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-xs font-black text-slate-500">{t.weeklyTrend}</div><div className="mt-1 text-3xl font-black">{primaryDisplay}$</div></div><div className="rounded-2xl bg-blue-50 p-4"><div className="text-xs font-black uppercase text-blue-700">{t.dailyGap}</div><div className="mt-1 text-3xl font-black text-blue-950">{secondaryDisplay}</div></div><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-xs font-black uppercase text-emerald-700">{t.tertiaryTag}</div><div className="mt-1 text-3xl font-black text-emerald-950">{tertiaryDisplay}</div></div></div></article>
