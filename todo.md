# 不良品退貨修復 — 中文版面監修
## 停止量產
- [x] 停止新增量產工具，切換為修復模式
## 第 19 項 Profit Margin Calculator 修復
- [x] 盤點 Profit Margin 中文版面英文殘留位置
- [x] 將 Profit Margin 中文版面非 hero 主工具名內容改為中文
- [x] 執行 TypeScript 與 Profit Margin QC
- [x] Commit / push 修復（b1b8f6b、b414b22）
- [x] Production verification：確認第 19 項中文主內容已修復；共用導覽/廣告仍列為全站監修風險
## Finance 1–19 監修
- [x] 列出 finance 前 19 個工具順序與第 19 項定位
- [x] 監查 1–19 中文版面英文殘留風險（初掃：#18 高風險、#19 已修主體、#1–17 需 production 抽查確認）
- [x] 修復 #18 roas-calculator 中文版面硬編英文
- [x] Production verification #18
- [x] 抽查 #1–17 production 中文可見內容
- [x] 修復 #17 pomodoro-calculator 中文版面硬編英文
- [x] Production verification #17
- [x] 修復 #16 meeting-cost-calculator 中文版面硬編英文
- [x] Production verification #16
- [x] 補修 #16 PremiumGate 小卡英文殘留
- [x] Production verification #16 補修
- [ ] 彙整需後續修復清單
- [x] 修復 #15 hourly-rate-calculator 中文版面硬編英文
- [x] Production verification #15
- [x] 修復 #14 salary-after-tax-calculator 中文版面硬編英文
- [ ] Production verification #14
